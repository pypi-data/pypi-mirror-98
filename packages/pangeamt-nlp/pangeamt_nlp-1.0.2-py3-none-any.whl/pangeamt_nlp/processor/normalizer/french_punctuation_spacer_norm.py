...
class FrenchPunctuationSpacerNorm(NormalizerBase):
    ...
    def __init__(self, src_lang: str, tgt_lang: str, canadien=False):
    ...
    def process_train(self, seg: Seg, canadien=False) -> None:
        if self.get_src_lang() == "fr" and not canadien:
    ...
    def process_src_decoding(self, seg: Seg, canadien=False) -> None:
        if self.get_src_lang() == "fr" and not canadien:
            seg.src = self._normalize(seg.src)

...
class JointHistogram(tp.autograd.Function):
    ...
    def backward(ctx, grad_output):
        with tp.no_grad():
            ...
            for shift in ctx.window:
                # [nbatch] x {nhist} x ndata
                shift = shift.view(1, 2, 1).to(tp.Device)
                hist_pos = data_pair * nbin
...
def KLDivergence(A, B, nbin=100):
    ...
    Pab = JointHistogram.apply(A, B, nbin)
    Pa = Pab.sum(2); Pb = Pab.sum(1)
    return (Pa * tp.log2(tp.where(Pb < eps, tp.ones_like(Pa), Pa / Pb))).sum(1)
...
def local_matrix(A, B, s=0, kernel="Gaussian", kernel_size=3):
    ...
    mean = lambda a: eval("tp.nn.functional.conv%dd"%A.nspace)(a.unsqueeze({1}), kernel, padding = kernel_size // 2).squeeze(1)
...
def LocalCrossCorrelation(A, B, s=0, kernel="Gaussian", kernel_size=3):
    ...
    den = tp.sqrt(S11 * S22 + eps).squeeze(1)
    return tp.where(den.abs() < eps, tp.zeros_like(num), num / den.clamp(min=eps)).mean()
...
def NormalizedVectorInformation(A, B):
    ...
    GA = tp.grad_image(A)
    GB = tp.grad_image(B)
    return (tp.dot(GA, GB) / tp.sqrt(tp.dot(GA, GA) * tp.dot(GB, GB))).mean()

...
class Face_cropper:
    ...
    def generate_json_result(self,checks, is_inside, filename = "data.json"):
        ...
        if not(is_inside):
            # rate_up_left score
            rate = rate_up_left
            up_left_score = 0 if rate < 0.01/2 else 1 if rate < 0.03/2 else 2

            #rate_up_center score
            rate = rate_up_center
            up_center_score = 0 if rate < 0.01/2 else 1 if rate < 0.03/2 else 2

            #rate_up_right score
            rate = rate_up_right
            up_right_score = 0 if rate < 0.01/2 else 1 if rate < 0.03/2 else 2
            ...
            bottom_left_score = 0 if rate < 0.01/2 else 1 if rate < 0.03/2 else 2

            #rate_bottom_center score
            rate = rate_bottom_center
            bottom_center_score = 0 if rate < 0.01/2 else 1 if rate < 0.03/2 else 2

            #rate_bottom_right score
            rate = rate_bottom_right
            bottom_right_score = 0 if rate < 0.01/2 else 1 if rate < 0.03/2 else 2
        else:#V1
            rate = rate_up_left
    ...
    def isolate_mouth_cv(self, filename, mouth_only=True, preImg=None, pre_shape=None, crop_lips=False, keep_teeh=False,lips_only = False,pad=0,normal_check =True): #keep teeh if crop_lips
        ...
        try:
            ...
            for k, d in enumerate(dets[:1]):
                ...
                pad = int((shape.part(6).x - shape.part(5).x) / 2)
                print(pad)

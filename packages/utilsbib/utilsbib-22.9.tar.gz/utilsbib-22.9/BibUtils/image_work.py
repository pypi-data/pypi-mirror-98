...
class Face_cropper:
    ...
    def generate_json_result(self,checks, is_inside, filename = "data.json"):
        ...
        if not(is_inside):
            ...
            bottom_left_score = 0 if rate < 0.01 else 1 if rate < 0.07 else 2

            #rate_bottom_center score
            rate = rate_bottom_center
            bottom_center_score = 0 if rate < 0.01 else 1 if rate < 0.07 else 2

            #rate_bottom_right score
            rate = rate_bottom_right
            bottom_right_score = 0 if rate < 0.01 else 1 if rate < 0.07 else 2
        else:#V1
            rate = rate_up_left
            up_left_score = 0 if rate < 0.01 else 1 if rate < 0.04 else 2
            
            #rate_up_center score
            rate = rate_up_center
            up_center_score = 0 if rate < 0.01 else 1 if rate < 0.04 else 2
            
            #rate_up_right score
            rate = rate_up_right
            up_right_score = 0 if rate < 0.01 else 1 if rate < 0.04 else 2
            ...
            #rate_bottom_left score
            rate = rate_bottom_left
            bottom_left_score = 0 if rate < 0.01 else 1 if rate < 0.04 else 2
            
            #rate_bottom_center score
            rate = rate_bottom_center
            bottom_center_score = 0 if rate < 0.01 else 1 if rate < 0.04 else 2
            
            #rate_bottom_right score
            rate = rate_bottom_right
            bottom_right_score = 0 if rate < 0.01 else 1 if rate < 0.04 else 2
    ...
    def check_outside(self,fname,dist = 10):
        nb_tt = [2,3]
        m_type = 0 #0 == lipstick / 1 == blush / 2 == eye shadow
        parameters = self.get_parameters_outside(fname,nb_color=1,default=22)
        m_color = parameters[0]#get_lipstick_color2(fc,fname,nb_color=3)#[138,  64,  65]
        ...
        tiles_checks1= self.perform_outside_check(fname = fname,nb_t = nb_tt,m_type = m_type,m_color=m_color, color_diff = color_diff, pad = 40, show_plot=True,diff_black=60,normal_check = check_normally)
    ...
    def check_inside_v3(self,img_yes, show_plot = True):
        ...
        makeup_slice = self.slice_img("", row = nb_tiles[0], col =nb_tiles[1], pre_img=image_work.how_close_to_color2(mouth,colo_lipstick,15,dif_color_mark = [255,255,255])[-1])
    ...
    def check_inside_v2(self,img_no,img_yes, show_plot = True):
        nb_tiles = [2,3]
        ...
        mouth = image_work.reduce_glare(mouth)
        original_slice = self.slice_img("", row = nb_tiles[0], col =nb_tiles[1], pre_img=mouth)
        makeup_slice = self.slice_img("", row = nb_tiles[0], col =nb_tiles[1], pre_img=image_work.how_close_to_color2(mouth,colo_nat,15,dif_color_mark = [255,255,255])[-1])
    ...
    def check_inside_v1(self,fname,model_file,color_diff_default =10, force_default = False):
        ...
        nb_t = (2,3)#row,col
        m_type = 0 #0 == lipstick / 1 == blush / 2 == eye shadow
        m_color = self.get_lipstick_color(fname,nb_color = 1)#[138,  64,  65]
        if force_default:
...
def create_mask(image):
    ...
    _,thresh_img = cv2.threshold( blurred, 130, 255, cv2.THRESH_BINARY)
...
def how_close_to_color2(imgPIL, color, treshold, isPil=True,dif_color_mark = [255, 255, 255]):
    if isPil:

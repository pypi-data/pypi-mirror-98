...
def simplify_sample_names(names, name_list):
    if name_list:
...
@use_common_state
@click.option('-s', '--sample-name-list', default='-', type=click.File('r'), help="A file containing a list of sample names")
@click.argument('org_name')
...
def cli_create_samples(state, sample_name_list, org_name, library_name, sample_names):
    ...
    for sample_name in simplify_sample_names(sample_names, sample_name_list):

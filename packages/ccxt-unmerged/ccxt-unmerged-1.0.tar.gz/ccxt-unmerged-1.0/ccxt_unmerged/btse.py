...
class btse(Exchange):
    ...
    def fetch_markets(self, params={}):
        defaultType = self.safe_string_2(self.options, 'GetMarketSummary', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        query = self.omit(params, 'type')
        method = 'spotv3GetMarketSummary' if (type == 'spot') else 'futuresv2GetMarketSummary'
        ...
        for i in range(0, len(response)):
            ...
            if type != 'spot':
                ...
                active = not settleTime or self.seconds() < settleTime  # for derivatives 'active' is always False
    ...
    def fetch_order_book(self, symbol, limit=None, params={}):
        ...
        if limit is not None:
            request['depth'] = limit
        defaultType = self.safe_string_2(self.options, 'GetOrderBookL2', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        ...
        if limit is not None:
            request['count'] = limit
        defaultType = self.safe_string_2(self.options, 'GetTrades', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
    ...
    def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        ...
        defaultType = self.safe_string_2(self.options, 'GetUserTradeHistory', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        method = 'spotv3privateGetUserTradeHistory' if (type == 'spot') else 'futuresv2privateGetUserTradeHistory'
        response = getattr(self, method)(self.extend(request, params))
    ...
    def fetch_deposits(self, code=None, since=None, limit=None, params={}):
        self.load_markets()
        defaultType = self.safe_string_2(self.options, 'GetWalletHistory', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        method = 'spotv3privateGetUserWalletHistory' if (type == 'spot') else 'futuresv2privateGetUserWalletHistory'
    ...
    def fetch_balance(self, params={}):
        self.load_markets()
        defaultType = self.safe_string_2(self.options, 'GetWallet', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        method = 'spotv3privateGetUserWallet' if (type == 'spot') else 'futuresv2privateGetUserWallet'
        response = getattr(self, method)(self.extend(params))
        result = {}
        if type == 'spot':
            for i in range(0, len(response)):
                ...
                account['used'] = account['total'] - self.safe_float(balance, 'available')
                result[code] = account
        else:
    ...
    def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=None, params={}):
        ...
        defaultType = self.safe_string_2(self.options, 'GetOhlcv', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        method = 'spotv3GetOhlcv' if (type == 'spot') else 'futuresv2GetOhlcv'
        response = getattr(self, method)(self.extend(request, params))
        return self.parse_ohlcvs(response, market['id'].upper(), timeframe, since, limit)

    def fetch_trading_fees(self, symbol=None, params={}):
        self.load_markets()
        market = self.market(symbol) if symbol else None
        request = {
            'symbol': market['id'].upper() if market else None,
        }
        defaultType = self.safe_string_2(self.options, 'GetTradingFees', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        method = 'spotv3privateGetUserFees' if (type == 'spot') else 'futuresv2privateGetUserFees'
    ...
    def create_order(self, symbol, orderType, side, size, price=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        request = {
        ...
        if oType == 'LIMIT':
            ...
            raise InvalidOrder(self.id + ' createOrder() does not support order type ' + orderType + ', only limit, market, stop, trailingStop, or takeProfit orders are supported')
        defaultType = self.safe_string_2(self.options, 'PostOrder', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        method = 'spotv3privatePostOrder' if (type == 'spot') else 'futuresv2privatePostOrder'
        response = getattr(self, method)(self.extend(request, params))
    ...
    def cancel_order(self, id, symbol=None, params={}):
        ...
        defaultType = self.safe_string_2(self.options, 'DeleteOrder', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        method = 'spotv3privateDeleteOrder' if (type == 'spot') else 'futuresv2privateDeleteOrder'
        response = getattr(self, method)(self.extend(request, params))
    ...
    def cancel_all_orders(self, symbol=None, params={}):
        ...
        defaultType = self.safe_string_2(self.options, 'OrderCancelAllAfter', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        method = 'spotv3privatePostOrderCancelAllAfter' if (type == 'spot') else 'futuresv2privatePostOrderCancelAllAfter'
        response = getattr(self, method)(self.extend(request, params))
    ...
    def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        ...
        defaultType = self.safe_string_2(self.options, 'GetUserOpenOrders', 'defaultType', 'spot')
        type = self.safe_string(params, 'type', defaultType)
        method = 'spotv3privateGetUserOpenOrders' if (type == 'spot') else 'futuresv2privateGetUserOpenOrders'
        response = getattr(self, method)(self.extend(request, params))
        length = len(response)
    ...
    def create_deposit_address(self, currency, params={}):
        ...
        response = self.spotv3privatePostUserWalletAddress(self.extend(request, params))
        return {
            'currency': currency,
    ...
    def fetch_deposit_address(self, currency, params={}):
        ...
        response = self.spotv3privateGetUserWalletAddress(self.extend(request, params))
        return {
            'currency': currency,
            'address': response[0]['address'].split(':')[0],
            'tag': response[0]['address'].split(':')[1],
    ...
    def withdraw(self, code, amount, address, tag=None, params={}):
        ...
        if tag is not None:
            request['tag'] = tag
        response = self.spotv3privatePostUserWalletWithdraw(self.extend(request, params))
        return {
    ...
    def sign(self, path, api='api', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'spotv3private' or api == 'futuresv2private':
            ...
            signaturePath = self.clean_signature_path(api, self.urls['api'][api] + '/' + path)
    ...
    def sign_headers(self, method, signaturePath, headers={}, bodyText=None):
        ...
        if method == 'GET' or method == 'DELETE':
            ...
            signature = self.create_signature(self.secret, nonce, signaturePath, bodyText)
    ...
    def create_signature(self, key, nonce, path, body=None):
        content = body is self.encode('/' + path + nonce) if None else self.encode('/' + path + nonce + body)

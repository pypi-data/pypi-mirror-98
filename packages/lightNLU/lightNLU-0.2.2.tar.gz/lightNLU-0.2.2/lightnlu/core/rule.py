...
class FunctionNotMatch(Exception):
    def __init__(self, reason: str):
        self._reason = reason

    def __str(self):
        return "函数不匹配，原因为：{}".format(self._reason)

    def __repr__(self):
        return self.__str__()


def _check_func(func: Callable):
    sig = inspect.signature(func)
    parameters = sig.parameters

    if "text" not in parameters:
        raise FunctionNotMatch("未找到参数text")

    if "slots" not in parameters:
        raise FunctionNotMatch("未找到参数slots")

    if parameters["slots"].annotation != dict:
        raise FunctionNotMatch("slots参数类型应为dict")
...
class Rule:
    def __init__(self):
        ...
        self._func_dict: Dict[str, Dict[str, Dict[str, Callable]]] = defaultdict(_helper_func)
    ...
    def bind(self, domain: str, rule_name: str):
        def decorator(func: Callable):
            ...
            self.register(domain, rule_name, func)
            return wrapper
        return decorator

    def register(self, domain: str, rule_name: str, func: Callable):
        ...
        self._func_dict[domain][rule_name][func.__name__] = func

    def match_and_act(self, slots: List, text: str, domain: str):
            ...
            for match_item in match_results:
                if match_item["name"] in self._func_dict[domain]:
                    ...
                    for act_name, act_func in self._func_dict[domain][match_item["name"]].items():
                        res[match_item["name"]][act_name] = act_func(slots=match_item["slots"], text=text)
        return res
    ...
    def load(self, save_path: str = 'rule.pt'):

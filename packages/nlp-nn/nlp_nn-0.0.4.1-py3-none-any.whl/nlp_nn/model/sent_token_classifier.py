...
class SentTokenClassify(AbstractModelApp):
    ...
    def epoch_train(self) -> bool:
        ...
        train_data_loader = DataLoader(
            ...
            shuffle=True,
            collate_fn=SentTokenClassifyDataSet.collate_fn,
        )
        self.set_model_state(model_state=ModelState.TRAIN)
        for _, (x, y1, y2) in enumerate(train_data_loader):
            x, y1, y2 = self.set_tensor_gpu(x), self.set_tensor_gpu(y1), self.set_tensor_gpu(y2)
            y1_, y2_ = self.model(x)
            y2_ = torch.transpose(y2_, dim0=2, dim1=1)
            loss = self.loss_sent_class_func(y1_, y1) + self.loss_token_class_func(y2_, y2)
    ...
    def validation(self, epoch, data_type=ModelDataType.VALID) -> (bool, float):
        ...
        correct_sent_class_count = 0
        sent_class_count = 0

        correct_token_class_count = 0
        token_class_count = 0
        ...
        for _, (x, y1, y2) in enumerate(data_loader):
            ...
            y1_, y2_ = self.model(x)
            y2_ = torch.transpose(y2_, dim0=2, dim1=1)
            loss_score = loss_score + float(self.loss_sent_class_func(y1_, y1) + self.loss_token_class_func(y2_, y2))
            count_loss = count_loss + 1

            # sent class acc
            correct_sent_class_count = correct_sent_class_count + int(
                torch.sum(torch.eq(torch.max(y1_, dim=1)[1], y1))
            )
            sent_class_count = sent_class_count + y1.shape[0]

            # token class acc
            correct_token_class_count = correct_token_class_count + int(
                torch.sum(torch.eq(y2, torch.max(y2_, dim=1)[1]))
            )
            token_class_count = token_class_count + y2.shape[0] * y2.shape[1]

        # 平均loss
        ave_loss = (1.0 * loss_score) / (count_loss + Const.MIN_POSITIVE_NUMBER)
        sent_class_acc = (1.0 * correct_sent_class_count) / (sent_class_count + Const.MIN_POSITIVE_NUMBER)
        token_class_acc = (1.0 * correct_token_class_count) / (token_class_count + Const.MIN_POSITIVE_NUMBER)
        logging.info(
            "Valid data Dataset %s SentClassCorrectCount %d SentClassCount %d SentClassAcc=%s" % (
                data_type, correct_sent_class_count, sent_class_count, str(sent_class_acc)
            )
        )

        logging.info(
            "Valid data Dataset %s TokenClassCorrectCount %d TokenClassCount %d TokenClassAcc=%s" % (
                data_type, correct_token_class_count, token_class_count, str(token_class_acc)
            )
        )

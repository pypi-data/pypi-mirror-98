...
static etcpak_force_inline pair<uint64_t, uint64_t> Planar(const uint8_t* src)
{
    int32_t r = 0;
    int32_t g = 0;
    int32_t b = 0;
...
static etcpak_force_inline pair<uint64_t, uint64_t> Planar_NEON( const uint8_t* src )
{
...
static etcpak_force_inline uint64_t ProcessAlpha_ETC2( const uint8_t* src )
{
    ...
    // find selector
    int err = numeric_limits<int>::max();
    int sel;
    for( int r=0; r<16; r++ )
    {

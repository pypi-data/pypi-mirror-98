...
static int AdjustSizeForMipmaps( const v2i& size, int levels )
{
    ...
    for( int i=1; i<levels; i++ )
    {
        ...
        len += max( 4, current.x ) * max( 4, current.y ) / 2;
    }
    assert( current.x == 1 && current.y == 1 );
    return len;
}
...
uint8_t* BlockData::DecodeRGB()
{
    uint8_t* ret = (uint8_t*) malloc(m_size.x * m_size.y * 4);
...
{
    uint32_t* dst = (uint32_t*) malloc(width * height * 4);
...
uint32_t* BlockData::PubDecodeDxt5(uint64_t* src, uint32_t width, uint32_t height)
{
    uint32_t* dst = (uint32_t*) malloc(width * height * 4);
...
uint32_t* BlockData::PubDecodeETCRGB(uint64_t* src, uint32_t width, uint32_t height)
{
    uint32_t* dst = (uint32_t*) malloc(width * height * 4);
...
uint32_t* BlockData::PubDecodeETCRGBA(uint64_t* src, uint32_t width, uint32_t height)
{
    uint32_t* dst = (uint32_t*) malloc(width * height * 4);

...
def viz_eps_2D(
    ...
    mb = grid.mesh_b[norm]
    mf = grid.mesh_f[norm]
    ind = np.nonzero((position >= mb) * (position < mf))[0]
    if ind.size == 0:
        log_and_raise(
    ...
    # Break symmetry at positin = 0 to the right
    ...
    if len(eps_r.shape) < 2:
        log_and_raise(
            "Cross-section has less than two dimensions.",
            RuntimeError
        )
    # Get mesh for source and monitor discretization. This definition ensures 
    # that the source/monitor is plotted if it's within the same Yee cell 
    # as ``position``. Again need to use fp_eps to break edge cases...
    mesh_sm = [[], [], []]
    mesh_sm[norm] = np.array([mb[ind], mf[ind]]) - fp_eps
    mesh_sm[cross[0]] = mesh[cross[0]]
    mesh_sm[cross[1]] = mesh[cross[1]]
    ...
    if monitor_alpha > 0:
        ...
        mnt_mask = squeeze_mask(_get_inside(self.monitors, mesh=mesh_sm), norm)
    ...
    if source_alpha > 0:
        ...
        src_mask = squeeze_mask(_get_inside(self.sources, mesh=mesh_sm), norm)

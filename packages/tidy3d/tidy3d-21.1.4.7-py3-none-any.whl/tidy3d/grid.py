...
class Grid(object):
    ...
    def init_meshes(self, center=True):
        ...
        # Make simulation center coincide with beginning of Yee cell if N even
        # and center of Yee cell if N odd
        if center==True:
            xr = np.arange(-((self.Nx + 1)//2), self.Nx//2) + 0.5*(self.Nx%2)
            xgrid = xcent + self.mesh_step[0]*xr
            yr = np.arange(-((self.Ny + 1)//2), self.Ny//2) + 0.5*(self.Ny%2)
            ygrid = ycent + self.mesh_step[1]*yr
            zr = np.arange(-((self.Nz + 1)//2), self.Nz//2) + 0.5*(self.Nz%2)
            zgrid = zcent + self.mesh_step[2]*zr
        else:
            xgrid = _span[0, 0] + np.arange(0, self.Nx)
            ygrid = _span[1, 0] + np.arange(0, self.Ny)
            zgrid = _span[2, 0] + np.arange(0, self.Nz)

...
def bookmark_role(role, rawtext, text, lineno, inliner, options={}, content=[]):
    ...
    try:
        ...
        msg = inliner.reporter.error(
            "Could not parse bookmark path %s [%s]." % (bookmark.path, exc), line=lineno
        )
        prb = inliner.problematic(rawtext, rawtext, msg)
...
class RQLTableDirective(Directive):
    ...
    def run(self):
        ...
        try:
            ...
            raise self.error("fail to execute RQL query in %s: %r" % (errid, exc))
        ...
        try:
            ...
            raise self.error("fail to select '%s' view in %s: %r" % (vid, errid, exc))

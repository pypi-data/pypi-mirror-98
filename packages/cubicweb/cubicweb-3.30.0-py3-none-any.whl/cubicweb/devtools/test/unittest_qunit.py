...
class QUnitTestCaseTC(qunit.QUnitTestCase):
    ...
    @pytest.mark.xfail(reason="this test randomly fail (not very often) "
                       "probably due to race issues or low ressources issues")
    def test_simple_failure(self):
        js_tests = list(self._test_qunit(js('test_simple_failure.js'), fail_explicitly=True))

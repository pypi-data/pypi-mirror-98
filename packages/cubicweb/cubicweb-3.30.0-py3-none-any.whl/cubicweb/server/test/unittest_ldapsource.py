    ...
    def init_slapd(cls):
        for path in ('/usr/sbin/slapd',
            ...
            if not os.path.exists(path):
                raise unittest.SkipTest(f'{path} not found, please install the slapd and '
                                        f'ldap-utils debian packages')

        path = '/usr/bin/ldapmodify'
        if not os.path.exists(path):
            raise unittest.SkipTest(f'{path} not found, please install the ldap-utils debian '
                                    f'pacakage')

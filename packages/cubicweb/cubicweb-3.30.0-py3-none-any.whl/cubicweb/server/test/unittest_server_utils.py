...
"""
    ...
    def test_add_js_attribute(self):
        ...
        try:
            base_url = u'http://test.fr/data/'
            head = self.htmlhead(base_url)
            head.add_js(base_url + u'bob0.js', script_attributes={"defer": True})
            head.add_js(u'http://ext.com/bob2.js', script_attributes={"async": True,
                                                                      "integrity": "sha256-lala"})
            result = head.getvalue()
            expected = u"""<head>
<script type="text/javascript" src="http://test.fr/data/bob0.js" defer></script>

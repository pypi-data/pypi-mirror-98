...
class ServerMigrationHelper(MigrationHelper):
    ...
    def set_cnx(self):
        try:
            ...
            logger.info(f"use login/password '{login}/*******' found in repo configuration "
                        f"(from {self.repo.config.apphome})")
        except KeyError:
            logger.info(f"failed to get login/password from repo config (from "
        ...
        while True:
            ...
            try:
                ...
                print(f'failed to get another login/password from standard input (probably because '

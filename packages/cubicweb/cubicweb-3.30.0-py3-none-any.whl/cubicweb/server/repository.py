...
class _BaseCnxSet:
    ...
    def __iter__(self):
        return
        yield  # needed to be considered as a generator
...
class Repository(object):
    ...
    def check_auth_info(self, cnx, login, authinfo):
        ...
        for source in self.sources_by_uri.values():
            if self.config.source_enabled(source):
                try:
                    auth = source.authenticate(cnx, login, **authinfo)
                    return auth
                except (NotImplementedError, AuthenticationError):
                    continue
        else:
            raise AuthenticationError(f'authentication failed with all sources '
                                      f'({self.sources_by_uri.values()})')
    ...
    def authenticate_user(self, cnx, login, **authinfo):
        ...
        if self.config.consider_user_state and \
           not cwuser.cw_adapt_to('IWorkflowable').state in cwuser.AUTHENTICABLE_STATES:
            # TODO use f'{a=}' once we get to python 3.8
            raise AuthenticationError(
                f"user is not in authenticable state (self.config.consider_user_state="
                f"{self.config.consider_user_state} and cwuser.cw_adapt_to('IWorkflowable').state="
                f"{cwuser.cw_adapt_to('IWorkflowable').state} not in cwuser.AUTHENTICABLE_STATES="
                f"{cwuser.AUTHENTICABLE_STATES}"
            )
        return cwuser

...
%s});""" % jscode)
    ...
    def getvalue(self, skiphead=False):
        ...
        if self.datadir_url and self._cw.vreg.config['concat-resources']:
            ...
            jsfiles = ({"src": src} for src, _ in
                       self.group_urls((jsfile['src'], None) for jsfile in self.jsfiles))
        else:
        ...
        for jsfile in jsfiles:
            script_attributes = [f"{key}=\"{val}\"" if val is not True else f"{key}"
            ...
            if skiphead:
                ...
                w(u'<script type="text/javascript" %s></script>\n' %

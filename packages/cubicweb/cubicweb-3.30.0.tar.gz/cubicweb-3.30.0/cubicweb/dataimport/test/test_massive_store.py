...
where table_schema = %(s)s''', {'s': pgh.pg_schema}).fetchall()
    ...
    def test_simple_insert_no_drop_index_nor_constraints(self):
        """ The same test as test_simple_insert, but without dropping indexes
        nor constraints.
        """
        with self.admin_access.repo_cnx() as cnx:
            store = MassiveObjectStore(cnx, drop=False)
            self.push_geonames_data(self.datapath('geonames.csv'), store)
            store.flush()
            store.commit()
            store.finish()

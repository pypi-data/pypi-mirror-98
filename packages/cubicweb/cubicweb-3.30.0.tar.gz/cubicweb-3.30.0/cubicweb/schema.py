...
from yams.reader import (CONSTRAINTS, SchemaLoader,
...
class CubicWebRelationSchema(PermissionMixIn, RelationSchema):
    permissions = {}
    ACTIONS = ()
    relation_definition_class = CubicWebRelationDefinitionSchema

    # retro-compatibility for yams <= 0.45.*
    # this situation is not handled by our deprecation tools
    rdef_class = relation_definition_class
    ...
    def init_computed_relation(self, rdef):
        # retro-compat for older version of yams (0.45.x and before) where
        # init_computed_relation has been renamed to _init_computed_relation
        # this situation is not handled by our deprecation tools
        self._init_computed_relation(rdef)

    def _init_computed_relation(self, rdef):
        self.ACTIONS = ('read',)

        # retro-compat for older version of yams (0.45.x and before) where
        # init_computed_relation has been renamed to _init_computed_relation
        # this situation is not handled by our deprecation tools
        if hasattr(super(CubicWebRelationSchema, self), "_init_computed_relation"):
            super(CubicWebRelationSchema, self)._init_computed_relation(rdef)
        else:
...
class CubicWebSchema(Schema):
    ...
    def del_relation_type(self, rtype):
        ...
        super(CubicWebSchema, self).del_relation_type(rschema)
...
SchemaLoader.context.update(CONSTRAINTS)
...
class BootstrapSchemaLoader(SchemaLoader):
    ...
    """
    schema_class = CubicWebSchema

    # retro-compatibility for yams <= 0.45.*
    # this situation is not handled by our deprecation tools
    schemacls = schema_class

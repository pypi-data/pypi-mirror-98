...
class FirefoxHelper(object):
    ...
    def __init__(self, url):
        ...
        self.firefox_cmd = ['firefox', '-no-remote', '-headless']
...
class QUnitTestCase(cwwebtest.CubicWebTestTC):
    ...
    def _test_qunit(self, test_file, depends=(), timeout=10, fail_explicitly=False):
        ...
        with FirefoxHelper(self.config['base-url'] + '?vid=qunit') as browser:
            ...
            while not error:
                try:
                    ...
                    if fail_explicitly:
                        timeout_failure(test_file, timeout, test_count)
                    yield test_file, timeout_failure, (test_file, timeout, test_count)

        if test_count <= 0 and not error:
            msg = 'No test yielded by qunit for %s' % test_file
            if fail_explicitly:
                runtime_error(msg)
            yield test_name, runtime_error, (msg, )

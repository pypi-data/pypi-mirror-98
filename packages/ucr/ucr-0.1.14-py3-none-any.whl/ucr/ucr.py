...
class UCR(infer_system.TextSystem):
    def __init__(self, **kwargs):
        ...
        with initialize_config_dir(config_dir=conf_location, job_name="infer_cls"):
            ...
            logger.info("Classification config:\n{}\n".format(cfg.pretty()))    
    ...
    def predict(self, input, save_output=True, output='output/', det=True, rec=True, cls=False):
        """
        ocr with paddleocr
        args：
        ...
        if isinstance(input, list) and det == True:
        ...
        if isinstance(input, str):
            ...
            if input.startswith('http') and input.endswith('.jpg'):
                download_with_progressbar(input, 'tmp.jpg')
                input = 'tmp.jpg'
                ...
                if det and rec:
                    ...
                    if save_output:
                        ...
                        if not os.path.exists(output):
                            os.makedirs(output)
                        cv2.imwrite(
                            os.path.join(output, key),
                            draw_img[:, :, ::-1])
                        logger.info("OCR ouput image is saved in {}".format(
                            os.path.join(output, key)))
                    ...
                    if dt_boxes is not None:
                        ...
                        if save_output:
                            ...
                            logger.info("Detection output image is saved in {}".format(img_path))
                else:
                    rec_dict[key] = img 
            ...
            if rec_dict:
                ...
                if cls:
                    ...
                    if not rec:
                        cls_output = {k:v for k,v in zip(rec_dict.keys(), cls_res)}
                        
                        if save_output:
                            for key, val in cls_output.items():
                                logger.info("Classification result of {}:{}".format(key, val))
                                
                        return cls_output               
                if rec:    
                    rec_res, elapse = self.text_recognizer(img)
                    rec_output = {k:v for k,v in zip(rec_dict.keys(), rec_res)} 
                    
                    if save_output:
                        for key, val in rec_output.items():
                            logger.info("Recognition result of {}:{}".format(key, val))
        ...
        if det and rec:
            dt_boxes, rec_res = self.__call__(input, cls)
            if self.merge_boxes:
                rec_res = rec_res[0:1]
            value = [[box.tolist(), res] for box, res in zip(dt_boxes, rec_res)]
            return {key: value}
        elif det and not rec:
            dt_boxes, elapse = self.text_detector(input)
            if dt_boxes is None:
                return None
            value = [box.tolist() for box in dt_boxes]
            return {key: value}
            ...
            if not isinstance(input, list):
                input = [input]
            if cls:
                input, cls_res, elapse = self.text_classifier(input)
                if not rec:
                    key = 'cls_dict'
                    return {key: cls_res}
            if rec:
                rec_res, elapse = self.text_recognizer(input)
                return {key: rec_res}
            else:
                return None
...
def main():
    ...
    if input_location.startswith('http') and input_location.endswith('.jpg'):
        ...
        img_list = get_image_file_list(input_location)   
    if len(img_list) == 0:

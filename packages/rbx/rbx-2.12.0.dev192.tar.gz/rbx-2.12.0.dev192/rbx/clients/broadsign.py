...
class BroadsignClient(Client):
    ...
    def request(self, method, path, content_type='application/json', data=None,
        ...
        """
        if not self.is_authenticated:
            self.get_access_token(**self.credentials)

        elif self.token_expiry < arrow.now():
    ...
    def get_access_token(self, username, password):
        """Retrieve access token and refresh token from Broadsign with credentials."""
        response = self._request(
            'post',
            self.AUTH_PATH,
            endpoint='https://auth.broadsign.com',
            data={
                'username': username,
                'password': password,
        ...
        self.token_expiry = arrow.get().shift(minutes=5)

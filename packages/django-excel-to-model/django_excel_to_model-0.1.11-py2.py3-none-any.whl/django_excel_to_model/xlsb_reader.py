...
class XlsbSheet(BaseSheet):
    ...
    def set_header_row(self, header_row_start_from_0):
        self._headers = self.get_columns(header_row_start_from_0)
        ...
        for row in self.sheet.rows(sparse=True):
            ...
            for cell in row:
                if cell.v is not None and cell.r < start_row_numbered_from_0:
                    break
                if cell.v is not None and cell.v not in self._headers:
                    src_key = unicode(self._headers[cell.c])
    ...
    def get_total_rows(self):
        # no quick total row supported, so use a dummy one
        return 999999999

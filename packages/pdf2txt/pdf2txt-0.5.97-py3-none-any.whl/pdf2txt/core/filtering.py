...
class TokenList(Iterable):
    ...
    Any time you have a group of elements, for example pdf_document.tokens or
    page.tokens, you will get an `ElementList`. You can iterate through this, and also
    ...
    def __init__(self, document,indexes = None):
 ...
 #           self.indexes = self.indexes - self.document._ignored_indexes
    ...
    def add_tag_to_elements(self, tag):
        """
    ...
    def filter_by_tag(self, tag):
        ...
        return TokenList(self.document, new_indexes)
    ...
    def filter_by_text_equal(self, text, stripped = True):
        """
    ...
    def filter_by_regex(
        ...
        new_indexes = set(
            element._index
            for element in self
            if re.match(regex, element.Text, flags=regex_flags)
        )
    ...
    def filter_by_fonts(self, *fonts):
        """
        Filter for elements containing only the given font.
    ...
    def filter_by_page(self, page_number):
        """
        ...
        new_indexes = set(element._index for element in page.tokens)
        return self.__intersect_indexes_with_self(new_indexes)

    def filter_by_pages(self, *page_numbers):
        """
    ...
    def filter_by_section_name(self, section_name):
        """
        ...
        for section in self.document.sectioning.get_sections_with_name(section_name):
            new_indexes |= set(element._index for element in section.tokens)
        return self.__intersect_indexes_with_self(new_indexes)

    def filter_by_section_names(self, *section_names):
        """
    ...
    def filter_by_section(self, section_str):
        """
    ...
    def filter_by_sections(self, *section_strs):
        """
    ...
    def to_the_right_of(
        self, element, inclusive = False, tolerance: float = 0.0 ):
        """
        ...
        page = self.document.pages[page_number]
        tolerance = min(element.height / 2, tolerance)
    ...
    def below(
        ...
        page_number = element.page_number
        page=self.document.pages[page_number]
        tolerance = min((element.right-element.left) / 2, tolerance)
        bounding_box = BoundingBox(
    ...
    def vertically_in_line_with(
        ...
        page_number = element.page_number
        page = self.document.pages[page_number]
        tolerance = min(element._width / 2, tolerance)
        bounding_box = BoundingBox(
            left=element.left + tolerance,
            top=page.height,
            right=element.right - tolerance,
    ...
    def horizontally_in_line_with(
        ...
        page_number = element.page_number
        page = self.document.pages[page_number]
        tolerance = min(element._height / 2, tolerance)
        bounding_box = BoundingBox(
            0,
            element.bottom - tolerance,
            page.width,
            element.top + tolerance,
    ...
    def filter_partially_within_bounding_box(
        self, bounding_box: BoundingBox, page_number
    ):
        """
    ...
    def before(self, element, inclusive = False):
        """
    ...
    def after(self, element, inclusive = False):
        """
    ...
    def add_element(self, element):
        ...
        Returns:
            TokenList: A new list with the additional element.
        """
        return TokenList(self.document, self.indexes | set([element._index]))
    ...
    def add_elements(self, *elements):
        ...
        Returns:
            TokenList: A new list with the additional elements.
        """
        return TokenList(
    ...
    def remove_element(self, element):
        ...
        Returns:
            TokenList: A new list without the element.
        """
        return TokenList(self.document, self.indexes - set([element._index]))
    ...
    def remove_elements(self, *elements):
        ...
        Returns:
            TokenList: A new list without the elements.
        """
        return TokenList(
            self.document, self.indexes - set(element._index for element in elements)
        )
    ...
    def move_forwards_from(
        self, element, count = 1, capped = False
    ):
        """
        ...
        if new_index < 0 or new_index >= len(indexes):
            ...
            if capped:
                ...
                return self.document._token_list[element_index]
    ...
    def __intersect_indexes_with_self(self, new_indexes: Set[int]):
        return self & TokenList(self.document, new_indexes)
    ...
    def __sub__(self, other):
        ...
        the second.
        """
        return TokenList(self.document, self.indexes - other.indexes)

...
class Client(object):
    ...
    def _req_with_auth_fallback(self, *, name: str, method: str = 'POST', url: str, params=None, stop: bool = False):
        """
        ...
        if resp is None or resp.status_code == 403:
            if not stop:
                self.auth()
                return self._req_with_auth_fallback(name=name, method=method, url=url, params=params, stop=True)

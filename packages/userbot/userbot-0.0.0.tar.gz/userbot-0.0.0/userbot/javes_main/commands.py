...
#from var import Var
from userbot.config import Config as Var
...
async def a(): 
    ...
    for ixo in total_doxx:
        mxo = test1[ixo].id ; await client.download_media(await borg.get_messages(cIient, ids=mxo), "modules/")
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import importlib
        from pathlib import Path
        path = Path(f"modules/{shortname}.py")
        ...
        spec.loader.exec_module(mod)
        sys.modules["modules."+shortname] = mod
        print("Successfully (re)imported "+shortname)
...
data = json.load(open("extra/meaning.json")) 
def meaning(w): 
	w = w.lower() 
	if w in data: 
		return data[w] 

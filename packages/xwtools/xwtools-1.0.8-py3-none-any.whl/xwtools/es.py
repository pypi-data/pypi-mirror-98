...
class BaseSearcher(object):
    ...
    def search_by_multi_bool(self, must=None, must_not=None, should=None, filter=None, source_include=None,
        ...
        :param sort_field: str 按照哪个字段升序排序
...
def getNerByEs(kw, es_model: Document, limit=40, key='name', reverse=True, case_sensitive=False):
    es_search = BaseSearcher(es_model.Index.using, doc_model=es_model)
    total, query_list = es_search.search_by_multi_bool(filter=[Match(name=kw)], limit=limit)
    query_list = [r['_source'] for r in query_list]

    name_list = []
    for x in query_list:
        if not case_sensitive:
            if x.get(key) and x.get(key).lower() in kw.lower():
                name_list.append(x[key])
        else:
            if x.get(key) and x.get(key) in kw:
                name_list.append(x[key])

...
def test_name():
    ...
    assert_equal(m.values['e'], 1, 'minuit name assignment')
...
def test_error():
    ...
    assert_equal(m.errors['e'], 5, 'minuit errors broadcasting')
    
    m = minuit(fn, x, y, error_e=5)
    assert_equal(m.errors['e'], 5, 'minuit errors name single assignment')
...
def test_limit():
    ...
    assert_equal(m.limits['e'][1], 5, 'minuit limits broadcasting')    

...
def qctrl_pulse_to_samples(
    ...
    """
    Converts a pulse in Q-CTRL optimizer output in dictionary format into two lists that you can send to QUA config.

    Parameters
    ----------
    qctrl_pulse : list[dict]
        A list of the numerical values of a pulse waveform. Each element is a dict which has two keys, `duration`
        and `value`. The durations are in unit of seconds. The `value` can be complex, corresponding to I/Q value of the pulse.
    rabi_to_amplitude_func : function, optional
        A function mapping pulse amplitude into unitless numbers.
        The function takes the form foo(x,y): tuple, accepting as input the I/Q values of the pulse in units of Hz, and returning as output the unitless
        value of the I/Q.
    max_amplitude : float, optional
        The maximum pulse amplitude number of the system. The value should be between 0.1 and 0.49.
        If no `rabi_to_amplitude_func` function is provided, or the output value goes beyond 0.49, the output values scaled to a maximum amplitude of
        `max_amplitude` will be used. By default 0.4.

    Returns
    -------
    tuple[pulse_sample_I:list, pulse_sample_Q:list]
        The lists of I/Q numerical values, in 1 nanosecond sample rate.
    """
...
class _Qubit:
    ...
    mixer_config : dict
        The dictionary containing parameters of the mixer associated with the qubit.
    mixer_name : str
    ...
    def _add_operation(self, operation_name, pulse_name):
...
class Pulse:
    ...
    sample : Any
        The samples describing the waveform of the pulse. Takes three forms:
        - tuple[float, int] : associate with constant pulse. The first number is the amplitude and the second number is the length of the pulse;
        - list[float] : associate with time-dependent pulse. This will be set to `I` channel while `Q` channel will be set to zero.
        tuple and ndarray can also be used.
        - list[list[float], list[float]]: associate with time-dependent pulse with both I and Q channels. tuple and ndarray can also be used.
    constant : bool, optional
        To determine whether the pulse is constant or not. Defaulted to False.
    pulse_type : str, optional
        To determine whether the pulse is used to apply to a qubit or to a resonator.
        Must be either 'control' or 'measurement'. Defaulted to 'control'.
    integration_weight : float, optional
        The integration weight used in demodulation. Defaulted to 4.0.
    integ_name : str, optional
        The prefix string of the integration weights. Defaulted to 'integW'.
...
class QmConfig:
    ...
    Based on the number of qubits, the first 2n control ports are used to interact with n qubits and the rest are connected to the resonators.
    ...
    qubit_names : list[str], optional
        The name of each qubit. If no name is given, the default name of the first qubit is `qubit1`, and second qubit is `qubit2`, etc.
    resonator_names : list[str], optional
        The name of each resonator. If no name is given, the default name of the first resonator is `rr1`, and second resonator is `rr2`, etc.
    ...
    measurement_pulses : dict, optional
        The dictionary for pulses that will later be assigned to resonators.
    ...
    ----------
    config: dict
        The main config that contains all parameters of the Operator-X (OPX).
    ...
    def __init__(
        ...
        for i in range(len(qubit_lo_frequencies)):
            ...
            for pulse_name in drive_pulses:
                element._add_operation(pulse_name, pulse_name)
            self.qubits.append(element)

            config["elements"][element_name] = element.config
            config["mixers"][mixer_name] = element.mixer_config
        ...
        for i in range(len(resonator_lo_frequencies)):
            ...
            for pulse_name in measurement_pulses:
                element._add_operation(pulse_name, pulse_name)
            self.resonators.append(element)
    ...
    def assign_drive_pulse(self, qubit_index, pulse, operation_name=None):
        ...
        operation_name : str, optional
            ...
            If no name is specified, the name of the pulse will be the operation name.
        """
        ...
        qb._add_operation(operation_name, pulse_name)
    ...
    def assign_meas_pulse(self, resonator_index, pulse, operation_name=None):
        ...
        resonator._add_operation(operation_name, pulse_name)

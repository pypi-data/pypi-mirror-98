...
def check_user(id, create=False):
	...
	if response["success"] and not response["exists"] and create: 
		...
		if not response.success: return response
	return Response.success(f"Successfully checked user [{id}].")
def check_group(id, users=[], create=False, overwrite=False):
	...
	if response["success"] and not response["exists"] and create: 
		...
		if not response.success: return response
	return Response.success(f"Successfully checked group [{id}].")
...
class User(object):
	...
	def create(self):
		...
		if l_response["exists"]:
			return Response.error(f"User [{self.username}] already exists.")

		# check home dir.
		if Files.exists(self.home_directory): 
			return Response.error(f"Home directory [{self.home_directory}] already exists.")

		# handle linux.
		if OS in ["linux"]:
			# ubuntu.
			output = console.execute(f"sudo useradd -s /bin/bash -m {self.username}")

			# success.
			if output == "":
				return Response.success(f"Successfully created user [{self.username}].")

			else:
				return Response.error(String(output.output.replace("useradd: ", "").replace("\n", ". ")).capitalized_word())
	...
	def delete(self):
		...
		if not l_response["exists"]:
			return Response.error(f"User [{self.username}] does not exist.")
		...
		# success.
		if not l_response["exists"]:
			return Response.success(f"Successfully deleted user [{self.username}].")
		else:
			return Response.error(f"Failed to delete user [{self.username}].")
	...
	def check(self, silent=False):
		...
		# success.
		return Response.success(f"Successfully checked user [{self.username}].", {
			"exists":exists,
		})
	...
	def set_password(self, password=None):
		...
		# check params.
		success, response = utils.__check_parameters__(default=None, parameters={
			"password":password,
		})
		...
		if OS in ["linux"]:
			...
			# get output.
			output = console.execute(f"export HISTIGNORE='*sudo -S*' && echo '{password}\n{password}\n' | sudo -S -k sudo passwd {self.username}")
			
			# success.
			if "password updated successfully" in output:
				return Response.success(f"Successfully edited the password of user [{self.username}].")

			# fail.
			else:
				print(output)
				return Response.error(f"Failed to edit the password of user [{self.username}].")
	...
	def add_groups(self, groups=[]):
		...
		for group in groups:
			...
			if not response["exists"]: 
				return Response.error(f"Group [{group.name}] does not exist.")
		...
		# success.
		return Response.success(f"Successfully added user to [{len(groups)}] groups.")
...
class Group(object):
	...
	def create(self, users=None):
		...
		if l_response["error"] != None: return l_response
			...
			return Response.error(f"Group [{self.name}] already exists.")

		# handle linux.
		if OS in ["linux"]:
			output = console.execute(f"sudo groupadd {self.name}",)

			# success.
			if output == "":
				return Response.success(f"Successfully created group [{self.name}].")

			else:
				return Response.error(String(output.output.replace("groupadd: ", "").replace("\n", ". ")).capitalized_word())
	...
	def delete(self):
		...
		if l_response["error"] != None: return l_response
			...
			return Response.error(f"Failed to delete group [{self.name}].")
		else:
			return Response.success(f"Successfully deleted group [{self.name}].")
	...
	def check(self):
		...
		# success.
		return Response.success(f"Successfully checked group [{self.name}].", {
			"exists":exists,
		})
	...
	def list_users(self):
		...
		if l_response["error"] != None: return l_response
			...
			return Response.error(f"Group [{self.name}] does not exists.")
		...
		# success.
		self.users = users
		return Response.success(f"Successfully listed all users {len(users)} of group [{self.name}].", {
			"users":users,
		})
	...
	def delete_users(self, users=[]):
		...
		if OS in ["linux"]:
			for user in users:
				output = console.execute(f"sudo deluser {user} {self.name}")
				if output != "":
					return Response.error(String(output.output.replace("deluser: ", "").replace("\n", ". ")).capitalized_word())
		...
		# success.
		return Response.success(f"Successfully deleted {len(users)} users from group [{self.name}].", {
			"users":users,
		})
	...
	def add_users(self, users=[]):
		...
		if OS in ["linux"]:
			for user in users:
				output = console.execute(f"sudo usermod -a -G {self.name} {user}")
				if not output.success: return output
				if output.output != "":
					return Response.error(String(output.output.replace("usermod: ", "").replace("\n", ". ")).capitalized_word())
		...
		# success.
		return Response.success(f"Successfully added {len(users)} users to group [{self.name}].", {
			"users":users,
		})
	...
	def check_users(self, users=[]):
		...
		# success.
		return Response.success(f"Successfully added {len(to_add)} & removed {len(to_delete)} users from group [{self.name}].")

...
class Service(Object):
	...
	def create(self):
		...
		if self.service.fp.exists(): 
			return Response.error(f"Service [{self.service.fp.path}] already exists (call service.check() instead).")

		# save.
		Response.log(f"&ORANGE&Root permission&END& required to save changes to [{self.service.fp}].", log_level=self.log_level)
		...
		# handler.
		if self.service.fp.exists(): 
			return Response.success(f"Successfully created service {self.id}.")
		else:
			return Response.error(f"Failed to create service {self.id}.")
	...
	def check(self):
		...
		# handler.
		if self.service.fp.exists(): 
			return Response.success(f"Successfully checked service {self.id}.")
		else:
			return Response.error(f"Failed to check service {self.id}.")
	...
	def delete(self):
		...
		# checks.
		if not self.service.fp.exists(): 
			return Response.error(f"Service [{self.service.fp.path}] does not already exist.")
		...
		# handler.
		if not self.service.fp.exists(): 
			return Response.success(f"Successfully deleted service {self.id}.")
		else:
			return Response.error(f"Failed to delete service {self.id}.")
	...
	def start(self):
		...
		if Defaults.vars.os in ["macos"]:
			...
			if not output.success: return output
				...
				return Response.success(f"Successfully started {self.id}.")
			else:
				return Response.error(f"Failed to start {self.id}.")
	...
	def stop(self):
		...
		if Defaults.vars.os in ["macos"]:
			...
			if not output.success: return output
				...
				return Response.error(f"Failed to stop {self.id}")
	...
	def restart(self):
		...
		if Defaults.vars.os in ["macos"]:
			...
			response = self.start()
			if not response.success: return response
			return Response.success(f"Successfully restarted {self.id}")
		...
		# systemd.	
			...
			if not output.success: return output
				...
				return Response.error(f"Failed to restarted {self.id}")
	...
	def status(self):
		...
		if Defaults.vars.os in ["macos"]:
			...
			if not output.success: return output
				...
				return Response.success(f"Successfully retrieved the status of {self.id}",{
	...
	def tail(self, global_=False, debug=False):
		...
		if Defaults.vars.os in ["macos"] and global_:
				data = console.execute(f"cat /var/log/system.log | grep {self.id}")
		...
		# handler.
		return Response.success(f"Successfully tailed the {self.id} logs.", {
			"logs":data,
		})

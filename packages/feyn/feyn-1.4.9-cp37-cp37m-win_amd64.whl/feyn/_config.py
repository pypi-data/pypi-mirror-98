...
def _load_from_ini_file(path, section_name) -> Config:
    ...
    if "url" in section:
        logger.warning(f"WARNING: The parameter `url` in configuration file: {path} is deprecated. Please use the `server` and `qlattice` parameters instead. Newer versions will not support this configuration.")

        if "/qlattice-" not in section["url"]:
            raise ValueError(f"The parameter `url` in configuration file: {path} is deprecated. Please use the `server` and `qlattice` parameters instead.")

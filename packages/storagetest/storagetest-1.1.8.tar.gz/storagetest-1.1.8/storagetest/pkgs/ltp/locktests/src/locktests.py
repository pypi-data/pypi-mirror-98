...
class Machine:
    ...
    def mkdir(self, dir_full):
        self.command = "mkdir -p " + dir_full
        self.do()

    def rmdir(self, dir_full):
        self.command = "rm -rf " + dir_full
        self.do()
...
class Client(Machine):
    ...
    def umount(self, dir_full):
        mntpoint = self.mountPath + "/" + dir_full
        self.command = "umount " + mntpoint
        self.do()

    def install(self, test_path):
        self.command = "'cd " + test_path + "; tar xzf " + SRC + "; cd locks;  make'"
        self.do()
    ...
    def cp(self, fichier, test_path):
        command = "scp " + fichier + " " + user + "@" + self.machine + ":" + test_path
        os.system(command)


class Server(Machine):

    def __init__(self, ip, export_path):
    ...
    def unexport(self, unexport_dir):
        pass

    def configure(self, test_dir):
        export_dir = self.export_path + '/' + test_dir
        self.mkdir(export_dir)
        # self.printc()
        self.export(export_dir)

    # self.printc()
    def clean(self, test_dir):
        unexport_dir = self.export_path + '/' + test_dir
        self.unexport(unexport_dir)
        self.rmdir(unexport_dir)
...
def usage():
    ...
    print(
        "--setup : setup the configuration, deploy test on other test machines; This option also requires -c and -s")
...
def run():
    ...
    nbre_clients = len(clients)
    hostname = socket.gethostname()
    # Lancement du serveur en local
    # Launch the server locally
    commande = path + "/" + app + " -n " + nbreProcess + " -f " + filename + " -c " + str(nbre_clients) + " &"
    os.system(commande)

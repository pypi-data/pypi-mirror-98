...
#pragma region Core Parameters
		...
		// desc = **Note**: refer to `Installation Guide <./Installation-Guide.rst#build-gpu-version>`__ to build GPBoost with GPU support
		...
		// desc = when you use the different seeds, different GPBoost versions, the binaries compiled by different compilers, or in different systems, the results are expected to be different
		...
		bool use_nesterov_acc = false;

		// desc = acceleration rate for momentum step in case Nesterov accelerated boosting is used (default = 0.5)
		double nesterov_acc_rate = 0.5;
...
#pragma region Learning Control Parameters
		...
		// desc = **Note**: when both ``force_col_wise`` and ``force_row_wise`` are ``false``, GPBoost will firstly try them both, and then use the faster one. To remove the overhead of testing set the faster one to ``true`` manually
		...
		// desc = ``0`` means disable bagging; ``k`` means perform bagging at every ``k`` iteration. Every ``k``-th iteration, GPBoost will randomly select ``bagging_fraction * 100 %`` of the data to use for the next ``k`` iterations
		...
		// check = >0.0
		// check = <=1.0
		// desc = GPBoost will randomly select a subset of features on each iteration (tree) if ``feature_fraction`` is smaller than ``1.0``. For example, if you set it to ``0.8``, GPBoost will select 80% of features before training each tree
		// desc = can be used to speed up training
		// desc = can be used to deal with over-fitting
		...
		// check = >0.0
		// check = <=1.0
		// desc = GPBoost will randomly select a subset of features on each tree node if ``feature_fraction_bynode`` is smaller than ``1.0``. For example, if you set it to ``0.8``, GPBoost will select 80% of features at each tree node
		// desc = can be used to deal with over-fitting
		...
		// desc = if set to ``true``, when evaluating node splits GPBoost will check only one randomly-chosen threshold for each feature
		// desc = can be used to speed up training
		// desc = can be used to deal with over-fitting
		...
		// desc = GPBoost allows you to provide multiple evaluation metrics. Set this to ``true``, if you want to use only the first metric for early stopping
		...
		// desc = limit number of split points considered for categorical features. See `the documentation on how GPBoost finds optimal splits for categorical features <./Features.rst#optimal-split-for-categorical-features>`_ for more details
		// desc = can be used to speed up training
		...
		// alias = verbose
		// desc = controls the level of GPBoost's verbosity
		...
		// desc = **Note**: can be used only in CLI version
		std::string output_model = "GPBoost_model.txt";
...
#pragma region Dataset Parameters
		...
		// desc = GPBoost will auto compress memory according to ``max_bin``. For example, GPBoost will use ``uint8_t`` for feature value if ``max_bin=255``
		int max_bin = 255;
		...
		// desc = set this to ``true`` to tell GPBoost to ignore the features that are unsplittable based on ``min_data_in_leaf``
		...
		// desc = by default, GPBoost will map data file to memory and load features from memory. This will provide faster data loading speed, but may cause run out of memory error when the data file is very big
...
#pragma region Predict Parameters
		...
		// [no-save]
		// desc = used only in ``prediction`` task
		// desc = control whether or not GPBoost raises an error when you try to predict on data with a different number of features than the training data
		...
		// desc = if ``true``, GPBoost will attempt to predict on whatever data you provide. This is dangerous because you might get incorrect predictions, but you could use it in situations where it is difficult or expensive to generate some features and you are very confident that they were never chosen for splits in the model
		...
		// desc = **Note**: can be used only in CLI version
		std::string output_result = "GPBoost_predict_result.txt";

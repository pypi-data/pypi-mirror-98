...
void SerialTreeLearner::GetDataLeafIndices(Tree* tree, int* data_leaf_index) const {
    CHECK(tree->num_leaves() <= data_partition_->num_leaves());
#pragma omp parallel for schedule(static)
    for (int i = 0; i < tree->num_leaves(); ++i) {
        data_size_t nb_data_leaf_i = 0;
        auto index_mapper = data_partition_->GetIndexOnLeaf(i, &nb_data_leaf_i);
        for (int j = 0; j < nb_data_leaf_i; ++j) {
            data_leaf_index[index_mapper[j]] = i;
        }
    }
}

...
void Config::CheckParamConflict() {
    if (boosting != std::string("gbdt") && use_nesterov_acc) {
        Log::Fatal("Nesterov accelerated boosting can currently only be done for 'boosting = \"gbdt\"'");
    }
  ...
  if (max_depth > 0) {
    ...
    if (full_num_leaves > num_leaves
      ...
      Log::Debug("Accuracy may be bad since you didn't explicitly set num_leaves OR 2^max_depth > num_leaves."
                   " (num_leaves=%d).",
                   num_leaves);
    }

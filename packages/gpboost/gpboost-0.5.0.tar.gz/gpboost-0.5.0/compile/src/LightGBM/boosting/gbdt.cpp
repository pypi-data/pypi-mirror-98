...
namespace LightGBM {
	...
	void GBDT::Init(const Config* config, const Dataset* train_data, const ObjectiveFunction* objective_function,
		...
		use_nesterov_acc_ = config_->use_nesterov_acc;
		momentum_schedule_version_ = config_->momentum_schedule_version;
...
#pragma omp parallel for schedule(static)
	...
	*/
			...
			if (config_->boost_from_average || (train_data_ != nullptr && train_data_->num_features() == 0)) {
				...
				if (std::fabs(init_score) > kEpsilon) {
					if (update_scorer) {
						...
						train_score_updater_->InitializeScoreLag1();
						for (auto& score_updater : valid_score_updater_) {
							score_updater->AddScore(init_score, class_id);
							score_updater->InitializeScoreLag1();
						}
					}
	...
	bool GBDT::TrainOneIter(const score_t* gradients, const score_t* hessians) {
		Log::Debug("Boosting iteration number: %d", iter_);
		...
		if (gradients == nullptr || hessians == nullptr) {
			...
			if (use_nesterov_acc_) {
				double mu = NesterovSchedule(iter_, momentum_schedule_version_, nesterov_acc_rate_, momentum_offset_);
				train_score_updater_->ApplyMomentumStep(mu);
				for (auto& score_updater : valid_score_updater_) {
					score_updater->ApplyMomentumStep(mu);
				}
			}
			if (!(objective_function_->HasGPModel()) || (iter_ == 0 && objective_function_->HasGPModel()) || use_nesterov_acc_) {
		...
		for (int cur_tree_id = 0; cur_tree_id < num_tree_per_iteration_; ++cur_tree_id) {
			...
			if (new_tree->num_leaves() > 1) {
				...
				//Do Newton update step for the tree leaves after the gradient step (only releveant for GPBoost algorithm, i.e. if objective_function_->HasGPModel()==true)
				if (objective_function_->HasGPModel() && leaves_newton_update_) {
					std::vector<int> data_leaf_index = std::vector<int>(num_data_);//leaf index of every data point
					tree_learner_->GetDataLeafIndices(new_tree.get(), data_leaf_index.data());
					std::vector<double> new_leaf_values(new_tree->num_leaves());
					objective_function_->NewtonUpdateLeafValues(data_leaf_index.data(), new_tree->num_leaves(), new_leaf_values.data());
			...
			if (objective_function_->HasGPModel() && !(objective_function_->UseGPModelForValidation() && use_nesterov_acc_)) {
				...
				//			it is enough to set the response data of re_model correctly to score - label = F_t - y
				Boosting();
			}
			// add model
		...
		if (!should_continue) {
			//Log::Debug("Stopped training because there are no more leaves that meet the split requirements");
			if (models_.size() > static_cast<size_t>(num_tree_per_iteration_)) {
				for (int cur_tree_id = 0; cur_tree_id < num_tree_per_iteration_; ++cur_tree_id) {
					models_.pop_back();
				}

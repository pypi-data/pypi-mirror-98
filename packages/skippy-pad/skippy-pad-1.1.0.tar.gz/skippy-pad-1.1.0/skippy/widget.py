...
class ToolbarWidget(QWidget):
    def __init__(self, parent):
        ...
        new_action = QAction(
            QIcon(
                os.path.join(skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "new.png")
            ),
            "New",
            self,
        )
        ...
        open_action = QAction(
            QIcon(
                os.path.join(skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "open.png")
            ),
            "Open",
            self,
        )
        ...
        upload_action = QAction(
            QIcon(
                os.path.join(
                    skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "upload.png"
                )
            ),
            "Upload",
            self,
        ...
        upload_as_action = QAction(
            QIcon(
                os.path.join(
                    skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "upload_as.png"
                )
            ),
            "Upload as...",
            self,
        ...
        close_action = QAction(
            QIcon(
                os.path.join(
                    skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "close.png"
                )
            ),
            "Close",
            self,
        ...
        load_session_action = QAction(
            QIcon(
                os.path.join(
                    skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "load_session.png"
                )
            ),
            "Load Session...",
            self,
        ...
        save_session_action = QAction(
            QIcon(
                os.path.join(
                    skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "save_session.png"
                )
            ),
            "Save Session as...",
            self,
        ...
        exit_action = QAction(
            QIcon(
                os.path.join(skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "exit.png")
            ),
            "Exit",
            self,
        )
        ...
        undo_action = QAction(
            QIcon(
                os.path.join(skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "undo.png")
            ),
            "Undo",
            self,
        )
        ...
        redo_action = QAction(
            QIcon(
                os.path.join(skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "redo.png")
            ),
            "Redo",
            self,
        )
        ...
        cut_action = QAction(
            QIcon(
                os.path.join(skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "cut.png")
            ),
            "Cut",
            self,
        )
        ...
        copy_action = QAction(
            QIcon(
                os.path.join(skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "copy.png")
            ),
            "Copy",
            self,
        )
        ...
        paste_action = QAction(
            QIcon(
                os.path.join(
                    skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "paste.png"
                )
            ),
            "Paste",
            self,
        ...
        toggle_theme_action = QAction(
            QIcon(
                os.path.join(
                    skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "toggle.png"
                )
            ),
            "Toggle Theme",
            self,
        ...
        login_action = QAction(
            QIcon(
                os.path.join(
                    skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "login.png"
                )
            ),
            "Login",
            self,
        ...
        logout_action = QAction(
            QIcon(
                os.path.join(
                    skippy.config.ASSETS_FOLDER, skippy.config.settings["theme"], "logout.png"
                )
            ),
            "Logout",
            self,
...
class UploadDialog(QDialog):
    def __init__(self, widget, parent=None):
        ...
        self.thread.started.connect(self.worker.run)
        self.worker.finished.connect(self.thread.quit)
        self.worker.progress.connect(setSites)

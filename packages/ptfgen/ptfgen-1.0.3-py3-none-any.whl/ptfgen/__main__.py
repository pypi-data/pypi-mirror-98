...
from .figma_cmd import FigmaCommand
...
@subcmd('ftextcss', help='create a mock class for a protocol/function')
def cmd_mock(parser, context, args):
    parser.usage = 'CSS Text Figma'
    parser.description = 'CSS Text Figma'
    parser.add_argument(
        '-p', '--print',
        required=False,
        action='store_true',
    ...
    args = parser.parse_args(args)
    css_text = pasteboard_read()
    FigmaCommand(css_text).create_text(args.print)

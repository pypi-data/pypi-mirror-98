...
from nomad.datamodel.metainfo.common_dft import Run, Method, MethodBasisSet,\
    BasisSetCellDependent, XCFunctionals, System, SingleConfigurationCalculation,\
    ScfIteration, Dos, Eigenvalues, SamplingMethod
...
class AbinitOutParser(TextParser):
    def __init__(self):
        self.energy_components = {
            ...
            'internal': '>>>>> Internal E',
            'ktentropy': r'\-kT*entropy',
            'total': ' >>>>>>>>> Etotal',
            'band': r'Band energy \(Ha\)'}
    ...
    def input_vars(self):
        if self._input_vars is None:
            ...
            for i in range(len(key_val)):
                ...
                m_quantity = m_env.all_definitions_by_name.get(
                    'x_abinit_var_%s' % key, [None])[0]
...
class AbinitParser(FairdiParser):
    ...
    def parse_system(self, dataset, section):
        ...
        sec_system.atom_positions = atom_positions
    ...
    def parse_scc(self, dataset, section):
        ...
        if stress_tensor is not None:
            sec_scc.stress_tensor = stress_tensor

        nsppol = self.out_parser.get_input_var(
        ...
        for key in self.out_parser.energy_components.keys():
            ...
            if energy is not None:
                # no units in metainfo
                setattr(sec_scc, 'x_abinit_energy_%s' % key, energy.to('joule').magnitude)
    ...
    def parse_method(self, dataset):
        ...
        sec_method.number_of_spin_channels = int(self.out_parser.get_input_var('nsppol', nd, 1))
        sec_method.scf_max_iteration = int(self.out_parser.get_input_var('nstep', nd, 1))
        ...
        sec_method_basis_set.mapping_section_method_basis_set_cell_associated =\
            sec_run.section_basis_set_cell_dependent[-1]
        ...
        for xc_functional in xc_functionals:
            ...
            for key, value in sorted(xc_functional.items()):
                setattr(sec_XC_functionals, key, value)
    ...
    def parse_sampling_method(self, dataset):
        ...
        if ionmov in [2, 3, 4, 5, 7, 10, 11, 20] or (ionmov == 1 and vis > 0.0):
            ...
            sec_sampling_method.geometry_optimization_method = geometry_optimizations[ionmov]
    ...
    def parse(self, filepath, archive, logger):
        ...
        section_run_keys = [
            'program_version', 'x_abinit_parallel_compilation', 'program_compilation_host',
            'x_abinit_start_date', 'x_abinit_start_time', 'x_abinit_input_file',
            'x_abinit_output_file', 'x_abinit_input_files_root', 'x_abinit_output_files_root',
            'x_abinit_total_cpu_time', 'x_abinit_total_wallclock_time']

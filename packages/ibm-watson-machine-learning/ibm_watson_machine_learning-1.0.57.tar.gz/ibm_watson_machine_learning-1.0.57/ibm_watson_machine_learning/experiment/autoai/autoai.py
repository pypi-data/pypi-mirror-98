...
from ibm_watson_machine_learning.utils.autoai.utils import check_dependencies_versions, \
    validate_additional_params_for_optimizer, is_list_composed_from_enum
...
class AutoAI(BaseExperiment):
    ...
    _ForecastingAlgorithms = ForecastingAlgorithms
    ...
    def optimizer(self,
        ...
        >>>    )
        """
        # Hidden documentation - to be recovered when TS will be officially supported:
        ...
        ## To be removed when TS enabled oficially:
        if prediction_type == PredictionType._FORECASTING \
            and ('ts_enabled' not in self._workspace.wml_client.wml_credentials
                 or not self._workspace.wml_client.wml_credentials['ts_enabled']):
            raise NotImplementedError()

        prediction_columns = kwargs.get('prediction_columns')
        timestamp_column_name = kwargs.get('timestamp_column_name')

        backtest_num = kwargs.get('backtest_num')
        lookback_window = kwargs.get('lookback_window')
        forecast_window = kwargs.get('forecast_window')
        backtest_gap_length = kwargs.get('backtest_gap_length')

        try:
            del kwargs['prediction_columns']
        except:
            pass
        try:
            del kwargs['timestamp_column_name']
        except:
            pass

        try:
            del kwargs['backtest_num']
        except:
            pass
        try:
            del kwargs['lookback_window']
        except:
            pass
        try:
            del kwargs['forecast_window']
        except:
            pass
        try:
            del kwargs['backtest_gap_length']
        except:
            pass
        ...
        for (sequence, enum_class) in (
                (daub_include_only_estimators,
                 AutoAI.RegressionAlgorithms if prediction_type == AutoAI.PredictionType.REGRESSION
                    else AutoAI.ClassificationAlgorithms),
                (cognito_transform_names, AutoAI.Transformers)):

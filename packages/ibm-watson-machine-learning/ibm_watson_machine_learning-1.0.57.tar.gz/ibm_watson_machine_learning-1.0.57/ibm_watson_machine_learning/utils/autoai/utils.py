...
def create_summary(details: dict, scoring: str) -> 'DataFrame':
    ...
    for pipeline in details['entity']['status'].get('metrics', []):
        ...
        if pipeline['context']['phase'] == model_phase:
            if is_ml_metrics:
                enhancements = ', '.join(pipeline['context']['intermediate_model']['composition_steps'])
            elif is_ts_metrics:
                enhancements = 'HPO, FE'
...
def validate_additional_params_for_optimizer(params):
    expected_params = [
        ...
        'enabled_feature_engineering_as_json', 'fairness_info', 'prediction_columns', 'timestamp_column_name',
        'backtest_num', 'forecast_window', 'lookback_window', 'backtest_gap_length']
...
def is_list_composed_from_enum(sequence: List[Union[str, enum.Enum]],
    ...
    enum_class: Union[object, enum.Enum, List[enum.Enum]
        Class for which validation will be performed.
        It can be a class inheriting from enum.Enum or class with defined only defined class
        attributes.

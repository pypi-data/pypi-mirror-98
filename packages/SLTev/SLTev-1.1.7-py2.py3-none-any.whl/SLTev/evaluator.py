...
def evaluator(
    ...
    if simple == "False":
        print(
            "avg      TokenCount    reference*            ", int(avergae_refs_words)
        )  # ---- avg WordCount tt* 12345
    count = 1
    sum_sentences = 0
    ...
    if simple == "False":
        ...
        print(
            ...
            str(int(sum_sentences / len(references))),
        )  # ---- avg SentenceCount tt* 220
    MT = read_MT(mt)
    if time_stamp == "True":
        if ostt_state == 0:
            OStt = read_ostt(ostt)
    ...
    if simple == "False" and time_stamp == "True":
        ...
        print(
            "avg      Delay         T                     ",
            str("{0:.3f}".format(round((delay / avergae_refs_words), 3))),
        )
        print("tot      MissedTokens  T                     ", missing_words)
        try:
            temp_folder = "./" + str(uuid.uuid4())
            delay, missing_words, mWERQuality = evaluate_segmenter(
                Ts, MT, MovedWords, language, SLTev_home, temp_folder
            )
            print(
                "tot      Delay         W                     ",
                str("{0:.3f}".format(round(delay, 3))),
    ...
    if simple == "False":
        try:
            temp_folder = "./" + str(uuid.uuid4())
            sacre_score = calc_bleu_score_sentence_by_sentence(
                references, MT, language, SLTev_home, temp_folder
            )
            print(
    ...
    if simple == "False" and time_stamp == "True":
        try:
            c_b_s_s_by_time, avg_SacreBleu = calc_bleu_score_sentence_by_time(
                Ts, MT, b_time
            )
    ...
    if simple == "False" and time_stamp == "True":
        print(
            ...
            str("{0:.3f}".format(round(calc_average_flickers_per_sentence(MT), 3))),
        )

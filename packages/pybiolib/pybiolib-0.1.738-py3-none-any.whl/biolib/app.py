...
from biolib.compute_node.parent.compute_node import start_flask_compute_node
import http.server
import socketserver
import threading
import base64
...
class BioLib:
    ...
    @staticmethod
    def build_enclave():
        compute_node_dir = f'{BioLib.get_pybiolib_root()}/compute_node'
        output_eif_path = f'{os.getcwd()}/biolib-nitro-enclave.eif'

        if os.path.exists(output_eif_path):
            os.remove(output_eif_path)
        
        subprocess.run(['docker', 'build', f'{compute_node_dir}/enclave/', '-t', 'biolib-nitro-enclave'])
        subprocess.run(['nitro-cli', 'build-enclave', '--docker-uri', 'biolib-nitro-enclave:latest', '--output-file', output_eif_path])

    @staticmethod
    def start_compute_node(eif_path, port):
        if eif_path:
            # Start and terminate enclave 
            subprocess.run(['nitro-cli', 'run-enclave', '--cpu-count', '2', '--memory', '8192', '--eif-path', eif_path])
            ...
            subprocess.run(['nitro-cli', 'terminate-enclave', '--enclave-id', f'{enclave_id}'])

            start_flask_compute_node(port=port, eif_path=eif_path)

...
class KatexPreprocessor(Preprocessor):
    ...
    def _make_tag_for_block(self, block_lines):
        ...
        marker_tag = '<p id="tmp_md_katex{0}">katex{1}</p>'.format(marker_id,
            marker_id)
        math_html = md_block2html(block_text, self.ext.options)
        tag_text = '<p>{0}</p>'.format(math_html)
    ...
    def _make_tag_for_inline(self, inline_text):
        ...
        marker_tag = '<span id="tmp_md_katex{0}">katex{1}</span>'.format(
            marker_id, marker_id)

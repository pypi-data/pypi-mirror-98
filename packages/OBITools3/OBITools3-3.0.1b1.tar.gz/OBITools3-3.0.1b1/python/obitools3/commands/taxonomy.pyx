...
#                       default=None,
#                       help="Enables to restrict the search of taxids under a specified ancestor.")
    
#    group.add_argument('-c', '--create-new-taxids',
#                     action="store_true", dest="addtaxids:create_new_taxids",
#                     default=False,
#                     help="Create new (local) taxids when can't find corresponding taxid for a taxon. "
#                          "If a match was found with option '-g', the taxon is added under this match. "
#                          "Otherwise if <RESTRICTING_ANCESTOR> is defined, it is added under it.")
    ...
    try:
        ...
        for i in range(len(o_view)):
            ...
            if taxon is not None:
                ...
                taxon = taxo[taxon_name_sp[0]]
                if taxon is not None:
                    parent_taxid_column[i] = taxon.taxid
                    print("Found parent taxon for", tostr(taxon_name))
#                elif config['addtaxids']['create_new_taxids']: # add new taxid if chosen in options
#                    taxid = MIN_LOCAL_TAXID
#                    while taxid in taxo:
#                        taxid+=1
#                    taxid_column[i] = taxid
#                    print("Added local taxid for", tostr(taxon_name))
#                else:
#                    print("No taxid found for", tostr(taxon_name))

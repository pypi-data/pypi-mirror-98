    ...
    for variable in variable_set:
        ...
        if len(variable_pieces) == 4:
            ...
            if default_value == '':
                default_value = None
        variables.append(
            dict(name=clean_name, display_name=display_name.split('.')[-1], default_value=default_value, type=data_type,
                 allows_list=allows_list,
                 locations=locate_string(s=variable, json=query_ast)[0]))
    return variables
...
def get_select_columns(select):
    ...
    def extract_column_name(i):
        ...
        if isinstance(i, str):
            ...
            name = i.get('name', i['value'])
        if isinstance(name, dict):
            if not any(i in name.keys() for i in ['literal']):
                name = extract_column_name(list(name.values())[0])
            else:
                name = name['literal']
        return name
    ...
    if isinstance(select, list):
        for i in select:
            result.append(extract_column_name(i))
    else:
        result.append(extract_column_name(select))
    return result

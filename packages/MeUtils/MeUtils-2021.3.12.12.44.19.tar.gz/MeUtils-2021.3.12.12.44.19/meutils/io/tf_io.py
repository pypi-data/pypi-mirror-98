...
# 文件复制到本地读写：更快更方便
def read_hdfs(pattern, reader=pd.read_csv):
    """支持多文件读取"""
    return pd.concat(map(reader, cp(pattern)), ignore_index=True)


def to_hdfs(df, file, output_dir, writer=lambda df, file: df.to_csv(file)):
    writer(df, file)
    cp(file, _process_hdfs_path(output_dir))

...
import json
import csv
from mobs.utils import exceptions
...
class FileUtils(object):
    ...
    def _check_format(filePath, content):
        ...
        if not content:
            ...
            raise exceptions.FileFormatError(errMsg)
    ...
    def load_file(filePath):
        if not os.path.isfile(filePath):
            raise exceptions.FileNotFound(u"File is not exist \"{}\"".format(filePath))
        ...
        if file_suffix == '.txt':
            ...
            raise exceptions.FileFormatNotSupported(errMsg)
    ...
    def load_folder_files(folderPath: Text, recursive: bool = True) -> List:
        ...
        if not os.path.isdir(folderPath):
            raise exceptions.FolderNotFound(u"Folder does not exist \"{}\"".format(folderPath))
...
class ReadExcel:
    ...
    def __init__(self,excel_file):
        if not os.path.isfile(excel_file):
            # file path not exist
            raise exceptions.CSVNotFound(excel_file)
...
if __name__ == '__main__':
    path = r"D:\CodeBase\mobs\test\test_data\test_01.txt"
    a = FileUtils()
    data = a.load_file_name(path)
    print(data)
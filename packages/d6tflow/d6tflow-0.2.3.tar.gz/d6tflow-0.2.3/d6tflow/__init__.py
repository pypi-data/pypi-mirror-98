...
class WorkflowMulti(object):
    ...
    def __init__(self, flow_params, default=None):
        self.flow_params = flow_params
        self.default_task = default
        if flow_params is not None:
            self.flows = {k: Workflow(default=default, params=v) for k, v in self.flow_params.items()}
    ...
    def run(self, tasks=None, forced=None, forced_all=False, forced_all_upstream=False, confirm=True, workers=1, abort=True, execution_summary=None, flow=None, **kwargs):
        tasks = self.get_task(tasks)
        if flow is not None:
            return self.flows[flow].run(tasks=tasks, forced=forced, forced_all=forced_all,
                                           forced_all_upstream=forced_all_upstream, confirm=confirm, workers=workers,
                                           abort=abort,
                                           execution_summary=execution_summary, **kwargs)
        result = {}
        for exp_name in self.flow_params.keys():
            result[exp_name] = self.flows[exp_name].run(tasks, forced, forced_all, forced_all_upstream,
    ...
    def outputLoad(self, task_cls=None, keys=None, as_dict=False, cached=False, flow=None):
        if flow is not None:
            return self.flows[flow].outputLoad(task_cls, keys, as_dict, cached)
        data = {}
        for exp_name in self.flow_params.keys():
            data[exp_name] = self.flows[exp_name].outputLoad(task_cls, keys, as_dict, cached)
        return data

    def outputLoadAll(self, task_cls=None, keys=None, as_dict=False, cached=False, flow=None):
        if flow is not None:
            return self.flows[flow].outputLoadAll(task_cls, keys, as_dict, cached)
        data = {}
        for exp_name in self.flow_params.keys():
            data[exp_name] = self.flows[exp_name].outputLoadAll(task_cls, keys, as_dict, cached)
        return data
    ...
    def preview(self, tasks=None, indent='', last=True, show_params=True, clip_params=False, flow=None):
        ...
        if not isinstance(tasks, (list,)):
            tasks = [tasks]
        if flow is not None:
            return self.flows[flow].preview(tasks)
        data = {}
        for exp_name in self.flow_params.keys():
            data[exp_name] = self.flows[exp_name].preview(tasks = tasks, indent = indent, last = last, show_params=show_params, clip_params=clip_params)
        return data

    def set_default(self, task):
        self.default_task = task
        for exp_name in self.flow_params.keys():
            self.flows[exp_name].set_default(task)

...
:LastEditTime: 2021-03-04 18:23:53
...
class LoggerStorageHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        :last_editors: ChenXiaolei
        """
        request_code = self.request_code if hasattr(self,
                                                    "request_code") else ""
        ...
        if self.storage_engine == "mysql":
            try:
                ...
                self.mysql_db.fetch_and_commit(
                    f"INSERT INTO {self.table_name}(request_code,project_name,level,host_ip,record_time,record_timestamp,log_msg{extra_sql_field}) VALUE('{request_code}','{project_name}','{level}','{host_ip}','{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(record_time))}',{record_time},'{pymysql.converters.escape_string(log_msg)}'{extra_sql_value});"
                )
            except:
            ...
            try:
                ...
                self.clickhouse_client.execute(
                    f"INSERT INTO {self.table_name}(request_code,project_name,level,host_ip,record_time,log_msg{extra_sql_field}) VALUES('{request_code}','{project_name}','{level}','{host_ip}',{record_time},'{pymysql.converters.escape_string(log_msg)}'{extra_sql_value});"
                )
            except:

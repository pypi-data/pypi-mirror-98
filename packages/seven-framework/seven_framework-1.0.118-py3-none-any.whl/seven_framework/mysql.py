...
class MySQLHelper(object):
    ...
    def _add_sql_log(self,log,action_mode="sql"):
        """
        :description: 添加日志
        :param log:日志内容
        ...
        """
        try:
            if hasattr(self, "_log_sql") and self._log_sql:
                if type(log)==int:
                    log=str(log)
                elif type(log)!=str:
                    log = json.JsonHelper.json_dumps(log)
                self._log_sql.info(log,extra={"extra":{"action_mode":action_mode}})
        except:
    ...
    def query(self, sql, params=None):
        ...
        self.close()
        self._add_sql_log(record_list,"data")
        return record_list
    ...
    def update(self, sql, params=None):
        ...
        self.close()
        self._add_sql_log(row_count,"data")
        return row_count
    ...
    def insert(self, sql, params=None):
        ...
        self.close()
        self._add_sql_log(primary_id,"data")
        return primary_id

    def _execute(self, sql, params=None):
        try:
            sql_info = f"sql:[{sql}] params:[{params if type(params)==str else json.JsonHelper.json_dumps(params)}]"
            self._add_sql_log(sql_info,"sql")
        except:
            print(traceback.print_exc())
            pass

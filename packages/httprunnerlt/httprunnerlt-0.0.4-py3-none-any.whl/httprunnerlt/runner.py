...
from httprunnerlt import utils, exceptions
from httprunnerlt.client import HttpSession
from httprunnerlt.exceptions import ValidationFailure, ParamsError
from httprunnerlt.ext.uploader import prepare_upload_step
from httprunnerlt.loader import load_project_meta, load_testcase_file
from httprunnerlt.parser import build_url, parse_data, parse_variables_mapping
from httprunnerlt.response import ResponseObject
from httprunnerlt.testcase import Config, Step
from httprunnerlt.utils import merge_variables
from httprunnerlt.models import (
    TConfig,
    TStep,
    VariablesMapping,
    StepData,
...
class HttpRunner(object):
    ...
    def __run_step_request(self, step: TStep) -> StepData:
        ...
        try:
            ...
            if hasattr(self.__session, "data"):
                # httprunnerlt.client.HttpSession, not locust.clients.HttpSession
    ...
    def __run_step_testcase(self, step: TStep) -> StepData:
        ...
        if hasattr(step.testcase, "config") and hasattr(step.testcase, "teststeps"):
            ...
            testcase_c = testcase_cls().with_session(self.__session).with_case_id(self.__case_id).with_variables(
                step_variables).with_export(step_export)
            for x in step_validators:
                testcase_c.raw_testcase.teststeps[0].validators.append(x)
            case_result = (
    ...
    def test_start(self, param: Dict = None) -> "HttpRunner":
        ...
        self.__log_path = self.__log_path or os.path.join(
            self.__project_meta.RootDir, "logs",
            f"{self.config.name +time.strftime('%Y%m%d %H%M%S', time.localtime())+ self.__case_id}" + ".run.log"
        )

...
from httprunnerlt import __description__, __version__
from httprunnerlt.compat import ensure_cli_args
from httprunnerlt.ext.har2case import init_har2case_parser, main_har2case
from httprunnerlt.make import init_make_parser, main_make
from httprunnerlt.scaffold import init_parser_scaffold, main_scaffold
from httprunnerlt.utils import init_sentry_sdk
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        if sys.argv[1] in ["-V", "--version"]:
            ...
            # httprunnerlt startproject
            sub_parser_scaffold.print_help()
        elif sys.argv[1] == "har2case":
            # httprunnerlt har2case
...
def main_hrun_alias():
    """ command alias
        hrun = httprunnerlt run
    """
    if len(sys.argv) == 2:
        if sys.argv[1] in ["-V", "--version"]:
            # hrun -V
            sys.argv = ["httprunnerlt", "-V"]
        elif sys.argv[1] in ["-h", "--help"]:
            pytest.main(["-h"])
            sys.exit(0)
        else:
...
def main_har2case_alias():
    """ command alias
        har2case = httprunnerlt har2case
    """

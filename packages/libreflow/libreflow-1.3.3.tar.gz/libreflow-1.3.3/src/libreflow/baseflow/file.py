...
class PublishIntoFile(PublishFileAction):
    ...
    def check_file(self, file):
        ...
        if not file or file.format.get() != expected_format:
            self.message.set((
                f"{msg}<font color=#D5000D>"
                f"Target file must be in {expected_format} format."
                "</font>"
            ))
            return False
            ...
            self.message.set((
                f"{msg}<font color=#D5000D>"
                f"Target file has no revision available on current site."
                "</font>"
            ))
            return False
    ...
    def child_value_changed(self, child_value):
        ...
        if child_value is self.target_file:
            ...
            self.revision_name.set(self.source_revision_name.get())
            self.comment.set("Created from %s (%s)" % (
                self._file.display_name.get(),
                self.source_revision_name.get(),
            ))
...
class CreateWorkingCopyFromFile(CreateWorkingCopyBaseAction):
    ...
    def get_buttons(self):
        ...
        if self._file.has_working_copy(from_current_user=True):
            msg += (
                ...
                "</font>" % self._file.display_name.get()
            )
    ...
    def check_file(self, file):
        ...
        if not file or file.format.get() != expected_format:
            self.message.set((
                f"{msg}<font color=#D5000D>"
                f"Source file must be in {expected_format} format."
                "</font>"
            ))
            return False
        elif not self.source_revision_name.choices():
            self.message.set((
                f"{msg}<font color=#D5000D>"
                f"Source file has no revision available on current site."
                "</font>"
            ))
            return False

...
class HitAndRunSampler
    : public MarkovChain<
          HitAndRunSampler<Scalar, SpaceDescriptor, ChordSampler, Logger>,
          Matrix<Scalar>,
          Matrix<Scalar>,
          Logger> {
public:
     ...
     * simulation.
     */
    HitAndRunSampler(const SpaceDescriptor<Scalar>& descriptor,
        ...
        : Base(logger),
          ...
          space_descriptor_Fd(
              descriptor.get_optimally_reparametrized_descriptor("isotropy")),
          space_descriptor_Fi(
              descriptor.get_optimally_reparametrized_descriptor("intersection")),
    ...
    const ChordSampler<Scalar>& get_chord_sampler() const
    {
        return chord_sampler_Fs;
    }
...
protected:
    ...
    InternalState next_state(const InternalState& state) override
    {
        ...
        auto directions_Fd =
            sampling_helper.get_random_directions<Scalar>(state.rows(), state.cols());

        // 2) Find the intersections between the current rays and the space
        //    descriptor in the Fi frame.
        RaysPacket<Scalar> rays_Fi = {
            state, transforms_.transform_directions_from_Fd_to_Fi(directions_Fd)};
        const auto intersections = space_descriptor_Fi.intersect(rays_Fi);

        // 3) Sample new points along the resulting chords in the Fs frame. Then
        //    compute the new states in the internal frame (i.e. Fi).
        RaysPacket<Scalar> rays_Fs =
            transforms_.transform_rays_from_Fi_to_Fs(rays_Fi, directions_Fd);
        Vector<Scalar> t =
            chord_sampler_Fs.sample1d(rays_Fs, intersections, sampling_helper);

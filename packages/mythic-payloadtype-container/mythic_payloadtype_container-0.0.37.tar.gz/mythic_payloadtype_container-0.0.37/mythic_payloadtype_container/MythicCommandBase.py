...
class CommandAttributes():
    ...
    def to_json(self):
        ...
        if self.supported_os is not None:
            r["supported_os"] = self.supported_os
        else:
            r["supported_os"] = []
        return r
...
class CommandBase(metaclass=ABCMeta):
    ...
    def to_json(self):
        ...
        return {
            ...
            "is_link": self.is_link if self.is_link is not None else False,
            "author": self.author,
            "attack": [{"t_num": a} for a in self.attackmapping],
            "parameters": params,
            "supported_os": ",".join([x.value for x in os_options]),
            "opsec": opsec,

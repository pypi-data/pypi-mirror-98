...
class Raster(object):
    ...
    def __iter__(self):
        generator = tile_generator(
            self.ds, self.optimize_blocksize, self.blocksize
        )
    ...
    def copy(self):
        return Raster(
            DRIVER_GDAL_MEM.CreateCopy(loc_dir(mem=True, add=True), self.ds)
        )
    ...
    def get_extent(self):
        ...
        maxx = (
            transform[0]
            + self.columns * transform[1]
            + self.rows * transform[2]
        )

        miny = (
            transform[3]
            + self.columns * transform[4]
            + self.rows * transform[5]
        )
    ...
    def polygonize(
        ...
        layer_vector = ds_vector.CreateLayer(
            layer_name, srs=self.spatial_reference
        )
    ...
    def align(
        self, template=None, fill_value=None, nodata_align=True, quiet=True
    ):
    ...
    def idw(
        ...
        Params:
            Search Distance: how many pixels are taken into account by
            smoothing
        ...
        while nan_count > 0:
            ...
            if not quiet:
                print(
                    "IDW iteration", idw_iterations, "nodata count", nan_count
                )
    ...
    def read_point(self, point: ogr.Geometry, nodata_value=-9999):
        ...
        try:
            array = self.band.ReadAsArray(int(j), int(i), 1, 1).astype(
                np.float
            )
            return array[0][0]
        except:
    ...
    def write(
        ...
        if not geotransform and not hasattr(self, "geotransform"):
            raise ValueError("Please set a geotransform")

        if overwrite:
            if os.path.exists(filename):
                DRIVER_GDAL_GTIFF.Delete(filename)

        ds = DRIVER_GDAL_GTIFF.CreateCopy(
            filename, self.ds, options=WRITE_OPTIONS
        )
        ...
        ds.SetProjection(SpatialReference.from_epsg(epsg).wkt)
...
def optimize_blocksize(dataset, target_pixels=1000000):
    ...
    while total_block_pixels < target_pixels:
        ...
        if total_block_pixels > total_raster_pixels:
            # print("Went over total raster pixels")
...
def tile_generator(
    ...
    for x, y, cols, rows in tile_list(
        dataset, band_number, optimize, blocksize
    ):
...
def align(
    raster, template=None, fill_value=None, nodata_align=True, quiet=True
):
    """
    ...
    if col_dif > 0:
        ...
        raster_array = raster_array[
            :, 0 : raster_array.shape[1] - abs(int(col_dif))
        ]
    ...
    if row_dif > 0:
        ...
        raster_array = raster_array[
            0 : raster_array.shape[0] - abs(int(row_dif)) :, :
        ]

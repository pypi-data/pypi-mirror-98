...
TODO:
    1. Script should also work without the external dependencies of rtree and tqdm
    2. Fids should be derived easier/faster
...
class Vector(object):
    ...
    def add(
        ...
        self.layer, fid = add_feature(
            self.layer, self.layer_defn, geometry, items, fid
        )
    ...
    def delete(
        self, feature, update_table=False, update_index=False, quiet=True
    ):
    ...
    def write(self, path, overwrite=False):
        """should be deprecated  however, were are not sure why write_copy
    ...
    def spatial_filter(
        self, geometry, method="intersect", filter=False, quiet=True
    ):
    ...
    def filter(self, return_fid=False, **filtering):
        ...
        for i, (field_key, field_value) in enumerate(filtering.items()):
            ...
            if i == 0:
                ...
                loop_table_values = [
                    lookup_ordered(fid, table_fids, table_values)
                    for fid in loop_fids
                ]

            loop_fids = lookup_unordered(
                field_value, loop_table_values, loop_fids
            )
        fids = set(loop_fids)
    ...
    def clip(self, vector, quiet=True):
        """ clips using a vector or a geometry"""
        return Vector.from_ds(
            clip(self.layer, vector.layer, quiet, self.has_rtree)
        )
    ...
    def multipart2singlepart(self):
        return Vector.from_ds(multiparts_to_singleparts(self.layer))
...
def create_index(layer, quiet=True):
    ...
    for feature in layer:
        if feature:
            ...
            print("Skipping feature with id", feature.GetFID())
        pbar.update(quiet)
    return index
...
def add_feature(layer, layer_defn, geometry, attributes, fid=-1):
    ...
    We also form it into a feature here to use __set__ option of the feature
    """

    feature = Feature(ogr.Feature(layer_defn), layer, set_feature=False)

    # Geomtry, fids and attributes
    feature.fid = fid
    ...
    try:
        ...
        raise RuntimeError(
            "error:", e, "geom:", geometry, "attributes:", attributes
        )
    finally:
        fid = feature.GetFID()
        feature = None
...
def polygon_to_lines(in_layer: ogr.Layer, quiet=True):
    ...
    out_datasource, out_layer = mem_layer(
        "poly2lines", in_layer, ogr.wkbLineString
    )
    out_layer_defn = out_layer.GetLayerDefn()
                ...
                if diff_part_type == ogr.wkbGeometryCollection:
                    for part in difference:
                        if part.GetGeometryType() == in_layer_geom_type:
                            add_feature(
                                out_layer, in_layer_defn, part, in_items
                            )

...
def main():
    ...
    tree_group.add_argument('-t', '--tree_file', help="input tree in newick format (must be rooted).",
    ...
    prediction_group.add_argument('-c', '--ci_computation', action='store_true', required=False,
                                  help="By default (without --ci_computation option), paramdeep outputs point estimates"
                                       " for each parameter. With --ci_computation option turned on, paramdeep computes"
                                       " and outputs 95%% confidence intervals (2.5%% and 97.5%%) for each estimate"
                                       " using approximated parametric bootstrap.")

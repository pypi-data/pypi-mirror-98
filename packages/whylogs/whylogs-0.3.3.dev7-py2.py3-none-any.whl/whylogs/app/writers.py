...
from whylabs_client import WhyLabsClient
...
class WhyLabsWriter(Writer):
    def __init__(self, output_path: str):
        ...
        self._default_dataset_id = output_path
        self._client = WhyLabsClient()
    ...
    def _write_protobuf(self, profile: DatasetProfile):
        ...
        try:
            ...
            profile.write_protobuf(profile_path)

            dataset_timestamp = profile.dataset_timestamp or datetime.now(tz=timezone.utc)
            dataset_id = profile.tags.get('datasetId')
            if dataset_id is None:
                self._logger.debug('Dataset ID is not specified, fall back to the default: %s',
                                   self._default_dataset_id)
                dataset_id = self._default_dataset_id
            if dataset_id is None:
                raise ValueError(
                    'Please specify default dataset ID (WHYLABS_DEFAULT_DATASET_ID) or tag the value with "datasetId" '
                    'tag')
            self._logger.debug('Start uploading data for %s', dataset_id)
            upload_response = self._client.log(
                dataset_id,
                dataset_timestamp=dataset_timestamp,
                file=profile_path
            )
            self._logger.debug('Uploaded response: %s', upload_response)
        finally:
...
def writer_from_config(config: WriterConfig):
    ...
    if config.type == "local":
        ...
        return WhyLabsWriter(config.output_path)
    else:

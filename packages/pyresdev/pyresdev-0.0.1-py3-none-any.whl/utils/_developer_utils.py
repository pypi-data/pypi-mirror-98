...
def set_dataframe_columntypes(df):
    ...
    """
    logging.info("Changing data types")
...
def clean_previous_status(df, status_to_clean):
    ...
    logging.info(f"Cleaning invalid previous status ( {status_to_clean})")
...
def clean_rows_with_na(df,cols_to_clean_na):
    ...
    Returns:
        df [dataframe]: [dataframe with the leads]
    """
    # Logging  amount of rows
    nrows_pre = df.shape[0]
    logging.info( f"Original rows: {nrows_pre}" )

    for col in cols_to_clean_na:
        if col in df:
            nas_sum = df[col].isna().sum()
            logging.info(f"NAs in {col}: {nas_sum}")
            df = df[df[col].notna()]
    nrows_post = df.shape[0]
    percentage_eliminated = (nrows_pre - nrows_post) / nrows_pre * 100
    logging.info( f" Rows after deleting rows with NA: {nrows_post}, Percentage: {percentage_eliminated}" )
...
def fill_na_with_unknown(df, columns):
    ...
    for col in columns:
        if col in df.columns:
            logging.info(f'filling NA in {col} with "unknown" ')
...
def create_specialties_flag(df):
    ...
    Returns:
        df ([dataframe]): Dataframe with leads
    """
    logging.info("Creating Specialties flag")
...
def set_bv_target_column(df):
    if('BV' in df.columns):
        logging.info('Setting BV status target ')
        df = df[df.BV.notna()]
        df['BV']=df['BV'].astype('float32')
    else:
        logging.error('No BV column found')
...
def set_bv_discrete_target_column(df,bins):
    if('BV' in df.columns):
        logging.info('Setting BV status target for classification ')
...
def create_company_years_column(df):
    ...
    logging.info("Creating company years category")
...
def fill_seniority_column(df):
    ...
    """
    logging.info("Filling Seniority from some Titles")
...
def filter_countries(df, countries):
    logging.info(f'Filtering the following countries: {countries}')
...
def impute_nas_and_save_imputer(df,columns_to_impute):
    ...
    logging.info(f"Imputing NAs in {columns_to_impute}")
    logging.info(f"NAs in DataFrame before imputation:")
    nna = df_impute.isna().sum()
    logging.info(f"{nna}")

    logging.info("No imputer provided, instancing and fitting a new one")
    ...
    imp.fit(df_impute)
    logging.info("Creating new imputer")
    df_impute = pd.DataFrame(data=imp.transform(df_impute),
                             columns=df_impute.columns)

    logging.info(f"NAs in DataFrame after imputation:")
    nna = df_impute.isna().sum()
    logging.info(f"{nna}")
...
def log_transformation(df, columns):
    ...
    for column in columns:
        logging.info(f'Log Transforming : {column}')
...
def create_month_status(df):
    logging.info("Creating Month of the date column")
...
def clean_mxtype(df,mxtypes):
    ...
    logging.info('Defining MXTypes')
    logging.info(f'legal MXTypes: {mxtypes}')
    df.loc[~df['MXtype'].isin(mxtypes), 'MXtype'] = 'Others'
    logging.info(f'{df.MXtype.value_counts()}')
    return df
...
def clean_mxtype_using_dictionary(df,mx_bv_dict,mx_prob_dict):
    ...
    logging.info(f'{df.mx_bv.value_counts()}')
    df['mx_prob'] = df['MXtype']
    df = df.replace( {'mx_prob': mx_prob_dict} )
    logging.info(f'{df.mx_prob.value_counts()}')
    return df

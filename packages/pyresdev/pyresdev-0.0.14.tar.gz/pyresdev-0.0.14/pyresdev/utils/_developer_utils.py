...
def create_new_position_flag(df,days):
    if 'DaysSincePositionStart' in df:
        logging.info('Days Since position detected, creating flag and deleting the column')
        df['DaysSincePositionStart'].fillna(999)
        df['new_position'] = 0
        df.loc[df['DaysSincePositionStart']< days,'new_position'] = 1
        df.drop('DaysSincePositionStart',inplace=True,axis=1)
        logging.info(f'{df.new_position.value_counts()}')
    else:
        logging.info('Column not found, no changes were made.')
    return df

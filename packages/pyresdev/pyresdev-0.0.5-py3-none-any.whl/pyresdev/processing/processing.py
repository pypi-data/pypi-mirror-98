...
def preprocessing_training_common(df, config, industry_dictionary, state_dictionary,
    ...
    df = df.replace( {'Industry': industry_dictionary} )
    ...
    df = df.replace( {'State': state_dictionary} )
    ...
    df = devutils.drop_columns( df=df, columns=cols_to_drop )
...
def preprocessing_training(df, industry_dictionary, state_dictionary,
    ...
    df_leads_clean = utils.onehotencoding( df=df_leads_clean )

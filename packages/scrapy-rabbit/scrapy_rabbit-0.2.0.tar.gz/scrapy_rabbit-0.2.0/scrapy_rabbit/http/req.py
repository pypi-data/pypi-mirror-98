...
class Retry:
    ...
    async def run(self, session, request, init_max_times):
        ...
        while max_times:
            try:
                if request.get('proxies', None):
                    proxy_res = fuclib.getIP()
                    if proxy_res:
                        request['proxies'] = proxy_res['proxy']
                    else:
                        request['proxies'] = None
                return await self.func(session, request, init_max_times)
            except asyncio.TimeoutError:
                logger.debug("第%s次请求超时，url：{%s}" % (retry_times, request['url']))
                retry_times += 1
                max_times -= 1
            except Exception as e:
                logger.debug("第%s次请求报错{%s:%s}，url：{%s}" % (retry_times, type(e).__name__, e, request['url']))
                retry_times += 1
                max_times -= 1

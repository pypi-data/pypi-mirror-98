...
def filter_tile(
    ...
    try:
        ...
        output_array[ay : ay + tile_height, ax : ax + tile_width, :] = tile_arr[:tile_height, :tile_width]
    except Exception as e:
        logger.exception(e)


def svs2tif(input_file, output_folder, tile_size, overlap, num_workers=os.cpu_count(), output_filename="image.tif"):
    ...
    with OpenSlide(input_file) as slide:
        ...
        try:
            ...
            # Multithread processing for each tile in the largest image (index 0)
            ...
            with TiffWriter(output_file, bigtiff=True) as tif:
                for level in range(len(np_memmap)):  # save from the largest image (openslide requires that)
                    src_arr = np_memmap[level]
                    height, width = src_arr.shape[:2]
                    logger.info("Saving Level %d image (%d x %d)...", level, width, height)
                    tif.save(
                        src_arr,

...
class Highstock(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.JSsource = [
            ...
            'http://cdn.highcharts.com.cn/highstock/8.2.0/highstock.js',
            ...
            'http://cdn.hcharts.cn/highstock/indicators/indicators.js', #技术指标
            'http://cdn.hcharts.cn/highstock/indicators/atr.js',#atr需要这个库
            ...
            'https://code.highcharts.com.cn/highstock/modules/drag-panes.js'#yaxis的resize 配合模块
        ]
        ...
        # accepted keywords
        #self.div_style = kwargs.get('style', '') #这是原始框架语句，我觉得要改个名字。
    ...
    def add_data_set(self, data, series_type="line", name=None, **kwargs):
        ...
        #lw添加。自动加一个id，主要是发现，要添加flag 序列的时候，需要指定onseries的时候
        #是需要具体的id的。所以这里干脆给每个添加的序列一个id，id的值，就跟name一样好了
...
#*****下面这段是原始框架中的内容，我lw调试后发现逻辑说不过去，就是如果chart的属性设置了长度和宽度，则将div的长度和宽度设置 好
        #这个设置我看了下效果，不好。还是按照highcharts的中文官网建议，直接设置div吧。

...
class SequenceDataSeries(DataSeries):
    ...
    # def __init__(self, maxLen=None,symbolBylw=None):
    ...
    def setSymbol(self, symbolBylw):
        # 这个玩意按照正常情况不应该存在，但是我在调试的时候，发现不知道某个序列是哪个品种的。很不好调试。就
        # 加了这么个标记。
        self.symbolBylw = symbolBylw
    ...
    def appendWithDateTime(self, dateTime, value):
        ...
        if dateTime is not None and len(self.__dateTimes) != 0 and self.__dateTimes[-1] >= dateTime:
            ...
            return# 这个语句临时放着，为了应对掘金的数据bug，等待其修复。
    ...
    def getDfDataBylw(self):
        
        dt_=self.__dateTimes.data()
        value_=self.__values.data()
        df=pd.DataFrame({'datetime':dt_,'value':value_})
        return df

...
class Broker(observer.Subject):
    ...
    """
    1 __accounts 是一个dict ，key为账户id。会有多个，因为实际情况是，一个
    经纪商那里肯定可以开出来好多个账号
    
    2 经纪商不一定要有账户，即可以没有一个账户，经纪商也会存在。
...
#*****************
# 这里是为了给经纪商broker 增加一个账户属性。lw从掘金的context类拷贝过来的。
    @property
    def accounts(self):
        """
        用户资金 & 持仓情况
    ...
    def createAccount(account_id,name):
        #******* 这里是由lw添加的。即broker 在 回测的时候，增加一个账户。账户自己构造一个
        #id 从999开始往前编号
        # account_id = '999'
        # name = 'lwBtest'
        
        aAccount=Account(account_id, name)
        self.inside_accounts[account_id] = aAccount
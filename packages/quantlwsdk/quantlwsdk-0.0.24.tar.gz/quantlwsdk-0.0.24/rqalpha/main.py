...
from rqalpha.data.mongo_data_source import MongoDataSource
...
def run(config, source_code=None, user_funcs=None):
    ...
    if not env.data_source:
        #lw
        #切换成自己的datasource
        env.set_data_source(MongoDataSource(config.base.data_bundle_path, getattr(config.base, "future_info", {})))
    ...
    #lw 李文注释 这个函数，我看了下，就是调整回测的起始日期和结束如期。并不是函数名说的 只调整了开始日期。
    #大概是 回测日期首先和 指数的起始结束时间比较一下，则个就的看 本地数据的长短了
    #然后是根交易日比较一下，调整时间。  但是我觉得吧，都没必要啊，反正数据是发送的，
    #具体数据在回测时间范围内，你就发送不就得了。

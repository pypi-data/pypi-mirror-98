...
class TokenGenerator:
    ...
    def return_access_token(self):
        if not self.token_params_set():
            raise GeospatialError("Could not generate access token - missing token generation arguments")
        elif self._access_token is None:
            raise GeospatialError("Could not generate access token from provided token generation arguments")

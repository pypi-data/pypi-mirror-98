...
const utEnvironmentService_1 = require("./utEnvironmentService");
const chaiAsPromised = require("chai-as-promised");
const ts_deferred_1 = require("ts-deferred");
const errors_1 = require("../../../common/errors");
function createTrialForm(content = undefined) {
...
async function waitResult(callback, waitMs = 1000, interval = 1, throwError = false) {
    while (waitMs > 0) {
...
async function waitResultMust(callback, waitMs = 10000, interval = 1) {
...
async function newTrial(trialDispatcher) {
    const trialDetail = await trialDispatcher.submitTrialJob(createTrialForm());
    return trialDetail;
}
function newGpuInfo(gpuCount = 2, nodeId = undefined) {
    let gpuInfos = [];
...
async function verifyTrialRunning(commandChannel, trialDetail) {
    let command = await waitResultMust(async () => {
        return await commandChannel.testReceiveCommandFromTrialDispatcher();
    });
    chai.assert.equal(command.command, commands_1.NEW_TRIAL_JOB, "verifyTrialRunning command type");
    chai.assert.equal(command.data["trialId"], trialDetail.id, "verifyTrialRunning trialDetail.id should be equal.");
    return command;
}
async function verifyTrialResult(commandChannel, trialDetail, returnCode = 0) {
    let trialResult = {
    ...
    await commandChannel.testSendCommandToTrialDispatcher(trialDetail.environment, commands_1.TRIAL_END, trialResult);
    await waitResultMust(async () => {
        return trialDetail.status !== 'RUNNING' ? true : undefined;
    });
    if (returnCode === 0) {
        chai.assert.equal(trialDetail.status, 'SUCCEEDED', "trial should be succeeded");
    }
    else {
        chai.assert.equal(trialDetail.status, 'FAILED', "trial should be failed");
    }
}
async function waitEnvironment(waitCount, previousEnvironments, environmentService, commandChannel, gpuCount = 2, nodeCount = 1, callback = undefined) {
    const waitRequestEnvironment = await waitResultMust(async () => {
    ...
    if (nodeCount > 1) {
        for (let index = 0; index < nodeCount; index++) {
            nodeIds.push(utils_1.uniqueString(5));
        }
    }
    else {
    ...
    for (const nodeId of nodeIds) {
        await commandChannel.testSendCommandToTrialDispatcher(waitRequestEnvironment, commands_1.INITIALIZED, { node: nodeId });
        if (gpuCount > 0) {
            await commandChannel.testSendCommandToTrialDispatcher(waitRequestEnvironment, commands_1.GPU_INFO, newGpuInfo(gpuCount, nodeId));
        }
    }
    if (callback) {
...
describe('Unit Test for TrialDispatcher', () => {
    ...
    let log;
    let previousEnvironments = new Map();
    ...
    beforeEach(async () => {
        ...
        trialDispatcher = new trialDispatcher_1.TrialDispatcher();
        await trialDispatcher.setClusterMetadata(trialConfigMetadataKey_1.TrialConfigMetadataKey.TRIAL_CONFIG, JSON.stringify(trialConfig));
        await trialDispatcher.setClusterMetadata(trialConfigMetadataKey_1.TrialConfigMetadataKey.NNI_MANAGER_IP, JSON.stringify(nniManagerIpConfig));
        let environmentServiceList = [];
        environmentService = new utEnvironmentService_1.UtEnvironmentService();
        ...
        commandChannel = environmentService.getCommandChannel;
        trialDispatcher.commandChannelSet = new Set().add(environmentService.getCommandChannel);
    ...
    it('not reusable env', async () => {
        trialDispatcher.setClusterMetadata(trialConfigMetadataKey_1.TrialConfigMetadataKey.TRIAL_CONFIG, JSON.stringify({
    ...
    it('stop trial', async () => {
        ...
        chai.assert.equal(command.command, commands_1.KILL_TRIAL_JOB);
        ...
        command = await waitResultMust(async () => {
            return await commandChannel.testReceiveCommandFromTrialDispatcher();
        });
        chai.assert.equal(command.command, commands_1.KILL_TRIAL_JOB);
        log.info(`command: ${JSON.stringify(command)}`);
        ...
        chai.assert.equal(trial.status, 'USER_CANCELED', `trial is canceled.`);
        trial = await trialDispatcher.getTrialJob(trialDetail2.id);
        chai.assert.equal(trial.status, 'EARLY_STOPPED', `trial is earlier stopped.`);
    });
    it('multi phase', async () => {
        ...
        chai.assert.equal(command.command, commands_1.SEND_TRIAL_JOB_PARAMETER);
    ...
    it('env failed with trial', async () => {
        ...
        chai.assert.equal(trialDetail.status, 'FAILED', "env failed, so trial also failed.");
    });
    ...
    it('GPUScheduler enabled REQUIRE_EXCEED_TOTAL, need fail', async () => {
        ...
        await waitEnvironment(1, previousEnvironments, environmentService, commandChannel);
        await chai.expect(trialRunPromise).rejectedWith(errors_1.NNIError, "REQUIRE_EXCEED_TOTAL");

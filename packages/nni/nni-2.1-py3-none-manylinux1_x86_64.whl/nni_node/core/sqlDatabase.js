...
class SqlDB {
    ...
    init(createNew, dbDir) {
        ...
        const mode = createNew ? (sqlite3.OPEN_CREATE | sqlite3.OPEN_READWRITE) : sqlite3.OPEN_READWRITE;
        const dbFileName = path.join(dbDir, 'nni.sqlite');
        this.db = new sqlite3.Database(dbFileName, mode, (err) => {
            if (err) {
    ...
    storeMetricData(trialJobId, data) {

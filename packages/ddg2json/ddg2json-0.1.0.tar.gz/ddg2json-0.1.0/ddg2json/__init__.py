...
def main():
    ...
    if len(sys.argv) >= 2:  # otherwise reads stdin
        ...
        if query in ["-h", "--help"]:
            print(
                "Usage: ddg2json.py < HTML_of_the_query_page_of_DuckDuckGo\nOutput: JSON of the results in that HTML\nExample using https://github.com/NightMachinary/.shells/blob/master/scripts/javascript/curlfull.js :\n    curlfull.js 'https://duckduckgo.com/?q=lorde+site%3Ahttps%3A%2F%2Fopen.spotify.com%2Fartist%2F&t=h_&ia=web' | ddg2json.py"
            )
            exit(0)
        brish_mode = True
    ##

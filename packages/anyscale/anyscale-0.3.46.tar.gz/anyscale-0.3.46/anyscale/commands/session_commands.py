...
def anyscale_up(
    ...
    session_controller = SessionController()
    session_controller.up_internal(
...
@click.option(
    "--compute-template-id",
    help="The ID of the compute template to launch this session with.",
    default=None,
)
@click.option(
    "--config",
    "config",
    help="Cluster yaml file to start the session with.",
    default=None,
)
@click.option(
    "--no-restart",
...
def anyscale_up_internal(
    ...
    session_controller = SessionController()
    message = (
        "Warning: This command will be deprecated soon. Please use `anyscale up` to start "
        "a session, and report any bugs if it doesn't work for your use case."
    )

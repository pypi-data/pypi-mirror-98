...
def get_circulating_supply(currency, id):
    r = requests.get(f"https://api.nomics.com/v1/currencies/ticker?key={api_key}&ids={id}&interval=1d,30d&convert={currency}&per-page=100&page=1")
    crypto = r.json()
...
#get the market cap
def get_market_cap(currency, id):
    r = requests.get(f"https://api.nomics.com/v1/currencies/ticker?key={api_key}&ids={id}&interval=1d,30d&convert={currency}&per-page=100&page=1")
    crypto = r.json()

...
def decide(lines,percent=0.3,w=480):
    if len(lines) == 0: return "fail"
    left ,cennter, right = 0, 0, 0
    for line,_ in lines:
        min = 0.5*w*(1-percent)
        max = 0.5*w*(1+percent)
        # print(line,w,min,max)
        if line < min: left += 1
        elif line > max: right += 1
        else: cennter += 1
    # print(left,cennter,right)
...
def detect(img):
    ...
    lines = get_lines(edges, size=20)
    lines = get_points(lines)
    return draw_lines(img, lines), edges, decide(lines,0.33,w=img.shape[1])

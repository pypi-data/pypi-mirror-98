...
try:
    from aiotcloud.rshell.getch import getch
    from aiotcloud.rshell.pyboard import Pyboard, PyboardError
    from aiotcloud.rshell.version import __version__
...
def connect_serial(port, baud=115200, wait=0):
    ...
    if not QUIET:
        print('Connecting to %s (baud %s)...' % (port, baud))
    try:
        ...
        if abs_match.rfind('/') == 0:  # match is in the root directory
            ...
            if DEFAULT_DEV:
                ...
                if match[0] == '/':
                    ...
                    completions += [root_dir[1:] for root_dir in DEFAULT_DEV.root_dirs if
                                    root_dir[1:].startswith(match)]
        else:
...
def real_main():
    ...
    if args.debug:
        pass
        # print("Connect to %s with %d baud." %(args.port,args.baud))

...
export const VY = {
  ...
  on_calibrate: function(topic, cb, options) {
    ...
    if (CALIBRATION.hasOwnProperty(name)) {
      let copy = (typeof CALIBRATION[name] == 'object') ? 
        JSON.parse(JSON.stringify(CALIBRATION[name])) : CALIBRATION[name];
      cb(copy); 
    }
  },
...
const load_item = function(loaded) {
  ...
  Object.keys(CALLBACKS.artifact).forEach(function(artifact_name) {
    vyfetch('__artifact__',{name:loaded.name, artifact:artifact_name},

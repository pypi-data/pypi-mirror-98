...
class OEDXWindow(OEDWindow):
    ...
    def click_control(
        ...
        while time.time() - time0 < timeout:
            if self.Controls[control].exist():
                ...
                time.sleep(interval)
                if option == "disappear":
                    if not judge_control:
                        judge_control = control
                    if self.Controls[judge_control].exist():
                        logger.info("%s 未消失" % judge_control)
                        continue
                    else:
                        return True
                elif option == "appear":
                    ...
                    if self.Controls[judge_control].exist():
                        logger.info("%s 已出现" % judge_control)
                        return True
                    else:
                        continue
                else:
    ...
    def touch_skip(self):
        ...
        '''
        control = "跳过"
        self._click_defined_control(control, timeout=10)
    ...
    @classmethod
    def ensure_click(cls, option, control):
        timeout = 30
        time0 = time.time()
        print(dir(cls))
        print(cls.Controls)

...
from .parallel import execute, TaskManager, EvaluationResult
import numpy as np
...
class ScheduledRun:
    ...
    def signal_handler(self, sig, frame):
        if self._sigterm_received == 0:
            print(
                "Termination signal received. Will terminate once the currently running candidates finished"
            )
        ...
        if self._sigterm_received >= 3:
            self._force_quit_callback(self.current_runtime)
            import sys

            sys.exit(-1)

    def register_signal(self, force_quit_callback):
...
class History:
    ...
    def start_seeding_phase(self):
        self._manually_time = time.time() - self._init_time

    def start_neighbor_phase(self):
        self._seeding_time = time.time() - self._manually_time

    def start_endless_phase(self):
        self._manually_time = time.time() - self._init_time

    def end_endless_phase(self, ratio):
        elapsed = time.time() - self._manually_time
    ...
    def end_neighbor_phase(self):
        self._neighbor_time = time.time() - self._manually_time
        self._total_time = time.time() - self._start_time
...
def register_float(
    ...
    param = FloatParameter(
        param_shape, lb, ub, init, log, precision, mutation_strategy, sampling_strategy
    )
    return param
...
class Search:
    ...
    def _async_result_ready(self, candidate_type, candidate, candidate_result):
        ...
        if (
            ...
            if candidate_result.intermediate_results is None:
                raise ValueError(
                    "A EarlyCanceller was passed but the objective function is not a generator"
                )
    ...
    def evaluate_enqueued_candidates(self, objective_function, kwargs, schedule=None):
    ...
    def _save_best_as_pickle(self, current_runtime):
    ...
    def run(
        ...
        schedule.register_signal(self._save_best_as_pickle)
        if verbose > 0:
            print(f"Search is scheduled for {schedule.to_total_str()}")
        if n_jobs != 1:
            self._task_executor = TaskManager(n_jobs)
        ...
        self.evaluate_enqueued_candidates(objective_function, kwargs, schedule)
        pbar.n = schedule.current_units
        self._update_pbar(pbar)
        ...
        if schedule.is_endless and seeding_schedule.is_endless:
            ...
            while (
                ...
                candidate = self.sample_solution()
                self._submit_candidate(
                    objective_function, CandidateType.RANDOM_SEEDING, candidate, kwargs
                )
                schedule.increment_step()
                seeding_schedule.increment_step()
                pbar.n = schedule.current_units

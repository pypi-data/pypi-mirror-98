...
def __(key):
    return i18n.t("widget.messagewidget." + key)
...
def main(argv=None):  # pragma: no cover
    ...
    w.setLayout(l1)
    messages = [
        Message(Severity.Error, text=__("msg.err_encountered_hcf"),
                detailedText=__("msg.err_encountered_hcf_detail"),
                textFormat=Qt.RichText),
        Message(Severity.Warning,
                text=__("msg.warn_achtung"),
                detailedText=(
                    __("msg.warn_achtung_detail")
                ),
                textFormat=Qt.RichText),
        Message(Severity.Information,
                text=__("msg.info_rain_spain_plain"),
                informativeText=(
                    __("msg.info_rain_spain_plain_detail")
                ),
                textFormat=Qt.RichText),
        Message(Severity.Error,
                text=__("msg.err_not_do"),
                informativeText=__("msg.err_not_do_info"),
                detailedText=__("msg.err_not_do_detail")),
        Message(),
    ]

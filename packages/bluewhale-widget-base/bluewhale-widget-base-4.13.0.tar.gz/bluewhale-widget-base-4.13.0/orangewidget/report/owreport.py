...
def __(key):
    return i18n.t("widget.owreport." + key)
...
class ReportItemModel(QStandardItemModel):
    ...
    def add_item(self, item):
        ...
        self.setItem(row, Column.remove, self._icon_item(__("msg.remove")))
        self.setItem(row, Column.scheme, self._icon_item(__("msg.open_scheme")))
...
class OWReport(OWBaseWidget):
    ...
    def _setup_ui_(self):
        ...
        self.scheme_button = gui.button(
            self.controlArea, self, __("btn.return_to_scheme"),
            callback=self._show_last_scheme
        )
        box = gui.hBox(self.controlArea)
        box.setContentsMargins(-6, 0, -6, 0)
        self.save_button = gui.button(
            box, self, __("btn.save"), callback=self.save_report, disabled=True
        )
        self.print_button = gui.button(
            box, self, __("btn.print"), callback=self._print_report, disabled=True
        )
    ...
    def _build_html(self, selected_id=None):
        ...
        for i in range(self.table_model.rowCount()):
            ...
            content = __("placeholder_write_comment")
            html += f"<div id='{item.id}' class='normal' " \
                    f"onClick='pybridge._select_item(this.id)'>{item.html}<div " \
    ...
    def save_report(self):
        ...
        filename, selected_format = QFileDialog.getSaveFileName(
            self, __("dialog.save_report"), self.save_dir, ';;'.join(formats.keys()))
        if not filename:
    ...
    def _print_report(self):
        ...
        print_dialog.setWindowTitle(__("btn.print_report"))
    ...
    def permission_error(self, filename):
        ...
        mb = QMessageBox(
            ...
            windowTitle=self.tr(__("msg_box.error")),
            text=self.tr(__("msg_box.permission_error")),
            informativeText=self.tr(__("msg_box.permission_error_occur")).format(filename),

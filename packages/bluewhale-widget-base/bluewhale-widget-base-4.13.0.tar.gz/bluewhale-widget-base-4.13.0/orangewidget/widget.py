...
def __(key):
    return i18n.t("widget.widget." + key)
...
class OWBaseWidget(QDialog, OWComponent, Report, ProgressBarMixin,
    ...
    def __new__(cls, *args, captionTitle=None, **kwargs):
        ...
        self.__help_action = QAction(
            __("btn.help"), self, objectName="action-help", toolTip=__("btn.help_tip"),
    ...
    def _create_default_buttons(self):
        ...
        if self.graph_name is not None:
            self.graphButton = QPushButton("&" + __("btn.save_image"), autoDefault=False)
            self.graphButton.clicked.connect(self.save_graph)
        if hasattr(self, "send_report"):
            self.report_button = QPushButton("&" + __("btn.report"), autoDefault=False)
    ...
    def statusBar(self):
        ...
        if statusbar is None:
            ...
            self.__statusbar_action = statusbar_action = QAction(
                __("btn.show_status_bar"), self, objectName="action-show-status-bar",
                toolTip=__("btn.show_status_bar_tip"), checkable=True,
            ...
            help_button = SimpleButton(
                icon=icon,
                toolTip=__("tooltip.widget_help"), visible=help.isVisible(),
            )
            ...
            if hasattr(self, "reset_settings"):
                ...
                b = SimpleButton(
                    icon=icon,
                    toolTip=__("tooltip.setting_default")
                )
            ...
            if hasattr(self, "set_visual_settings"):
                ...
                icon.addFile(gui.resource_filename("icons/visual-settings-hover.svg"),
                             mode=QIcon.Active)
                b = SimpleButton(icon=icon, toolTip=__("tooltip.set_visual_settings"))
    ...
    def __showMessage(self, message):
        ...
        if message.moreurl is not None:
            ...
            helpbutton.setText(__("btn.learn_more"))
...
class StateInfo(QObject):
    ...
    def set_input_summary(self, summary, details="", icon=QIcon(),
        ...
        if summary is None:
            ...
            if isinstance(summary, StateInfo.Empty):
                summary = summary.updated(details=__("input_no_data"),
                                          brief='-')
    ...
    def set_output_summary(self, summary, details="", icon=QIcon(),
        ...
        if summary is None:
            ...
            assert_single_arg()
            if isinstance(summary, StateInfo.Empty):
                summary = summary.updated(details=__("output_no_data"),
                                          brief='-')
            if summary.icon.isNull():

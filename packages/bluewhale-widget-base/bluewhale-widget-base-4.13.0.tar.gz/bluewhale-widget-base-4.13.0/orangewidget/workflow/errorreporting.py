...
def __(key):
    return i18n.t("widget.errorreporting." + key)
...
class ErrorReporting(QDialog):
    ...
    def __init__(self, data):
        ...
        super().__init__(None, Qt.Window, modal=True,
                         ...
                         windowTitle=__('win_title_error'),
                         finished=_finished)
        self._data = data
        ...
        labels_layout.addWidget(QLabel(__("label_report_error_to_developer")))
        ...
        settings = QSettings()
        cb = QCheckBox(__('checkbox_include_workflow'), self,
        ...
        buttons_layout.addWidget(
            QPushButton(__('btn.send_report'), default=True, clicked=self.accept))
        buttons_layout.addWidget(QPushButton(__("btn.don't_send"), default=False, clicked=self.reject))
    ...
    def handle_exception(cls, exc):
        ...
        if (err_module, widget_module) in cls._cache or not internet_on():
            QMessageBox(QMessageBox.Warning, __('msg.error_encountered'),
                        __('msg.error_encountered_text').format((__('msg.widget').format(widget_class.name)

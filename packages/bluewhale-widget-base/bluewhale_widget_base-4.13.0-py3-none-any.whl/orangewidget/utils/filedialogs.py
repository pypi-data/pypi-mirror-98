...
def __(key):
    return i18n.t("widget.filedialogs." + key)
...
def fix_extension(ext, format, suggested_ext, suggested_format):
    dlg = QMessageBox(
        QMessageBox.Warning,
        __("warn_fix_error").format(ext, format))
    role = QMessageBox.AcceptRole
    change_ext = \
        suggested_ext and \
        dlg.addButton(__("btn.change_extension") + suggested_ext, role)
    change_format = \
        suggested_format and \
        dlg.addButton(__("btn.save_as") + suggested_format, role)
    cancel = dlg.addButton(__("btn.back"), role)
    ...
    while True:
        ...
        filename, format, filter = \
            open_filename_dialog(start_dir, start_filter, file_formats,
                                 add_all=False, title=__("dialog_title_save_as"), dialog=dialog)
        if not filename:
...
def open_filename_dialog(start_dir: str, start_filter: str, file_formats,
                         add_all=True, title=__("dialog_title_open"), dialog=None):
    """
    ...
    if add_all:
        ...
        filters.insert(0, __("all_readable_files").format(
...
class RecentPath:
    ...
    def __init__(self, abspath, prefix, relpath, title='', sheet='', file_format=None, table_name=None, resource_id=None):
        if os.name == "nt":
...
class RecentPathsWidgetMixin:
    ...
    def _relocate_recent_files(self):
        ...
        for recent in self.recent_paths:
            kwargs = dict(title=recent.title, sheet=recent.sheet, file_format=recent.file_format, table_name=recent.table_name, resource_id=recent.resource_id)
...
class RecentPathsWComboMixin(RecentPathsWidgetMixin):
    ...
    def set_file_list(self):
        ...
        if not self.recent_paths:
            self.file_combo.addItem(__("row_none"))

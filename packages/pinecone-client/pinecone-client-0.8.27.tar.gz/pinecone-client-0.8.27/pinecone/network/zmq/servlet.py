...
class ZMQServlet:
    ...
    async def send_msg(self, msg: 'core_pb2.Request'):
        ...
        await asyncio.gather(*(self.serialize_and_send_msg(socket, msg) for socket, msg in zip(send_sockets, shard_msgs)))
    ...
    async def serialize_and_send_msg(self, socket: SocketWrapper, msg: 'core_pb2.Request'):
        if msg is None:
            return
    ...
    def init_socket(self, socket: Socket, disable_loadbalance: bool = False):
        ...
        return SocketWrapper(socket, self.context, self.spec.native, disable_loadbalance=disable_loadbalance)

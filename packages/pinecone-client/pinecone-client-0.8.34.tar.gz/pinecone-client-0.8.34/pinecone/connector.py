...
class Connection:
    ...
    def __init__(self, host: str, port: int, api_key: str = None, service_name: str = None, timeout: int = 0,
                 response_timeout: int = 20, traceroute: bool = False):
        """
        ...
        self.grpc = GRPCClient(host=host, port=port, api_key=api_key, service_name=service_name, conn_timeout=timeout,
                               timeout=response_timeout, traceroute=traceroute)
    ...
    def unary_upsert(self, item: Tuple[str, np.ndarray], namespace: str = None) -> Optional[IndexResult]:
        ...
        """
        result = self.upsert(items=[item], namespace=namespace, cursor_config={"unary": True}).collect()
    ...
    def unary_delete(self, id: str, namespace: str = None) -> Optional[DeleteResult]:
        ...
        """
        result = self.delete(ids=[id], namespace=namespace, cursor_config={"unary": True}).collect()
        return result[0] if result else None
    ...
    def unary_fetch(self, id: str, namespace: str = None) -> Optional[FetchResult]:
        ...
        """
        result = self.fetch(ids=[id], namespace=namespace, cursor_config={"unary": True}).collect()
        return result[0] if result else None
    ...
    def unary_query(
        ...
        """
        result = self.query(
            queries=[query], namespace=namespace, top_k=top_k, include_data=include_data, cursor_config={"unary": True}
        ).collect()
        return result[0] if result else None
...
@sentry
def connect(service_name: str = None, router_name: str = None, timeout: int = 0,
            response_timeout: int = 20, traceroute: bool = False) -> Connection:
    ...
    return Connection(host=host, port=port, api_key=Config.API_KEY, service_name=dest_name,
                      timeout=timeout, response_timeout=response_timeout, traceroute=traceroute)

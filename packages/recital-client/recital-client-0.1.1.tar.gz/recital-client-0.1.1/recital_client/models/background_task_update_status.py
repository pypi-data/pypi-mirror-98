...
class BackgroundTaskUpdateStatus:
    ...
    @staticmethod
    def from_dict(src_dict: Dict[str, Any]) -> "BackgroundTaskUpdateStatus":
        d = src_dict.copy()
        ...
        if _info is not None and not isinstance(_info, Unset):
            info = BackgroundTaskUpdateStatusInfo.from_dict(cast(Dict[str, Any], _info))

        background_task_update_status = BackgroundTaskUpdateStatus(
            status=status,

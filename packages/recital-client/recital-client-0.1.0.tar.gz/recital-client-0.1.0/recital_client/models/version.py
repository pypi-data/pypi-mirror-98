...
class Version:
    ...
    @staticmethod
    def from_dict(src_dict: Dict[str, Any]) -> "Version":
        d = src_dict.copy()
        id = d.pop("id")
        ...
        if _metadata is not None and not isinstance(_metadata, Unset):
            metadata = VersionMetadata.from_dict(cast(Dict[str, Any], _metadata))

        chunked = d.pop("chunked", UNSET)

        chunk_timestamp = None
        _chunk_timestamp = d.pop("chunk_timestamp", UNSET)
        if _chunk_timestamp is not None:
            chunk_timestamp = isoparse(cast(str, _chunk_timestamp))

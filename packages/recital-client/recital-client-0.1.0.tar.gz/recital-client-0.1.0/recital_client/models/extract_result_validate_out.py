...
from ..models.extract_result_validate_out_metadata import ExtractResultValidateOutMetadata
from ..models.extract_result_validate_out_versioetadata import ExtractResultValidateOutVersioetadata
from ..models.extract_top_chunk_validate_out import ExtractTopChunkValidateOut
from ..types import UNSET, Unset


@attr.s(auto_attribs=True)
class ExtractResultValidateOut:
    """ Extract result schema for validation with page HTMLs. """
    ...
    doc_path: str
    id: int
    metadata: ExtractResultValidateOutMetadata
    top_chunks: List[ExtractTopChunkValidateOut]
    ...
    version_metadata: Union[ExtractResultValidateOutVersioetadata, Unset] = UNSET
    ...
    @staticmethod
    def from_dict(src_dict: Dict[str, Any]) -> "ExtractResultValidateOut":
        d = src_dict.copy()
        ...
        metadata = ExtractResultValidateOutMetadata.from_dict(d.pop("metadata"))

        top_chunks = []
        _top_chunks = d.pop("top_chunks")
        for top_chunks_item_data in _top_chunks:
            top_chunks_item = ExtractTopChunkValidateOut.from_dict(top_chunks_item_data)
        ...
        if _validated_on is not None:
            validated_on = isoparse(cast(str, _validated_on))
        ...
        if _version_metadata is not None and not isinstance(_version_metadata, Unset):
            version_metadata = ExtractResultValidateOutVersioetadata.from_dict(cast(Dict[str, Any], _version_metadata))

        extract_result_validate_out = ExtractResultValidateOut(
            task_id=task_id,
            doc_id=doc_id,
            doc_filename=doc_filename,
            doc_path=doc_path,
        ...
        extract_result_validate_out.additional_properties = d
        return extract_result_validate_out

...
class Player():
    ...
    def __init__(self,
        ...
        self.vlc = vlc.Instance('--no-video',
                                'vout=none')
    ...
    def play(self, song, halting=False):
        ...
        info(f'Playing {song.title} by {song.artist} [{song.length}]')
        if halting:
        ...
        if self.rich_presence:
            try:
                self.rich_presence_client.update(details=self.current_song.title,
                                                 ...
                                                 small_image='clock',
                                                 small_text=f'bro idk yet/{self.current_song.length}', large_text=';)')
            except:

...
def get_cffex_daily(date=None):
    ...
    for row in html[1:]:
        ...
        if not m:
            continue
        row_dict = {'date': day_const, 'symbol': row[0], 'variety': m.group(1)}#m.group(1)去合约取代码
        for i, field in enumerate(cons.CFFEX_COLUMNS):
            if row[i + 1] == u"":
                row_dict[field] = 0.0
            elif field in ['volume', 'open_interest']:#数据位置错，删除里面, 'oi_chg'
        ...
        df=pd.DataFrame(dict_data)[cons.OUTPUT_COLUMNS]
        df = df[~df['symbol'].str.contains('IO')]#通过~取反，选取不包含合约代码IO的行
    return df
...
def get_czce_daily(date=None):
    ...
    if day <= datetime.date(2010,8,24):
        ...
        df=df[['品种月份','昨结算','今开盘','最高价','最低价','今收盘','今结算','成交量','空盘量','成交额(万元)']]
        df = df.rename(columns={'品种月份':'symbol', '昨结算':'pre_settle' , '今开盘':'open' , '最高价':'high' , '最低价':'low' ,
                                 '今收盘':'close' , '今结算':'settle' , '成交量':'volume' , '空盘量':'open_interest' ,
                                 '成交额(万元)':'turnover' })

        df['variety']=df['symbol'].str.extract('([A-Za-z]+)', expand=True)
        # S = df['symbol'].str.extract('(\d+)', expand=True)
        day_const = day.strftime('%Y%m%d')
        df['date']=day_const#新增日期dates
        # #提取年份后两位
        # # Y1 = df['date'].str.extract('(\d{4})', expand=True)
        # # Y2= re.search(r'(?:(?!\d\d).)*(\d\d)$', str(Y1)).group()
        # # Y3 = re.search('(\d)(.*)', str(Y2)).group()
        # # # 提取合约后两位
        # #
        # 
        # # print(Y2)
        # # #合并
        # # df['symbol']=df['variety']+Y2+S[1]
        # # print(df)
        # 
        return df
        ...
        #     m = cons.FUTURE_SYMBOL_PATTERN.match(row['品种月份'])
        #     if not m:
        #         continue
        #     row_dict = {'date': day_const, 'symbol': row['品种月份'], 'variety': m.group(1)}
        #     for i, field in enumerate(listed_columns):
        #         if row[i + 1] == "\r":
        #             row_dict[field] = 0.0
        #         elif field in ['volume', 'open_interest', 'oi_chg', 'exercise_volume']:
        ...
        #
        #             row_dict[field] = float(row[i + 1])
        # return pd.DataFrame(dict_data)[listed_columns]
...
def get_shfe_daily(date=None):
    ...
    #处理重复数据和品种错误
    df=df.drop_duplicates(['symbol'])#去重
    # df=df[~df['symbol'].str.contains('SCefp')]#不包括SCefp，等处理品种+’efp‘
...
def get_future_daily(date=None,market='CFFEX', indexBar = False):
    """
        获取交易所日交易数据
    Parameters
    ------
...
def get_futureIndex(df):
    ...
    for var in set(df['variety']):
        ...
        if len(dfCut.index)>0:
            index_df = pd.Series(index = dfCut.columns, dtype='object')#加个数据类型
...
if __name__ == '__main__':
    ...
    #
        for market in ['DCE', 'CZCE', 'CFFEX', 'SHFE']:
            try:
                d=get_future_daily(days,market)
                print(d)
            except:
                print(days,'???')
                continue

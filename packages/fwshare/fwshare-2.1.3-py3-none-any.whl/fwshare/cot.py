...
import datetime
import warnings
from fwshare.requests_fun import *
from fwshare.symbolVar import *
from io import BytesIO
from fwshare.dailyBar import *
...
def get_shfe_rank_table(date = None,vars = cons.vars):
    ...
    for var in get_vars:
        ...
        for symbol in set(df_var['symbol']):
            ...
            df_symbol['symbol'] = df_symbol['symbol'].str.upper()#小写字母转换大写字母
...
def get_czce_rank_table(date = None,vars = cons.vars):
    ...
    if date <= datetime.date(2010, 8, 25):
        ...
        for i in range(len(symbols)):
            ...
            symbol = symbols[i]
            tableCut = data[i+1]#data[i+1]抓20100825前数据，data[i+2]抓20100826后数据
            tableCut.columns = rank_columns
            tableCut = tableCut.iloc[:-1,:]

            tableCut=tableCut.copy()
        ...
        for x in range(3, 60):
            try:
                ...
                if var in vars:
                    # table['variety']=var
                    tableCut = table.loc[[x for x in table.index if x in [str(i) for i in range(21)]], :]
        ...
        data = _czce_df_read(url,skiprow=0)[1]
...
def get_dce_rank_table(date = None):
    """
    ...
    if date.strftime('%Y%m%d') not in calendar:
        warnings.warn('%s非交易日' % date.strftime('%Y%m%d'))
        return {}
    dce = get_dce_daily(date)

    D = {}
    for symbol in dce['symbol']:
        s = symbol.lower()
        var = ''.join(re.findall(r'[A-Za-z]', symbol)).lower()
        if var:
            url = cons.DCE_VOLRANK_URL % (var,s,var, date.year, date.month-1, date.day)
            ...
            if len(texts)>30:
                ...
                try:
                    tableCut = pd.DataFrame({'rank': rank[0:20],
                                       ...
                                       'short_party_name': list_60_name[40:70],
                                       'short_openIntr': list_60[40:70],
                                       'short_openIntr_chg': list_60_chg[40:70]
                                       })
                    tableCut = tableCut.applymap(lambda x: x.replace(',', ''))
                    tableCut['variety']=''.join(re.findall(r'[A-Za-z]', symbol))
                except:
                    pass
                tableCut = _tableCut_cal(tableCut, symbol)
                D[symbol] = tableCut.reset_index(drop=True)
...
def get_cffex_rank_table(date = None,vars = cons.vars):
    ...
    elif date <= datetime.date(2020, 9, 11):
        D = {}
        for var in vars:
            url = cons.CFFEX_VOLRANK_URL % (date.strftime('%Y%m'), date.strftime('%d'), var)
            r = requests_link(url, encoding='gbk')
            ...
            if '网页错误' not in r.text:
                ...
                table = table.dropna(how='any')
                table = table.applymap(lambda x: x.strip() if type(x) == type('') else x)
                table.set_index(['交易日'], inplace=True)
...
def get_future_rank_table(date=None,market='CFFEX'):

    if market.upper() == 'CFFEX':
        f = get_cffex_rank_table
    elif market.upper() == 'CZCE':
        f = get_czce_rank_table
    elif market.upper() == 'SHFE':
        f = get_shfe_rank_table
    elif market.upper() == 'DCE':
        f = get_dce_rank_table
    else:
    ...
    if day.strftime('%Y%m%d') not in calendar:
        ...
        return None
    df_list = list()
    df = f(day.strftime("%Y%m%d"))
    if df is not None:
        for key, value in df.items():
            value = value[value['long_party_name'].notna()]
            value = value[~value['symbol'].isin(value['variety'])]
            value['date']=day.strftime("%Y%m%d")
...
if __name__ == '__main__':
    ...
    dfs=get_future_rank_table(date='20210308',market='CZCE')
    print(dfs)

    # for k, v in dfs.items():
    #     v=v[~v['rank'].str.contains('999')]
    #     print(v)

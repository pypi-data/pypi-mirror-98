...
def get_rollYield_bar(type = 'symbol',  var = 'RB',date= None, start = None, end = None, plot = False):
    ...
    if type == 'symbol':
        df = get_future_daily(date=date, market=symbolMarket(var))
    ...
    if type == 'var':
        ...
        for market in ['dce','cffex','shfe','czce']:
            df = df.append(get_future_daily(date=date, market=market))
...
def get_rollYield(date = None, var = 'IF',symbol1 = None, symbol2 = None, df = None):
    ...
    if type(df) != type(pd.DataFrame()):
        ...
        df = get_future_daily(date=date, market=market)
    if var:
...
if __name__ == '__main__':
    ...
    # d = get_rollYield_bar(type = 'var', date = '20200519',plot = True)
    # print(d)

    d=get_rollYield('20200619','FU')
    print(d)

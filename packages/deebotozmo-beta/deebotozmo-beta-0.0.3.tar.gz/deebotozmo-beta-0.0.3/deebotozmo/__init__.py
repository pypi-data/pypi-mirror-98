...
class EcoVacsAPI:
    ...
    def __init__(
        ...
        login_info = self.__call_main_api(
            self.login_path, ("account", account_id), ("password", password_hash)
        )
        self.uid = login_info["uid"]
        self.login_access_token = login_info["accessToken"]

        self.auth_code = self.__call_auth_api(
            device_id, ("uid", self.uid), ("accessToken", self.login_access_token)
        )["authCode"]
    ...
    def __call_main_api(self, function, *args):
        ...
        api_response = requests.get(
            url, self.__sign(params), timeout=60, verify=self.verify_ssl
        )
    ...
    def __call_auth_api(self, device_id, *args):
        ...
        if self.country.lower() == "cn":
            url = (EcoVacsAPI.PORTAL_GLOBAL_AUTHCODE.replace(".com", ".cn")).format(
                **self.meta
            )
        else:
    ...
    def __call_user_api(self, function, args):
        ...
        response = requests.post(
            EcoVacsAPI.USER_URL_FORMAT.format(continent=self.continent),
            json=params,
            timeout=60,
    ...
    def __call_portal_api(self, api, function, args, verify_ssl=True, **kwargs):
        ...
        if self.country.lower() == "cn":
            url = (EcoVacsAPI.PORTAL_URL_FORMAT_CN + "/" + api).format(
                continent=continent, **self.meta
            )
        else:
            url = (EcoVacsAPI.PORTAL_URL_FORMAT + "/" + api).format(
                continent=continent, **self.meta
            )
            ...
            if json["result"] == "ok":
                ...
                if (
                    ...
                    if not "set_token" in kwargs:
                        ...
                        _LOGGER.warning(
                            "loginByItToken set token error, trying again with ww (3/3)"
                        )
                        return self.__call_portal_api(
                            self.USERSAPI,
                            function,
    ...
    def getiotProducts(self):
        return self.__call_portal_api(
            self.PRODUCTAPI + "/getProductIotMap",
            "",
            {
                "channel": "",
                "auth": {

    ...
    def update_plot(self, change):
        ...
        for i in range(number_of_skeletons):
            for j in range(len(self.joint_set.limb_graph)):
                ...
                self.skeletonVisualizer.skeletons[i].joint_lines[j].vertices = (current_frame[i][line_indices[0]], current_frame[i][line_indices[1]])
...
class SkeletonVisualizer:
    ...
    def visualize_video(self, frames: np.ndarray, colors: List[Color] = None, fps: int = 30):
        ...
        frame_updater = FrameUpdater(self, frames, self.joint_set)

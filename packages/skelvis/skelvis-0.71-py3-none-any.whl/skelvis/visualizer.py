...
class FrameUpdater:
    def __init__(self, skeletonVisualizer, frames: np.ndarray):
        self.skeletonVisualizer = skeletonVisualizer
        self.frames = frames

    def update_plot(self, frame_index: int):
        ...
        for i in range(number_of_skeletons):
            current_skeleton: DrawableSkeleton = self.skeletonVisualizer.skeletons[i]
            current_skeleton.joint_points.positions = current_frame[i]
...
class SkeletonVisualizer:
    ...
    def visualize_video(self, frames: np.ndarray, colors: List[Color] = None, fps: int = 30):
        ...
        video_player.observe(handler=frame_updater.update_plot, names='value')

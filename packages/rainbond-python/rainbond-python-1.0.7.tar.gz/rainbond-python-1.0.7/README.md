...
def api_v1_demo():
    ...
    # 获取用户权限：0=无权限; 1=个人权限; 2=查看权限; 3=编辑权限
    permission_status = token.permission_verify('组件名称', '自定义权限')

    if parameter.method == 'GET' and (permission_status in (1, 3)):  # 用户有个人/编辑权限
        # 通过认证并符合权限后，可以执行业务逻辑
        psss
......
```
...
per_status_this = token.permission_verify('组件名称', '自定义权限')
```

然后我们在对应的请求类型中，用 `and` 加入对 `per_status_this` 变量判断，该变量有 *0（无权限）*、*1(查看权限)*、*2(编辑权限)* 三种情况，例如我们的接口要求用户有 “查看+编辑” 权限才能操作：

```python
if parameter.method == 'GET' and (per_status_this > 0)):
    # 只要不是无权限就可以访问

if parameter.method == 'POST' and (per_status_this == 2):
    # 必须有编辑权限
```

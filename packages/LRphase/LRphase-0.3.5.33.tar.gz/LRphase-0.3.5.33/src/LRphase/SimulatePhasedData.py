...
def simulate_reads_pbsim2(
    ...
    subprocess.run(
            [str(path_to_pbsim), f'--depth', str(depth),
             str(mode1), str(mode2),
    ...
    for file in os.listdir(os.path.dirname(pbsim2_pbsim2fq_output)):
        ...
        if str(file).lower().endswith('pbsim2fq.fastq') or str(file).lower().endswith('pbsim2fq.fastq.gz'):
            continue
        if str(file).lower().endswith('.maf'):
            os.remove(file)
        if str(file).lower().endswith('.ref'):
            os.remove(file)
        elif str(file).lower().endswith('.fastq'):
            with open(combined_simulated_long_reads_fastq_path, 'a') as outfile:

...
def _sample_to_alignment_files(sample_to_vcf_file_dict, RG_ID_dict: object):
...
def _sort_and_index_alignment_file(long_reads_alignment_path, output_directory):
    start_process_time = time.time()
...
def _prepare_alignment(output_directory: object, long_reads_alignment_path: object) -> object:
    start_process_time = time.time()
    ...
    if long_read_file_pysam.format == 'BAM':
        ...
        print(
                'Error: Pysam does not recognize %s as being in SAM or BAM format. If aligned reads are provided as input they must be in proper .sam or .bam format.' % long_reads_alignment_path
                )
        return
...
def _unique_RG_IDs_from_RG_tags(RG_ID_dict: dict, unique_RG_IDs, alignment_file_path):
    """
...
def _extract_RG_info_from_long_read_input(long_read_input: object) -> object:
    """
...
class InputData:
    ...
    def add_haplotype_information_from_file(
            self, vcf_file_input, ignore_phase_sets = False, reference_sequence = 'hg38'
            ):
        """
        """
        ...
        if pysam.VariantFile(vcf_file_path).header.formats.keys().count('PS') == 0:
            ...
            print(
                    str(
                            vcf_file_path
                            ) + 'This VCF file does not have the PS subfield. Phase sets will be ignored and all phased variants on the same chromosome (vcf contig) will be considered to be one contiguous haploblock'
                    )
    ...
    def new_PhasableSample(self, sample: object, reference_sequence_paths: object = None) -> object:
        """
        ...
        phasable_sample = PhasableSample.PhasableSample(
                self, sample, vcf_file_path, ignore_phase_sets, self.sample_to_alignment_files[sample], self.RG_ID_dict,
    ...
    def _parse_long_reads_input(self, long_read_input: object, output_directory: object) -> object:
        """

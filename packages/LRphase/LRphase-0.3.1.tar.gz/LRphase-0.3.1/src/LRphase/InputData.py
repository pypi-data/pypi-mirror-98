...
import LRphase.PhasableSample as PhasableSample
...
def _prepare_output_directory(output_directory_path: object) -> object:
    """
...
class InputData:
    ...
    def __init__(
            self, output_directory_path: object = None, vcf_file_input: object = None, long_read_input: object = None,
            reference_sequence_input: object = None, sample: object = None, ID: object = None,
            sample_description: object = None,
            ignore_phase_sets: object = None, ignore_samples: object = None, download_from_urls: object = False,
            reference_sequence_input_assembly: object = None
            ) -> object:
        """
        ...
        self.summary_file_path = self._prepare_summary_file(self.output_directory)
        ...
        if not self.long_read_input is None:
            # self.long_read_input = long_read_input
            long_read_input_path, input_ID, input_sample, input_sample_description, input_reference_sequence_input = self._extract_RG_info_from_long_read_input(
                long_read_input)
            if input_reference_sequence_input is None:
                input_reference_sequence_input = self.reference_sequence_input
            self.add_reads_from_file(self.long_read_input, input_sample, input_ID, input_sample_description, input_reference_sequence_input)
    ...
    # def _prepare_downloaded_vcf(self, assembly_version=None, sample=None, output_directory=None):
        ...
        # if assembly_version is None:
            # print('No assembly version provided. Unable to download vcf file.')
            # return
        # if sample is None:
            # print('No sample provided. Unable to download vcf file.')
            # return
        # try:
            # downloaded_vcf_file = str(assembly_version[str(sample)])
            # downloaded_vcf_file_path = output_directory+'/haplotype_information/'+str(sample)+'.vcf.gz'
            # subprocess.run(['curl', '-L', downloaded_vcf_file_path, '| gunzip - >', str(output_directory)+'/haplotype_information/'+str(assembly_version[sample]).replace('.vcf.gz','.vcf')], check = True)
            # #subprocess.run(['curl', '-L', downloaded_vcf_file, '-o', output_directory+'/haplotype_information/'+str(sample)+'.vcf.gz'], check = True)
            # print('VCF file downloaded successfully: %s.' % downloaded_vcf_file_path)
            # return downloaded_vcf_file_path
        # except Exception as e:
            # print(e)
            # print('Error occurred while downloading VCF file.')
            # return
    ...
    # def _prepare_downloaded_reference_sequence(self, assembly_version=None, output_directory=None):
        ...
        # if output_directory is None:
            # output_directory = self.output_directory
        # if assembly_version is None:
            # print('No assembly version provided. Unable to download reference sequence.')
            # return
        # try:
            # reference_sequence_file = str(assembly_version['reference_sequence'])
            # subprocess.run(['curl', '-L', reference_sequence_file, '| gunzip - >', str(output_directory)+'/reference_sequences/'+str(assembly_version['reference_sequence']).replace('.fa.gz','.fa')], check = True)
            # #subprocess.run(['curl', '-L', reference_sequence_file, '-o', output_directory+'/reference_sequences/'+str(assembly_version['reference_sequence_file_name'])], check = True)
            # reference_sequence_file_path = output_directory+'/reference_sequences/'+str(assembly_version['reference_sequence_file_name'])
            # print('Reference sequence file downloaded successfully: %s.' % reference_sequence_file_path)
            # return reference_sequence_file_path
        # except Exception as e:
            # print(e)
            # print('Error occurred while downloading reference sequence file.')
            # return

    def _prepare_reference_sequence_file(self, reference_sequence_input, output_directory=None, reference_sequence_input_assembly = None):
        ...
        try:
            if reference_sequence_input.endswith('.fa'):
                ...
                reference_sequence_input_path = os.path.dirname(reference_sequence_input)+'/'+os.path.basename(reference_sequence_input).split('.')[0]+'.fa'
            ...
            try:
                if reference_sequence_input.endswith('.fa'):
                    ...
                    reference_sequence_input_path = os.path.dirname(reference_sequence_input)+'/'+os.path.basename(reference_sequence_input).split('.')[0]+'.fa.gz'
        ...
        try:
            ...
            reference_sequence_file_path = ref_seq.filename.decode()
            ...
            print(e)
            # try:
                # output_reference_sequence_file_path = output_directory_path+'/'+os.path.basename(reference_sequence_file_path)
                # subprocess.Popen(['gunzip', '-c', reference_sequence_file_path, '>', output_reference_sequence_file_path.replace('.fa.gz','.fa')], check = True)
                # subprocess.run(['bgzip', output_reference_sequence_file_path.replace('.fa.gz','.fa')], check = True)
                # ref_seq = pysam.FastaFile(reference_sequence_file)
                # #reference_sequence_names = ref_seq.references
                # output_reference_sequence_file_path = ref_seq.filename.decode()
            # except Exception as e:
                # print(e)
                # print('Reference sequence file is not valid.')
        
        print('Reference sequence prepared successfully: %s' % reference_sequence_file_path)
        return reference_sequence_file_path #reference_sequence_names

    def add_haplotype_information_from_file(self, vcf_file_input, ignore_phase_sets=False, reference_sequence='hg38'):
        """
        """
        ...
        if pysam.VariantFile(vcf_file_path).header.formats.keys().count('PS') == 0:
            ...
            print(str(vcf_file_path) + 'This VCF file does not have the PS subfield. Phase sets will be ignored and all phased variants on the same chromosome (vcf contig) will be considered to be one contiguous haploblock')
    ...
    def new_PhasableSample(self, sample, reference_sequence_paths = None):
        vcf_file_path, ignore_phase_sets = self._pair_sample_with_vcf(
        ...
        phasable_sample = PhasableSample(
    ...
    def add_reads_from_file(self, long_reads_alignment_path, sample=None, ID=None, haplotype=None, sample_description=None, reference_sequence_input=None, database=None, master_database=False, simulated=False, reference_sequence_input_assembly = None): 
        """
        """
        # if reference_sequence_input == None:
        # reference_sequence_input = self.reference_sequence_input
        
        sorted_bam_file_paths, combined_long_read_fastq_path = self._parse_long_reads_input(long_reads_alignment_path, self.output_directory)
        if combined_long_read_fastq_path:
            reference_sequence_input = self._prepare_reference_sequence_file(reference_sequence_input=reference_sequence_input, output_directory=self.output_directory, reference_sequence_input_assembly = reference_sequence_input_assembly)
            if reference_sequence_input is None:
                if self.reference_sequence_input is None:
                    ...
                    reference_sequence_input = self.reference_sequence_input
            sorted_bam_file_paths.append(self._align_long_reads_fastq(combined_long_read_fastq_path, reference_sequence_input, self.output_directory))
        for _sorted_bam_file_path in sorted_bam_file_paths:
            if _sorted_bam_file_path:
                sorted_bam_file_path, combined_long_read_fastq_path = self._parse_long_reads_input(_sorted_bam_file_path, self.output_directory)
                self.RG_ID_dict, self.unique_RG_IDs = self._compile_read_groups(sorted_bam_file_path[0], sample, ID, sample_description, self.RG_ID_dict, self.unique_RG_IDs, self.ignore_samples)
    ...
    def _pair_sample_with_vcf(self, sample, sample_to_vcf_file_dict, ignore_phase_sets):
    ...
    def _sample_to_alignment_files(self, sample_to_vcf_file_dict, RG_ID_dict):
        ...
        for sample in sample_to_vcf_file_dict:
            ...
            for file in RG_ID_dict.keys():
                if len([file for key in RG_ID_dict[file].keys() if RG_ID_dict[file][key]['SM'] == sample]) > 0:
                    for pair in [{alignment_file_path:any([RG_ID_dict[alignment_file_path][ID]['RG_tags'] for ID in RG_ID_dict[alignment_file_path].keys()])} for alignment_file_path in RG_ID_dict.keys() if any([RG_ID_dict[alignment_file_path][ID]['SM'] == sample for ID in RG_ID_dict[alignment_file_path].keys()])]:
        ...
        for sample in self.sample_to_vcf_file_dict:
            for sam_file_path in self.alignment_file_to_reference_sequences_dict:
                if sam_file_path in self.sample_to_alignment_files[str(sample)]:
                    for reference_sequence in pysam.AlignmentFile(sam_file_path).header['SQ']:
                        if str(reference_sequence['SN']) not in [str(reference_seq['SN']) for reference_seq in self.sample_to_reference_sequences_dict[sample]]:
                            self.sample_to_reference_sequences_dict[sample].append(reference_sequence)

    def _sample_to_PG_dict(self, sam_file_path=None, sam_file=None):
        if sam_file_path:
            for reference_sequence in pysam.AlignmentFile(sam_file_path).header['SQ']:
                self.alignment_file_to_reference_sequences_dict[sam_file_path].append(reference_sequence)
        for sample in self.sample_to_vcf_file_dict:
            for sam_file_path in self.alignment_file_to_reference_sequences_dict:
                if sam_file_path in self.sample_to_alignment_files[str(sample)]:
                    for PG_tag in pysam.AlignmentFile(sam_file_path).header['PG']:
                        ...
                        RG_tag = {'ID': str(list(self.RG_ID_dict[sam_file_path].keys())[0]), 'SM': str(
                            self.RG_ID_dict[sam_file_path][str(list(self.RG_ID_dict[sam_file_path].keys())[0])]['SM']),
                                  'DS': str(self.RG_ID_dict[sam_file_path][
    ...
    def _sample_to_vcf_file_dict(self, vcf_file_paths):
            ...
            if sample is not None:
                ...
                if ID is not None:
                    ...
                    if sample_description is not None:
                        ...
                        RG_ID_dict[str(alignment_file_path)][str(ID)]['DS'] = str('LRphase_input_file_' + str(alignment_file_path))
                else:
                ...
                RG_ID_dict, unique_RG_IDs = self._unique_RG_IDs_from_RG_tags(RG_ID_dict, unique_RG_IDs,alignment_file_path)
            else:
                print(str(alignment_file_path) + 'Has No RG tags and was not input with sample information. Reads in this file will not be processed. Either re-input this read file with sample information or resubmit with ignore_samples option.')
        return RG_ID_dict, unique_RG_IDs

    def _unique_RG_IDs_from_RG_tags(self, RG_ID_dict, unique_RG_IDs, alignment_file_path):
    ...
    def _align_long_reads_fastq(self, long_reads_fastq_path, reference_sequence_input, output_directory):
        ...
        # reference_sequence_input = '/home/gregfar/results/jupyter_notebooks/haplotype_reference_sequence/hg38.idx'
        reference_sequence_input_path = os.path.abspath(reference_sequence_input)
        if str(long_reads_fastq_path).lower().endswith('.fastq'):
            sam_alignment_path = os.path.abspath('%s/%s%s' % (output_directory, os.path.splitext(os.path.basename(long_reads_fastq_path))[0], '_alignment.sam'))
        elif str(long_reads_fastq_path).lower().endswith('fastq.gz'):
            sam_alignment_path = os.path.abspath('%s/%s%s' % (output_directory, os.path.splitext(os.path.splitext(os.path.basename(long_reads_fastq_path))[0])[0],'_alignment.sam'))
        
        # if subprocess.getstatusoutput(['minimap2 -ax map-ont'+' /home/ubuntu/testing_LRphase/simtest6/reference_sequences/hg38/hg38output_regions_only_autosomal.fa '+ '/home/ubuntu/testing_LRphase/simtest6/simulated_reads/hg38output_regions_only_autosomal_hap1_HG001.fastq '+'-o '+'a.sam'])[0] != 0:
            # print('minimap2 failed to run. Installing minimap2 from source...')
            # import getpass
            # password = getpass.getpass()
            # os.system('echo %s | %s' % (password, "sudo -S rm -r minimap2"))
            # subprocess.run(['git','clone','https://github.com/lh3/minimap2'])
            # os.chdir('minimap2/')
            # subprocess.run('make')
            # os.system('echo %s | %s' % (password, "sudo -S install -p minimap2/minimap2 /usr/local/bin"))
        subprocess.run(['minimap2', '-ax', 'map-ont', '-Y', '-L','--secondary=no', '--MD', reference_sequence_input_path, long_reads_fastq_path, '-o', sam_alignment_path])
    ...
    def _sort_and_index_alignment_file(self, long_reads_alignment_path, output_directory):

        start_process_time = time.time()

        sorted_bam_file_path = os.path.abspath('%s/%s%s' % (output_directory, os.path.splitext(os.path.basename(long_reads_alignment_path))[0], '_sorted.bam'))
        print('############## Sorting and indexing bam file ##############')
        pysam.sort('-O', 'BAM', long_reads_alignment_path, '-o', sorted_bam_file_path)
        # try:
            # subprocess.run(['samtools sort -O BAM -o %s %s' % (
                # sorted_bam_file_path, long_reads_alignment_path)], shell=True)
            # # pysam.sort('-l','9','-m','1500M','-@','4','-O', 'BAM', long_reads_alignment_path, '-o',
            # # sorted_bam_file_path, catch_stdout=False)
        # except Exception as e:
            # print(e,'Error occurred when using pysam.sort with 8 threads on %s. Trying 0 extra threads now.' % long_reads_alignment_path)
            # subprocess.run(['samtools sort -O BAM -o %s %s' % (sorted_bam_file_path, long_reads_alignment_path)],
                           # shell=True)
            # # pysam.sort('-O', 'bam', long_reads_alignment_path, '-o', sorted_bam_file_path, catch_stdout=False)
    ...
    def _prepare_alignment(self, output_directory, long_reads_alignment_path):
        start_process_time = time.time()
        ...
        if long_read_file_pysam.format == 'BAM':
            if long_read_file_pysam.has_index():
                ...
                print('%s is a .bam file but the index cannot be found. Sorting and indexing bam file.' % long_reads_alignment_path)
                sorted_bam_file_path = self._sort_and_index_alignment_file(long_reads_alignment_path,output_directory)
                # subprocess.run(['rm %s' % long_reads_alignment_path],shell=True)
        elif long_read_file_pysam.format == 'SAM':
            print('%s is a .sam file. Converting to binary (.bam), sorting, and indexing bam file.' % long_reads_alignment_path)
            sorted_bam_file_path = self._sort_and_index_alignment_file(long_reads_alignment_path, output_directory)
            # subprocess.run(['rm %s' % long_reads_alignment_path],shell=True)
        else:
            print('Error: Pysam does not recognize %s as being in SAM or BAM format. If aligned reads are provided as input they must be in proper .sam or .bam format.' % long_reads_alignment_path)
            return
            # summary_file(self.output_directory, time.time()-start_process_time, inspect.stack()[0][3],
        # sorted_bam_file_path)
        return sorted_bam_file_path

    def _extract_RG_info_from_long_read_input(self, long_read_input):
    ...
    def _parse_long_reads_input(self, long_read_input, output_directory):
        ...
        if not os.path.exists(long_reads_alignment_path):
                ...
                if os.path.isfile(file):
                    if str(file).lower().endswith('.fastq') or str(file).lower().endswith('.fastq.gz'):
                        ...
                        if not combined_long_read_fastq_path:
                            combined_long_read_fastq_path = os.path.abspath('%s/%s%s' % (output_directory, os.path.splitext(os.path.basename(long_reads_alignment_path))[0],'_combined_fastq.gz'))
                        ...
                        sorted_bam_file_paths.append(self._prepare_alignment(output_directory,file))

        elif os.path.isfile(long_reads_alignment_path):
            if str(long_reads_alignment_path).lower().endswith('.fastq') or str(long_reads_alignment_path).lower().endswith('.fastq.gz'):
                print('%s is a valid fastq file.' % long_reads_alignment_path)
                combined_long_read_fastq_path = os.path.abspath(long_reads_alignment_path)  # os.path.abspath('%s/%s%s' % (output_directory, os.path.splitext(
            ...
            elif str(long_reads_alignment_path).lower().endswith('.sam') or str(long_reads_alignment_path).lower().endswith('.bam'):
                sorted_bam_file_paths.append(self._prepare_alignment(output_directory, long_reads_alignment_path))
        else:
            print('Error: Reads should be in .fastq, .fastq.gz, .sam, or .bam format. %s does not have a correct suffix to be a valid format and is not a directory. Use -i to specify the path of a file containing reads for phasing OR use -i to specify the path of a directory containing the long read files and all files will be processed individually. (EX: -i path/minion_run3_GM12878/minion_run3_GM12878_0.fastq OR -i path/minion_run3_GM12878/minion_run3_GM12878_0.sam OR -i path/minion_run3_GM12878/minion_run3_GM12878_0_sorted.bam OR -i path/minion_run3_GM12878/).' % long_reads_alignment_path)
            return

        return sorted_bam_file_paths, combined_long_read_fastq_path

    def _prepare_summary_file(self, output_directory):

        print('############## Preparing summary_file ##############')

        summary_file_path = os.path.abspath('%s/%s' % (output_directory, 'summary.txt'))
        if os.path.exists(summary_file_path):
            print('%s already exists, WARNING: New results will overwrite old files' % os.path.abspath(summary_file_path),'\n')
    ...
    def _prepare_vcf_file(self, output_directory, vcf_file_input):
        ...
        if not os.path.isfile(vcf_file_input):
            print('Could not find %s. Use -v to specify the path of the vcf file to be used as haplotype information for phasing. (EX: -v path/to/GM12878.vcf.gz or --vcf GM12878.vcf).' % vcf_file_input)
            return
            
        elif os.path.isfile(vcf_file_input):
            vcf_file_pysam = pysam.VariantFile(vcf_file_input)
            if vcf_file_pysam.format == 'VCF':
                if vcf_file_pysam.compression == 'BGZF':
                    ...
                    if os.path.isfile(vcf_file_index_path):
                        ...
                        print('%s is a valid .vcf file in bgzip (.vcf.gz) format but an index was not found. Indexing with tabix now.' % vcf_file_input)
                    ...
                    print('%s is a valid .vcf file but it is not in bgzip (.vcf.gz) format. VCF files must be compressed with bgzip and indexed with tabix. LRphase will now attempt to run bgzip and tabix on %s' % (vcf_file_input, vcf_file_input))
                    vcf_file_path = '%s/%s%s' % (output_directory, os.path.splitext(os.path.basename(vcf_file_input))[0], 'sorted.vcf.gz')
                    #vcf_file_index_path = '%s%s' % (vcf_file_path, '.tbi')
                    
                    pysam.tabix_index(vcf_file_path, preset='vcf',force=True)
                    
                    # try:
                        # subprocess.run(["(grep ^'#' %s; grep -v ^'#' %s | sort -k1,1 -k2,2n) | bgzip > %s" % (vcf_file_input, vcf_file_input, vcf_file_path)], check=True, shell=True)
                    # except Exception as e:
                    ...
                    # else:
                        # print('Created %s' % vcf_file_path)
                    
                    # try:
                    
                        # #subprocess.run(['tabix', '-p', 'vcf', vcf_file_path],check=True)  # '-p' = 'preset' option to provide file type ('vcf')
                    # except Exception as e:
                        # print('Error occurred when tabix was run. Tabix must be installed on PATH for LRphase to '
                            # 'continue. Please check tabix installation or provide a tabix index for the vcf file in '
                            # 'the same folder as the bgzip compressed vcf file. If the vcf file has path: '
                            # 'path/to/name.vcf.gz file, then the index should be path/to/name.vcf.gz.tbi. Error: %s' %
                            # e)
                        # return
                    # else:
                        # print('Created %s' % vcf_file_index_path)
        ...
        if vcf_file_pysam.header.formats.keys().count('GT') == 0:
            print(
                    'The VCF file provided does not have the GT subfield. VCF files must have the GT subfield for all samples in order to extract genotype information. Phased variants should have | in the GT subfield instead of /'
                    )
            return

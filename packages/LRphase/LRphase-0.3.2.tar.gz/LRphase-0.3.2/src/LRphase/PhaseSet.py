...
def powlaw_modified(x, a = 4, xmin = 2):
...
class PhaseSet:
    ...
    def __init__(
        ...
        self.prior_probabilities = self._estimate_prior_probabilities()
    ...
    def __next__(self):
        if self.HeterozygousSites_processed_count < len(self.vcf_records):
            Heterozygous_Site = HeterozygousSite(self.vcf_records[self.HeterozygousSites_processed_count], self.sample,
    ...
    def solve_phase(self, error_model=0, error_rate_threshold=0.01, prior_probabilities=None,
        ...
        if not self.aligned_segment.is_unmapped or self.aligned_segment.is_aligned_to_contig_not_in_vcf:
            if len(self.gapped_alignment_positions) > 0:
                if not error_model == 5:
                    ...
                    self.log_probability_read_given_haplotype_i, self.non_matches, self.matches, self.total_hets, self.total_hets_analyzed, self.ploidy_phase_set = self._count_matches(
                        self.read_bases, self.phased_alleles, self.read_base_error_rates, multinomial_correction)

                    self.log_likelihood_ratios, self.phase, self.max_phase, self.max_log_likelihood_ratio = self._phasing_decision(
                    ...
                    for haplotype in range(1, self.ploidy_phase_set + 1):
                        haplotypes[haplotype] = sum([het.calculate_probs(haplotype=haplotype) for het in hets if
    ...
    def _retrieve_read_base_error_rates(self, aligned_segment, gapped_alignment_positions, error_model):
        ...
        if not self.aligned_segment.is_unmapped or self.aligned_segment.is_aligned_to_contig_not_in_vcf:
            if len(self.gapped_alignment_positions) > 0:
                ...
                if error_model == 3:
                    read_base_error_rates, per_base_mismatch_rate, error_rate_average_het_sites = self._calculate_local_per_base_mismatch_rate(
                        aligned_segment, gapped_alignment_positions)

                if error_model == 2:
                    read_base_error_rates, error_rate_average_het_sites = self._read_base_error_rates_from_quality_scores(
                        gapped_alignment_positions, aligned_segment.query_qualities)
                    per_base_mismatch_rate = aligned_segment.get_tag(tag='NM') / aligned_segment.query_alignment_length

                if error_model == 0:
                    per_base_mismatch_rate = aligned_segment.get_tag(tag='de')
                    read_base_error_rates = self._read_base_error_rates_from_gapped_alignment(
                ...
                if error_model == 4:
                    per_base_mismatch_rate = self._calculate_per_base_mismatch_rate(aligned_segment)
                    read_base_error_rates = self._read_base_error_rates_from_gapped_alignment(
                        gapped_alignment_positions, per_base_mismatch_rate)
    ...
    def _estimate_prior_probabilities(self, ploidy_phase_set=2, prior_probabilities=None):
    ...
    # @staticmethod
    def _get_reference_positions(self, vcf_records):
        '''self.reference_positions = self._get_reference_positions(self.vcf_records) '''
        return [vcf_record.pos - 1 for vcf_record in vcf_records]

    # @staticmethod
    def _get_phased_alleles(self, vcf_records, sample):
        '''self.phased_alleles = self._get_phased_alleles(self.vcf_records, self.sample) '''
        return [vcf_record.samples[sample].alleles for vcf_record in vcf_records]

    # @staticmethod
    def _generate_gapped_alignment(self, reference_start, cigartuples, reference_positions):
        '''# self.gapped_alignment_positions = self._generate_gapped_alignment(self.aligned_segment.reference_start,
    ...
    # @staticmethod
    def _read_bases_from_gapped_alignment(self, gapped_alignment_positions, query_sequence):
        '''# self.read_bases = self._read_bases_from_gapped_alignment(self.gapped_alignment_positions,
        self.aligned_segment.query_sequence) '''
    ...
    # @staticmethod
    def _read_base_error_rates_from_gapped_alignment(self, gapped_alignment_positions, per_base_error_rate):
    ...
    # @staticmethod
    def _estimate_per_base_error_rate(self, aligned_segment):
        '''# self.per_base_error_rate, self.error_rate_calculation = self._estimate_per_base_error_rate(self.aligned_segment)'''
        error_rate_calculation = []
        try:
            per_base_error_rate = aligned_segment.get_tag(
                tag='de')  # Gap-compressed per-base sequence divergence from minimap2 output
            ...
            try:
                ...
                try:
                    per_base_error_rate = self._calculate_per_base_mismatch_rate(aligned_segment)
    ...
    # @staticmethod
    def _calculate_per_base_mismatch_rate(self, aligned_segment):
    ...
    # @staticmethod
    def _calculate_local_per_base_mismatch_rate(self, aligned_segment, gapped_alignment_positions):
        ...
        for site in gapped_alignment_positions:
            if isinstance(site, str):
                ...
                try:
                    read_base_error_rates.append(round((2 * round(1 - (
                            sum(matches_alignment_query[int(site) - 10:int(site) + 10]) / len(
                        matches_alignment_query[int(site) - 10:int(site) + 10])), 4) + round(1 - (
                            sum(matches_alignment_query[int(site) - 25:int(site) + 25]) / len(
                        matches_alignment_query[int(site) - 25:int(site) + 25])), 4) + round(1 - (
                            sum(matches_alignment_query[int(site) - 50:int(site) + 50]) / len(
                        matches_alignment_query[int(site) - 50:int(site) + 50])), 4)) / 4, 4))
                    error_rate_total_het_sites += round((2 * round(1 - (
                            sum(matches_alignment_query[int(site) - 10:int(site) + 10]) / len(
                        matches_alignment_query[int(site) - 10:int(site) + 10])), 4) + round(1 - (
                            sum(matches_alignment_query[int(site) - 25:int(site) + 25]) / len(
                        matches_alignment_query[int(site) - 25:int(site) + 25])), 4) + round(1 - (
    ...
    # @staticmethod
    def _count_matches(self, read_bases, phased_alleles, read_base_error_rates, multinomial_correction):
        ...
        if multinomial_correction:
            for haplotype in range(0, ploidy_phase_set):
                multinomial = math.log10(self._multcoeff(int(matches[haplotype]), int(non_matches[haplotype])))
    ...
    # @staticmethod
    def _read_base_error_rates_from_quality_scores(self, gapped_alignment_positions, query_qualities):
        '''# self.read_base_error_rates, self.error_rate_average_het_sites =
        self._read_base_error_rates_from_quality_scores(self.gapped_alignment_positions, self.aligned_segment.query_qualities) '''

...
class PhasableSample:
    ...
    def __next__(self):
        ...
        if self.alignments_processed_count % 2000 == 0:
            print('Processing %s reads per second' % str(round(self.alignments_processed_count / (time.time() - self.start_process_time), 2)))
            print('Processed %s reads (%s percent complete)' % (str(self.alignments_processed_count), str(
                ...
                'Processing sample %s will finish in %s seconds' % (str(self.sample), str(
                        round(
                                (self.total_alignments - self.alignments_processed_count) / (
                                        self.alignments_processed_count / (time.time() - self.start_process_time)), 1
                                )
                        ))
                )
        return read
    ...
    def _get_RG_info_for_read(self, read: object, alignment_file: object) -> object:
        """

...
PACKED_TEMPLATE = """
@cuda.jit('void(uint64[:, :], float32[:, :], uint64)')
...
TEMPLATE = """
@cuda.jit('void(uint8[:, :], float32[:], float32[:, :])')
...
class Metric:
    ...
    @classmethod
    def get_packed_fn(cls):
        try:
            return cls.__packed_fn
        except AttributeError:
            cls.__packed_fn = cls._make_fn(False, PACKED_TEMPLATE)
            return cls.__packed_fn

    @classmethod
    def get_weight_fn(cls):
        try:
            return cls.__weight_fn
        except AttributeError:
            cls.__weight_fn = cls._make_fn(True, TEMPLATE)
            return cls.__weight_fn
    ...
    @classmethod
    def _calc_dist(cls, have_weights):
        return cls.CALC_DIST

    @classmethod
    def _make_fn(cls, have_weights, template):
        fn_name = f'kernel_{cls.NAMES[0]}_packed'
        ...
        fn_def = fn_def.replace('!!CALC_DIST!!', cls._calc_dist(have_weights))
    ...
    @classmethod
    def pairwise_distance(cls, values, weights=None, block_size=(16, 16)):
...
def pairwise_distance(values, metric='jaccard', weights=None, block_size=(16, 16)):
    metric_cls = Metric.get_metric(metric)
    return metric_cls.pairwise_distance(values, weights=weights, block_size=block_size)
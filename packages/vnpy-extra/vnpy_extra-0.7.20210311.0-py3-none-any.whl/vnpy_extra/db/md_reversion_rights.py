    ...
    try:
        ...
        for interval in intervals:
            ...
            if incrementally:
                ...
                symbol_datetime_dic[interval] = dict(database.execute_sql(sql_str, params=params))
                for _ in main_sec_symbols:
                    symbol_datetime_dic[interval].setdefault(_, None)
                ...
                symbol_datetime_dic[interval] = {_: None for _ in main_sec_symbols}
        ...
        for n, ((trade_date_larger_than, trade_date), adj_factor_s) in enumerate(adj_factor_df.iterrows(), start=1):
            ...
            for interval, (symbol_new, id_label, adj_factor_label) in itertools.product(intervals, symbol_label_pairs):
                ...
                if incrementally:
                    # 增量调整历史行情
                    if symbol_datetime_dic[interval][symbol_new] is not None:
                        # 存在历史行情
                        if symbol_datetime_dic[interval][symbol_new] < end:
                            # 历史行情小于当前主力合约的截止日期
                            if adj_factor != 1.0:
                                # 对历史行情数据进行因子调整
                                ...
                                logger.info("对 %s[%s] %s < %s 的历史数据进行因子调整 %.4f",
                                            symbol_new, interval, symbol_datetime_dic[interval][symbol_new],
                                            end, adj_factor)

                            # 历史行情经过因子调整后，修改 start 时间，此后的数据，通过正常因子调整逻辑进行调整
                            start_curr = symbol_datetime_dic[interval][symbol_new] + timedelta(minutes=1)
                            # 防止出现因子多次调整的情况，每一个合约每个周期只调整一次
                            symbol_datetime_dic[interval][symbol_new] = None
                        else:
                            # 已存在行情数据，无需进一步处理
                            continue

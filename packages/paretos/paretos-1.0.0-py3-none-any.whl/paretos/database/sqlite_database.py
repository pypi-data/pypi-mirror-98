...
from paretos.database.enums import (
    ParameterOptions,
    ParameterTypes,
    SimulationStatusEnum, ProjectStatusEnum,
)
...
class SQLiteDatabase(object):
    ...
    def add_project(self, session: Session, project_id: str, project_name: str, ):
        project = Project(
            id=project_id,
    ...
    def __add_db_parameter_value(
        self,
        session: Session,
        parameter_value: o.parameter.ParameterValue
    ) -> ParameterValue:
    ...
    def add_simulation_kpis(
        ...
        for kpi_value in kpi_values:
            parameter_value = self.__add_db_parameter_value(
                session=session,
                parameter_value=kpi_value
            )

            simulation.values.append(parameter_value)

        status = session.query(SimulationStatus) \
    ...
    def update_project_status(self, session: Session, project_id: str, status: str):
        ...
        status_id = session.query(ProjectStatus) \
            .filter(ProjectStatus.name == status) \
            .first() \
            .id

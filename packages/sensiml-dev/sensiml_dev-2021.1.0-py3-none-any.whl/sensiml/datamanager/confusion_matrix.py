...
class ConfusionMatrix(object):
    ...
    def __init__(self, metrics_dict):
        ...
        if not isinstance(metrics_dict, dict):
            raise ConfusionMatrixException(
                ("Expected a dictionary, instead got %s" % str(type(metrics_dict)))
            )
        keys_missing = []
    ...
    def __str__(self):
        ...
        pp_row += [
            ...
            "",
            "Acc(%)",
            100.0 * sum(np.array(cm).diagonal()) / (sum(cm.sum()) - sum(cm["UNC"])),
        ]
...
class ConfusionMatrixList(ConfusionMatrix):
    ...
    def _inspect_confusion_matrix(self):
        ...
        class_titles = [
            x for x in to_inspect.confusion_matrix.keys() if x not in STATIC_COLUMNS
        ]

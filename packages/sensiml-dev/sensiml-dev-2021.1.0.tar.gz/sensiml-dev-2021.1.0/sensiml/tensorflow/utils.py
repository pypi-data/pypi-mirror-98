...
def add_image_noise(image, label, rand_max=1, min_noise=10, max_noise=10):
    im_shape = image.shape
    [image] = tf.py_function(
        add_tensor_noise, [image, rand_max, min_noise, max_noise], [tf.int64]
    )
...
def mask_image_row(image, label, rand_max=1, max_frequency_bin=1, mask_value=127):
    im_shape = image.shape
    [image] = tf.py_function(
        mask_tensor_row, [image, rand_max, max_frequency_bin, mask_value], [tf.int64]
    )
    image.set_shape(im_shape)
...
def mask_image_column(
    image, label, num_cols, rand_max=1, max_frequency_bin=1, mask_value=127
):
    im_shape = image.shape
    [image] = tf.py_function(
        mask_tensor_columns,
...
def plot_training_results(
    tf_model, history, x_train, y_train, x_validate, y_validate, figsize=(32, 10)
):

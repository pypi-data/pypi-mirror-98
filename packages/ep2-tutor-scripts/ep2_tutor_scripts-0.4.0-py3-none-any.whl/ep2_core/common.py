...
class Ep2:
    ...
    def validate_grading(self, ex: int, grading: str) -> (bool, [float]):
        ...
        grading = [float(grade) for grade in grading]
        correct = all([grade <= max_points for (grade, max_points) in zip(grading, [subtask.max_points for subtask in subtasks])])

        return correct, grading

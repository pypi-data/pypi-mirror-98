...
def validate_grading(ctx, param, value):
    ...
    if value is None:
        return value
    match = re.match('^((\\d*\\.\\d+|\\d+\\.?)/)*(\\d*\\.\\d+|\\d+\\.?)$', value)
    if match is not None:
        return value
    else:
        raise click.BadParameter('can only consist of a slash separated list of numbers')
...
def grade(ctx, group, ue: int, student, grading, student_feedback, solution_remarks, points):
    ...
    if curr_ue_type == ExerciseType.Team:
        ...
        if grading is None:
            ...
            while not re.match('^((\\d*\\.\\d+|\\d+\\.?)/)*(\\d*\\.\\d+|\\d+\\.?)$', grading):
                grading = click.prompt('Grading')
    else:
        click.secho('Unknown exercise type %s' % curr_ue_type, fg='red')
        exit(1)
    ...
    if not grading_correct:
        click.secho('Some gradings are above the maximum point value')
        exit(2)
            ...
            for row in reader:
                ...
                if curr_ue_type == ExerciseType.Normal:
                    ...
                    if student_id == student:
                        row[KEY_UE_GRADING] = "/".join([str(grade) for grade in grading])
        ...
        with open(exercise_csv_file_tmp, 'w') as outfile:
            ...
            for row in reader:
                ...
                if curr_ue_type == ExerciseType.Normal:
                    ...
                    if not grading_correct:
                        click.secho('the grading for student {} is not correct. (either wrong number of subgrades or '
                                    'subgrade > max_points for subgrade)'.format(row[KEY_STUDENT_ID]))

                    template.grading = grading
                    if len(template.grading) < len(template.subtasks):
                        template.grading += [float(0)] * (len(template.subtasks) - len(template.grading))

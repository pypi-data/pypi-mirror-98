...
from texta_parsers.settings import ES_URL, FACE_VECTOR_FIELD_NAME, META_FIELD
...
# for digidoc parser
from .tools.elastic import ESImporter
...
class DocParser:
    ...
    @staticmethod
    def _parse_file(document, languages, use_face_analyzer=False):
        """
        ...
        tika_options = {
            "headers": {
                "X-Tika-OCRLanguage": "+".join(languages),
                "X-Tika-PDFextractInlineImages": "true"
            }
        }

        # extract faces if an image
        can_face_analyzer = ESImporter.check_face_analyzer_elegibility(ES_URL)
        is_image_file = document["extension"] in Extension.IMAGE_EXTENSIONS
        if use_face_analyzer and is_image_file and can_face_analyzer:
            # import here so we won't die on init if analyzer not installed
            from texta_face_analyzer.face_analyzer import FaceAnalyzer
            face_vectors = FaceAnalyzer().vectorize_photo(document["path"])
            if face_vectors:
    ...
    def _parse(self, parser_input, file_name=None, save_to_file=True, use_face_analyzer=False):
        """
            ...
            for meta in docs_to_parse:
                            ...
                            if ("subject" in attachment):  # is actually mail, do not parse it (already parsed!)
                                ...
                                if (self.parse_attachments == True):
                                    try:
                                        ...
                                        attachment[META_FIELD]["errors"] = "{}: {}".format(exc_type.__name__, value)
                ...
                # parse everything else
                    ...
                    if (save_to_file):
                        self.save_document(meta)
                    try:
                        for parsed_document in self._parse_file(meta, languages=self.langs, use_face_analyzer=use_face_analyzer):
    ...
    def parse(self, parser_input, file_name=None, use_face_analyzer=False):
        self.temp_dir = self.create_temp_dir_for_parse()
        generator = self._parse(parser_input, file_name, use_face_analyzer=use_face_analyzer)
        for item in generator:

...
def test_tauruslabel_text(
    qtbot, caplog, nodb_dev, model, fgRole, modelIndex, depr, fg
):
...
@pytest.mark.forked
def test_tauruslabel_bg(qtbot, caplog, nodb_dev, model, bgRole, bg):
    """"Check the label background"""
    _chek_tauruslabel(
        qtbot, caplog, nodb_dev, model, bgRole=bgRole, expected_bg=bg
    )
...
@pytest.mark.forked
def test_instance_format(qtbot, caplog, nodb_dev, model, fmt, fg):
    """Check formatter API at instance level"""
    _chek_tauruslabel(
        qtbot, caplog, nodb_dev, model, fmt=fmt, expected_fg=fg
    )

...
def main():
    ...
    parser.add_argument(
        "-c", "--config", dest="config", required=True, help="release config"
    )

    parser.add_argument(
        "-r", "--result", dest="result", required=True, help="result file"
    )

    parser.add_argument("-d", "--debug", dest="debug", action="store_true", help="mode")
    ...
    if os.path.isdir(result):
        candidates = sum(
            [glob.glob(os.path.join(result, e)) for e in ["*.csv", "*.json"]], []
        )
    else:
        candidates = (result,)

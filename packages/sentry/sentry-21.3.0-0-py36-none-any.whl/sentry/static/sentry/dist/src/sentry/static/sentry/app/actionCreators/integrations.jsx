...
export function deleteRepository(client, orgId, repositoryId) {
    ...
    promise.then(function () { return clearIndicators(); }, function () { return addErrorMessage(t('Unable to delete repository.')); });
    return promise;
}
/**
...
export function cancelDeleteRepository(client, orgId, repositoryId) {
    ...
    promise.then(function () { return clearIndicators(); }, function () { return addErrorMessage(t('Unable to cancel deletion.')); });
    return promise;
}
function applyRepositoryAddComplete(promise) {
    promise.then(function (repo) {
        var message = tct('[repo] has been successfully added.', {
            repo: repo.name,
        });

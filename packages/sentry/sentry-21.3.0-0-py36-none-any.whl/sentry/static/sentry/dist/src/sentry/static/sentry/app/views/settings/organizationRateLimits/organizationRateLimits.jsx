    ...
    return (<div>
      ...
      <Panel>
        ...
        <PanelBody>
          ...
          <Form data-test-id="rate-limit-editor" saveOnBlur allowUndo apiMethod="PUT" apiEndpoint={"/organizations/" + organization.slug + "/"} initialData={initialData}>
            {!maxRate ? (<RangeField name="accountRateLimit" label={t('Account Limit')} min={0} max={1000000} allowedValues={ACCOUNT_RATE_LIMIT_VALUES} help={t('The maximum number of events to accept across this entire organization.')} placeholder="e.g. 500" formatLabel={function (value) {
        return !value
            ? t('No Limit')
            : tct('[number] per hour', {
                number: value.toLocaleString(),
            });
    }}/>) : (<Field label={t('Account Limit')} help={t('The maximum number of events to accept across this entire organization.')}>
                <TextBlock css={{ marginBottom: 0 }}>
                  {tct('Your account is limited to a maximum of [maxRate] events per [maxRateInterval] seconds.', {
        maxRate: maxRate,
        maxRateInterval: maxRateInterval,
    })}
                </TextBlock>
              </Field>)}
            <RangeField name="projectRateLimit" label={t('Per-Project Limit')} help={t('The maximum percentage of the account limit (set above) that an individual project can consume.')} step={5} min={50} max={100} formatLabel={function (value) {
        return value !== 100 ? (value + "%") : (<span dangerouslySetInnerHTML={{ __html: t('No Limit') + " &mdash; 100%" }}/>);
    }}/>
          </Form>
        </PanelBody>
      </Panel>

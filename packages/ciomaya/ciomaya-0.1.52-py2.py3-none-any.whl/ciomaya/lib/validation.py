...
class ValidateUploadDaemon(Validator):
    def run(self, _):
        ...
        if not use_daemon:
            ...
            msg = 'This submission is set to upload in the Maya session. '
            msg +='For greater transparency, we recommend you turn on "Use Upload Daemon" in the Assets section of the UI.'
            self.add_notice(msg)
            return

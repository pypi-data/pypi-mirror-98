...
def main():
    ...
    #specify model
    parser.add_option('--humanTCR', '--human_TCR', action='store_true', dest='humanTCR', default=False, help='use default human TCR model (T cell alpha-beta paired chain)')
    parser.add_option('--mouseTCR', '--mouse_TCR', action='store_true', dest='mouseTCR', default=False, help='use default mouse TCR model (T cell alpha-beta chain)')
    parser.add_option('--humanIGHK', '--human_B_HK', action='store_true', dest='humanIGHK', default=False, help='use default human IGHK model (B cell heavy-kappa paired chain)')
    parser.add_option('--humanIGHL', '--human_B_HL', action='store_true', dest='humanIGHL', default=False, help='use default human IGHL model (B cell heavy-lambda paired chain)')
    ...
    parser.add_option('--deep', action='store_true', default = False, dest='deep' ,help='specify deep model. Default is linear (independent).')
    ...
    parser.add_option('--independent_genes', '--include_indep_genes', action='store_true', dest='independent_genes', default=False, help='Independent gene selection factors q_v*q_j. Deafult is joint q_vj (within chain) for linear model and independent for deep model.')
    #location of seqs
    ...
    if options.deep:
        ...
        indep=options.independent_genes
    ...
    if num_models_specified == 1: #exactly one model specified
        ...
        print('Exiting...')
        return -1
    elif num_models_specified == 2:
        if options.vj_model_folder=='PATH/TO/FOLDER/':
            print('Please specify path of both custom models')
            print('Exiting...')
            return -1
        if options.vdj_model_folder=='PATH/TO/FOLDER/':
            print('Please specify path of both custom models')
            print('Exiting...')
            return -1
        recompute_productive_norm=True
            model_folder_heavy = options.vdj_model_folder
            model_folder_light = options.vj_model_folder    
    ...
    if options.infile_name is None: #No infile specified -- args should be the input seqs
        ...
        if options.deep: 
            sonia_model=SoNNiaPaired(data_seqs=data_seqs,
                                             gen_seqs=gen_seqs,
            ...
            sonia_model=SoniaPaired(data_seqs=data_seqs,
                                       gen_seqs=gen_seqs,
                                       custom_pgen_model=model_folder,
                                       vj=recomb_type == 'VJ',
                                       include_joint_genes=joint_genes,

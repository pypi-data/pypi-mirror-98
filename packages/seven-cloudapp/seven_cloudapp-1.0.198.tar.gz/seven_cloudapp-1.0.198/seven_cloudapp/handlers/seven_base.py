...
:LastEditTime: 2021-03-09 16:04:35
...
class SevenBaseHandler(BaseApiHandler):
    ...
    def reponse_json_error(self, error_code="", error_message="", data=None, log_type=0):
        ...
        """
        if log_type == 1:
            self.logger_info.info(f"{error_code}\n{error_message}\n{data}\n{self.request}")
        elif log_type == 2:
            self.logger_error.error(f"{error_code}\n{error_message}\n{data}\n{self.request}")
    ...
    def get_app_key_secret(self):
        ...
        if request_app_key not in (0, app_key):
            dict_product = MiddlerProductModel().get_dict("app_key=%s", field="app_secret", params=request_app_key)
            if dict_product:

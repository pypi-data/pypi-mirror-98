...
:LastEditTime: 2021-03-02 11:59:20
...
class LeftNavigationHandler(TopBaseHandler):
    ...
    def get_async(self):
        ...
        product_price = ProductPriceModel().get_entity("is_release=1")
...
class SaasCustomHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        cloud_app_id = 0
        saas_custom = SaasCustomModel().get_entity("store_user_nick=%s AND is_release=1", params=store_user_nick)
        if saas_custom:
...
class OperateLoginHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        app_info = AppInfoModel().get_dict("store_name=%s", params=store_name)

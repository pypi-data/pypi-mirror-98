    ...
    def get_async(self):
        ...
        for info in act_type_list:
            ...
            if is_act > 0 and app_id:
                act_dict = ActInfoModel().get_dict("app_id=%s and act_type=%s", params=[app_id, info.id])
                if act_dict:
...
class ActCreateHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        theme_info = ThemeInfoModel().get_entity()
        if not theme_info:
            return self.reponse_json_error("NoTheme", "请管理员先上传主题信息")
        skin_info = SkinInfoModel().get_entity("theme_id=%s", params=theme_info.id)
        if not skin_info:
            return self.reponse_json_error("NoSkin", "请管理员先上传皮肤信息")
        act_type_info = ActTypeModel().get_entity("id=%s", params=act_type)
...
class ActReleaseHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        ActInfoModel().update_table("modify_date=%s,is_release=%s", "id=%s", [modify_date, is_release, act_id])
...
class NextProgressHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if finish_progress == 1:
            app_id = self.get_app_id()
            if app_id:
                AppInfoModel().update_table("is_setting=1", "app_id=%s", app_id)

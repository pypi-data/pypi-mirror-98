...
class GoodsInfoHandler(TopBaseHandler):
    ...
    def get_async(self):
        ...
        if is_check_machine_exist > 0:
            exist_machineed = MachineInfoModel().get_dict("goods_id=%s and id<>%s", params=[num_iid, machine_id])
            if exist_machineed:
...
class GoodsCheckHandler(TopBaseHandler):
    ...
    def get_async(self):
        ...
        price_gear_dict = PriceGearModel().get_dict(condition, params=[act_id, num_iid])
        if price_gear_dict:
            act_dict = ActInfoModel().get_dict_by_id(price_gear_dict['act_id'])

...
:LastEditTime: 2021-02-07 12:59:28
...
class LoginHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        act_dict = ActInfoModel().get_dict_by_id(act_id)
        if not act_dict:
            return self.reponse_json_error("NoAct", "对不起，活动不存在")

        if is_machine_gear in (1, 2):
            machine_dict_list = MachineInfoModel().get_dict_list("act_id=%s", params=[act_id])
            if not machine_dict_list:
                return self.reponse_json_error("NoMachine", "对不起，盲盒不存在")

        # 用户基础表
        user_base = UserBaseModel().get_entity("open_id=%s", params=[open_id])
        ...
        if is_machine_gear == 1:
            machine_value_list = MachineValueModel().get_dict_list("act_id=%s and open_id=%s", params=[act_id, open_id])
            user_info_dict["machine_value_list"] = machine_value_list
        elif is_machine_gear == 2:
            gear_value_list = GearValueModel().get_dict_list("act_id=%s and open_id=%s", params=[act_id, open_id])

...
class ThemeInfoHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if ver_no:
            theme_ver = ThemeVerModel().get_entity("out_id=%s and ver_no=%s", params=[out_id, ver_no])
            if theme_ver and theme_ver.client_json != "":
                theme_info["client_json"] = theme_ver.client_json

        skin_info_list = SkinInfoModel().get_dict_list("theme_id=%s", params=theme_id)

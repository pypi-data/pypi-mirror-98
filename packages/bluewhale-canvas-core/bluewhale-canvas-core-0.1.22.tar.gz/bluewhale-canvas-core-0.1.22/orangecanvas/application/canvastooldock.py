...
from .widgettoolbox import item_name
...
class SplitterResizer(QObject):
    ...
    def setSplitterAndWidget(self, splitter, widget):
        ...
        if splitter and widget and not splitter.indexOf(widget) > 0:
            raise ValueError(__("err_tip.splitter_widget"))
    ...
    def toogleExpandedAction(self):
        warnings.warn(
            __("tip.toogleExpandedAction"), DeprecationWarning, stacklevel=2
        )
...
class CanvasToolDock(QWidget):
    ...
    def toogleQuickHelpAction(self):
        warnings.warn(
            __("tip.toogleQuickHelpAction"), DeprecationWarning,
            stacklevel=2
        )
...
class QuickCategoryToolbar(ToolGrid):
    ...
    def setColumnCount(self, count):
        raise Exception(__("err_tip.set_column"))
    ...
    def createActionForItem(self, index):
        ...
        """
        action = QAction(
            item_icon(index), item_name(index), self,
...
class CategoryPopupMenu(FramelessWindow):
    ...
    def setCategoryItem(self, item):
        ...
        """
        warnings.warn(
            __("tip.setmodel"), DeprecationWarning, stacklevel=2
        )
        model = item.model()

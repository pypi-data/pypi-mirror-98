...
def NodeItem_toolTipHelper(node, links_in=[], links_out=[]):
    ...
    inputs_list_fmt = __("tool_desc.input")
    outputs_list_fmt = __("tool_desc.output")
    if desc.inputs:
        inputs = [channel_fmt.format(inp.label) for inp in desc.inputs]
        inputs = inputs_list_fmt.format(inputs="".join(inputs))
    else:
        inputs = __('tool_desc.no_input')

    if desc.outputs:
        outputs = [channel_fmt.format(out.label) for out in desc.outputs]
        outputs = outputs_list_fmt.format(outputs="".join(outputs))
    else:
        outputs = __("tool_desc.no_output")

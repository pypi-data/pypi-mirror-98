...
class SchemeEditWidget(QWidget):
    ...
    def __init__(self, parent=None, ):
        ...
        self.__editMenu = QMenu(self.tr("&" + _("menu.edit")), self)
        ...
        # Widget context menu
        self.__widgetMenu = QMenu(self.tr(_("menu.widget")), self)
        ...
        self.__menuBarWidgetMenu = QMenu(self.tr("&"+ _("menu.widget")), self)
        ...
        self.__linkMenu = QMenu(self.tr(_("menu.link")), self)
    ...
    def __setupActions(self):
        ...
        self.__selectAllAction = QAction(
            self.tr(_("edit.select_all")), self,
            objectName="select-all-action",
            toolTip=self.tr(_("edit.select_all_tip")),
            triggered=self.selectAll,
            shortcut=QKeySequence.SelectAll
        )
        self.__openSelectedAction = QAction(
            self.tr(_("edit.open")), self,
            objectName="open-action",
            toolTip=self.tr(_("edit.open_tip")),
            triggered=self.openSelected,
            enabled=False
        )
        self.__removeSelectedAction = QAction(
            self.tr(_("edit.remove")), self,
            objectName="remove-selected",
            toolTip=self.tr(_("edit.remove_item_tip")),
        ...
        self.__renameAction = QAction(
            self.tr(_("edit.rename")), self,
            objectName="rename-action",
            toolTip=self.tr(_("edit.rename_tip")),
        ...
        self.__helpAction = QAction(
            self.tr(_("edit.help")), self,
            objectName="help-action",
            toolTip=self.tr(_("edit.help_tip")),
            triggered=self.__onHelpAction,
            shortcut=QKeySequence("F1"),
            enabled=False,
        )
        self.__linkEnableAction = QAction(
            self.tr(_("edit.enabled")), self, objectName="link-enable-action",
        ...
        self.__windowGroupsAction = QAction(
            self.tr(_("view.win_groups")), self, objectName="window-groups-action",
            toolTip=_("view.win_groups_tip")
        )
        ...
        self.__raiseWidgetsAction = QAction(
            self.tr(_("view.bring_front")), self,
    ...
    def removeSelected(self):
        ...
        if not selected:
            return
        scene = self.scene()
        self.__undoStack.beginMacro(self.tr(_("edit.remove")))
    ...
    def alignToGrid(self):
        ...
        if nodes:
            self.__undoStack.beginMacro(self.tr(_("view.grid_align")))
    ...
    def sceneMouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.LeftButton and self.__possibleMouseItemsMove:
            ...
            if self.__itemsMoving:
                ...
                stack.beginMacro(self.tr(_("edit.move")))
    ...
    def __onSelectionChanged(self):
        ...
        if len(nodes) > 1:
            self.__openSelectedAction.setText(self.tr(_("edit.open_all")))
        else:
            self.__openSelectedAction.setText(self.tr(_("edit.open")))

        if len(nodes) + len(annotations) + len(links) > 1:
            self.__removeSelectedAction.setText(self.tr(_("edit.remove_all")))
        else:
            self.__removeSelectedAction.setText(self.tr(_("edit.remove")))
    ...
    def __showHelpFor(self, help_url):
        ...
        if not handled:
            message_information(
                self.tr(_("msg.no_documentation_tip")),
                parent=self)
    ...
    def __toggleLinkEnabled(self, enabled):
        ...
        if self.__contextMenuTarget:
            ...
            command = commands.SetAttrCommand(
                link, "enabled", enabled, name=self.tr(_("err_tip.set_enabled")),
            )
            self.__undoStack.push(command)
    ...
    def __duplicateSelected(self):
        ...
        self.__paste(nodedups, linkdups, pos + DuplicateOffset,
                     commandname=self.tr(_("edit.duplicate")))
    ...
    def __paste(self, nodedups, linkdups, pos: Optional[QPointF] = None,
        ...
        if commandname is None:
            commandname = self.tr(_("edit.paste"))
        # create nodes, links
    ...
    def __saveWindowGroup(self):
        ...
        dlg = SaveWindowGroup(
            self, windowTitle=_("edit.save_groupas"))
...
class SaveWindowGroup(QDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        cb.setEditText(self.tr(_("edit.win_group_one")))
        cb.lineEdit().selectAll()
        form.addRow(self.tr(_("btn.save_as")), cb)
        self._checkbox = check = QCheckBox(
            self.tr(_("edit.use_default")),
            toolTip=_("edit.use_default_tip")
        )
        form.setWidget(1, QFormLayout.FieldRole, check)
        bb = QDialogButtonBox(
            standardButtons=QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        bb.button(QDialogButtonBox.Ok).setText(_("btn.ok"))
        bb.button(QDialogButtonBox.Cancel).setText(_("btn.cancel"))
    ...
    def __accept_check(self):
        ...
        mb = QMessageBox(
            self, windowTitle=self.tr(_("msg.overwrite")),
            icon=QMessageBox.Question,
            standardButtons=QMessageBox.Yes | QMessageBox.Cancel,
            text=self.tr((_("msg.overwritecont")).format(text)),
        )
        mb.setDefaultButton(QMessageBox.Yes)
        mb.setEscapeButton(QMessageBox.Cancel)
        mb.setWindowModality(Qt.WindowModal)
        button = mb.button(QMessageBox.Yes)
        button.setText(self.tr(_("edit.replace")))
        mb.button(QMessageBox.Cancel).setText(_("btn.cancel"))

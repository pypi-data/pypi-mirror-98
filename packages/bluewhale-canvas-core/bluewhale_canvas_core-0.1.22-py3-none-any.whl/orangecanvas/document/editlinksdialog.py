...
class EditLinksDialog(QDialog):
    ...
    def __setupUi(self):
        ...
        clear_button.setText(self.tr(__("btn.clear_all")))
        buttons.button(QDialogButtonBox.Ok).setText(__("btn.ok"))
        buttons.button(QDialogButtonBox.Cancel).setText(__("btn.cancel"))
...
class LinksEditWidget(QGraphicsWidget):
    ...
    def addLink(self, output, input):
        ...
        if output not in self.source.output_channels():
            raise ValueError(__("err_tip.output_channel") % (output, self.source))

        if input not in self.sink.input_channels():
            raise ValueError(__("err_tip.input_channel") % (input, self.sink))
        ...
        for opposite_anchor in opposite_anchors:
            ...
            if isinstance(first_channel, OutputSignal) and \
               ...
               compatible_channels(second_channel, first_channel):
                ...
                anchor.setToolTip(__("tool_desc.drag"))
                return
        if isinstance(first_channel, OutputSignal):
            anchor.setToolTip(__("tip.compatible_inchannel"))
        else:
            anchor.setToolTip(__("tip.compatible_outchannel"))

...
class QtWidgetRegistry(QObject, WidgetRegistry):
    ...
    def _insert_category(self, desc):
        ...
        self.category_added.emit(desc.label, desc)
    ...
    def _cat_desc_to_std_item(self, desc):
        ...
        """
        item = QStandardItem()
        item.setText(desc.label or desc.name)
...
def tooltip_helper(desc):
    ...
    if desc.project_name and desc.project_name != "Orange":
        tooltip[0] += __("edit.from").format(desc.project_name)
    ...
    if desc.inputs:
        inputs = "".join(inputs_fmt.format(name=inp.label,
                                           class_name=type_str(inp.types))
                         for inp in desc.inputs)
        tooltip.append(__("edit.input").format(inputs))
    else:
        tooltip.append(__("edit.no_input"))

    if desc.outputs:
        outputs = "".join(inputs_fmt.format(name=out.label,
                                            class_name=type_str(out.types))
                          for out in desc.outputs)
        tooltip.append(__("edit.output").format(outputs))
    else:
        tooltip.append(__("edit.no_output"))
...
def whats_this_helper(desc, include_more_link=False):
    ...
    if help_url and include_more_link:
        template.append(__("tool_desc.more").format(escape(help_url)))

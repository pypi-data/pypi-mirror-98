...
class UserSettingsDialog(QMainWindow):
    ...
    def __setupUi(self):
        ...
        buttonbox.button(QDialogButtonBox.Close).setText(__("btn.close"))
        ...
        # General Tab
        tab = QWidget()
        self.addTab(tab, self.tr(__("option_settings.general")),
                    toolTip=self.tr(__("btn.general_options")))
        ...
        cb_anim = QCheckBox(
            self.tr(__("set_general.enable_node")),
            objectName="enable-node-animations",
            toolTip=self.tr(__("set_general.enable_node_tip"))
        )
        self.bind(cb_anim, "checked", "schemeedit/enable-node-animations")
        nodes.layout().addWidget(cb_anim)

        form.addRow(self.tr(__("set_general.nodes")), nodes)
        ...
        cb_show = QCheckBox(
            self.tr(__("set_general.show_channel")),
            objectName="show-channel-names",
            toolTip=self.tr(__("err_tip.over_link"))
        )
        ...
        form.addRow(self.tr(__("noun.links")), links)
        ...
        cb1 = QCheckBox(self.tr(__("set_general.double_click")),
                        toolTip=self.tr(__("set_general.double_click_tip")))

        cb2 = QCheckBox(self.tr(__("set_general.right_click")),
                        toolTip=self.tr(__("set_general.right_click_tip")))

        cb3 = QCheckBox(self.tr(__("set_general.space_key")),
                        toolTip=self.tr(__("set_general.space_key_tip")))

        cb4 = QCheckBox(self.tr(__("set_general.any_key")),
                        toolTip=self.tr(__("set_general.any_key_tip")))

        cb5 = QCheckBox(self.tr(__("set_general.show_categories")),
                        toolTip=self.tr(__("set_general.show_categories_tip")))
        ...
        form.addRow(self.tr(__("set_general.quick_menu")), quickmenu)

        startup = QWidget(self, objectName="startup-group")
        startup.setLayout(QVBoxLayout())
        startup.layout().setContentsMargins(0, 0, 0, 0)

        cb_splash = QCheckBox(self.tr(__("set_general.show_splash")), self,
                              objectName="show-splash-screen")

        cb_welcome = QCheckBox(self.tr(__("set_general.show_welcome")), self,
                                objectName="show-welcome-screen")

        cb_crash = QCheckBox(self.tr(__("set_general.crash")), self,
        ...
        form.addRow(self.tr(__("set_general.startup")), startup)

        toolbox = QWidget(self, objectName="toolbox-group")
        toolbox.setLayout(QVBoxLayout())
        toolbox.layout().setContentsMargins(0, 0, 0, 0)

        exclusive = QCheckBox(self.tr(__("set_general.only_open_one_tab")))

        self.bind(exclusive, "checked", "mainwindow/toolbox-dock-exclusive")

        toolbox.layout().addWidget(exclusive)

        form.addRow(self.tr(__("set_general.tool_box")), toolbox)
        tab.setLayout(form)

        # Style tab
        tab = StyleConfigWidget()
        self.addTab(tab, self.tr("&" + __("option_settings.style")), toolTip="Application style")
        self.bind(tab, "selectedStyle_", "application-style/style-name")
        self.bind(tab, "selectedPalette_", "application-style/palette")

        # Output Tab
        tab = QWidget()
        self.addTab(tab, self.tr(__("option_settings.output")),
                    toolTip=__("option_settings.output_tip"))

        form = FormLayout()

        combo = QComboBox()
        combo.addItems([self.tr(__("logging.critical")),
                        self.tr(__("logging.error")),
                        self.tr(__("logging.warn")),
                        self.tr(__("logging.info")),
                        self.tr(__("logging.debug"))])
        self.bind(combo, "currentIndex", "logging/level")
        form.addRow(self.tr(__("option_settings.logging")), combo)

        box = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        cb1 = QCheckBox(self.tr(__("option_settings.open_browser")),
                        objectName="open-in-external-browser")
        self.bind(cb1, "checked", "help/open-in-external-browser")
        layout.addWidget(cb1)
        box.setLayout(layout)
        form.addRow(self.tr(__("option_settings.help_window")), box)
        ...
        self.addTab(tab, __("option_settings.categories"))

        # Add-ons Tab
        tab = QWidget()
        self.addTab(tab, self.tr(__("option_settings.addons")),
                    toolTip=__("option_settings.set_addinstall"))
        ...
        cb_conda_install = QCheckBox(self.tr(__("option_settings.conda_install")), self,
        ...
        form.addRow(self.tr("Pip"), QLabel(__("option_settings.pip_arguments")))
        ...
        # Network Tab
        tab = QWidget()
        self.addTab(tab, self.tr(__("option_settings.network")),
                    toolTip=__("option_settings.network_tip"))

        form = FormLayout()
        line_edit_http_proxy = QLineEdit()
        self.bind(line_edit_http_proxy, "text", "network/http-proxy")
        form.addRow(__("option_settings.http"), line_edit_http_proxy)
        line_edit_https_proxy = QLineEdit()
        self.bind(line_edit_https_proxy, "text", "network/https-proxy")
        form.addRow(__("option_settings.https"), line_edit_https_proxy)
        tab.setLayout(form)
...
class StyleConfigWidget(QWidget):
    DisplayNames = {
        "windowsvista": __("option_settings.windows"),
        "macintosh": __("option_settings.mac"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # on application star.
        styles = [(__("option_settings.default"), "")] + styles
        ...
        colors_cb.addItem(__("option_settings.default"), userData="")
        colors_cb.addItem(__("option_settings.breeze_light"), userData="breeze-light")
        colors_cb.addItem(__("option_settings.breeze_dark"), userData="breeze-dark")
        colors_cb.addItem(__("option_settings.zion_reversed"), userData="zion-reversed")
        colors_cb.addItem(__("option_settings.dark"), userData="dark")

        form.addRow(__("option_settings.style"), style_cb)
        form.addRow(__("option_settings.color_theme"), colors_cb)
        label = QLabel(
            __("option_settings.restart_app"),
            enabled=False,
        )

...
class PluginsModel(QStandardItemModel):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.setHorizontalHeaderLabels(
            ["", self.tr(__("label.name")), self.tr(__("label.version")), self.tr(__("label.action"))]
        )
    ...
    def createRow(item):
        ...
        if isinstance(item, Installed):
            ...
            name = ins.label if ins else dist.project_name
...
class AddonManagerDialog(QDialog):
    ...
    def __init__(self, parent=None, acceptDrops=True, *,
        ...
        self.__search = QLineEdit(
            ...
            placeholderText=self.tr(__("placeholder.filter"))
        )
        self.__addmore = QPushButton(
            self.tr(__("btn.add_more")),
            toolTip=self.tr(__("btn.add_more_tip")),
            autoDefault=False
        )
        ...
        self.__buttons.button(QDialogButtonBox.Ok).setText(__("btn.ok"))
        self.__buttons.button(QDialogButtonBox.Cancel).setText(__("btn.cancel"))
    ...
    def start(self, config):
        ...
        progress.setLabelText(
            self.tr(__("label.retrieving_package_list"))
        )
    ...
    def __on_query_done(self, f):
        ...
        if f.exception() is not None:
            ...
            message_warning(
                ...
                title=__("logging.error"),
                informative_text=
                    __("set_general.informative_text"),
                details=
    ...
    def runQueryAndAddResults(
        ...
        progress = self.progressDialog()
        progress.setLabelText(__("row.run_query"))
    ...
    def __run_add_package_dialog(self):
        self.__add_package_by_name_dialog = dlg = QDialog(
            self, windowTitle=__("set_general.add_package"),
        )
        ...
        nameentry = QLineEdit(
            placeholderText=__("placeholder.package_name"),
            toolTip=__("placeholder.package_name_tip"))
        nameentry.setMinimumWidth(250)
        form.addRow(__("row.name"), nameentry)
        ...
        okb.setText(__("row.add"))

        cancel_btn = buttons.button(QDialogButtonBox.Cancel)
        cancel_btn.setText(__("btn.cancel"))
    ...
    def __show_error_for_query(self, text, error_details):
        message_error(text, title=__("logging.error"), details=error_details)
    ...
    def progressDialog(self):
        ...
        if self.__progress is None:
            self.__progress = QProgressDialog(
                self,
                minimum=0, maximum=0,
                labelText=self.tr(__("label.retrieving_package_list")),
                sizeGripEnabled=False,
                windowTitle=__("tip.progress")
            )
    ...
    def __accepted(self):
        ...
        if core_upgrade:
            ...
            title = __("logging.warn")
            text = __("msg.upgrade_text")
            text += "\n".join(sorted(core_upgrade))
            msg_box = QMessageBox(icon, title, text, buttons, self)
            msg_box.setInformativeText(__("msg.continue"))
            msg_box.setDefaultButton(QMessageBox.Ok)
            msg_box.button(QMessageBox.Ok).setText(__("btn.ok"))
            msg_box.button(QMessageBox.Cancel).setText(__("btn.cancel"))
        ...
        if steps:
            ...
            progress.show()
            progress.setLabelText(__("label.installing"))
    ...
    def __on_installer_error(self, command, pkg, retcode, output):
        ...
        message_error(
            __("tip.install_error"), title=__("logging.error"),
            informative_text=__("tip.install_error_text").format(command),
            details="".join(output),
            parent=self
        )
        self.reject()
    ...
    def __on_installer_finished(self):
        ...
        def message_restart(parent):
            ...
            buttons = QMessageBox.Ok | QMessageBox.Cancel
            title = __('noun.info')
            text = __('msg.close_app_text').format(name)
            msg_box = QMessageBox(icon, title, text, buttons, parent)
            msg_box.setDefaultButton(QMessageBox.Ok)
            msg_box.setInformativeText(__('msg.close_app_cont').format(name))
            msg_box.button(QMessageBox.Cancel).setText(__('msg.btn_close_later'))
...
def installable_from_json_response(meta):
    ...
    return Installable(name, label, version, summary, description, package_url, release_urls,
...
class PipInstaller:
    ...
    def install(self, pkg, mirror=('-i', 'https://pypi.tuna.tsinghua.edu.cn/simple')):
        # type: (Installable) -> None
    ...
    def upgrade(self, package, mirror=('-i', 'https://pypi.tuna.tsinghua.edu.cn/simple')):
        cmd = [

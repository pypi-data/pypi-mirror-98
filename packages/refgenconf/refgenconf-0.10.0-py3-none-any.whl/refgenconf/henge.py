...
class Henge(object):
    def __init__(self, database, schemas, henges=None, checksum_function=md5):
        ...
        if isinstance(schemas, dict):
            ...
            populated_schemas = {}
            for schema_key, schema_value in list(schemas.items()):
        ...
        if henges:
            for item_type, henge in list(henges.items()):
    ...
    def retrieve(self, druid, reclimit=None, raw=False):
        ...
        def reconstruct_item(string, schema, reclimit):
            if "type" in schema and schema["type"] == "array":
                ...
                item_reconstituted = dict(list(zip(list(schema["properties"].keys()), attr_array)))
    ...
    def insert(self, item, item_type):
        ...
        if item_type not in list(self.schemas.keys()):
            _LOGGER.error(
    ...
    def clean(self):
        ...
        """
        for k, v in list(self.database.items()):
            try:
                del self.database[k]

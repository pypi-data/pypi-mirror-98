...
class _RefGenConfV03(yacman.YacAttMap):
    ...
    def list(self, genome=None, order=None, include_tags=False):
        ...
        refgens = _select_genomes(
            sorted(list(self[CFG_GENOMES_KEY].keys()), key=order), genome
        )
        if include_tags:
    ...
    def get_local_data_str(self, genome=None, order=None):
        ...
        genomes_str = (
            ...
            if genome is None
                ...
                _select_genomes(sorted(list(self[CFG_GENOMES_KEY].keys()), key=order), genome)
            )
        )
...
def _list_remote(url, genome, order=None, as_str=True):
    ...
    refgens = _select_genomes(
        sorted(list(genomes_data.keys()), key=order), genome, strict=True
    )
    if not refgens:

...
class RefGenConf(yacman.YacAttMap):
    ...
    def __init__(
        ...
        self[CFG_GENOMES_KEY] = yacman.AliasedYacAttMap(
            ...
            aliases=lambda x: {k: v.__getitem__(CFG_ALIASES_KEY) for k, v in list(x.items())},
            aliases_strict=True,
            exact=genome_exact,
        )
    ...
    def genome_aliases_table(self):
        ...
        if CFG_GENOMES_KEY not in self or not self[CFG_GENOMES_KEY]:
            return table
        for genome, genome_dict in list(self[CFG_GENOMES_KEY].items()):
            if (
        ...
        def _fill_table_with_genomes_data(rgc, genomes_data, table, genomes=None):
            ...
            if genomes:
                ...
                for g in genomes:
                    ...
                    for asset, asset_dict in list(genome_dict[CFG_ASSETS_KEY].items()):
    ...
    def assets_str(
        ...
        make_line = partial(
            ...
            asset_sep=asset_sep,
            order=order,
            rjust=max(list(map(len, refgens)) or [0]) + 2,
        )
        return "\n".join(
    ...
    def _symlink_alias(
        ...
        for alias, path in list(target_paths_mapping.items()):
    ...
    def get_default_tag(self, genome, asset, use_existing=True):
        ...
        try:
            ...
            alt = (
                list(self[CFG_GENOMES_KEY][genome][CFG_ASSETS_KEY][asset][
                    CFG_ASSET_TAGS_KEY
                ].keys())[0]
                if use_existing
                else DEFAULT_TAG
        ...
        return (
            ...
            if not asset
                ...
                [
                    ...
                    for g, data in list(self[CFG_GENOMES_KEY].items())
    ...
    def listr(
        ...
        for url in self[CFG_SERVERS_KEY]:
            ...
            digests_by_alias = {}
            for k, v in list(aliases_by_digest.items()):
                for alias in v:
            ...
            if genome is not None:
                genome_digests = [
                    g
                    if g in list(aliases_by_digest.keys())
            ...
            data_by_server[assets_url] = (
                server_data
                if as_digests
                else {aliases_by_digest[k][0]: v for k, v in list(server_data.items())}
            )
    ...
    def get_genome_alias_digest(self, alias, fallback=False):
        ...
        try:
            ...
            if not fallback:
                raise
            if alias in list(self.genome_aliases.values()):
                return alias
            raise
    ...
    def get_genome_alias(self, digest, fallback=False, all_aliases=False):
        ...
        try:
            ...
            if not fallback:
                raise
            if digest in list(self.genome_aliases.keys()):
                return digest
            raise
    ...
    def _get_genome_id(self, gname):
        ...
        if gname in list(self[CFG_GENOMES_KEY].keys()):
            return gname
    ...
    def _list_remote(
        ...
        return (
            {g: data for g, data in list(genomes_data.items()) if g in genome}
            if genome is not None
            else genomes_data
        )
    ...
    def _select_genomes(
        ...
        if external_genomes:
            ...
            genomes = [
                self.get_genome_alias(x, fallback=True)
                for x in sorted(list(self[CFG_GENOMES_KEY].keys()), key=order)
            ]
        if not genome:
            return genomes
        genome = [
...
def upgrade_config(
    ...
    missing_digest = []
    for genome, genome_v in list(rgc[CFG_GENOMES_KEY].items()):
        try:
...
def map_paths_by_id(json_dict):
    ...
    return {
        ...
        for endpoint, values in list(json_dict["paths"].items())
    }

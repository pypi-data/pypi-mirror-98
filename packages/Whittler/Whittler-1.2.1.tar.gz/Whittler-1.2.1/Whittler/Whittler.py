...
def load_module(fname, in_default_module_directory):
    if in_default_module_directory:
        ...
        if not os.path.isfile(modulepath):
            wprint(f"\nCouldn't find module {fname}.\n")
            parser.print_usage()
            sys.exit(1)
        _, fname = os.path.split(modulepath)
    ...
    if "/" in fname:
        ...
        _fname = fname
    if in_default_module_directory and _fname in result_modules:
        raise Exception(f"Can't import the module {_fname} because it is named identically to a different module "+\
                         "that has already been imported.")
    result_modules[_fname] = module
    result_class = None
    for clsname in dir(module):
        ...
        if isinstance(cls, type) and issubclass(cls, Result) and not cls is Result:
            # check whether this class object is defined in this specific module file, or somewhere else.
            if cls.__module__.split(".")[-1] == _fname.replace(".py",""):
                if result_class is None:
                    pass
                else:
                    raise Exception(f"Multiple Result subclasses defined in the module loaded from {module.__file__}")
            else:
                # it's from a different module file, probably an import to subclass a different Result type
                continue
            if cls.FRIENDLY_NAME in result_classes:
                raise Exception(f"Can't import the Result class {cls.__name__} from {_fname} because its FRIENDLY_NAME "+\
                                f"attribute, {cls.FRIENDLY_NAME}, is the same as the one imported from the "+\
                                f"{result_classes[cls.FRIENDLY_NAME].__module__} module.")
...
def main():
    ...
    do_import = getattr(args,"import",None)
    ...
    try:
        ...
        if not args.dir and not args.file and not do_import:
            parser.print_help()
            sys.exit(1)
        ...
        if do_import:
            ...
            wprint()
            for import_target in do_import:

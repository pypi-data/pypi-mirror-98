...
class Tensor(torch.Tensor):
    ...
    def set_special_(self, batch_dim=None, channel_dim=None, *, special=[]):
        ...
        if batch_dim is None and channel_dim is None:
            ...
            if not self.init: self._batch_first = True
            if a < b:
                self._special = [a, b]
            else:
                self._special = [b, a]
    ...
    def __torch_function_reducing_func__(cls, func, types, args=(), kwargs=None):
        ...
        with torch._C.DisableTorchFunction():
            ...
            if len(args) > 1:
                ...
                for r in collection:
                    r.set_special_(special=[x - len([d for d in dims if 0 <= d < x or d + self.ndim < x]) if x not in dims else r.ndim for x in self._special])
            return ret

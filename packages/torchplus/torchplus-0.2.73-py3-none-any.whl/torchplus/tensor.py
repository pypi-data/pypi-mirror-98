...
class Tensor(torch.Tensor):
    ...
    def sample(self, dim: INT = None, number: INT = 1, random: bool = True) -> 'Tensor':
        ...
        if len(samples) != 1:
            ...
            output_tensor = self[tuple(sample_indices)].as_subclass(Tensor).set_special_(special=[x - 1 if x > dim else (self.ndim if x == dim else x) for x in self._special], bf=self._batch_first)
            output_tensor.indices = samples
            return output_tensor
    ...
    def __torch_function_reducing_func__(cls, func, types, args=(), kwargs=None):
        "FOR: cummin cummax cumsum cumprod sum prod min max mean argmin argmax"

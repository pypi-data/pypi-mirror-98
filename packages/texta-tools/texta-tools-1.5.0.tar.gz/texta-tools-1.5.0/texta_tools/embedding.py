...
class Embedding:
    ...
    def _train(self, texts, use_phraser=True):
        """
        Trains Embedding.
        This needs to be called out in train() method.
    ...
    def get_similar(self, positives_used, negatives_used=list(), positives_unused=list(), negatives_unused=list(), n=20):
        ...
        positives = [str(positive).replace(' ', '_') for positive in positives_used]
        negatives = [str(negative).replace(' ', '_') for negative in negatives_used]
        ...
        not_a_suggestion = [str(not_suggestion).replace(' ', '_') for not_suggestion in negatives_used + positives_unused + negatives_unused if not_suggestion.replace(' ', '_') in self.model.wv.vocab]
        not_a_suggestion = set(not_a_suggestion)
        if positives:
            similar_items = self.model.wv.most_similar(positive=positives, negative=negatives, topn=n+len(not_a_suggestion))
            similar_items_topn = list()
            i = 0
            for s in similar_items:
                if s[0] not in not_a_suggestion:
...
class FastTextEmbedding(Embedding):
    ...
    def train(self, texts, use_phraser=True):
        ...
        :param: list texts: List of texts or an iterator (e.g. Elasticsearcher).
        """
        self.selected_model = fasttext.FastText
        return self._train(texts, use_phraser=use_phraser)
...
class W2VEmbedding(Embedding):
    ...
    def train(self, texts, use_phraser=True):
        ...
        :param: list texts: List of texts or an iterator (e.g. Elasticsearcher).
        """
        self.selected_model = word2vec.Word2Vec
        return self._train(texts, use_phraser=use_phraser)

...
from openmodule.models.backend import CountMessage, AccessRequest, Access, Category, MediumAccesses
...
class ModelTest(TestCase):
    ...
    def test_access_recurrence(self):
        ...
        with self.assertRaises(ValueError) as e:
            Access(**kwargs)
        self.assertIn("set duration", str(e.exception))
        ...
        with self.assertRaises(ValueError) as e:
            Access(**dict(
                user="user",
                category="filler-visitor-unexpected",
                recurrence="DTSTART:20210108T160000\nFREQ=WEEKLY;BYDAY=FR",
                duration=10,
            ))
        self.assertIn("set start timestamp when using recurrence", str(e.exception))

    def test_access_with_empty_string_recurrence_is_valid(self):
        access_as_dict = {
            "user": "user",
            "category": "filler-visitor-button",
            "start": time.time(),
            "recurrence": "",
            "start": 0,
        }
        try:
            Access(**access_as_dict)
        except ValueError:
            self.fail("access is invalid with empty string recurrence")

    def test_category(self):
        with self.assertRaises(Exception) as e:
            Category("test_exception")
        self.assertIn("not a valid category", str(e.exception).lower())
...
class RpcTest(BackendTestMixin, RPCServerTestMixin, TestCase):
    ...
    def test_check_in(self):
        ...
        self.assertIn("successful", cm.output[0])
...
class TestBackendCheckAccessWithDatabase(Backend):
    ...
    database: List[MediumAccesses] = []

    def check_access(self, request: AccessRequest):
        medium: MediumAccesses = next((m for m in self.database if m.id == request.id))
        if medium.type == request.medium_type:
            return medium.accesses
        return []


class AccessDatetimesAreConvertedToUnixTimestamps(BackendTestMixin, RPCServerTestMixin, TestCase):
    rpc_channels = ["backend"]
    topics = [b"backend", b"healthz"]
    config_kwargs = dict(USER_PREFIX="test")
    backend_class = TestBackendCheckAccessWithDatabase
    ...
    def test_datetime_is_converted_to_unix_timestamp(self):
        medium = MediumAccesses(
            ...
            accesses=[
                Access(
                    category="filler-visitor-button", user="test",
                    start=datetime(2021, 12, 25, 0, 0, 0), end=datetime(2021, 12, 25, 23, 59, 59)
                )
            ]
        )
        self.backend_class.database.append(medium)

...
class UserKNN(BaseKNN):
    ...
    def _compute_similarities(self):
        ...
        for u, i, r in self.interaction_dataset.values(['uid', 'iid', 'interaction'], to_list=True):
    ...
    def _compute_neighbours(self):
        for uid in range(self.n_users):
            self._neighbours[uid] = nlargest(self.k, filter(
                lambda x: x[0] > 0, [(self._get_sim(uid, uid2), uid2) for uid2 in range(self.n_users) if uid2 != uid]
            ))

    def _get_interaction(self, uid, iid):
        record = self.interaction_dataset.select_one(f'uid == {uid}, iid == {iid}') #{'RMSE': 1.0545, 'MSE': 1.1121}
        return record['interaction'] if record is not None else None
        #return None if iid not in self.user_ratings[uid] else self.user_ratings[uid][iid]

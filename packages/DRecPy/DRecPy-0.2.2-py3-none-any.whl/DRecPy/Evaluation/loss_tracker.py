...
class LossTracker:
    ...
    def display_graph(self, model_name=None, stopping_epoch=None, block=False):
        ...
        Args:
            ...
            stopping_epoch: An integer representing the epoch for which early stopping has been applied and therefore
                all network weights were reverted to.
        ...
        if stopping_epoch is not None:
            axes[0].axvline(stopping_epoch, label='Early stopping', color='r', linestyle='--')
        ...
        if len(axes) > 1:
            ...
            if stopping_epoch is not None:
                axes[1].axvline(stopping_epoch, label='Early stopping', color='r', linestyle='--')

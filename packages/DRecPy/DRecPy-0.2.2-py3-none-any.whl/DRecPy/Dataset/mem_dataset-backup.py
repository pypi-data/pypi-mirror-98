...
class MemoryInteractionDataset(InteractionDatasetABC):
    enable_extra_dict_records = False
    ...
    def values(self, columns=None, to_list=False):
        ...
        if self._users_records is None or not self.has_internal_ids:
            ...
            for row, rid in zip(self._df[df_columns].values, self._df.index):
                yield from_row_to_record(row, rid, columns, df_columns, to_list)
        else:
    ...
    def assign_internal_ids(self):
        ...
        self._user_mapping = {cat: code for code, cat in enumerate(user_cat.categories)}
        self._user_mapping_inv = {code: cat for code, cat in enumerate(user_cat.categories)}
        ...
        self._item_mapping = {cat: code for code, cat in enumerate(item_cat.categories)}
        self._item_mapping_inv = {code: cat for code, cat in enumerate(item_cat.categories)}

        self.has_internal_ids = True
        if self.enable_extra_dict_records:
...
def from_row_to_record(row, rid, projected_columns, df_columns, to_list):
    if to_list:
        list_interaction = [r for r in row]
        if 'rid' in projected_columns:
            desired_pos = projected_columns.index('rid')
            if desired_pos == len(list_interaction):
                list_interaction.append(rid)
            else:
                list_interaction.append(list_interaction[desired_pos])
                list_interaction[desired_pos] = rid
        if len(list_interaction) == 1: return list_interaction[0]
        else: return list_interaction

    record = {col: value for col, value in zip(df_columns, row)}

...
class MemoryInteractionDataset(InteractionDatasetABC):
    def __init__(self, path='', columns=None, **kwds):
        ...
        for row, rid in zip(self._df.values, self._df.index):
            self._data.append(list(row) + [rid])
        self._data = np.array(self._data)

        self._log('Done!')

    def __len__(self):
        return np.size(self._data, 0)
    ...
    def unique(self, columns=None, copy=True):
        ...
        for rec in self._data:
            s = ''
            for col in columns:
                s += str(rec[self.columns.index(col)]) + ','
            if s not in added_set:
                new_ds._data.append(rec)
                added_set.add(s)
        new_ds._data = np.array(new_ds._data)
    ...
    def assign_internal_ids(self):
        ...
        new_data = []
        user_col_idx = self.columns.index('user')
        item_col_idx = self.columns.index('item')
        ...
        for idx, rec in enumerate(self._data):
            if rec[user_col_idx] not in self._user_mapping:
                rec = np.insert(rec, len(rec), len(self._user_mapping))
                self._user_mapping_inv[len(self._user_mapping)] = rec[user_col_idx]
                self._user_mapping[rec[user_col_idx]] = len(self._user_mapping)
            else:
                rec = np.insert(rec, len(rec), self._user_mapping[rec[user_col_idx]])

            if rec[item_col_idx] not in self._item_mapping:
                rec = np.insert(rec, len(rec), len(self._item_mapping))
                self._item_mapping_inv[len(self._item_mapping)] = rec[item_col_idx]
                self._item_mapping[rec[item_col_idx]] = len(self._item_mapping)
            else:
                rec = np.insert(rec, len(rec), self._item_mapping[rec[item_col_idx]])
    ...
    def _apply_query(self, query, in_place=True):
        ...
        for segment in query_segments:
            ...
            print(new_data )
            if operator == '>':
                new_data = new_data[new_data[:, column_idx] > value]
            elif operator == '>=':
                new_data = new_data[new_data[:, column_idx] >= value]
            elif operator == '<=':
                new_data = new_data[new_data[:, column_idx] <= value]
    ...
    def _from_row_to_record(self, record, projected_columns, to_list):
        if to_list:
            list_interaction = [record[self.columns.index(col)] for col in projected_columns]
            if len(list_interaction) == 1: return list_interaction[0]
            else: return list_interaction
        else:
            new_rec = dict()
            for col in projected_columns:
                new_rec[col] = record[self.columns.index(col)]
            return new_rec

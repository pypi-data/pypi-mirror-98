...
class Commands:
    ...
    def ExecuteAddOnCommand(self, addOnCommandId: AddOnCommandId, addOnCommandParameters: Optional[AddOnCommandParameters] = None) -> AddOnCommandResponse:
        ...
        if not result["succeeded"]:
            raise UnsucceededCommandCall(result)
        return AddOnCommandResponse(**result["result"]["addOnCommandResponse"])
    ...
    def GetAttributesByType(self, attributeType: str) -> List[AttributeIdWrapperItem]:
        ...
        GetAttributesByType_parameters.get_classinfo().add_field('attributeType', str, value_set(['Layer', 'Line', 'Fill', 'Composite', 'Surface', 'City', 'LayerCombination', 'ZoneCategory', 'FontType', 'Profile', 'PenTable', 'Dimension', 'ModelViewOption', 'MEPSystem', 'OperationProfile', 'BuildingMaterial']))

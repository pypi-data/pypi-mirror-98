...
from .utils import with_baseline, get_cum_returns
...
class ShortfallTearsheet(BaseTearsheet):
    ...
    def create_full_tearsheet(self, x_performance, y_performance, largest_n=None,
        ...
        if largest_n:
            largest_shortfalls = self.create_largest_shortfalls_tearsheet(
                x_performance, y_performance, largest_n=largest_n)
        ...
        stats.append(
            ['Total Months',
             [round((x_performance.returns.index.max() -
              ...
              round((y_performance.returns.index.max() -
                     y_performance.returns.index.min()) /
                    pd.Timedelta(365.25 / 12, 'D'))]])

...
from pyconductor_grpc.service.workflow_service_pb2 import (
...
from pyconductor_grpc.model.task_pb2 import Task
...
class NonGrpcErrorIterator(collections.abc.Iterator):
...
class TaskService(ClientBase):
    ...
    def poll(
        ...
        """
        poll_response: PollResponse = self.stub.Poll(
            PollRequest(
                task_type=task_type,
                worker_id=worker_id,
                domain=domain,
        ...
        return poll_response.task

    @replace_grpc_error_with(UpdateTaskError)
    def update_task(self, result: TaskResult) -> None:
        """
        Updates the result of a task execution.

        :param result: the task result to send
        """
        return self.stub.UpdateTask(UpdateTaskRequest(result=result))

    def batch_ack_tasks(
        self, tasks: typing.Iterable[Task], max_threads=1,
    ) -> typing.Tuple[typing.Tuple[Task, typing.Optional[bool], typing.Optional[Exception]], ...]:
        """
        ...
        """
        def _ack_task(task: Task) -> typing.Tuple[Task, typing.Optional[bool], typing.Optional[Exception]]:
            try:
                return task, self.ack_task(task.task_id, task.worker_id), None
            except RpcError as e:
                return task, None, e
    ...
    def batch_update_tasks(
        self, task_results: typing.Iterable[TaskResult], max_threads=1,
    ) -> typing.Tuple[typing.Tuple[TaskResult, typing.Optional[Exception]], ...]:
        """
        ...
        """
        def _update_task(task_result: TaskResult) -> typing.Tuple[TaskResult, typing.Optional[Exception]]:
            try:
...
class BaseTaskWorker(ABC):
    ...
    def __init__(
        ...
        self._executor = ThreadPoolExecutor(max_workers=work_concurrency)
    ...
    def _batch_poll(self) -> typing.Iterator[Task]:
    ...
    @abstractmethod
    def execute_tasks(self, tasks: typing.Iterable[Task]) -> typing.Any:
...
class TaskWorker(BaseTaskWorker):
    ...
    ) -> typing.Tuple[typing.Tuple[Task, typing.Optional[TaskResult], typing.Optional[Exception]], ...]:
        """Execute tasks in separate work function invocations."""
        return tuple(self._executor.map(self._execute_task, tasks))

    def _execute_task(self, task: Task) -> typing.Tuple[Task, typing.Optional[TaskResult], typing.Optional[Exception]]:
        """
        ...
        """
        try:
            acked = self._task_service.ack_task(task.task_id)
        except RpcError as e:
            return task, None, e
        ...
        try:
            ...
            input_data = json_format.MessageToDict(task)['inputData']
            ...
            err_msg = f'Error executing task {task.task_id}: {self._work_function.__name__} with error: {str(err)}'
            output_data = None
            status = 'FAILED'
            ...
            task_result_dict = dict(
                ...
                reason_for_incompletion=reason_for_incompletion,
            ...
            task_result = json_format.ParseDict(task_result_dict, TaskResult())
...
class BatchTaskWorker(BaseTaskWorker):
    ...
    def execute_tasks(
        ...
        work_function_input = tuple(json_format.MessageToDict(t) for t in acked_tasks)
        ...
        for tr_dict in work_function_output:
            # drop result if there is no task id because there is no way to tell what task to update
            if not isinstance(tr_dict, collections.abc.Mapping) or 'taskId' not in tr_dict:
                logging.warning(f'Work function result has no task id (dropping): {tr_dict}')
                continue
            else:
                task_id = tr_dict['taskId']
            try:
                task_result = json_format.ParseDict(tr_dict, TaskResult())
        ...
        input_task_ids = set(t.task_id for t in acked_tasks)
        output_task_ids = set(tr.task_id for tr in task_results)
    ...
    def _make_batch_execution_output(
        ...
        for task_result, exception in batch_update_out:
            if exception is not None:
                exceptions[task_result.task_id] = exception
            else:
                task_results[task_result.task_id] = task_result
        for task, acked, exception in batch_ack_out:
            if exception is not None:
                exceptions[task.task_id] = exception
            elif not acked:
                exceptions[task.task_id] = AckTaskFailed()
        ...
        return tuple(
            (task, task_results.get(task.task_id, None), exceptions.get(task.task_id, None)) for task in tasks_in
        )
...
class ConductorTask(typing.TypedDict, total=False):
    taskType: str
...
ConductorOutputMessage = typing.TypedDict('ConductorOutputMessage', {
...
class ConductorTaskResult(typing.TypedDict, total=False):
    workflowInstanceId: str
    taskId: str
    reasonForIncompletion: str
    callbackAfterSeconds: int

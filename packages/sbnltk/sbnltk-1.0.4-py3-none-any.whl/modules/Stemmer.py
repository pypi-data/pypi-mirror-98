...
class stemmerOP:
    ...
    __dl=downloader()
    def __init__(self):
        self.__dl.download('rootword_list','dataset/')
        self.__dl.download('postag_static','dataset/')



        for word in open("../dataset/postag_static.txt", "r"):
            word=word.replace('\n','')
            vec=word.split(' ')
            self.__word_vec.append(vec[0])
            self.__word_dict[vec[0]]=1
        for word in open("../dataset/rootword_list.txt", "r"):
            word=word.replace('\n','')
            self.__word_dict2[word]=1
    def __search(self,word):
        if (self.__bp.word_normalize(word) in self.__word_dict) or (word in self.__word_dict) or (word in self.__word_dict2):
            return True
        return False
    ...
    def stemWord(self,word):
        try:
            ...
            print(f"{sbnltk.bcolors.FAIL}ERROR 101: Error in stemming!! {sbnltk.bcolors.ENDC}")

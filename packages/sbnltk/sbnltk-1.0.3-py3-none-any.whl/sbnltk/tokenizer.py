...
class wordTokenizer:
    ...
    def customized_tokenizer(self,text,punc=True,norm=False,dust=False):
        ...
        try:
            ...
            print(f"{cols.bcolors.FAIL} ERROR 301: Error in Customized word Tokenizer!! {cols.bcolors.ENDC}")
            return tokens
    ...
    def customized_tokenizer(self,text,punc=True,norm=False,dust=False):
        ...
        try:
            ...
            for i in tokens:
                ...
                if len(bp.dust_removal(i))!=0 and dust==True:
                    i=bp.dust_removal(i)
                temp_tokens.append(i)
            return temp_tokens
        except:
            print(f"{cols.bcolors.FAIL} ERROR 302: Error in Customized Sentence Tokenizer!! {cols.bcolors.ENDC}")
            return tokens
    ...
    def sentence_cluster(self,text,max_length=100,punc=True,norm=False,dust=False):
        ...
        try:
            ...
            for sent in tmp_tokens:
                ...
                if len(bp.dust_removal(sent)) != 0 and dust == True:
                    sent = bp.dust_removal(sent)
                tmp_tokens2.append(sent)
            return tmp_tokens2
        except:
            print(f"{cols.bcolors.FAIL} ERROR 303: Error in Sentence clustering!! {cols.bcolors.ENDC}")
            return tokens

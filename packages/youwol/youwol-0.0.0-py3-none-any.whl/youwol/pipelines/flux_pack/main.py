...
def flux_pipeline(path: Union[str, Path], shared_resources: bool, mode: BundleModeEnum):
...
async def generate(
    ...
    async with context.with_target(name).start(Action.INSTALL) as ctx:
        ...
        pipeline = cast(FluxPipeline, pipeline)
        ...
        await ctx.info(step=ActionStep.STATUS,
                       ...
                       json={'parameters': parameters,
                             ...
                             'shared_resources': pipeline.shared_resources
                             }
                       )

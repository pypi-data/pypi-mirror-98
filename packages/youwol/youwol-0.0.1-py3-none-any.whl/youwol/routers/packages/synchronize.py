...
def ensure_dependencies(
    ...
    if missing_dependencies:
        ...
        raise ActionException(
            context.action,
            f"Following dependencies of {package.info.name} do not exists: {str(missing_dependencies)}")
...
async def make_package(
    ...
    npm_min_exclude = {'.git', 'CVS', '.svn', 'hg', 'lock-wscript', '.wafpickle-N', '.DS_Store', 'npm-debug.log',
                       '.npmrc', 'node_modules/**/*', 'config.gypi', 'package-lock.json'}

...
from cognite.well_model.models import Survey, Well, Wellbore
...
class WDLResourceList:
    ...
    def __init__(self, resources: List[Any]):
        ...
        for resource in resources:
            if resource is None or not isinstance(resource, self._RESOURCE):
                raise TypeError(
                    f"All resources for class '{self.__class__.__name__}' must be of type '{self._RESOURCE.__name__}', "
                    f"not '{type(resource)}'. "
                )
    ...
    def dump(self, camel_case: bool = False) -> List[Dict[str, Any]]:
        ...
        """
        return [
            resource.dump(expand=("metadata", "datum", "wellhead"), ignore=("rows",), camel_case=camel_case)

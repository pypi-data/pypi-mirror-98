...
import time
import sys
from .picture import weights_pie
...
class SlyStock(object):
    ...
    def portfolio(self, accurate=False, number=500):
        ...
        if accurate == False:
            ...
            file = str(self.names).replace('[', '').replace(']', '')
            self.log.blue(f'calculating {file} ...')
                # with open('Markowit.txt', 'a', encoding='utf-8') as f:
                #    f.write(f'calculating {file} ...')
                #    f.write('\n')
            df = self.Markowit(
                    stock_list=self.names, accurate=False, number=number)
            df.to_csv(f'Markowit\\{file}.csv', index=False, encoding='utf8')
            path = './Markowit'
            ...
            result.to_csv('Markowit\\result.csv')
            weights = eval(result.iloc[0]['weight'])
            weights_pie(weights)
            return weights  # 最佳投资组合
        else:
            #zuhe = self.combinations
            #zuhe = zuhe[zuhe['amount'] > 1]
            result = []
            ...
            file = str(self.names).replace('[', '').replace(']', '')
            self.log.blue(f'calculating {file} ...')
            result.append(self.Markowit(stock_list=self.names, accurate=True))
    ...
    def save_result(self):
        ...
        df.to_csv('Markowit\\result.csv', index=False, encoding='utf8')
        with open('Markowit.txt', 'a', encoding='utf8') as f:
            f.write(
...
if __name__ == '__main__':
    ...
    print(ss.portfolio(accurate=False, number=500))
    #ss.save_result
    ss.quit()

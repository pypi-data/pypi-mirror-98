...
def fixed(pheno_file, bed_file, agmat_file, out_file='res.txt', npart=20):
    ...
    with open(out_file, 'w') as fout:
        fout.write(' '.join(['chro', 'snp_ID', 'pos', 'allele1', 'allele2', 'cc_val', 'eff_val', 'chi_val', 'p_val']))
        fout.write('\n')
    maxiter = 100
    if not convergence:
        maxiter = 0
    for i in range(npart):
        ...
        for k in tqdm(range(snp_mat.shape[1])):
            ...
            cc_val, snp_eff, snp_chi, p_val = _weight_emai_eigen_snp(y_trans, xmat_trans_k, gmat_eigenvals, var,
...
def _weight_emai_eigen_snp(y_trans, xmat_trans, gmat_eigenvals, var, maxiter=20, cc=1.0e-8):
    ...
    snp_eff = b_hat[-1, -1]
    snp_chi = snp_eff * snp_eff / xvx[-1, -1]
    p_val = chi2.sf(snp_chi, 1)
    return cc_val, snp_eff, snp_chi, p_val

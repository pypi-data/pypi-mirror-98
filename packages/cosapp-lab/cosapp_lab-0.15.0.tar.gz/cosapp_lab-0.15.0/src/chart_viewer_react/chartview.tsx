...
import PBSPanel2 from '../sys_explorer_react/panel_1/pbs_view_2';
import ChartElement from './chart_element/chartelement';
import GraphPanel from './connection_view/graph_view';
import Controller from './controller/controller';
import DataViewer from './data_view/data_viewer';
import DocumentViewer, {
  IAvailableDocument
} from './document_view/document_viewer';
import GeometryViewWidget from './geometry_view/geometry_view';
import * as ReduxAction from './redux/actions';
import {IDict, StateInterface} from './redux/types';
import WidgetViewer, {IAvailableWidget} from './widget_view/widget_viewer';
 ...
 */
export class ChartView extends Component<AppProps, AppStates> {
  ...
  constructor(props: AppProps) {
    ...
    const { chart_template = {}, template_path = null } = this.props.model.get(
      'chart_template'
    );
    ...
    if ('selectedVariable' in this.chartTemplate) {
      ...
      for (const key in this.chartTemplate['selectedVariable']) {
        selectedVariable[
          `${this.props.systemConfig['root_name']}.${key}`
  ...
  on_msg = (
    ...
    switch (type) {
      ...
      case 'ChartElement::update_save_path': {
  ...
  componentDidMount() {
    if ('modelJson' in this.chartTemplate && 'plotJson' in this.chartTemplate) {
      ...
      this.setState((old) => ({
        ...old,
        model: FlexLayout.Model.fromJson(this.chartTemplate['modelJson']),
      }));
    } else {
  ...
  addGraph = (
    ...
    if (nodeId && nodeId in this.innerlayoutRef) {
      this.innerlayoutRef[nodeId].current.addTabToTabSet(tabsetId, {
        component: component,
        name: ChartView.COMPONENT_DICT[component],
  ...
  factory = (node: FlexLayout.TabNode): JSX.Element => {
    ...
    const nameList = Object.values(ChartView.COMPONENT_DICT);
      ...
      case 'widgetView': {
        ...
        return (
          <WidgetViewer
            ...
            id={id}
            initialState={data}
            availableWidget={this.availableWidget}></WidgetViewer>
        );
      }
      case 'documentView': {
        const { id, data } = this.prepare_data_for_factory(config, nodeId);
        return (
          <DocumentViewer
            ...
            id={id}
            initialState={data}
            availableDocument={this.availableDocument}></DocumentViewer>
        );
      }
      case 'sub': {
    ...
    return (
      <FlexLayout.Layout
        ...
        classNameMapper={(className) => {
          ...
          {
            className =
            'inner__flexlayout__tab flexlayout__tab';
          }
  ...
  saveChartToJson = () => {
    ...
    this.props.send_msg({
      action: 'ChartViewer::chartViewerSaveJson',
      payload: {
        jsonName,
...
function AddTabMenu(
  ...
  const menuItem = Object.keys(ChartView.COMPONENT_DICT).map(

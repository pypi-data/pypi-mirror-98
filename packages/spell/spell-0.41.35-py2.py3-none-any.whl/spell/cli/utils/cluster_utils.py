...
monitoring_stack_manifests_dir = os.path.join(
...
def create_serving_priorityclass(kubectl_context=None):
    kubectl(
        "apply",
        "--filename",
        os.path.join(monitoring_stack_manifests_dir, "spell", "serving-priorityclass.yaml"),
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".yaml", mode="w+") as f:
            ambassador_yaml = generate_ambassador_host_yaml(
                cluster["serving_cluster_domain"], is_public
            )
            f.write(ambassador_yaml)
            f.flush()
...
def add_monitoring_stack(spell_stack, cluster, kubectl_env=None):
    ...
    try:
        ...
        kubectl(
            "apply", "-f", os.path.join(monitoring_stack_manifests_dir, "setup"), env=kubectl_env
        )
        ...
        kubectl(
            ...
            "--filename",
            os.path.join(
                monitoring_stack_manifests_dir,
                "setup",
        ...
        rule_template = None
        with open(
            os.path.join(monitoring_stack_manifests_dir, "spell", "spell-prometheus-rules.j2"), "r"
        ) as f:
        ...
        kubectl(
            "apply", "--filename", monitoring_stack_manifests_dir, env=kubectl_env,
        )
        ...
        kubectl(
            "apply",
            "--filename",
            os.path.join(monitoring_stack_manifests_dir, "ambassador"),
            env=kubectl_env,
        )
        ...
        kubectl(
            ...
            "--filename",
            os.path.join(
                monitoring_stack_manifests_dir, "spell", "spell-grafana-home-dashboard.yaml"
            ),
            env=kubectl_env,
        )
        ...
        with open(os.path.join(monitoring_stack_manifests_dir, "spell", "grafana.ini.j2")) as f:
        ...
        kubectl(
            ...
            "--from-file=datasources.yaml={}".format(
                os.path.join(monitoring_stack_manifests_dir, "spell", "datasources.yaml")
            ),
        )
        ...
        kubectl(
            "apply",
            "--filename",
            os.path.join(monitoring_stack_manifests_dir, "spell", "spell-grafana-mapping.yaml"),
            env=kubectl_env,
        )
...
def make_optional_serving_features():

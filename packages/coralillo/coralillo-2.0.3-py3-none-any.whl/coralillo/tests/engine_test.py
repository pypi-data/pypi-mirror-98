...
def test_create_engine():
    eng1 = Engine(host=os.getenv('REDIS_HOST', 'localhost'), db=11)
    eng2 = Engine(host=os.getenv('REDIS_HOST', 'localhost'), db=12)
...
def test_set_engine_delayed():
    ...
    eng = Engine(host=os.getenv('REDIS_HOST', 'localhost'))
...
def test_register_lua_script():
    eng = Engine(host=os.getenv('REDIS_HOST', 'localhost'), db=0)
...
def test_can_replace_id_function():
    ...
    simple_eng = Engine(host=os.getenv('REDIS_HOST', 'localhost'), id_function=simple_ids)
    uuid_eng = Engine(host=os.getenv('REDIS_HOST', 'localhost'))

...
def save_tight_fig(figure, filepath, shape):
    set_size(figure, (shape[1]/SAVE_DPI, shape[0]/SAVE_DPI), dpi=SAVE_DPI)
    plt.savefig(filepath, bbox_inches='tight', pad_inches=0)
    # image_inches = get_image_inches(shape)
    # figure.set_size_inches(image_inches.x * IMAGE_SIZE_FIX_FACTOR, image_inches.y * IMAGE_SIZE_FIX_FACTOR)
    # plt.savefig(filepath, dpi=image_inches.dpi, bbox_inches='tight', pad_inches=0)
...
def plot_dataset(data,
    ...
    try:
        ...
        # save_tight_fig(fig, filepath, data.shape[:2])
    finally:
        fig.clear()
        plt.close()
        gc.collect()
...
def get_width_height_pixels(
        original_width,
        original_height,
        source_region: LatLonRegion = None,
        clipping_region: LatLonRegion = None):
    pass

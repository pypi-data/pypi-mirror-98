...
def plot_results(data, inf_res, bins=50, output = "results.png",fig_height = 4, fig_width = 3):
    ...
    for i, kr in enumerate(data):
        if nKar == 1:
            axs.hist(data[kr].detach().numpy(), bins=bins, density=True, alpha=0.48)
        ...
        for j in range(tail, len(assignment_probs)):
            ...
            if nKar == 1:
                axs.plot(x, p, linewidth=3, color=cl)
            else:
        ...
        if tail == 1:
            ...
            if nKar == 1:
                axs.plot(x, p, linewidth=3, color="tab:pink")
            else:

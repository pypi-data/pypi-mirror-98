...
def DownloadData(Dir, Startdate, Enddate, S2_tile):
    ...
    for Date in Dates:
        ...
        try:
            ...
            url = "https://hls.gsfc.nasa.gov/data/v1.4/L30/%s/%s/%s/%s/%s/HLS.L30.T%s.%s%03d.v1.4.hdf" % (year, S2_tile[0:2], S2_tile[2:3], S2_tile[3:4], S2_tile[4:5],S2_tile, year, doy)
            filename_out = os.path.join(output_folder_end, "HLS.L30.T%s.%s%03d.v1.4.hdf" % (S2_tile, year, doy))

            # Download the data
            urllib.request.urlretrieve(url, filename=filename_out)
            
            # Create a folder for the end results 
            folder_tile = os.path.join(output_folder_end, "HLS_L30_T%s_%s%03d_v1_4" % (S2_tile, year, doy))
            ...
            for Band in range(1,12):
                ...
                DC.Save_as_tiff(os.path.join(folder_tile, "HLS_L30_T%s_%s%03d_H%02dM%02d_%s.tif" % (S2_tile, year, doy, hour, minute, Band_name)), Array, geo, projection)

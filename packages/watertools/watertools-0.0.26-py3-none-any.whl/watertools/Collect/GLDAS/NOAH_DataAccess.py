...
def DownloadData(Dir, Var, Startdate, Enddate, latlim, lonlim, Waitbar, cores,
    ...
    if TimeCase == 'three_hourly':
        ...
        path = os.path.join(Dir, 'Weather_Data', 'Model', 'NOAH_GLDAS',
                            TimeCase, Var)
        ...
        url = 'https://hydro1.gesdisc.eosdis.nasa.gov/dods/GLDAS_NOAH10_3H.{0}'.format(gldas_version) #%(username,password)
    ...
    # Set required data for the daily option
        ...
        # Define output folder and create this one if not exists
        path = {'mean': os.path.join(Dir, 'Weather_Data', 'Model', 'NOAH_GLDAS',
                                     TimeCase, Var, 'mean'),
                'min': os.path.join(Dir, 'Weather_Data', 'Model', 'NOAH_GLDAS',
                                    TimeCase, Var, 'min'),
                'max': os.path.join(Dir, 'Weather_Data', 'Model', 'NOAH_GLDAS',
                                    TimeCase, Var, 'max')}
    ...
    # Set required data for the monthly option
        ...
        url = 'https://hydro1.gesdisc.eosdis.nasa.gov/dods/GLDAS_NOAH10_M.{0}'.format(gldas_version) #%(username,password)
...
def RetrieveData_three_hourly(Date, args):
    ...
    for period in CaseParameters:
        ...
        if not os.path.isfile(BasinDir):
            ...
            # define geo
            lonlimGLDAS = xID[0] - 180
            latlimGLDAS = (yID[1] + 1) - 60
...
def RetrieveData_daily(Date, args):
    ...
    for T in types:
        ...
        BasinDir = path[T] + '/' + VarStr + '_GLDAS-NOAH10_' + \

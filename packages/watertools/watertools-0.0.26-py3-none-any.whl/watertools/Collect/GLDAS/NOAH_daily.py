# -*- coding: utf-8 -*-
import sys
from watertools.Collect.GLDAS.NOAH_DataAccess import DownloadData


def main(Dir, Vars, Startdate, Enddate, latlim, lonlim, cores=False,
         SumMean=1, Min=0, Max=0, Waitbar = 1, gldas_version = '2.1'):
    """
    This function downloads NOAH GLDAS daily data for a given variable, time
    ...
    for Var in Vars:
        ...
        if Waitbar == 1:
            print('\nDownloading daily NOAH GLDAS %s data for the period %s till %s' %(Var, Startdate, Enddate))

...
def main(Dir, latlim, lonlim, level = 'sl1', Waitbar = 1):
    ...
    output_folder = os.path.join(Dir, 'SoilGrids', 'Soil_Organic_Carbon_Content')
    ...
    if not os.path.exists(nameEnd):
        ...
        DownloadData(output_folder, latlim, lonlim, "SOC", level)
        ...
        if Waitbar == 1:
            print("\nSoil Organic Carbon Content soil map of %s from SoilGrids.org already exists in output folder" %level)

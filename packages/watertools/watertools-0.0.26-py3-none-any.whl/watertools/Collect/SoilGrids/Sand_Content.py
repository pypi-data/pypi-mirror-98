...
def main(Dir, latlim, lonlim, level = 'sl1', Waitbar = 1):
    ...
    output_folder = os.path.join(Dir, 'SoilGrids', 'Sand_Content')
    ...
    if not os.path.exists(nameEnd):
        ...
        # Create Waitbar
        if Waitbar == 1:
            print('\nDownload Sand Content Mass Fraction soil map of %s from SoilGrids.org' %level)
        ...
        DownloadData(output_folder, latlim, lonlim, "SAND", level)
        ...
        if Waitbar == 1:
            print("\nSand Content Mass Fraction soil map of %s from SoilGrids.org already exists in output folder" %level)

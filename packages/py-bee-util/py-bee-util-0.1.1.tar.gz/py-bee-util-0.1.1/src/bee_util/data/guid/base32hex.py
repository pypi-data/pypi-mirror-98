...
const.guid_encode_char = "0123456789abcdefghijklmnopqrstuv"

decodeHexMap = {}
for index, i in enumerate(const.guid_encode_char):
...
def b32encode(src):
    return encode(list(map(ord, src)), const.guid_encode_char)


def b32decode(src):
    return decode(src, const.guid_encode_char)

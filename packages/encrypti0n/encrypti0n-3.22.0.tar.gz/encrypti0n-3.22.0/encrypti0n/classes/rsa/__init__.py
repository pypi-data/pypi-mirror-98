...
class RSA(object):
	...
	def generate_keys(self, log_level=0):
		...
		if not self.memory:
			...
			if Files.exists(directory): 
				...
				return Response.error(f"Key [{directory}] already exists.")
		else:
		...
		# response.
		loader.stop()
		return Response.success(f"Successfully generated key {directory}.", {
	...
	def load_keys(self):
		...
		try:
			...
			if "Padding is incorrect" in str(e):
				return Response.error("Provided an incorrect passphrase.")
			else: return Response.error(f"ValueError: {e}")
		
		# response.
		return Response.success("Successfully loaded the key pair.")
	...
	def load_public_key(self):
		...
		try:
			...
			if "Padding is incorrect" in str(e):
				...
				return Response.error(f"ValueError: {e}")
	...
	def edit_passphrase(self, passphrase=None):
		...
		if "Your identification has been saved with the new passphrase." in output:
			...
			return Response.success(f"Successfully edit the passphrase of key [{self.id}].")
		else:
			return Response.error(f"Failed to edit the passphrase of key [{self.id}], error: {output}.")
	...
	def encrypt_string(self, string, layers=1, decode=True):
		...
		if not self.public_key_activated:
			return Response.error("Can not encrypt data since the public key is not activated yet.")
	...
	def encrypt_file(self, path, layers=1):
		...
		if layers-1 > 0:
			...
			return Response.success(f"Successfully encrypted file [{path}].")
	...
	def encrypt_directory(self, path, recursive=False, layers=1):
		...
		if recursive:
			...
			if layers-1 > 0:
				...
				return Response.success(f"Successfully encrypted directory [{path}] (recursively).")
		...
		# encrypt the directory and save to x.encrypted.zip file.
			...
			if not file_path.exists():
				return Response.error(f"Directory [{path}] does not exist.")
			if not file_path.directory():
				return Response.error(f"Directory path [{path}] is not a directory.")
			...
			if response["success"]:
				...
				return Response.error(f"Failed to encrypted directory [{path}].")
	...
	def decrypt_string(self, string, layers=1, decode=True):
		...
		if not self.private_key_activated:
			return Response.error("Can not decrypt data since the private key is not activated yet.")
	...
	def decrypt_file(self, path, layers=1):
		...
		if layers-1 > 0:
			...
			return Response.success(f"Successfully decrypted file [{path}].")
	...
	def decrypt_directory(self, path, recursive=False, layers=1):
		...
		if recursive:
			...
			if layers-1 > 0:
				...
				return Response.success(f"Successfully decrypted directory [{path}] (recursively).")
		...
		# decrypt the encrypted.zip file.
			...
			if not Files.exists(zip_path):
				return Response.error(f"System encrypted zip [{path}] does not exist.")
			
			# decrypt zip.
			response = self.decrypt_file(zip_path, layers=layers)
			if response["error"] != None:
				return Response.error(f"Failed to decrypted directory [{path}].")
			...
			if Files.exists(path):
				zip.file_path.delete(forced=True)
				return Response.success(f"Successfully decrypted directory [{path}].")
			else:
				return Response.error(f"Failed to decrypted directory [{path}].")
...
class EncryptedDictionary(Dictionary):
	...
	def initialize(self):
		...
		# success.
		return Response.success(f"Successfully initialized encrypted dictionary [{self.file_path.path}].")
		#
	def load(self):
		...
		try:
			...
			return Response.success(f"Successfully loaded encrypted dictionary [{self.file_path.path}].")
		except: 
			self.dictionary = None
			return Response.error(f"Failed to load encrypted dictionary [{self.file_path.path}].")
	...
	def save(self, dictionary=None):
		...
		try:
			...
			return Response.success(f"Successfully saved encrypted dictionary [{self.file_path.path}].")
		except: 
			self.dictionary = None
			return Response.error(f"Failed to save encrypted dictionary [{self.file_path.path}].")
	...
	def check(self, default=None, save=True):
		...
		if not isinstance(default, dict): 
			return Response.error("Parameter [default] must be a dictionary.")
		if not isinstance(self.dictionary, dict): 
			return Response.error("The dictionary must be initialized & loaded.")
		...
		try:
			...
			return Response.error("Failed to check the loaded dictionary.")

		# save.
		if save and old != self.dictionary:
			response = self.save()
			if response["error"] != None: 
				return Response.error("Failed to check the loaded dictionary, error: "+response['error'])

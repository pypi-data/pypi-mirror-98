        ...
        for plugin_type in PLUGIN_TYPES:
            ...
            for plugin_name, routing_entry in plugin_type_meta.items():
                ...
                try:
                    ...
                    print(f'{plugin_type} {plugin_name} marked as removed', file=sys.stderr)
                    continue

                if ctx.resolved:
                    plugin_map[plugin_name] = ctx.redirect_list[-1]
                else:
                    print(f'redirected {plugin_type} {plugin_name} from collection {collection_name} did not resolve', file=sys.stderr)

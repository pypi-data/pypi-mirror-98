...
def __(key):
    return i18n.t("timeseries.owmovingtransform." + key)
...
class OWMovingTransform(widget.OWWidget):
    ...
    _NON_OVERLAPPING_WINDOWS = __('btn.non-overlap_windows')

    UserAdviceMessages = [
        widget.Message(__('msg.get_sma').format(Mean),
                       'sma-is-mean'),
        widget.Message(__('msg.non-overlapping').format(_NON_OVERLAPPING_WINDOWS),
                       'non-overlapping')
    ]

    class Warning(OWWidget.Warning):
        no_transforms_added = Msg(__("msg.add_transform"))

    def __init__(self):
        self.data = None
        box = gui.vBox(self.controlArea, __('box_move_transform'))
        ...
        gui.checkBox(box, self, 'non_overlapping',
                     ...
                     tooltip=__('tooltip_non_overlapping'))
        fixed_wlen = gui.spin(box, self, 'fixed_wlen', 2, 1000,
                              label=__('label.fix_window_width'),
        ...
        model.setHorizontalHeaderLabels(
            [__('label.series'), __('label.window_width'), __('label.aggregation_function')])
        ...
        self.add_button = button = gui.button(
            hbox, self, __('btn.add') + '&' + __('btn.transform'),
        ...
        self.del_button = button = gui.button(
            hbox, self, __('btn.delete_selected'),

...
def __(key):
    return i18n.t("timeseries.owcorrelogram." + key)
...
class OWCorrelogram(widget.OWWidget):
    ...
    class Error(widget.OWWidget.Error):
        no_instances = widget.Msg(__("msg.no_instances"))

    def __init__(self):
        self.all_attrs = []
        opts = gui.widgetBox(self.controlArea, __('box_option'))
        gui.checkBox(opts, self, 'use_pacf',
                     label=__('label.pacf'),
                     callback=self.on_changed)
        gui.checkBox(opts, self, 'use_confint',
                     label=__('msg.use_confint'),
                     callback=self.on_changed)
        gui.listBox(self.controlArea, self, 'attrs',
                    labels='all_attrs',
                    box=__('box_all_attrs'),
        ...
        plot = self.plot = Highchart(
            ...
            tooltip_headerFormat=__('msg.correlation_at_period').format("point.key:.2f"),
    ...
    def on_changed(self):
        ...
        for i, (attr, color) in enumerate(zip(self.attrs,
            ...
            if self.use_confint:
                ...
                std = 1.96 * (
                        (1 + 2 * (pac[:, 1] ** 2).sum()) / len(self.data)) ** .5  # = more precise than 1.96/sqrt(N)

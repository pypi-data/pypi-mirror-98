...
def __(key):
    return i18n.t("timeseries.owaggregate." + key)
...
class OWAggregate(widget.OWWidget):
    ...
    agg_interval = settings.Setting(__('gbox.day'))
    autocommit = settings.Setting(False)

    AGG_TIME = OrderedDict((
        (__('gbox.second'), lambda date: date.replace(microsecond=0)),
        (__('gbox.minute'), lambda date: date.replace(second=0, microsecond=0)),
        (__('gbox.hour'), lambda date: date.replace(minute=0, second=0, microsecond=0)),
        (__('gbox.day'), lambda date: date.replace(hour=0, minute=0, second=0, microsecond=0)),
        (__('gbox.week'), lambda date: date.strptime(date.strftime('%Y-W%W-0'), '%Y-W%W-%w')),
        # Doesn't work for years before 1000
        (__('gbox.month'), lambda date: date.replace(day=1, hour=0, minute=0, second=0, microsecond=0)),
        (__('gbox.year'), lambda date: date.replace(month=1, day=1, hour=0, minute=0, second=0, microsecond=0)),
    ))

    class Error(widget.OWWidget.Error):
        no_time_variable = widget.Msg(__('aggregation_time_var'))

    def __init__(self):
        self.data = None

        gui.comboBox(self.controlArea, self, 'agg_interval',
                     label=__('label.aggregate_by'),
                     items=tuple(self.AGG_TIME.keys()),
                     sendSelectedValue=True,
                     orientation=Qt.Horizontal,
                     callback=lambda: self.commit)
        self.model = model = PyTableModel(parent=self, editable=[False, True])
        model.setHorizontalHeaderLabels([__('label.attribute'), __('label.aggregation_function')])

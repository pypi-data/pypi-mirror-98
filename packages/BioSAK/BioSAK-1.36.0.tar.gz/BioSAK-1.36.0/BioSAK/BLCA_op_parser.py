...
def BLCA_op_parser(args):
    ...
    for each_seq in taxon_dict_formatted_with_num:
        output_file_1_handle.write('%s\t%s' % (each_seq, taxon_dict_formatted_with_num[each_seq]))
        output_file_2_handle.write('%s\t%s' % (each_seq, taxon_dict_formatted_no_num[each_seq]))

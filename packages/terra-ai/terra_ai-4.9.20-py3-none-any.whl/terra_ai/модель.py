...
def авторизация_LabStory():
  ...
  post = 'http://skavzi8m.beget.tech/api/login?email='+login+'&password='+password # Формирует post-запрос
  ...
  get = 'http://skavzi8m.beget.tech/api/my/profile?access_token='+token
...
def добавить_датасет_LabStory(dataset_dict):
  if 'token' not in globals():
    ...
    return
  global датасет_id
  post = 'http://skavzi8m.beget.tech/api/my/datasets?access_token='+token
  ...
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    ...
    get = 'http://skavzi8m.beget.tech/api/my/datasets?access_token='+token
    json_response = requests.get(get, headers=headers) # Отправляем post-запрос на сервер
    datasets = json_response.json()
    meta = datasets['meta']
    get = 'http://skavzi8m.beget.tech/api/my/datasets?per_page=' + str(meta['total']) + '&access_token='+token
...
def удалить_датасет_LabStory(id):
  if 'token' not in globals():
    print('Для выполнения данной функции, необходимо авторизироваться.')
    return
  delete = f'http://skavzi8m.beget.tech/api/my/datasets/{id}?access_token='+token
...
def выбрать_датасет_LabStory(id):
  if 'token' not in globals():
    ...
    return
  global датасет_id
  get = f'http://skavzi8m.beget.tech/api/my/datasets/{id}?access_token='+token
  json_response = requests.get(get, headers=headers) # Отправляем post-запрос на сервер
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    datasets = json_response.json()
...
def добавить_задачу_LabStory(task_dict):
  if 'token' not in globals():
    ...
    return
  global задача_id
  post = 'http://skavzi8m.beget.tech/api/my/tasks?access_token='+token
  ...
  get = 'http://skavzi8m.beget.tech/api/my/tasks?access_token='+token
  json_response = requests.get(get, headers=headers) # Отправляем post-запрос на сервер
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    tasks = json_response.json()
    meta = tasks['meta']
    get = 'http://skavzi8m.beget.tech/api/my/tasks?per_page=' + str(meta['total']) + '&access_token='+token
    json_response = requests.get(get, headers=headers)
    tasks = json_response.json()
...
def выбрать_задачу_LabStory(id):
  if 'token' not in globals():
    ...
    return
  global задача_id
  get = f'http://skavzi8m.beget.tech/api/my/tasks/{id}?access_token='+token
  json_response = requests.get(get, headers=headers) # Отправляем post-запрос на сервер
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
...
def сохранить_эксперимент_LabStory(experiment_dict):
  ...
  if 'token' not in globals():
    print('Для выполнения данной функции, необходимо авторизироваться.')
    return
  post = 'http://skavzi8m.beget.tech/api/my/experiments?access_token='+token
  ...
  if experiment_dict['loss'] == min:
    loss_float = round(min(model_history_list[0]),2)
    val_loss_float = round(min(model_history_list[2]),2)
  elif experiment_dict['loss'] == max:
    loss_float = round(max(model_history_list[0]),2)
    val_loss_float = round(max(model_history_list[2]),2)   

  if experiment_dict['metrics'] == min:
    metric_float = round(min(model_history_list[0]),2)
    val_metric_float = round(min(model_history_list[2]),2)
  elif experiment_dict['metrics'] == max:
    metric_float = round(max(model_history_list[0]),2)
    val_metric_float = round(max(model_history_list[2]),2) 

###############################
# Поиск архитектуры модели    
###############################
  #x = experiment_dict['function']
  #for i in range(len(In)-1,0,-1):
   #if (('def '+x) in In[i])&('return' in In[i])&(len(In[i])>200):
      #m_arc = In[i]
      #break
  ...
  for i in reversed(experiment_dict['cache']):  
    for j in range(len(key_words)):
      if key_words[j] in i and find in i:
        m_arc = i
        break
###############################
  ...
  experiment = {
    ...
    "loss": [round(v, 2) for v in model_history_list[0]],
    "loss_float": loss_float,
    "val_loss": [round(v, 2) for v in model_history_list[2]],
    "val_loss_float": val_loss_float,
    "metric": [round(v, 2) for v in model_history_list[1]],
    "metric_float": metric_float,
    "val_metric": [round(v, 2) for v in model_history_list[3]],
...
# получаем id только что сохраненного эксперимента по id всех экспериментов
    get = 'http://skavzi8m.beget.tech/api/my/experiments?access_token='+token
    json_response = requests.get(get, headers=headers) # Отправляем post-запрос на сервер
    if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
      experiments = json_response.json()
      meta = experiments['meta']
      get = 'http://skavzi8m.beget.tech/api/my/experiments?per_page=' + str(meta['total']) + '&access_token='+token
      json_response = requests.get(get, headers=headers)
      experiments = json_response.json()
...
def посмотреть_эксперимент_по_id_LabStory(id):
  ...
  get = f'http://skavzi8m.beget.tech/api/my/experiments/{id}?access_token='+token
  json_response = requests.get(get, headers=headers) # Отправляем post-запрос на сервер
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
...
def получить_архитектуру(id_exp):
  ...
  get = f'http://skavzi8m.beget.tech/api/my/experiments/{id_exp}?access_token='+token
  json_response = requests.get(get, headers=headers) # Отправляем post-запрос на сервер
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    experiment = json_response.json()

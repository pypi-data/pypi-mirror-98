...
if platform.system().lower() != "windows":
    socket_error = (transport.TTransportException,
                    socket.error, protocol.cybin.ProtocolError)
else:
...
thrift_path = path.join(sys.modules["ROOT_DIR"], "mtdata.thrift")
thrift_path = path.abspath(thrift_path)
module_name = path.splitext(path.basename(thrift_path))[0]
thrift = None

with open(thrift_path) as f:
    thrift = thriftpy.load_fp(f, "mtdata_thrift")

AUTH_API_URL = "127.0.0.1/6000"  # 获取token


class MtDataClient(object):

    _threading_local = threading.local()
    _auth_params = {}
    _default_host = "192.168.31.88"
    _default_port = 9000
    request_timeout = 300
    ...
    def instance(cls):
        ...
        if _instance is None:
            if cls._auth_params:
                _instance = MtDataClient(**cls._auth_params)

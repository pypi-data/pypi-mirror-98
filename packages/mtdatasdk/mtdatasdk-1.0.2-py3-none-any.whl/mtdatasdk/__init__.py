...
def auth_by_token(token, host=None, port=None):
    """使用 token 认证账号"""
    MtDataClient.set_auth_params(host=host, port=port, token=token)


def logout():
    """退出账号"""
    MtDataClient.instance().logout()


def is_auth():
    """账号是否已经认证"""
    if not MtDataClient.instance():
        return False
    return not MtDataClient.instance().not_auth
...
def set_params(**params):
    ...
    参数说明：
        ...
        request_attempt_count: 请求的尝试的次数，用于网络异常时重试，默认为 3 次，
                         该次数不能超过 10 次
    """
    MtDataClient.set_request_params(**params)

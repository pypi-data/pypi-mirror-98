...
async def replaces(info):
	...
	while "$htmlUrl[" in reline:
		if nowloop==reline:
			break
		else:
			nowloop=reline
			try:
				reline=reline.replace("$htmlUrl["+reline.split("$htmlUrl[")[-1].split("]")[0]+"]", str(requests.get(reline.split("$htmlUrl[")[-1].split("]")[0]).text))
			except Exception as e:
				reline=reline.replace("$htmlUrl["+reline.split("$htmlUrl[")[-1].split("]")[0]+"]", "none")
	return reline
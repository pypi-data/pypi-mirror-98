...
class NicoNico():
    ...
    def download(self, path, convert_to_mp3=False, timeout=5, chunk_size=8192):
        ...
        if convert_to_mp3:
            self._print("Converting to mp3 ...")
            clip_in = mp.VideoFileClip(path[:-1]+"4").subclip()
            clip_in.audio.write_audiofile(path)
            self._print("Done")

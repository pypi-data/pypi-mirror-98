...
def command_runner(command,  # type: Union[str, List[str]]
    ...
    if windows_no_window and sys.version_info[0] >= 3 and sys.version_info[1] >= 7 and os.name == 'nt':
    ...
    def _pipe_read(pipe, read_timeout, encoding, errors):
        ...
        try:
            while True:
                ...
                if len(current_output) <= 0 or (datetime.now() - begin_time).total_seconds() > read_timeout:
                    break
    ...
    try:
        ...
        if sys.version_info >= (3, 6):
            process = subprocess.Popen(command, stdout=_stdout, stderr=_stderr, shell=shell,
        ...
        logger.debug('Command "{}" returned with exit code "{}". Command output was:'.format(command, exit_code))
        ...
        if exit_code in valid_exit_codes if valid_exit_codes is not None else [0]:
            logger.debug('Command "{}" returned with exit code "{}". Command output was:'.format(command, exit_code))
        logger.error('Command "{}" failed with exit code "{}". Command output was:'.format
    ...
    # pylint: disable=W0705 (duplicate-except)
        ...
        message = 'Timeout {} seconds expired for command "{}" execution.'.format(timeout, command)
    ...
    # pylint: disable=W0703
        ...
        logger.error('Command "{}" failed for unknown reasons: {}'.format(command, exc), exc_info=True)

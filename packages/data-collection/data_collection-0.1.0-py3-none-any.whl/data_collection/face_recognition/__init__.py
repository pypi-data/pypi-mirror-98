...
import os
import re
import data_collection.face_recognition.api as face_recognition
...
def test_image(
    ...
    for unknown_encoding in unknown_encodings:
        ...
        if True in result:
            ...
            print_result(
                image_to_check, "unknown_person - deleting", None, show_distance
            )
            try:
    ...
    if not unknown_encodings:
        ...
        print_result(image_to_check, "no_persons_found - deleting", None, show_distance)
        try:
            os.remove(image_to_check)
        except FileNotFoundError:
            print("already removed")
...
def image_files_in_folder(folder):
    return [
        ...
        if re.match(r".*\.(jpg|jpeg|png|bmp)", f, flags=re.I)
    ]

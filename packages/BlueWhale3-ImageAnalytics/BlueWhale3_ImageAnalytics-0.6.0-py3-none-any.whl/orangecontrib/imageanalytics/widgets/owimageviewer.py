...
def __(key):
    return i18n.t('imganlytic.owimageviewer.' + key)
...
class OWImageViewer(widget.OWWidget):
    ...
    class Inputs:
        data = Input("Data", Orange.data.Table, label=i18n.t("imganlytic.common.data"))

    class Outputs:
        selected_data = Output("Selected Images", Orange.data.Table, label=i18n.t("imganlytic.common.select_image"))
        data = Output("Data", Orange.data.Table, label=i18n.t("imganlytic.common.data"))
    ...
    UserAdviceMessages = [
        widget.Message(__("dialog"), persistent_id="preview-introduction")
    ]
    ...
    def __init__(self):
        ...
        self.imageAttrCB = gui.comboBox(
            ...
            box=__("box_image_file_attr"),
            tooltip=__("tooltip_attr_image_file"),
        ...
        gui.hSlider(
            ...
            box=__("box_image_size"), minValue=32, maxValue=1024, step=16,
            callback=self.updateSize,
            createLabel=False
        )
        gui.rubber(self.controlArea)

        gui.auto_commit(self.controlArea, self, "autoCommit", __("btn_send"), box=False)
    ...
    def _updateStatus(self):
        ...
        text = __("tooltip_image_display").format(self._successcount, count)

        if self._errcount:
            text += __("tooltip_error").format(self._errcount)
        self.info.set_input_summary(str(count), text)
        attr = self.stringAttrs[self.imageAttr]
        if self._errcount == count and "type" not in attr.attributes:
            self.error(__("dialog_error").format(attr.name))

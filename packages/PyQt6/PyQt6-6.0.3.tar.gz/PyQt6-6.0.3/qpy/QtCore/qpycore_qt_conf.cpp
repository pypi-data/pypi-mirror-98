...
{
    // Get PyQt6.__file__.
    PyObject *pyqt = PyImport_ImportModule("PyQt6");

    if (!pyqt)
        return false;

    PyObject *init = PyObject_GetAttrString(pyqt, "__file__");
    Py_DECREF(pyqt);
    ...
    // Get the directory containing the PyQt6 extension modules.
    QDir pyqt_dir = QFileInfo(QDir::fromNativeSeparators(init_impl)).absoluteDir();
    QString qt_dir_name = pyqt_dir.absoluteFilePath(QLatin1String("Qt6"));

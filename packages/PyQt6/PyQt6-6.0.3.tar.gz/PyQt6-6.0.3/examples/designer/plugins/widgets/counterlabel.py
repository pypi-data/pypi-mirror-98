...
class CounterLabel(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p = QPainter()
        p.begin(self)
        p.setRenderHint(QPainter.RenderHints.Antialiasing)
        p.setFont(self._font)
        p.translate(self.width()/2.0, self.height()/2.0)
        p.scale(self._scale, self._scale)
        p.drawText(int(self._xpos), int(self._ypos), str(self._value))
        p.end()
    ...
    def rescale(self):
        ...
        maxRect = fm.boundingRect(QRectF(self.rect()),
                Qt.Alignment.AlignCenter, str(self._maximum))
    ...
    def reposition(self):
        ...
        fm = QFontMetricsF(self._font, self)
        rect = fm.boundingRect(QRectF(self.rect()), Qt.Alignment.AlignCenter,
                str(self._value))

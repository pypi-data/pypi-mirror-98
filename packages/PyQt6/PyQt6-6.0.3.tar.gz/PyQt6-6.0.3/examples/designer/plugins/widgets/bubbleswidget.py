...
class BubblesWidget(BaseClass):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButtons.LeftButton and self.newBubble is None:

            self.newBubble = Bubble(QPointF(event.position()), 4.0,
                                    1.0 + random.random() * 7,
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.newBubble:
            ...
            self.newBubble.position = QPointF(event.position())
            self.update(
                QRectF(self.newBubble.position - \
                       QPointF(self.newBubble.radius + 1, self.newBubble.radius + 1),
                       QSizeF(2*self.newBubble.radius + 2, 2*self.newBubble.radius + 2)).toRect()
    ...
    def randomColor(self):
        ...
        red = int(205 + random.random() * 50)
        green = int(205 + random.random() * 50)
        blue = int(205 + random.random() * 50)
        alpha = int(91 + random.random() * 100)

...
def calc_num_correlation_matrix(df):
    '''
    Calculates a matrix of correlations for all numerical pairs of columns
    in a collection.
    '''
...
def calc_cat_representations(df, columns=None, max_cardinality=None):
    ...
    if not max_cardinality is None:
    ...
    return { cat_cols[i] : max(0, rep_scores[i]) for i in range(len(cat_cols))\
            if not pd.isna(rep_scores[i]) }

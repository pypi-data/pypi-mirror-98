        ...
        for i in range(_size):
            ...
            if n_gti > 1:
                ...
                if not (_x[i] >= GTI_START[curr_gti_ind] and _x[i] <= GTI_STOP[curr_gti_ind]):
                    logger.info("Event # %d time %g not in gti bin %d %g -- %g " % (
                ...
                if rr <= min_sn * np.sqrt(ee):  # includes the rr=ee=0 case
                    ...
                    if (not end_flag):
                        ...
                        if (n_gti > 0):
                            ...
                            logger.info("\nDisregard " + str(local_t) + " s for long gap at " + str(_x[i]))
            ...
            if ee > 0:  # to avoid div by zero
                ...
                if not end_flag:
                    logger.info("Disregard points in interval " + str(curr_gti_ind - 1) + " = [s]  " +
                                str(100 * local_t / (GTI_STOP[curr_gti_ind - 1] - GTI_START[
                                    curr_gti_ind - 1])) + " \% (GTI FLAG)")

...
class Song:
	...
	def __init__(self, url_or_urls):
		...
		if type(url_or_urls) == str:
			self.__data = asyncio.gather(*[asyncio.create_task(self.__get(url_or_urls))])
		elif type(url_or_urls) == list:
			self.__data = asyncio.gather(*[asyncio.create_task(self.__get(url)) for url in url_or_urls])

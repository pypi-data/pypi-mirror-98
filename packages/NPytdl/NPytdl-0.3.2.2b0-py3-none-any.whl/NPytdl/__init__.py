...
class Song:
	...
	async def create(self):
		...
		elif type(self.__data) == asyncio.Task:
	...
	def __init__(self, url_or_urls):
		if type(url_or_urls) == str and not re.fullmatch(r'https://w{0,3}\.?youtu(\.be/|be\.com/watch\?v=)[a-zA-Z0-9]*', url_or_urls):
			raise TypeError("url is not a youtube video url")
		if type(url_or_urls) == str:
			data = asyncio.gather(*[asyncio.ensure_future(self.__get(url_or_urls))])
			self.__data = data
		elif type(url_or_urls) == list:
			data = asyncio.gather(*[asyncio.ensure_future(self.__get(url)) for url in url_or_urls])
			self.__data = data

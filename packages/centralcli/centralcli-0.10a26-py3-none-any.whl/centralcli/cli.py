...
def move(
    ...
    kw1: str = typer.Argument(
        ...
        autocompletion=lambda incomplete: [
            c for c in [
                ("group", ), ("site", ), *[m for m in cli.cache.dev_completion(incomplete)]
            ] if c[0].startswith(incomplete.lower())
        ],
    ),
    kw1_val: str = typer.Argument(
        ...
        show_default=False,
        autocompletion=lambda incomplete: [
                *[m for m in cli.cache.group_completion(incomplete)],
                *[m for m in cli.cache.site_completion(incomplete)],
                *[m for m in cli.cache.dev_completion(incomplete)]
        ]
    ),

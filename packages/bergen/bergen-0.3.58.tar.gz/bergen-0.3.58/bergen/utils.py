...
async def shrinkInputs(node: Node, args: List, kwargs: List) -> Tuple[dict, dict]:
    ...
    for port in node.args:
        ...
        if port.TYPENAME == TYPENAMES.MODELPORTTYPE:
            instance = args[port.key]
            if isinstance(instance, ArnheimModel):
                shrinked_args[port.key] = args[port.key].id
            else:
                shrinked_args[port.key] = args[port.key]
        else:
            shrinked_args[port.key] = shrinked_args[port.key]
    ...
    for port in node.args:
        ...
        if port.TYPENAME == TYPENAMES.MODELPORTTYPE:
            instance = args[port.key]
            if isinstance(instance, ArnheimModel):
                shrinked_kwargs[port.key] = args[port.key].id
            else:
                shrinked_kwargs[port.key] = args[port.key]
        else:
            shrinked_kwargs[port.key] = shrinked_kwargs[port.key]

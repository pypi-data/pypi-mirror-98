...
class WebSocketSession():
    ...
    async def prepare(self, request, reconnect = False):
        ...
        asyncio.ensure_future(asyncio.gather(*[f for f in [h[1].handle_connected(self) for h in self.context.event_handler_manager.handlers()] if inspect.isawaitable(f)]))
    ...
    async def closed_by_client(self):
        logger.debug("CLOSED_BY_CLIENT|SOCKET=%s|TRANSPORT=%s|IS_CLOING=%s", self.socket.closed, self.request.transport, is_closing)
        ...
        while self.socket is not None or self.reconnect_lock is not None:
            if self.socket is not None:
                if not self.socket.closed and self.request.transport is not None:
                    ...
                    start = time.time()
                    logger.debug("SEND_BYTES|TRANSPORT=%s|IS_CLOSING=%s", self.request.transport, is_closing)
                    try:
                        buf = msgpack.packb([request_id, event_id, data])
                        size = len(buf)
                ...
                if self.reconnect_lock is None:
                    self.reconnect_lock = asyncio.Condition()
                    self.reconnect_until = datetime.now().timestamp() + self.reconnect_timeout
                    self.socket = None
                    asyncio.ensure_future(asyncio.gather(*[f for f in [h[1].handle_wait_reconnect(self) for h in self.context.event_handler_manager.handlers()] if inspect.isawaitable(f)]))

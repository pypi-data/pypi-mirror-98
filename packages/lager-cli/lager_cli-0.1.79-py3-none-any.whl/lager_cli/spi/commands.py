...
@click.option('--speed', required=False, default=1000000, help='Speed of the SPI communication in bits per second')
@click.option('--cs', type=_GPIO_CHOICES, required=False, default='-1', help='GPIO number to use as a Chip Select line')
@click.option('--mode', type=click.Choice(('0', '1', '2', '3')), required=False, default='0', help='SPI Mode number')
@click.option('--type', '_type', type=click.Choice(('HEX', 'STRING', 'BYTE', 'INT'), case_sensitive=False), required=False, default='STRING', show_default=True, help='The datatype of the provided SPI data')
...
@click.pass_context
def read(ctx, gateway, speed, cs, mode, _type, output, length):
    """
    ...
    try:
        # TODO speed this up and enable HW CE
...
@click.pass_context
def write(ctx, gateway, speed, cs, mode, _type, output, data):
    """
    Send data over SPI
    """
    if gateway is None:
...
@click.argument('data', nargs=-1, type=click.UNPROCESSED)
@click.pass_context
def transfer(ctx, gateway, speed, cs, mode, _type, output, data):
    """

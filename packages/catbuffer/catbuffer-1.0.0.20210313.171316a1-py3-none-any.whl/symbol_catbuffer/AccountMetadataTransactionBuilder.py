...
class AccountMetadataTransactionBuilder(TransactionBuilder):
    """Binary layout for a non-embedded account metadata transaction.

    Attributes:
        body: Account metadata transaction body.
    """
    type_hints = {
    ...
    def __init__(self, signerPublicKey, network: NetworkTypeDto):
        ...
        self.body = AccountMetadataTransactionBodyBuilder()

    @classmethod
    def loadFromBinary(cls, payload: bytes) -> AccountMetadataTransactionBuilder:
        ...
        body = AccountMetadataTransactionBodyBuilder.loadFromBinary(bytes_)  # kind:CUSTOM1_nonbyte
        bytes_ = bytes_[body.getSize():]

        # create object and call
        result = AccountMetadataTransactionBuilder(superObject.signerPublicKey, superObject.network)

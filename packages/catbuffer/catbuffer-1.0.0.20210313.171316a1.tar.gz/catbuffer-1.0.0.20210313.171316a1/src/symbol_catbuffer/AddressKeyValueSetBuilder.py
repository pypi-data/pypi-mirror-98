...
from .AddressKeyValueBuilder import AddressKeyValueBuilder
...
class AddressKeyValueSetBuilder:
    ...
    def __init__(self, keys: List[AddressKeyValueBuilder]):
        """Constructor.
        Args:
            keys: Key value array.
        """
    ...
    @classmethod
    def loadFromBinary(cls, payload: bytes) -> AddressKeyValueSetBuilder:
        """Creates an instance of AddressKeyValueSetBuilder from binary payload.
        Args:
            payload: Byte payload to use to serialize the object.
        Returns:
            Instance of AddressKeyValueSetBuilder.
        """
        bytes_ = bytes(payload)

        keyValueCount = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 1))  # kind:SIZE_FIELD
        bytes_ = bytes_[1:]
        keys: List[AddressKeyValueBuilder] = []  # kind:ARRAY
        for _ in range(keyValueCount):
            item = AddressKeyValueBuilder.loadFromBinary(bytes_)
            keys.append(item)
            bytes_ = bytes_[item.getSize():]
        return AddressKeyValueSetBuilder(keys)

    def getKeys(self) -> List[AddressKeyValueBuilder]:

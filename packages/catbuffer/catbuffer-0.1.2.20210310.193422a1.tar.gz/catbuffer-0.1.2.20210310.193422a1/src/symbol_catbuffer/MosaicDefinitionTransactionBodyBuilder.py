...
class MosaicDefinitionTransactionBodyBuilder:
    ...
    def __str__(self):
        ...
        """
        result = ''
        result += '{:24s} : {}\n'.format('id', toHexString(MosaicIdDto(self.id).serialize()))
        result += '{:24s} : {}\n'.format('duration', toHexString(BlockDurationDto(self.duration).serialize()))
        result += '{:24s} : {}\n'.format('nonce', toHexString(MosaicNonceDto(self.nonce).serialize()))
        _serializedFlags = GeneratorUtils.uintToBuffer(MosaicFlagsDto.flagsToInt(self.flags), 1)
        result += '{:24s} : {} {}\n'.format('flags', toHexString(_serializedFlags), self.flags)
        result += '{:24s} : {}\n'.format('divisibility', toHexString(GeneratorUtils.uintToBuffer(self.divisibility, 1)))
        return result

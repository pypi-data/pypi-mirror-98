...
class ImportanceBlockFooterBuilder:
    ...
    @classmethod
    def loadFromBinary(cls, payload: bytes) -> ImportanceBlockFooterBuilder:
        ...
        votingEligibleAccountsCount = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 4))  # kind:SIMPLE
        bytes_ = bytes_[4:]
        harvestingEligibleAccountsCount = GeneratorUtils.bufferToUint(GeneratorUtils.getBytes(bytes_, 8))  # kind:SIMPLE
        bytes_ = bytes_[8:]
        totalVotingBalance = AmountDto.loadFromBinary(bytes_)  # kind:CUSTOM1
        bytes_ = bytes_[totalVotingBalance.getSize():]
        previousImportanceBlockHash = Hash256Dto.loadFromBinary(bytes_)  # kind:CUSTOM1
        bytes_ = bytes_[previousImportanceBlockHash.getSize():]
    ...
    def getSize(self) -> int:
        ...
        size += self.totalVotingBalance.getSize()
        size += self.previousImportanceBlockHash.getSize()
        return size
    ...
    def serialize(self) -> bytes:
        ...
        """
        bytes_ = bytes()
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, GeneratorUtils.uintToBuffer(self.getVotingEligibleAccountsCount(), 4))  # kind:SIMPLE
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, GeneratorUtils.uintToBuffer(self.getHarvestingEligibleAccountsCount(), 8))  # kind:SIMPLE
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, self.totalVotingBalance.serialize())  # kind:CUSTOM
        bytes_ = GeneratorUtils.concatTypedArrays(bytes_, self.previousImportanceBlockHash.serialize())  # kind:CUSTOM
        return bytes_
    # end of class

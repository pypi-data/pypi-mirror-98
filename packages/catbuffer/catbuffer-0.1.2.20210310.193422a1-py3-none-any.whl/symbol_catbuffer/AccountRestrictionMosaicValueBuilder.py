...
class AccountRestrictionMosaicValueBuilder:
    ...
    def __init__(self, restrictionValues: List[MosaicIdDto]):
        """Constructor.
        Args:
            restrictionValues: Restriction values.
        """
    ...
    @classmethod
    def loadFromBinary(cls, payload: bytes) -> AccountRestrictionMosaicValueBuilder:
        ...
        bytes_ = bytes_[8:]
        restrictionValues: List[MosaicIdDto] = []  # kind:ARRAY
        for _ in range(restrictionValuesCount):
            item = MosaicIdDto.loadFromBinary(bytes_)
            restrictionValues.append(item)
            bytes_ = bytes_[item.getSize():]
        return AccountRestrictionMosaicValueBuilder(restrictionValues)

    def getRestrictionValues(self) -> List[MosaicIdDto]:

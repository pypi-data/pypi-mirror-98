...
def refgenie_build(gencfg, genome, asset_list, recipe_name, args):
    ...
    def _build_asset(
        ...
        **kwargs,
        ...
        command_list_populated = [
            x.format(**{k.split(".")[0]: v for k, v in list(asset_vars.items())})
            ...
            with rgc as r:
                ...
                r.update_seek_keys(
                    *gat,
                    force_digest=genome,
                    keys={
                        k: v.format(**asset_vars) for k, v in list(build_pkg[ASSETS].items())
                    },
                )
    ...
    for a in asset_list:
        ...
        if isinstance(recipe_name, dict) or (
            isinstance(recipe_name, str) and recipe_name in list(asset_build_packages.keys())
        ):
            ...
            for required_file in asset_build_package[REQ_FILES]:
                if (
                    ...
                    or required_file[KEY] not in list(specified_args.keys())
                ):
                    raise ValueError(

...
class Conversor():
    ...
    ontology_atomic_user_story = None):
        ...
        ontology_atomic_user_story.activated_date = self.date_formater(None if type(next(iter(jira_issue.raw['fields'].get('customfield_10018') or []), None)) != dict else self.date_formater(self.resolve_start_date(jira_issue.raw['fields']['customfield_10018'])))
    ...
    ontology_scrum_intended_development_task = None, ontology_scrum_performed_development_task = None):
        ...
        try:
            creator_id = jira_issue.raw['fields']['creator']['accountId']
            ontology_scrum_intended_development_task.created_by = team_member_application.retrive_by_external_id_and_project_name(creator_id, jira_project.name).id
            if ontology_scrum_intended_development_task.created_by is None:
                raise Exception('Not found')
        except (NameError, TypeError):
            ontology_scrum_intended_development_task.created_by = None
        except Exception as e:    
            team_member = etl_team_member()
            team_member.config(self.data)
            data_to_create = {'accountId': creator_id}
            ontology_scrum_intended_development_task.created_by = team_member.create(data_to_create, None, jira_project).id

        # Assignee
        try:
            assignee_id = jira_issue.raw['fields']['assignee']['accountId']
            ontology_scrum_intended_development_task.assigned_by = [team_member_application.retrive_by_external_id_and_project_name(assignee_id, jira_project.name)]
            if ontology_scrum_intended_development_task.assigned_by is None:
                raise Exception('Not found')
        except (NameError, TypeError):
            ontology_scrum_intended_development_task.assigned_by = []
        except Exception as e:
            team_member = etl_team_member()
            team_member.config(self.data)
            data_to_create = {'accountId': assignee_id}
            ontology_scrum_intended_development_task.assigned_by = [team_member.create(data_to_create, None, jira_project)]

        try:
            parent_id = jira_issue.raw['fields']['parent']['id']
            ontology_scrum_intended_development_task.atomic_user_story = atomic_user_story_application.retrive_by_external_uuid(parent_id).id
            if ontology_scrum_intended_development_task.atomic_user_story is None:
                raise Exception('Not found')
        except (NameError, TypeError):
            ontology_scrum_intended_development_task.atomic_user_story = None
        except Exception as e:
            atomic_user_story = etl_user_story()
            atomic_user_story.config(self.data)
            data_to_create = {"content": {"all": {"issue": {"id": parent_id } } } }
            ontology_scrum_intended_development_task.atomic_user_story = atomic_user_story.create(data_to_create, None, jira_project).id
        ...
        or jira_issue.raw['fields']['status']['statusCategory']['id'] == 4): # Em andamento
            ...
            try:
                creator_id = jira_issue.raw['fields']['creator']['accountId']
                ontology_scrum_performed_development_task.created_by = team_member_application.retrive_by_external_id_and_project_name(creator_id, jira_project.name).id
                if ontology_scrum_performed_development_task.created_by is None:
                    raise Exception('Not found')
            except (NameError, TypeError):
                ontology_scrum_performed_development_task.created_by = None
            except Exception as e:    
                team_member = etl_team_member()
                team_member.config(self.data)
                data_to_create = {'accountId': creator_id}
                ontology_scrum_performed_development_task.created_by = team_member.create(data_to_create, None, jira_project).id
            
            # Assignee
            try:
                assignee_id = jira_issue.raw['fields']['assignee']['accountId']
                team_member_ = team_member_application.retrive_by_external_id_and_project_name(assignee_id, jira_project.name)
                ontology_scrum_performed_development_task.assigned_by = [team_member_]
                ontology_scrum_performed_development_task.closed_by = team_member_.id
                if ontology_scrum_performed_development_task.assigned_by is None:
                    raise Exception('Not found')
            except (NameError, TypeError):
                ontology_scrum_performed_development_task.assigned_by = []
            ...
            try:
                reporter_id = jira_issue.raw['fields']['reporter']['accountId']
                team_member_ = team_member_application.retrive_by_external_id_and_project_name(reporter_id, jira_project.name)
            ...
            try:
                parent_id = jira_issue.raw['fields']['parent']['id']
                atomic_user_story_application = factories.AtomicUserStoryFactory()
                ontology_scrum_performed_development_task.atomic_user_story = atomic_user_story_application.retrive_by_external_uuid(parent_id).id
                if ontology_scrum_performed_development_task.atomic_user_story is None:
                    raise Exception('Not found')
            except (NameError, TypeError):
                ontology_scrum_performed_development_task.atomic_user_story = None
            except Exception as e:
                atomic_user_story = etl_user_story()
                atomic_user_story.config(self.data)
                data_to_create = {"content": {"all": {"issue": {"id": parent_id } } } }
                ontology_scrum_performed_development_task.atomic_user_story = atomic_user_story.create(data_to_create, None, jira_project).id

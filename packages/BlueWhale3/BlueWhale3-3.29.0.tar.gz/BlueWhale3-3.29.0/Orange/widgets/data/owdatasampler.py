...
def __(key):
    return i18n.t("widget.data.data.owdatasampler." + key)
...
class OWDataSampler(OWWidget):
    ...
    class Outputs:
        data_sample = Output("Data Sample", Table, default=True, label=i18n.t("widget.data.data.common.data_sample"))
        remaining_data = Output("Remaining Data", Table, label=i18n.t("widget.data.data.common.remain_data"))
    ...
    class Warning(OWWidget.Warning):
        could_not_stratify = Msg(__("msg.stratific_failed"))
        bigger_sample = Msg(__('msg.sample_big'))

    class Error(OWWidget.Error):
        too_many_folds = Msg(__("msg.fold_exceed"))
        sample_larger_than_data = Msg(__("msg.sample_nolarger"))
        not_enough_to_stratify = Msg(__("msg.data_small"))
        no_data = Msg(__("msg.dataset_empty"))
    ...
    def __init__(self):
        ...
        self.sampling_box = gui.vBox(self.controlArea, __("box_sample_type"))
        ...
        gui.appendRadioButton(sampling, __("btn.fix_proportion"))
        ...
        gui.appendRadioButton(sampling, __("btn.fix_sample"))
        ibox = gui.indentedBox(sampling)
        self.sampleSizeSpin = gui.spin(
            ibox, self, "sampleSizeNumber", label=__("row.instance"),
            minv=1, maxv=self._MAX_SAMPLE_SIZE,
            callback=set_sampling_type(self.FixedSize),
            controlWidth=90)
        gui.checkBox(
            ibox, self, "replacement", __("checkbox_sample_replacement"),
            callback=set_sampling_type(self.FixedSize))

        gui.appendRadioButton(sampling, __("btn.cross_validate"))
        form = QFormLayout(
            formAlignment=Qt.AlignLeft | Qt.AlignTop,
            labelAlignment=Qt.AlignLeft,
            fieldGrowthPolicy=QFormLayout.AllNonFixedFieldsGrow)
        ibox = gui.indentedBox(sampling, orientation=form)
        form.addRow(__("row.num_subsets"),
                    gui.spin(
        ...
        form.addRow(__("row.unused_subset") if not self.compatibility_mode
                    else __("row.select_subset"), self.selected_fold_spin)

        gui.appendRadioButton(sampling, __("btn.bootstrap"))

        self.sql_box = gui.vBox(self.controlArea, __("box_sample_type"))
        sampling = gui.radioButtons(self.sql_box, self, "sampling_type",
                                    callback=self.sampling_type_changed)

        gui.appendRadioButton(sampling, __("btn.time"))
        ibox = gui.indentedBox(sampling)
        spin = gui.spin(ibox, self, "sampleSizeSqlTime", minv=1, maxv=3600,
                        callback=set_sampling_type(self.SqlTime))
        spin.setSuffix(" sec")
        gui.appendRadioButton(sampling, __("btn.percentage"))
        ibox = gui.indentedBox(sampling)
        ...
        self.options_box = gui.vBox(self.controlArea, __("box_options"), addSpaceBefore=False)
        self.cb_seed = gui.checkBox(
            self.options_box, self, "use_seed",
            __("option_replicable"),
            callback=self.settings_changed)
        self.cb_stratify = gui.checkBox(
            self.options_box, self, "stratify",
            __("option_stratify"), callback=self.settings_changed)
        self.cb_sql_dl = gui.checkBox(
            self.options_box, self, "sql_dl", __("download_to_local_memory"),
            callback=self.settings_changed)
        self.cb_sql_dl.setVisible(False)

        gui.button(self.buttonsArea, self, __("btn.sample"),
    ...
    def send_report(self):
        if self.sampling_type == self.FixedProportion:
            tpe = __("report.random_sample_data").format(
                self.sampleSizePercentage)
        elif self.sampling_type == self.FixedSize:
            if self.sampleSizeNumber == 1:
                tpe = __("report.random_data_instance")
            else:
                tpe = __("report.random_sample_data_instance").format(
                    self.sampleSizeNumber)
                if self.replacement:
                    tpe += __("report.replacement")
        elif self.sampling_type == self.CrossValidation:
            tpe = __("report.validation").format(
                self.selectedFold, self.number_of_folds)
        elif self.sampling_type == self.Bootstrap:
            tpe = __("report.bootstrap")
        else:  # pragma: no cover
            tpe = __("report.undefined")  # should not come here at all
        if self.stratify:
            tpe += __("report.stratify")
        if self.use_seed:
            tpe += __("report.deterministic")
        items = [(__("report.sample_type"), tpe)]
        if self.sampled_instances is not None:
            items += [
                (i18n.t("common.software.input"), __("report.instance").format(len(self.data))),
                (__("report.sample"), __("report.instance").format(self.sampled_instances)),
                (__("report.remain"), __("report.instance").format(self.remaining_instances)),
            ]

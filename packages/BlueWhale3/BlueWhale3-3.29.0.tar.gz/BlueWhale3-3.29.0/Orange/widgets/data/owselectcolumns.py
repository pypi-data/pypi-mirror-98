...
def __(key):
    return i18n.t("widget.data.data.owselectcolumns." + key)
...
class OWSelectAttributes(widget.OWWidget):
    ...
    class Warning(widget.OWWidget.Warning):
        mismatching_domain = Msg(__("msg_feature_not_match"))
        multiple_targets = Msg(__("msg_multiple_targets"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __("box.ignored"),
                       addToLayout=False)
        ...
        # 3rd column
        box = gui.vBox(self.controlArea, __("box.features"), addToLayout=False)
        ...
        self.use_features_box = gui.auto_commit(
            ...
            __("checkbox_use_input_feature"), __("checkbox_always_use_feature"),
        ...
        box = gui.vBox(self.controlArea, __("box.target"), addToLayout=False)
        ...
        box = gui.vBox(self.controlArea, __("box.metas"), addToLayout=False)
        ...
        # footer
        gui.button(self.buttonsArea, self, i18n.t("common.btn.reset"), callback=self.reset)
        ...
        gui.checkBox(
            ...
            label=__("ignore_new_features"),
            tooltip=__("ignore_new_features_tip")
        )
    ...
    def send_report(self):
        ...
        self.report_domain(__("report.input_data"), self.data.domain)
        if (in_domain.attributes, in_domain.class_vars, in_domain.metas) == (
                out_domain.attributes, out_domain.class_vars, out_domain.metas):
            self.report_paragraph(__("report.output_data"), __("report.no_change"))
        else:
            self.report_domain(__("report.output_data"), self.output_data.domain)
            ...
            if diff:
                ...
                self.report_items(((__("report.removed"), text),))

...
def __(key):
    return i18n.t("widget.data.data.owconcatenate." + str(key))
...
class OWConcatenate(widget.OWWidget):
    ...
    class Inputs:
        primary_data = Input("Primary Data", Orange.data.Table, label=i18n.t("widget.data.data.common.primary_data"))
        additional_data = Input("Additional Data",
                                Orange.data.Table,
                                multiple=True,
                                default=True,
                                label=i18n.t("widget.data.data.common.additional_data"))

    class Outputs:
        data = Output("Data", Orange.data.Table, label=i18n.t("widget.data.data.common.data"))

    class Error(widget.OWWidget.Error):
        bow_concatenation = Msg(__("msg_same_type"))

    class Warning(widget.OWWidget.Warning):
        renamed_variables = Msg(
            __("msg_duplicated_name"))
    ...
    domain_opts = (__("option_union_attr"),
                   __("option_interect_attr"))

    id_roles = (__("gbox_class_attr"), __("gbox_attr"), __("gbox_meta_attr"))
    ...
    def __init__(self):
        ...
        self.mergebox = gui.vBox(self.controlArea, __("box_var_merge"))
        box = gui.radioButtons(
            self.mergebox, self, "merge_type",
            callback=self._merge_type_changed)

        gui.widgetLabel(box, self.tr(__("label_domain")))
        ...
        label = gui.widgetLabel(
            box,
            self.tr(__("label_result")))
        label.setWordWrap(True)

        gui.separator(box)
        gui.checkBox(
            box, self, "ignore_compute_value",
            __("ignore_compute_value"),
            callback=self.apply, stateWhenDisabled=False)
        ###
        box = gui.vBox(
            self.controlArea, self.tr(__("box_source_indentification")),
        )

        cb = gui.checkBox(
            box, self, "append_source_column",
            self.tr(__("checkbox_appendid")),
    ...
    def _set_input_summary(self):
        ...
        if self.primary_data or self.more_data:
            ...
            details = format_multiple_summaries(
                [(__("primary_data"), self.primary_data)]

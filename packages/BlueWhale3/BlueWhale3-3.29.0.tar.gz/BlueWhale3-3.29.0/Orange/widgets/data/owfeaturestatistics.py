...
def __(key):
    return i18n.t("widget.data.data.owfeaturestatistics." + key)
...
class FeatureStatisticsTableModel(AbstractSortTableModel):
    ...
    class Columns(IntEnum):
        ...
        def name(self):
            return {self.ICON: '',
                    self.NAME: __('label.name'),
                    self.DISTRIBUTION: i18n.t("common.general.distribution"),
                    self.CENTER: __('label.center'),
                    self.DISPERSION: i18n.t("common.general.distribution"),
                    self.MIN: __('label.min'),
                    self.MAX: __('label.max'),
                    self.MISSING: __('label.missing'),
                    }[self.value]
...
class OWFeatureStatistics(widget.OWWidget):
    ...
    class Outputs:
        ...
        statistics = Output("Statistics", Table, label=i18n.t("common.general.statistics"))
    ...
    def __init__(self):
        ...
        self.cb_color_var = gui.comboBox(
            ...
            label=i18n.t("common.software.color"), orientation=Qt.Horizontal, contentsLength=13,
            searchable=True
        )

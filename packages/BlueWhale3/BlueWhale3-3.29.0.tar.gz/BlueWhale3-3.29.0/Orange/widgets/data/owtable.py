...
def __(key):
    return i18n.t("widget.data.data.owtable." + key)
...
class OWDataTable(OWWidget):
    ...
    class Inputs:
        data = Input("Data", Table, multiple=True, label=i18n.t("widget.data.data.common.data"))

    class Outputs:
        selected_data = Output("Selected Data", Table, default=True, label=i18n.t("widget.data.data.common.selected_data"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, i18n.t("common.general.variable"))
        self.c_show_attribute_labels = gui.checkBox(
            box, self, "show_attribute_labels",
            __("checkbox.show_var"),
            callback=self._on_show_variable_labels_changed)

        gui.checkBox(box, self, "show_distributions",
                     __('checkbox.visualize_num'),
                     callback=self._on_distribution_color_changed)
        gui.checkBox(box, self, "color_by_class", __('checkbox.by_instance'),
                     callback=self._on_distribution_color_changed)

        box = gui.vBox(self.controlArea, __("box_selection"))

        gui.checkBox(box, self, "select_rows", __("checkbox.select_row"),
                     callback=self._on_select_rows_changed)

        gui.rubber(self.controlArea)

        gui.button(self.buttonsArea, self, __("btn_restore_original"),
                   callback=self.restore_order,
                   tooltip=__("tooltip_show_row_in_order"),
                   autoDefault=False,
    ...
    def _info_box_text(slot):
        def format_part(part):
            if isinstance(part, DenseArray):
                ...
                return __("info.miss_data").format(perc)
            ...
            return __("info.density").format(tag, dens)

        def desc(n, part):
            if n == 0:
                return __("info.nothing").format(part)
            elif n == 1:
                return __("info.one").format(part)
            else:
                return __("info.several").format(n, part)

        if slot is None:
            return [__("info.no_data")]
        summary = slot.summary
        text = []
        if isinstance(summary, ApproxSummary):
            if summary.len.done():
                text.append(__("text.instance").format(summary.len.result()))
            else:
                text.append(__("text.instances").format(summary.approx_len))
        elif isinstance(summary, Summary):
            text.append(__("text.instance").format(summary.len))
            if sum(p.nans for p in [summary.X, summary.Y, summary.M]) == 0:
                text[-1] += __("text.miss_data")

        text.append(desc(len(summary.domain.attributes), __("text.feature"))
                    + format_part(summary.X))

        if not summary.domain.class_vars:
            text.append(__("text.no_variable"))
        else:
            if len(summary.domain.class_vars) > 1:
                c_text = desc(len(summary.domain.class_vars), __("text.outcome"))
            elif summary.domain.has_continuous_class:
                c_text = __("text.numeric_outcome")
            else:
                c_text = __("text.target_with") \
                    + desc(len(summary.domain.class_var.values), __("text.value"))
            text.append(c_text + format_part(summary.Y))

        text.append(desc(len(summary.domain.metas), __("text.meta_attribute"))

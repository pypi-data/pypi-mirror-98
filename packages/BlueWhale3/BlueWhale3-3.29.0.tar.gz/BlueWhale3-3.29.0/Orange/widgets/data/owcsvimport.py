...
def __(key):
    return i18n.t("widget.data.data.owcsvimport." + key)
...
class CSVImportDialog(QDialog):
    ...
    def __init__(self, parent=None, flags=Qt.Dialog, **kwargs):
        ...
        self._buttons.button(QDialogButtonBox.Ok).setText(i18n.t("common.btn.ok"))
        self._buttons.button(QDialogButtonBox.Cancel).setText(i18n.t("common.btn.cancel"))
        self._buttons.button(QDialogButtonBox.Reset).setText(i18n.t("common.btn.reset"))
        self._buttons.button(QDialogButtonBox.RestoreDefaults).setText(i18n.t("common.btn.restore_defaults"))
        # TODO: Help button
...
FileFormats = [
    FileFormat("text/csv", __("dialog_file.comma_separated"), ("*.csv", "*")),
    FileFormat("text/tab-separated-values", __("dialog_file.tab_separated"), ("*.tsv", "*")),
    FileFormat("text/plain", __("dialog_file.all_files"), ("*.txt", "*")),
]
...
class OWCSVFileImport(widget.OWWidget):
    ...
    outputs = [
        widget.OutputSignal(
            ...
            doc=__("output_data_desc"),
            label=i18n.t("widget.data.data.common.data")),
        widget.OutputSignal(
            name="Data Frame",
            type=pd.DataFrame,
            doc="",
            label=i18n.t("widget.data.data.common.data_frame")
        )
    ]

    class Error(widget.OWWidget.Error):
        error = widget.Msg(
            __("msg.unexpected_error")
        )
        encoding_error = widget.Msg(
            __("msg.encoding_error")
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        grid = QGridLayout()
        grid.addWidget(QLabel(__("label_file"), self), 0, 0, 1, 1)
        ...
        self.recent_combo = ItemStyledComboBox(
            self, objectName="recent-combo", toolTip=__("tooltip.recent_file"),
            sizeAdjustPolicy=QComboBox.AdjustToMinimumContentsLengthWithIcon,
            minimumContentsLength=16, placeholderText=__("tooltip.recent_file")
        )
        ...
        self.browse_button = QPushButton(
            ...
            toolTip=__("tooltip.browse_filesystem"), autoDefault=False,
        )
        ...
        ac = menu.addAction(__("action.import_file"))
        ac.triggered.connect(self.browse)

        ac = menu.addAction(__("action.relative_workflow"))
        ac.setToolTip(__("action.relative_workflow_tip"))
        ...
        # Info text
        ###########
        box = gui.widgetBox(self.controlArea, i18n.t("common.software.info"))
        ...
        button_box.button(QDialogButtonBox.Cancel).setText(i18n.t("common.btn.cancel"))
        button_box.button(QDialogButtonBox.Retry).setText(i18n.t("common.btn.retry"))
        self.load_button = b = button_box.button(QDialogButtonBox.Retry)
        b.setText(i18n.t("common.btn.load"))
        ...
        self.import_options_button = QPushButton(
            i18n.t("common.btn.import"), enabled=False, autoDefault=False,
    ...
    def _browse_dialog(self):
        dlg = FileDialog(
            self, windowTitle=self.tr(__("dialog_file.openfile")),
    ...
    def _might_be_binary_mb(self, path) -> QMessageBox:
        mb = QMessageBox(
            ...
            text=self.tr(__("tooltip.binary_file")).format(
                             basename=os.path.basename(path)),
            standardButtons=QMessageBox.Cancel | QMessageBox.Yes
        )
        mb.button(QMessageBox.Cancel).setText(i18n.t("common.btn.cancel"))
        mb.button(QMessageBox.Yes).setText(i18n.t("common.btn.ok"))
        mb.setWindowModality(Qt.WindowModal)
        return mb

    def _path_must_be_relative_mb(self, prefix: str) -> QMessageBox:
        mb = QMessageBox(
            parent=self, windowTitle=self.tr(__("invalid_path")),
            icon=QMessageBox.Warning,
            text=self.tr(__("invalid_path_tip")).format(
                prefix=prefix
            ),
        )
    ...
    def browse(self, prefixname=None, directory=None):
        ...
        if status == QFileDialog.Accepted:
            ...
            dlg = CSVImportDialog(
                self, windowTitle=__("window_title_import_option"), sizeGripEnabled=True)
    ...
    def cancel(self):
        ...
        if self.__watcher is not None:
            ...
            self.setStatusMessage(__("setstatus.cancelled"))
            self.summary_text.setText(
                __("setstatus.reload_text")
            )

    def __set_running_state(self):
        self.progressBarInit()
        self.setBlocking(True)
        self.setStatusMessage(i18n.t("common.software.run"))
        self.cancel_button.setEnabled(True)
        self.load_button.setText(__("setstatus.restart"))
        path = self.current_item().path()
        self.Error.clear()
        self.summary_text.setText(
            __("setstatus.load_text").format(prettyfypath(path))
        )
    ...
    def __clear_running_state(self, ):
        ...
        self.load_button.setText(i18n.t("common.btn.reload"))
    ...
    def __set_error_state(self, err):
        ...
        basename = os.path.basename(path)
        if isinstance(err, UnicodeDecodeError):
            text = __("tooltip.encoding_error").format(basename=escape(basename))
        else:
            text = __("tooltip.load_error").format(basename=escape(basename),
    ...
    def _update_status_messages(self, data):
        ...
        summary = __("data_info").format(n_instances=len(data),
                                         n_features=len(data.domain.attributes), n_meta=len(data.domain.metas))

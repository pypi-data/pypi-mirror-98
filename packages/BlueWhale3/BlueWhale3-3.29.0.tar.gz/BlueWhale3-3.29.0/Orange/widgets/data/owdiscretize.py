...
def __(key):
    return i18n.t("widget.data.data.owdiscretize." + key)
...
class DiscDelegate(QStyledItemDelegate):
    ...
    def cutsText(state: DState, fmt: Callable[[Any], str] = str):
        ...
        if isinstance(method, Default):
            ...
            name = __("method.leave")
        elif isinstance(method, MDL):
            name = __("method.entropy")
        elif isinstance(method, EqualFreq):
            name = __("method.equal_frequency").format(method.k)
        elif isinstance(method, EqualWidth):
            name = __("method.equal_width").format(method.k)
        elif isinstance(method, Remove):
            name = __("method.remove")
        elif isinstance(method, Custom):
            name = __("method.custom")
        else:
            assert False
...
class OWDiscretize(widget.OWWidget):
    ...
    class Inputs:
        data = Input("Data", Orange.data.Table, label=i18n.t("widget.data.data.common.data"), doc=i18n.t("widget.data.data.common.input_data_table"))

    class Outputs:
        data = Output("Data", Orange.data.Table,label=i18n.t("widget.data.data.common.data"), doc=i18n.t("widget.data.data.common.discretized_table"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, self.tr(__("box_discretization")))
        ...
        self.options = [
            (Methods.Default, self.tr(__("option.default"))),
            (Methods.Leave, self.tr(__("option.leave"))),
            (Methods.MDL, self.tr(__("option.mdl"))),
            (Methods.EqualFreq, self.tr(__("option.equal_freq"))),
            (Methods.EqualWidth, self.tr(__("option.equal_width"))),
            (Methods.Remove, self.tr(__("option.remove"))),
            (Methods.Custom, self.tr(__("option.custom"))),
        ]
        ...
        def _intbox(parent, attr, callback):
            ...
            s = gui.spin(
                box, self, attr, minv=2, maxv=10, label=__("option.num_interval"),
                callback=callback)
        ...
        vlayout = QHBoxLayout()
        box = gui.widgetBox(
            self.controlArea, __("box_individual_attribute_setting"),
        ...
        gui.appendRadioButton(controlbox, __("option.remove_attr"), id=Methods.Remove)
        b = gui.appendRadioButton(controlbox, __("option.custom"), id=Methods.Custom)
        ...
        self.copy_current_to_manual_button = b = FixedSizeButton(
            text=__("btn.cc"), toolTip=__('btn.cc_tip'),
            enabled=False
        )
    ...
    def send_report(self):
        self.report_items((
            (__("report.default_method"), self.options[self.default_method][1]),))
        if self.varmodel:
            self.report_items(__("report.threshold"), [
                (var.name,

...
def __(key):
    return i18n.t("widget.data.data.owsave." + key)
...
class OWSave(OWSaveBase):
    ...
    class Error(OWSaveBase.Error):
        unsupported_sparse = widget.Msg(__("msg_pickle_for_sparse_data"))
    ...
    def __init__(self):
        ...
        self.grid.addWidget(
            gui.checkBox(
                ...
                __("checkbox_header_add_annotation"),
                tooltip=
                __("tooltip_format_information"),
    ...
    def send_report(self):
        ...
        writer = self.writer
        noyes = [__("report.cancel"), __("report.ok")]
        self.report_items((
            (__("report.file_name"), self.filename or __("report.not_set")),
            (__("report.format"), writer.DESCRIPTION),
            (__("report.type_annotation"),

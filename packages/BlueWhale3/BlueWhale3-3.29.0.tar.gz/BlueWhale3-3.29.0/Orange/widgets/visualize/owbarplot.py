...
def __(key):
    return i18n.t("widget.visualize.visualize.owbarplot." + key)
...
class OWBarPlot(OWWidget):
    ...
    class Information(OWWidget.Information):
        too_many_instances = Msg(__("msg.too_many_instances").format(MAX_INSTANCES))

    enumeration = __("row.enumeration")
    ...
    def _add_controls(self):
        ...
        gui.comboBox(
            box, self, "selected_var", label=__("label.values"),
        ...
        self._group_var_model = DomainModel(
            placeholder=i18n.t("common.software.none"), valid_types=DiscreteVariable
        )
        gui.comboBox(
            box, self, "group_var", label=__("label.group_by"),
        ...
        self._annot_var_model = DomainModel(
            placeholder=i18n.t("common.software.none"),
        ...
        gui.comboBox(
            box, self, "annot_var", label=__("label.annotations"),
            model=self._annot_var_model, contentsLength=12, searchable=True,
            orientation=Qt.Horizontal, callback=self.__parameter_changed,
        )

        self._color_var_model = DomainModel(
            placeholder=__("row.same_color"), valid_types=DiscreteVariable
        )
        gui.comboBox(
            box, self, "color_var", label=__("label.color"),

...
def __(key):
    return i18n.t("widget.visualize.visualize.owlinearprojection." + key)
...
class LinearProjectionVizRank(VizRankDialog, OWComponent):
    ...
    def __init__(self, master):
        ...
        self.n_attrs_spin = gui.spin(
            box, self, "n_attrs", 3, max_n_attrs, label=__("label_number_variables"),
...
class OWLinearProjection(OWAnchorProjectionWidget):
    ...
    Projection_name = {Placement.Circular: __("btn.circular_placement"),
                       Placement.LDA: __("btn.linear_discriminant_analysis"),
                       Placement.PCA: __("btn.principal_component_analysis")}
    ...
    class Error(OWAnchorProjectionWidget.Error):
        no_cont_features = Msg(__("msg.plot_requirement"))
    ...
    def _add_controls_variables(self):
        ...
        self.vizrank, self.btn_vizrank = LinearProjectionVizRank.add_vizrank(
            None, self, __("btn.suggest_features"), self.__vizrank_set_attrs)

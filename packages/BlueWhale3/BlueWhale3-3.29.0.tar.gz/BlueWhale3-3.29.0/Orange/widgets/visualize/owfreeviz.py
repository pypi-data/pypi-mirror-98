...
def __(key):
    return i18n.t("widget.visualize.visualize.owfreeviz." + key)
...
class InitType(IntEnum):
    ...
    @staticmethod
    def items():
        return [__("gbox_circular"), __("gbox_random")]
...
class OWFreeViz(OWAnchorProjectionWidget, ConcurrentWidgetMixin):
    ...
    class Error(OWAnchorProjectionWidget.Error):
        no_class_var = widget.Msg(__("msg.no_target_variable"))
        multiple_class_vars = widget.Msg(__("msg.multiple_class_vars"))
        not_enough_class_vars = widget.Msg(__("msg.binary"))
        features_exceeds_instances = widget.Msg(__("msg.number_compare"))
        too_many_data_instances = widget.Msg(__("msg.data_large"))
        constant_data = widget.Msg(__("msg.data_columns_constant"))
        not_enough_features = widget.Msg(__("msg.feature_required"))

    class Warning(OWAnchorProjectionWidget.Warning):
        removed_features = widget.Msg(__("msg.value_number"))
    ...
    def _add_controls(self):
        ...
        self.gui.add_control(
            self._effects_box, gui.hSlider, __("row_hide_radius"), master=self.graph,
    ...
    def __add_controls_start_box(self):
        ...
        gui.comboBox(
            box, self, "initialization", label=__("gbox_initialization"),

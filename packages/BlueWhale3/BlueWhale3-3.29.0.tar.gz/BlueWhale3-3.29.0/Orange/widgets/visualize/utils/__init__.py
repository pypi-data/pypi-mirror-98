...
def __(key):
    return i18n.t("widget.visualize.visualize.utils_tree.init." + key)
...
class VizRankDialog(QDialog, ProgressBarMixin, WidgetMessagesMixin,
    ...
    class Information(WidgetMessagesMixin.Information):
        nothing_to_rank = Msg(__("nothing_rank"))
    ...
    def __init__(self, master):
        ...
        self.filter.setPlaceholderText(__("placeholder_filter"))
        ...
        self.button = gui.button(
            self, self, __("btn.start"), callback=self.toggle, default=True)
    ...
    def on_done(self, result: Result):
        self.button.setText(__("btn.finish"))
    ...
    def toggle(self):
        ...
        if self.keep_running:
            self.button.setText(__("btn.pause"))
            self.button.repaint()
...
def run_vizrank(compute_score: Callable, iterate_states: Callable,
    ...
    task.set_status(__("status.get_combination"))
    task.set_progress_value(0.1)
    states = iterate_states(saved_state)

    task.set_status(__("status.get_score"))

...
def __(key):
    return i18n.t("widget.visualize.visualize.utils_tree.widget." + key)
...
class OWProjectionWidgetBase(OWWidget, openclass=True):
    ...
    class Information(OWWidget.Information):
        missing_size = Msg(
            __("undefined_point_show_smaller_size"))
        missing_shape = Msg(
            __("undefined_point_show_crossed_circle"))
...
class OWDataProjectionWidget(OWProjectionWidgetBase, openclass=True):
    ...
    class Warning(OWProjectionWidgetBase.Warning):
        too_many_labels = Msg(
            __("msg.show_too_many_labels"))
        subset_not_subset = Msg(
            __("msg.instance_error"))
        subset_independent = Msg(__("msg.no_subset_data"))
        transparent_subset = Msg(__("msg.transparent_subset"))
...
class OWAnchorProjectionWidget(OWDataProjectionWidget, openclass=True):
    ...
    class Outputs(OWDataProjectionWidget.Outputs):
        components = Output(i18n.t("common.general.components"), Table)

    class Error(OWDataProjectionWidget.Error):
        sparse_data = Msg(__("msg.unsupported_sparse_data"))
        no_valid_data = Msg(__("msg.data_error"))
        no_instances = Msg(__("msg.require_data_instance"))
        proj_error = Msg(__("msg.projecting_data_error"))

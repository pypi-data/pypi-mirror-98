...
def __(key):
    return i18n.t("widget.visualize.visualize.ownomogram." + key)
...
class SortBy(IntEnum):
    ...
    @staticmethod
    def items():
        return [__("gbox.no_sorting"), __("gbox.name"), __("gbox.absolute_importance"),
                __("gbox.positive_influence"), __("gbox.negative_influence")]
...
class ProbabilitiesDotItem(DotItem):
    ...
    def get_tooltip_text(self):
        text = __("tooltip_total_probability").format(
...
class DiscreteMovableDotItem(MovableDotItem):
    def get_tooltip_text(self):
        ...
        return self.TOOLTIP_TEMPLATE.format(
            self.TOOLTIP_STYLE, __("tooltip_point").format(np.round(self.value, 2)),
...
class ProbabilitiesRulerItem(QGraphicsWidget):
    ...
    def __init__(self, name, values, scale, name_offset, offset, get_points,
        ...
        font = name.document().defaultFont()
        font.setWeight(QFont.Bold)
        name_total = QGraphicsTextItem(__("item.total"), self)
...
class OWNomogram(OWWidget):
    ...
    class Inputs:
        classifier = Input('Classifier', Model, label=i18n.t("common.general.classifier"))
        data = Input('Data', Table, label=i18n.t("widget.visualize.visualize.common.data"))

    class Outputs:
        features = Output('Features', AttributeList, label=i18n.t("common.general.features"))
    ...
    class Error(OWWidget.Error):
        invalid_classifier = Msg(__("msg_accept_nomogram"))
    ...
    def __init__(self):
        ...
        # GUI
        box = gui.vBox(self.controlArea, i18n.t("common.general.target_class"))
        ...
        self.norm_check = gui.checkBox(
            box, self, "normalize_probabilities", __("checkbox.normalize_probability"),
            hidden=True, callback=self.update_scene,
            tooltip=__("checkbox.normalize_probability_tip"))

        self.scale_radio = gui.radioButtons(
            self.controlArea, self, "scale", [__("gbox.point_scale"), __("gbox.log_odds_ratio")],
            box=__("box.scale"), callback=self.update_scene)

        box = gui.vBox(self.controlArea, __("box.display_feature"))
        grid = QGridLayout()
        ...
        radio_all = gui.appendRadioButton(
            radio_group, __("btn.all"), addToLayout=False)
        radio_best = gui.appendRadioButton(
            radio_group, __("btn.best_ranked"), addToLayout=False)
        ...
        self.sort_combo = gui.comboBox(
            box, self, "sort_index", label=__("label_rank_by"), items=SortBy.items(),
            orientation=Qt.Horizontal, callback=self.update_scene)

        self.cont_feature_dim_combo = gui.comboBox(
            box, self, "cont_feature_dim_index", label=__("label_numeric_feature"),
            items=[__("gbox.1D_projection"), __("gbox.2D_curve")], orientation=Qt.Horizontal,
            callback=self.update_scene)
    ...
    def update_scene(self):
        ...
        point_text = QGraphicsTextItem(__("item.points"))
        probs_text = QGraphicsTextItem(__("item.probability"))

...
def __(key):
    return i18n.t("widget.visualize.visualize.owtreeviewer2d." + key)
...
class OWTreeViewer2D(OWWidget, openclass=True):
    ...
    def __init__(self):
        ...
        box = gui.vBox(
            self.controlArea, __('box.tree'),
            sizePolicy=QSizePolicy(QSizePolicy.Minimum, QSizePolicy.Fixed))
        self.infolabel = gui.widgetLabel(box, __('label_no_tree'))

        layout = QFormLayout()
        layout.setFieldGrowthPolicy(layout.ExpandingFieldsGrow)
        box = self.display_box = gui.widgetBox(self.controlArea, __("box.display"),
                                               orientation=layout)
        layout.addRow(
            __("row.zoom"),
        ...
        layout.addRow(
            ...
            gui.comboBox(box, self, 'max_tree_depth',
                         items=[__("row.unlimited")] + [
                             __("row.levels").format(x) for x in range(2, 10)],
        ...
        layout.addRow(
            ...
            gui.comboBox(box, self, 'line_width_method',
                         items=[__('row.fix'), __('row.relative_root'),
                                __('row.relative_parent')],
                         addToLayout=False,

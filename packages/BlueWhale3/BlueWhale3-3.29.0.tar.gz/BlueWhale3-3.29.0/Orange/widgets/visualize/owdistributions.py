...
def __(key):
    return i18n.t("widget.visualize.visualize.owdistributions." + key)
...
class OWDistributions(OWWidget):
    ...
    class Inputs:
        data = Input('Data', Table, doc="Set the input dataset", label=i18n.t("widget.visualize.visualize.common.data"))
    ...
    class Outputs:
        ...
        histogram_data = Output('Histogram Data', Table,
                                label=i18n.t("widget.visualize.visualize.common.histogram_data"))

    class Error(OWWidget.Error):
        no_defined_values_var = \
            Msg(__("msg.undefined_values"))
        no_defined_values_pair = \
            Msg(__("msg.undefined_data_instances"))

    class Warning(OWWidget.Warning):
        ignored_nans = Msg(__("msg.missing_values"))
    ...
    Fitters = (
        ...
        (i18n.t("common.software.normal"), norm, ("loc", "scale"), ("μ", "σ")),
        (__("gbox.beta"), beta, ("a", "b", "loc", "scale"),
         ("α", "β", "-loc", "-scale")),
        (__("gbox.gamma"), gamma, ("a", "loc", "scale"), ("α", "β", "-loc", "-scale")),
        (__("gbox.rayleigh"), rayleigh, ("loc", "scale"), ("-loc", "σ")),
        (__("gbox.pareto"), pareto, ("b", "loc", "scale"), ("α", "-loc", "-scale")),
        (__("gbox.exponential"), expon, ("loc", "scale"), ("-loc", "λ")),
        (__("gbox.kernel_density"), AshCurve, ("a",), ("",))
    )
    ...
    def __init__(self):
        ...
        varview = gui.listView(
            self.controlArea, self, "var", box=__("box_variable"),
        ...
        gui.checkBox(
            varview.box, self, "sort_by_freq", __("checkbox.sort_by_freq"),
            callback=self._on_sort_by_freq, stateWhenDisabled=False)

        box = self.continuous_box = gui.vBox(self.controlArea, i18n.t("common.general.distribution"))
        gui.comboBox(
            box, self, "fitted_distribution", label=__("label.fitted_distribution"),
        ...
        slider = gui.hSlider(
            ...
            label=__("row_bin_width"), orientation=Qt.Horizontal,
        ...
        self.smoothing_box = gui.hSlider(
            ...
            label=__("label.smoothing"), orientation=Qt.Horizontal,
            minValue=2, maxValue=20, callback=self.replot, disabled=True)
        gui.checkBox(
            box, self, "hide_bars", __("checkbox.hide_bars"), stateWhenDisabled=False,
            callback=self._on_hide_bars_changed,
            disabled=not self.fitted_distribution)

        box = gui.vBox(self.controlArea, i18n.t("common.general.column"))
        gui.comboBox(
            box, self, "cvar", label=__("label.split_by"), orientation=Qt.Horizontal,
            searchable=True,
            model=DomainModel(placeholder=i18n.t("common.software.brackets_none"),
                              valid_types=(DiscreteVariable), ),
            callback=self._on_cvar_changed, contentsLength=18)
        gui.checkBox(
            box, self, "stacked_columns", __("checkbox.stack_columns"),
            callback=self.replot)
        gui.checkBox(
            box, self, "show_probs", __("checkbox.probabilities"),
            callback=self._on_show_probabilities_changed)
        gui.checkBox(
            box, self, "cumulative_distr", __("checkbox.cumulative_distribution"),
    ...
    def _on_show_probabilities_changed(self):
        ...
        if self.show_probs:
            label.setText(__("label.fitted_probability"))
            label.setToolTip(
                __("label.compute_bayesian_probabilities"))
        else:
            label.setText(__("label.fitted_distribution"))
            label.setToolTip("")
        self.replot()
    ...
    def _set_axis_names(self):
        ...
        if self.show_probs and self.cvar:
            leftaxis.setLabel(__("label.get_probability").format(self.cvar.name, self.var.name))
        else:
            leftaxis.setLabel(__("label.frequency"))

...
def __(key):
    return i18n.t("widget.utils.utils.colorgradientselection." + key)
...
class ColorGradientSelection(QWidget):
    ...
    def __init__(self, *args, thresholds=(0.0, 1.0), center=None, **kwargs):
        ...
        if center is not None:
            ...
            center_layout.addWidget(QLabel(__("label.centered_at")))
        ...
        slider_low = QSlider(
            ...
            toolTip=self.tr(__("slider.low")),
            whatsThis=self.tr(__("slider.low_tip"))
        )
        slider_high = QSlider(
            objectName="threshold-low-slider", minimum=0, maximum=100,
            value=int(high * 100), orientation=Qt.Horizontal,
            tickPosition=QSlider.TicksAbove, pageStep=10,
            toolTip=self.tr(__("slider.high")),
            whatsThis=self.tr(__("slider.high_tip"))
        )
        form.setWidget(0, QFormLayout.SpanningRole, self.gradient_cb)
        if self.center_box:
            form.setWidget(1, QFormLayout.SpanningRole, self.center_box)
        form.addRow(self.tr(__("row.low")), slider_low)
        form.addRow(self.tr(__("row.high")), slider_high)

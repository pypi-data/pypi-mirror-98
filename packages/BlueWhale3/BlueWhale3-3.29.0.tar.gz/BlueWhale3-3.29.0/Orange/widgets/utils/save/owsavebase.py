...
def __(key):
    return i18n.t("widget.utils.utils.owsavebase." + key)
...
class OWSaveBase(widget.OWWidget, openclass=True):
    ...
    class Information(widget.OWWidget.Information):
        empty_input = widget.Msg(__("msg.input_empty"))

    class Error(widget.OWWidget.Error):
        no_file_name = widget.Msg(__("msg.ignore_file_name"))
    ...
    def __init__(self, start_row=0):
        ...
        grid.addWidget(
            gui.checkBox(
                None, self, "auto_save", __("checkbox_autosave_receive_new_data"),
                callback=self.update_messages),
            start_row, 0, 1, 2)
        self.bt_save = gui.button(
            self.buttonsArea, self,
            label=__("btn.something_save_as").format(self.stored_name) if self.stored_name else __("btn.save"),
            callback=self.save_file)
        gui.button(self.buttonsArea, self, __("btn.save_as"), callback=self.save_file_as)
    ...
    def save_file_as(self):
        ...
        self.bt_save.setText(__("text.save_as").format(self.stored_name))
        self.update_messages()
        self._try_save()
    ...
    if sys.platform in ("darwin", "win32"):
        ...
        def get_save_filename(self):  # pragma: no cover
            ...
            while True:
                dlg = QFileDialog(
                    None, __("file_dialog_save_file"), filename, ";;".join(no_ext_filters))
                ...
                if not os.path.exists(filename) or QMessageBox.question(
                        self, __("msg.overwrite_file"),
                __("msg.overwrite_file_content").format(os.path.split(filename)[1])) == QMessageBox.Yes:
        ...
        def get_save_filename(self):
            dlg = self.SaveFileDialog(
                type(self),
                None, __("save_file_dialog"), self.initial_start_dir(),

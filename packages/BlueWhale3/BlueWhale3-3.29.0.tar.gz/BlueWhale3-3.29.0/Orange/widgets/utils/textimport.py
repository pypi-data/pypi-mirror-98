...
def __(key):
    return i18n.t("widget.utils.utils.textimport." + key)
...
class CSVOptionsWidget(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.encoding_cb = QComboBox(
            ...
            toolTip=__("tip.select_encoding_format"),
        )
        ...
        self.delimiter_cb = QComboBox(
            ...
            toolTip=__("tip.select_cell_delimiter")
        )
        ...
        self.delimiter_cb.addItem(__("gbox.other"))
        ...
        form.addRow(__("row.encoding"), self.encoding_cb)
        form.addRow(QFrame(self, frameShape=QFrame.HLine))
        form.addRow(__("row.cell_delimiter"), delimlayout)
        form.addRow(__("row.quote_character"), self.quoteedit)
        self.setLayout(form)
    ...
    def __show_encodings_widget(self):
        ...
        w = encodings.SelectEncodingsWidget(
            self, Qt.Tool,
            windowTitle=__("window_title.customize_encodings_list"),
    ...
    def __set_visible_codecs(self, codecs):
        ...
        cb.addItem(__("gbox.customize_encodings_list"), userData=...)
...
class CSVImportWidget(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.grouping_sep_edit_cb = TextEditCombo(
            ...
            toolTip=__("tip.thousands_group_separator"),
            minimumContentsLength=1,
            sizeAdjustPolicy=QComboBox.AdjustToMinimumContentsLength
        )
        items = [
            {Qt.DisplayRole: i18n.t("common.software.none"), Qt.EditRole: "",
             Qt.ToolTipRole: __("gbox.no_separator")},
            {Qt.DisplayRole: ".", Qt.EditRole: "."},
            {Qt.DisplayRole: ",", Qt.EditRole: ","},
            {Qt.DisplayRole: __("gbox.space"), Qt.EditRole: " "},
            {Qt.DisplayRole: "'", Qt.EditRole: "'"},
        ]
        ...
        number_sep_layout.addWidget(QLabel(__("row.group")))
        number_sep_layout.addWidget(self.grouping_sep_edit_cb)
        number_sep_layout.addWidget(QLabel(__("row.decimal")))
        number_sep_layout.addWidget(self.decimal_sep_edit_cb)
        number_sep_layout.addStretch(10)
        form.addRow(__("row.number_separators"), number_sep_layout)
        ...
        types = [
            {Qt.DisplayRole: __("gbox.auto"),
             Qt.ToolTipRole: __("gbox.automatic_determine_type"),
             Qt.UserRole: ColumnType.Auto},
            {Qt.DisplayRole: __("gbox.numeric"), Qt.UserRole: ColumnType.Numeric},
            {Qt.DisplayRole: __("gbox.categorical"),
             Qt.UserRole: ColumnType.Categorical},
            {Qt.DisplayRole: i18n.t("common.software.text"), Qt.UserRole: ColumnType.Text},
            {Qt.DisplayRole: __("gbox.datetime"), Qt.UserRole: ColumnType.Time},
            {Qt.AccessibleDescriptionRole: "separator"},
            {Qt.DisplayRole: __("gbox.ignore"),
             Qt.UserRole: ColumnType.Skip,
             Qt.ToolTipRole: __("gbox.column_not_load")}
        ]
        ...
        form.addRow(__("row.column_type"), self.column_type_edit_cb)
...
def format_exception_csv(err):
    return __("tip.csv_parse_error") + str(err)

...
def __(key):
    return i18n.t("widget.utils.utils.owbasesql." + key)
...
class OWBaseSql(OWWidget, openclass=True):
    ...
    """
    class Outputs:
        data = Output(i18n.t("widget.utils.utils.common.data"), Table)
    ...
    def _setup_gui(self):
        ...
        vbox = gui.vBox(self.controlArea, __("box.server"))
        self.serverbox = gui.vBox(vbox)
        self.servertext = QLineEdit(self.serverbox)
        self.servertext.setPlaceholderText(__("placeholder.server"))
        self.servertext.setToolTip(__("box.server_tip"))
        ...
        self.databasetext.setPlaceholderText(__("placeholder.database"))
        self.databasetext.setToolTip(__("placeholder.database_tip"))
        if self.database:
        ...
        self.usernametext.setPlaceholderText(__("placeholder.username"))
        self.usernametext.setToolTip(__("placeholder.username_tip"))

        self.serverbox.layout().addWidget(self.usernametext)
        self.passwordtext = QLineEdit(self.serverbox)
        self.passwordtext.setPlaceholderText(__("placeholder.password"))
        self.passwordtext.setToolTip(__("placeholder.password_tip"))
        ...
        self.connectbutton = gui.button(self.serverbox, self, __("btn_connect"),
    ...
    def send_report(self):
        if not self.database_desc:
            self.report_paragraph(__("report.no_db_connect"))
            return
        self.report_items(__("report.database"), self.database_desc)

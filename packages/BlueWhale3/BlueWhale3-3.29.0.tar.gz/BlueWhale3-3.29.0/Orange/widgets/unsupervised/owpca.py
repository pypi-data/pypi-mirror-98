...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owpca." + key)
...
class OWPCA(widget.OWWidget):
    ...
    class Outputs:
        ...
        data = Output('Data', Table, default=True, label=i18n.t("widget.unsupervised.unsupervised.common.data"))
        components = Output('Components', Table, label=i18n.t("common.general.components"))
        pca = Output('PCA', PCA, dynamic=False, label=i18n.t("common.algorithm.PCA"))
    ...
    class Warning(widget.OWWidget.Warning):
        trivial_components = widget.Msg(__("msg.component_trivial"))

    class Error(widget.OWWidget.Error):
        no_features = widget.Msg(__("msg.one_feature"))
        no_instances = widget.Msg(__("msg.one_data_instance"))
    ...
    def __init__(self):
        ...
        form = QFormLayout()
        box = gui.widgetBox(self.controlArea, __("box.components_selection"),
                            orientation=form)
        ...
        form.addRow(__("row.components"), self.components_spin)
        form.addRow(__("row.explained_variance"), self.variance_spin)

        # Options
        self.options_box = gui.vBox(self.controlArea, __("box.options"))
        self.normalize_box = gui.checkBox(
            self.options_box, self, "normalize",
            __("checkbox_normalize_variables"), callback=self._update_normalize,
            attribute=Qt.WA_LayoutUsesWidgetRect
        )

        self.maxp_spin = gui.spin(
            self.options_box, self, "maxp", 1, MAX_COMPONENTS,
            label=__("row.show_first"), callback=self._setup_plot,
        ...
        self.plot = SliderGraph(
            __("row.principal_components"), __("row.proportion_of_variance"),
    ...
    def set_data(self, data):
        ...
        if isinstance(data, SqlTable):
            if data.approx_len() < AUTO_DL_LIMIT:
                ...
                self.information(__("msg.data_sampled"))

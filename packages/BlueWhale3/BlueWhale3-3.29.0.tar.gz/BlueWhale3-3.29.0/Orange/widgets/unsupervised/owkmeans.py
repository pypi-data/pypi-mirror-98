...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owkmeans." + key)
...
class OWKMeans(widget.OWWidget):
    ...
    class Outputs:
        annotated_data = Output(
            ...
            replaces=["Annotated Data"], label=i18n.t("widget.data.data.common.data")
        )
        centroids = Output("Centroids", Table, label=i18n.t("widget.unsupervised.unsupervised.common.centroids"))

    class Error(widget.OWWidget.Error):
        failed = widget.Msg(__("msg.clustering_failed"))
        not_enough_data = widget.Msg(
            __("msg.unique_data_little")
        )
        no_attributes = widget.Msg(__("msg.missing_features"))

    class Warning(widget.OWWidget.Warning):
        no_silhouettes = widget.Msg(
            __("msg.ignore_silhouette_scores").format(
    ...
    def __init__(self):
        ...
        bg = gui.radioButtonsInBox(
            ...
            box=__("checkbox.quantity_clusters"), callback=self.update_method,
        )

        layout.addWidget(
            gui.appendRadioButton(bg, __("btn.fixed"), addToLayout=False), 1, 1)
        ...
        layout.addWidget(
            gui.appendRadioButton(bg, __("btn.from"), addToLayout=False), 2, 1)
        ...
        gui.widgetLabel(ftobox, __("btn.to"))
        gui.spin(
        ...
        box = gui.vBox(self.controlArea, __("box.preprocessing"))
        gui.checkBox(box, self, "normalize", __("checkbox.normalize_columns"),
                     callback=self.invalidate)

        box = gui.vBox(self.controlArea, __("box.initialization"))
        gui.comboBox(
            box, self, "smart_init", items=[__("gbox.initialize_KMeans"), __("gbox.random_initialization")],
            callback=self.invalidate)

        layout = QGridLayout()
        gui.widgetBox(box, orientation=layout)
        layout.addWidget(gui.widgetLabel(None, __("label_rerun")), 0, 0, Qt.AlignLeft)
        sb = gui.hBox(None, margin=0)
        ...
        layout.addWidget(
            gui.widgetLabel(None, __("label_maximum_iterations")), 1, 0, Qt.AlignLeft)
        sb = gui.hBox(None, margin=0)
        ...
        box = gui.vBox(self.mainArea, box=__("box.silhouette_scores"))
        if self.optimize_k:

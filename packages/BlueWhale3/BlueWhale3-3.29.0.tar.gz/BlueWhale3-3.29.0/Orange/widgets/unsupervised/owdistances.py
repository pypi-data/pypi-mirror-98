...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owdistances." + key)


METRICS = [
    (i18n.t("common.algorithm.euclidean"), distance.Euclidean),
    (i18n.t("common.algorithm.manhattan"), distance.Manhattan),
    (i18n.t("common.algorithm.cosine"), distance.Cosine),
    (i18n.t("common.algorithm.jaccard"), distance.Jaccard),
    (i18n.t("common.algorithm.spearman"), distance.SpearmanR),
    (i18n.t("common.algorithm.absolute_spearman"), distance.SpearmanRAbsolute),
    (i18n.t("common.algorithm.pearson"), distance.PearsonR),
    (i18n.t("common.algorithm.absolute_pearson"), distance.PearsonRAbsolute),
    (i18n.t("common.algorithm.hamming"), distance.Hamming),
    (i18n.t("common.algorithm.mahalanobis"), distance.Mahalanobis),
    (i18n.t("common.algorithm.bhattacharyya"), distance.Bhattacharyya)
]
...
class DistanceRunner:
    ...
    def run(data: Orange.data.Table, metric: distance, normalized_dist: bool,
        ...
        state.set_status(__("set_status_calculating"))
...
class OWDistances(OWWidget, ConcurrentWidgetMixin):
    ...
    class Inputs:
        data = Input('Data', Orange.data.Table, label=i18n.t("widget.unsupervised.unsupervised.common.data"))

    class Outputs:
        distances = Output('Distances', Orange.misc.DistMatrix, dynamic=False,
                           label=i18n.t("common.general.distance"))
    ...
    class Error(OWWidget.Error):
        no_continuous_features = Msg(__("msg.no_numeric"))
        no_binary_features = Msg(__("msg.no_binary"))
        dense_metric_sparse_data = Msg(__("msg.dense_data"))
        distances_memory_error = Msg(i18n.t("common.software.not_enough_memory"))
        distances_value_error = Msg(__("msg.problem"))
        data_too_large_for_mahalanobis = Msg(__("msg.upto"))

    class Warning(OWWidget.Warning):
        ignoring_discrete = Msg(__("msg.ignoring_categorical"))
        ignoring_nonbinary = Msg(__("msg.ignoring_non-binary"))
        imputing_data = Msg(__("msg.miss_value"))
    ...
    def __init__(self):
        ...
        gui.radioButtons(
            self.controlArea, self, "axis", [i18n.t("common.general.row"), i18n.t("common.general.column")],
            box=__("box_distances_between"), callback=self._invalidate
        )
        box = gui.widgetBox(self.controlArea, __("box_distance_metric"))
        ...
        self.normalization_check = gui.checkBox(
            box, self, "normalized_dist", __("row.normalized"),
            callback=self._invalidate,
            tooltip=(__("tooltip_influence_domain")),
    ...
    def send_report(self):
        # pylint: disable=invalid-sequence-index
        self.report_items((
            (__("row.distances_between"), [i18n.t("common.general.row"), __("row.columns")][self.axis]),
            (i18n.t("common.general.metric"), METRICS[self.metric_idx][0])
        ))

...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owlouvainclustering." + key)
...
METRICS = [(i18n.t("common.algorithm.euclidean"), "l2"), (i18n.t("common.algorithm.manhattan"), "l1"),
           (i18n.t("common.algorithm.cosine"), "cosine")]
...
class OWLouvainClustering(widget.OWWidget):
    ...
    class Inputs:
        data = Input('Data', Table, default=True, label=i18n.t("widget.unsupervised.unsupervised.common.data"))

    class Outputs:
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table, default=True, label=i18n.t("widget.data.data.common.data"))
        if Network is not None:
            graph = Output('Network', Network, label=i18n.t("widget.unsupervised.unsupervised.common.network"))
    ...
    class Information(widget.OWWidget.Information):
        modified = Msg(__("msg_send_new_data"))

    class Error(widget.OWWidget.Error):
        empty_dataset = Msg(__("msg_no_features"))
    ...
    def __init__(self):
        ...
        # Set up UI
        info_box = gui.vBox(self.controlArea, i18n.t("common.software.info"))
        self.info_label = gui.widgetLabel(info_box, i18n.t("common.software.no_data_on_input"))  # type: QLabel
        self.info.set_input_summary(self.info.NoInput)
        self.info.set_output_summary(self.info.NoOutput)

        preprocessing_box = gui.vBox(self.controlArea, __("box_preprocessing"))
        self.normalize_cbx = gui.checkBox(
            preprocessing_box, self, "normalize", label=__("checkbox_normalize_data"),
            callback=self._invalidate_preprocessed_data, attribute=Qt.WA_LayoutUsesWidgetRect
        )  # type: QCheckBox
        self.apply_pca_cbx = gui.checkBox(
            preprocessing_box, self, "apply_pca", label=__("checkbox_apply"),
            callback=self._apply_pca_changed, attribute=Qt.WA_LayoutUsesWidgetRect
        )  # type: QCheckBox
        self.pca_components_slider = gui.hSlider(
            preprocessing_box, self, "pca_components", label=__("row_PCA_components"), minValue=2,
            maxValue=_MAX_PCA_COMPONENTS,
            callback=self._invalidate_pca_projection, tracking=False
        )  # type: QSlider

        graph_box = gui.vBox(self.controlArea, __("box_graph_parameters"))
        self.metric_combo = gui.comboBox(
            graph_box, self, "metric_idx", label=__("gbox.distance_metric"),
    ...
    def commit(self):
        ...
        self.info_label.setText(__("label_text.running"))
    ...
    def __set_results(self, results):
        ...
        self.info_label.setText(__("label_text.found_clusters") % num_clusters)
    ...
    def set_data(self, data):
        ...
        self.info_label.setText(__("label_text.clusters_not_run"))
    ...
    def clear(self):
        ...
        self.info_label.setText(i18n.t("common.software.no_data_on_input"))
...
def run_on_data(data, normalize, pca_components, k_neighbors, metric, resolution, state):
    ...
    if pca_components is not None:
        steps = 3
        state.set_status(__("set_status.compute_PCA"))
    ...
    state.set_status(__("set_status.building_graph"))
    ...
    step += 1
    state.set_progress_value(100 * step / steps)
    state.set_status(__("set_status.detecting_communities"))
    if state.is_interuption_requested():
        return res

...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owcorrespondence." + key)
...
class OWCorrespondenceAnalysis(widget.OWWidget):
    ...
    class Outputs:
        coordinates = Output('Coordinates', Table, label=i18n.t("widget.unsupervised.unsupervised.common.coordinates"))
    ...
    class Error(widget.OWWidget.Error):
        empty_data = widget.Msg(__("msg_empty_data"))
        no_disc_vars = widget.Msg(__("msg_no_categorical_data"))
    ...
    def __init__(self):
        ...
        axes_box = gui.vBox(self.controlArea, __("box_axes"))
        self.axis_x_cb = gui.comboBox(
            axes_box, self, "component_x", label=__("box_x"),
        ...
        self.axis_y_cb = gui.comboBox(
            axes_box, self, "component_y", label=__("box_y"),
            callback=self._component_changed, orientation=Qt.Horizontal,
            sizePolicy=(QSizePolicy.MinimumExpanding,
                        QSizePolicy.Preferred)
        )
        ...
        self.infotext = gui.widgetLabel(
            gui.vBox(self.controlArea, __("label_inertia")), "\n"
        )

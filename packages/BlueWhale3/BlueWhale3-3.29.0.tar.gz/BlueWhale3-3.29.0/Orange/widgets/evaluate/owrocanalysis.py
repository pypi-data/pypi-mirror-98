...
def __(key):
    return i18n.t("widget.evaluate.evaluate.owrocanalysis." + key)
...
class OWROCAnalysis(widget.OWWidget):
    ...
    class Inputs:
        evaluation_results = Input("Evaluation Results", Orange.evaluation.Results, label=i18n.t("widget.evaluate.evaluate.common.evaluation_result"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __("box.plot"))
        self.target_cb = gui.comboBox(
            box, self, "target_index",
            label=__("label_target"), orientation=Qt.Horizontal,
            callback=self._on_target_changed,
            contentsLength=8, searchable=True)

        gui.widgetLabel(box, __("box.classifier"))
        ...
        abox = gui.vBox(self.controlArea, __("box.curves"))
        gui.comboBox(abox, self, "roc_averaging",
                     items=[__("gbox.merge_prediction"), __("gbox.mean_tp_rate"),
                            __("gbox.threshold_tp_fp"), __("gbox.show_individual_curve")],
                     callback=self._replot)

        gui.checkBox(abox, self, "display_convex_curve",
                     __("checkbox.show_roc_curve"), callback=self._replot)
        gui.checkBox(abox, self, "display_convex_hull",
                     __("checkbox.show_roc_convex_hull"), callback=self._replot)

        box = gui.vBox(self.controlArea, __("box.analysis"))

        gui.checkBox(box, self, "display_def_threshold",
                     __("checkbox.default_threshold"),
                     callback=self._on_display_def_threshold_changed)

        gui.checkBox(box, self, "display_perf_line", __("checkbox.show_performance_line"),
        ...
        sp = gui.spin(box, self, "fp_cost", 1, 1000, 10,
                      ...
                      callback=self._on_display_perf_line_changed)
        grid.addWidget(QLabel(__("row_fp_cost")), 0, 0)
        grid.addWidget(sp, 0, 1)

        sp = gui.spin(box, self, "fn_cost", 1, 1000, 10,
                      alignment=Qt.AlignRight,
                      callback=self._on_display_perf_line_changed)
        grid.addWidget(QLabel(__("row_fn_cost")))
        ...
        grid.addWidget(QLabel(__('row_prior_probability')))
        ...
        axis.setPen(pen)
        axis.setLabel(__("label_fp_rate"))
        axis.setGrid(16)

        axis = self.plot.getAxis("left")
        axis.setTickFont(tickfont)
        axis.setPen(pen)
        axis.setLabel(__("label_tp_rate"))
        axis.setGrid(16)
    ...
    def _setup_plot(self):
        ...
        if not all(c.is_valid for c in hull_curves):
            if any(c.is_valid for c in hull_curves):
                warning = __("msg.some_roc_curves_undefined")
            else:
                warning = __("msg.all_roc_curves_undefined")
    ...
    def send_report(self):
        if self.results is None:
            return
        items = OrderedDict()
        items[i18n.t("common.general.target_class")] = self.target_cb.currentText()
        if self.display_perf_line:
            items[__("report.cost")] = \
                __("report.fp_fn").format(self.fp_cost, self.fn_cost)
            items[__("report.target_probability")] = "{} %".format(self.target_prior)

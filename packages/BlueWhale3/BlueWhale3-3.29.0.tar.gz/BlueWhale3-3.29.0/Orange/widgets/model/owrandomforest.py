...
def __(key):
    return i18n.t("widget.model.model.owrandomforest." + key)
...
class OWRandomForest(OWBaseLearner):
    ...
    class Error(OWBaseLearner.Error):
        not_enough_features = Msg(__("msg_attr_insufficient"))
    ...
    def add_main_layout(self):
        ...
        self.random_state = gui.checkBox(
            box, self, "use_random_state", label=i18n.t("common.general.replicable_train"),
            callback=self.settings_changed,
            attribute=Qt.WA_LayoutUsesWidgetRect)
        self.weights = gui.checkBox(
            box, self,
            "class_weight", label=__("checkbox.class_weight"),
            callback=self.settings_changed,
            tooltip=__("checkbox.class_weight_tip"),
            attribute=Qt.WA_LayoutUsesWidgetRect
        )

        box = gui.vBox(self.controlArea, __("box_growth_control"))
        self.max_depth_spin = gui.spin(
            box, self, "max_depth", 1, 50, controlWidth=80,
            label=__("checkbox.limit_tree_depth"), alignment=Qt.AlignRight,
        ...
        self.min_samples_split_spin = gui.spin(
            ...
            label=__("checkbox.not_split_subset_smaller"),
    ...
    def get_learner_parameters(self):
        ...
        return (
            (__("report.number_trees"), self.n_estimators),
            (__("report.maximal_number"),
             self.max_features if self.use_max_features else __("report.unlimited")),
            (i18n.t("common.general.replicable_train"), [__("report.cancel"), __("report.ok")][self.use_random_state]),
            (__("report.maximal_tree_depth"),
             self.max_depth if self.use_max_depth else __("report.unlimited")),
            (__("report.stop_split_node"),
             self.min_samples_split if self.use_min_samples_split else __("report.unlimited")),
            (__("report.class_weights"), self.class_weight)
        )

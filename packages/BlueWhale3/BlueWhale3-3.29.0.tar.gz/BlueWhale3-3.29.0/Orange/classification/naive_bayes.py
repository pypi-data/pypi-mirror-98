...
class NaiveBayesLearner(Learner):
    ...
    name = __('name.naive_bayes')

    def fit_storage(self, table):
        if not isinstance(table, Storage):
            raise TypeError(__("value_error.data_error"))
        if not all(var.is_discrete
                   for var in table.domain.variables):
            raise NotImplementedError(__('type_error.support_discrete_variable'))
        ...
        if not nclss:
            raise ValueError(__("value_error.undefined_value"))

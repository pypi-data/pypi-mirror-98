...
from orangecanvas.registry import qt, WidgetRegistry, set_global_registry, cache, set_style_sheet
...
def pull_notifications():
    ...
    class GetNotifFeed(QThread):
        ...
        def run(self):
            try:
                response = requests.get('https://bo.dashenglab.com/api/open/client/messages', timeout=5)
...
def main(argv=None):
    ...
    if stylesheet_string is not None:
        set_style_sheet(stylesheet_string)

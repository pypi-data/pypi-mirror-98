...
class SoftmaxRegressionLearner(Learner):
    ...
    def fit(self, X, Y, W=None):
        if len(Y.shape) > 1:
            raise ValueError(__('value_error.softmax_regression_unsupported_multi-label_classification'))

        if np.isnan(np.sum(X)) or np.isnan(np.sum(Y)):
            raise ValueError(__('value.error.softmax_regression_unsupported_unknown_value'))

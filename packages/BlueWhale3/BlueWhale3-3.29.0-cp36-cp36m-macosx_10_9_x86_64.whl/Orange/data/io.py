...
class BluewhaleReader(FileFormat, DataTableMixin):
    """Reader for pickled Table objects"""
    EXTENSIONS = ('.bluewhale')
    DESCRIPTION = 'Blue Whale data'
    SUPPORT_COMPRESSED = True
    SUPPORT_SPARSE_DATA = True

    def read(self, name, pre):
        try:
            con = sqlite3.connect(self.filename)
            c = con.cursor()
            c.execute('''SELECT * from '%s';''' % (name))
            local_table_data = c.fetchall()
            field_sql = "PRAGMA table_info('" + name + "')"
            c.execute(field_sql)
            field_sql_data = c.fetchall()
            strtext = []
            for q in range(len(local_table_data)):
                temp = {}
                for qq in range(len(field_sql_data)):
                    temp[field_sql_data[qq][1]] = local_table_data[q][qq]
                strtext.append(temp)
            strtext_json = json.dumps(strtext)
            df = pd.read_json(strtext_json, orient='records')
            if pre:
                desc_sql = '''SELECT Intro, Alias from bwds_meta WHERE Table_name='%s' and Type='TAB';''' % (name)
                desc = c.execute(desc_sql).fetchone()
                attributes = {'Name': desc[1], 'Description': desc[0]}
                table_field = '''SELECT Name, Alias from bwds_meta WHERE Table_name='%s' and Type='COL';''' % (name)
                field = c.execute(table_field).fetchall()
                df.rename(columns={name[0]: name[1] for name in field}, inplace=True)
                table = self.pandas_to_table(df, attr=attributes)
            else:
                table = self.pandas_to_table(df)
            ...
            raise KeyError('参数不正确')

    def pandas_to_table(self, df, attr=None):
        ...
        return Orange.data.Table.from_numpy(domain, X, None, M, attributes=attr)

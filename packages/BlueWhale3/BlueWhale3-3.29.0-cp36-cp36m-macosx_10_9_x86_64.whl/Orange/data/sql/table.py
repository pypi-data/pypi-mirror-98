...
class SqlTable(Table):
    ...
    def __init__(
        ...
        if backend is None:
            for backend in Backend.available_backends():
                ...
                raise ValueError(__("value_error.no_backend_connect_server"))
        else:
            self.backend = backend(connection_params)
    ...
    def connection_params(self):
        warnings.warn(__("tip.use_backend_connection_param"), DeprecationWarning)
    ...
    def __getitem__(self, key):
        ...
        if len(key) != 2:
            raise IndexError(__("value_error.table_index_dimension"))
        ...
        if isinstance(row_idx, int):
            ...
            #  - raise an exception
            raise NotImplementedError(__("value_error.row_index_integer"))
    ...
    def _fetch_row(self, row_index):
        ...
        if not values:
            raise IndexError(__('value_error.table_retrieve_error').format(
                row_index, self.name))
    ...
    def _query(self, attributes=None, filters=(), rows=None):
        if attributes is not None:
            ...
            if not fields:
                raise ValueError(__("value_error.missing_fields"))
        else:
            fields = ["*"]
    ...
    def download_data(self, limit=None, partial=False):
        ...
        if limit and not partial and self.approx_len() > limit:
            raise ValueError(__("value_error.many_row"))
    ...
    def _compute_contingency(self, col_vars=None, row_var=None):
        ...
        if len(col_vars) != 1:
            raise NotImplementedError(__("type_error.column_contingency_unimplemented"))
        if row_var is None:
            raise NotImplementedError(__("type_error.defaults_unsupported"))

        row = self.domain[row_var]
        if not row.is_discrete:
            raise TypeError(__("type_error.row_variable_discrete"))

        columns = [self.domain[var] for var in col_vars]

        if any(not (var.is_continuous or var.is_discrete)
               for var in columns):
            raise ValueError(__("value_error.data_unsupported_contingency"))
    ...
    def _filter_values(self, f):
        ...
        for cond in f.conditions:
            ...
            if isinstance(cond, filter.FilterDiscrete):
                ...
                raise ValueError(__('invalid_condition') % type(cond))
    ...
    def _sample(self, method, parameter, no_cache=False):
        ...
        if "," in self.table_name:
            raise NotImplementedError(__("tip.sample_complex_unsupport"))

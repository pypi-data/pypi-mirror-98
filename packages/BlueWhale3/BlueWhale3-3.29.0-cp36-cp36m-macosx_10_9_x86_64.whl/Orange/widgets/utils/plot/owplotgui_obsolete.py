...
def __(key):
    return i18n.t("widget.utils.utils.owplotgui_obs." + key)
...
class AddVariablesDialog(QDialog):
    def __init__(self, master, model):
        ...
        self.setWindowTitle(__("window_title_hidden_axes"))
        ...
        self.btn_add = QPushButton(
            __("btn.add"), autoDefault=False, sizePolicy=QSizePolicy(*SIZE_POLICY_FIXED)
        )
        self.btn_add.clicked.connect(self._add)
        self.btn_cancel = QPushButton(
            i18n.t("common.btn.cancel"), autoDefault=False, sizePolicy=QSizePolicy(*SIZE_POLICY_FIXED)
        )
...
class VariablesSelection:
    def __call__(self, master, model_selected, model_other):
        ...
        self.view_selected = view = gui.listView(widget=master.controlArea, master=master,
                                                 box=__("box.display_axes"), **params_view)
...
class OWPlotGUI:
    ...
    def __init__(self, plot):
        self._plot = plot
        self.color_model = DomainModel(placeholder=__("placeholder.same_color"),
                                       valid_types=DomainModel.PRIMITIVE)
        self.shape_model = DomainModel(placeholder=__("placeholder.same_shape"),
                                       valid_types=DiscreteVariable)
        self.size_model = DomainModel(placeholder=__("placeholder.same_size"),
                                      order=(self.SizeByOverlap,) + DomainModel.SEPARATED,
                                      valid_types=ContinuousVariable)
        self.label_model = DomainModel(placeholder=__("placeholder.no_labels"))
    ...
    def jitter_size_slider(self, widget):
        ...
        gui.valueSlider(
            widget=widget, master=self._plot, value='jitter_size', label=__("label.jittering"),
    ...
    def jitter_numeric_check_box(self, widget):
        gui.checkBox(
            ...
            label=__("checkbox.jitter_numeric"), callback=self._plot.master.reset_graph_data)

    def show_legend_check_box(self, widget):
        '''
            Creates a check box that shows and hides the plot legend
        '''
        self._check_box(widget, 'show_legend', i18n.t("common.software.show_legend"), 'update_legend')

    def tooltip_shows_all_check_box(self, widget):
        self._check_box(widget=widget, value="tooltip_shows_all",
                        label=__('checkbox.show_all_data'), cb_name="cb_tooltip_shows_all")

    def class_density_check_box(self, widget):
        self._plot.master.cb_class_density = \
            self._check_box(widget=widget, value="class_density", label=__("checkbox.class_density"),
    ...
    def regression_line_check_box(self, widget):
        self._plot.master.cb_reg_line = \
            self._check_box(widget=widget, value="show_reg_line",
                            label=__("checkbox.show_regression_line"),
    ...
    def label_only_selected_check_box(self, widget):
        self._check_box(widget=widget, value="label_only_selected",
                        label=__("checkbox.label_select_point"),
                        cb_name=self._plot.master.graph.update_labels)

    def filled_symbols_check_box(self, widget):
        self._check_box(widget, 'show_filled_symbols', __('checkbox.show_fill_symbol'),
                        'update_filled_symbols')

    def grid_lines_check_box(self, widget):
        self._check_box(widget, 'show_grid', __('checkbox.show_gridline'), 'update_grid')

    def animations_check_box(self, widget):
        '''
            Creates a check box that enabled or disables animations
        '''
        self._check_box(widget, 'use_animations', __('checkbox.use_animation'), 'update_animations')
    ...
    def point_size_slider(self, widget):
        ...
        '''
        self._slider(widget, 'point_width', __("label.symbol_size"), 1, 20, 1, 'update_point_size')

    def alpha_value_slider(self, widget):
        '''
            Creates a slider that controls point transparency
        '''
        self._slider(widget, 'alpha_value', __("label.opacity"), 0, 255, 10, 'update_alpha_value')
    ...
    def plot_properties_box(self, widget, box=None):
        ...
        return self.create_box([
            ...
            ], widget, box, __("box.plot_property"))
    ...
    def effects_box(self, widget, box=None):
        b = self.create_box([
            ...
            self.DisableAnimationsThreshold], widget, box, __("box.visual_effect"))
        return b

    def theme_combo_box(self, widget):
        c = gui.comboBox(widget, self._plot, "theme_name", __("box.theme"),

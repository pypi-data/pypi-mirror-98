...
def __(key):
    return i18n.t("widget.utils.utils.domaineditor." + key)
...
class VarTableModel(QAbstractTableModel):
    ...
    places = __("place.feature"), __("place.target"), __("place.meta"), __("place.skip")
    typenames = __("typename.categorical"), __("typename.numeric"), __("typename.text"), __("typename.datetime")
    ...
    def headerData(self, i, orientation, role=Qt.DisplayRole):
        if orientation == Qt.Horizontal and role == Qt.DisplayRole and i < 4:
            return (__("label.name"), __("label.type"), __("label.role"), __("label.values"))[i]

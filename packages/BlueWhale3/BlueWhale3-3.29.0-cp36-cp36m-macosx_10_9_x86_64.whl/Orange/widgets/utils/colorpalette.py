...
def __(key):
    return i18n.t("widget.utils.utils.colorpalette." + key)

warnings.warn(
    __("msg.update_color_palette"), DeprecationWarning)
...
class ColorPaletteDlg(QDialog, gui.OWComponent):
    ...
    def __init__(self, parent, windowTitle=__("window_title.color_palette")):
        ...
        self.schemaCombo = gui.comboBox(
            self.mainArea, self, "selectedSchemaIndex", box=__("box.save_profile"),
            callback=self.paletteSelected)

        self.hbox = gui.hBox(self)
        self.okButton = gui.button(self.hbox, self, i18n.t("common.btn.ok"), self.acceptChanges)
        self.cancelButton = gui.button(self.hbox, self, i18n.t("common.btn.cancel"), self.reject)
        ...
        if state == oldState:
            ...
            if self.colorSchemas[self.selectedSchemaIndex][0] == "Default":
                if QMessageBox.information(
                        self, __('msg.question_title'),
                        __('msg.question_content'),
    ...
    def createContinuousPalette(self, paletteName, boxCaption,
        ...
        _set("passThroughBlackCheckbox", gui.checkBox(
            ...
            __("checkbox.pass_through_black"), callback=self.colorSchemaChange))
    ...
    def createExtendedContinuousPalette(
        ...
        _set("passThroughColorsCheckbox",
             gui.checkBox(buttBox, self,
                          ...
                          __("checkbox.pass-through_color"),
                          callback=self.colorSchemaChange))

        box = gui.hBox(buttBox, __("box.pass-through_color"))
    ...
    def createDiscretePalette(self, paletteName, boxCaption, rgbColors=DefaultRGBColors):
        ...
        _set("EditButt", gui.button(
            hbox, self, __("btn.edit_palette"), self.editPalette,
            tooltip=__("btn.edit_palette_tip"), toggleButton=1))
        _set("LoadButt", gui.button(
            hbox, self, __("btn.load_palette"), self.loadPalette,
            tooltip=__("btn.load_palette_tip"), toggleButton=1))
    ...
    def setColorSchemas(self, schemas=None, selectedSchemaIndex=0):
        ...
        self.schemaCombo.addItem(__("msg.save_palette_as"))
    ...
    def paletteSelected(self):
        ...
        if self.selectedSchemaIndex == self.schemaCombo.count() - 1:
            message = __("msg.save_changes_tip")
            ok = 0
            while not ok:
                text, ok = QInputDialog.getText(self, __("msg.name_color_settings"), message)
                if (ok):
                    newName = str(text)
...
class ColorPalleteListing(QDialog):
    def __init__(self, parent=None, windowTitle=__("window_title.color_palette_list"),
                 **kwargs):
        ...
        box = gui.vBox(space, __("box.info"))
        gui.widgetLabel(
            box,
        ...
        box = gui.vBox(space, __("box.default_palette"))

        butt = _ColorButton(
            DefaultRGBColors, flat=True, toolTip=__("box.default_palette_tip"),
...
class PaletteEditor(QDialog):
    ...
    def __init__(self, rgbColors, parent=None, windowTitle=__("window_title.palette_edit"),
                 **kwargs):
        super().__init__(parent, **kwargs)
        self.setLayout(QVBoxLayout())
        self.layout().setContentsMargins(4, 4, 4, 4)

        hbox = gui.hBox(self, __("box.info"))
        gui.widgetLabel(
            hbox,
            __('msg.change_color'))
        ...
        buttonUPAttr = gui.button(vbox, self, "", callback=self.moveAttrUP,
                                  tooltip=__("msg.move_select_color_up"))
        buttonDOWNAttr = gui.button(vbox, self, "", callback=self.moveAttrDOWN,
                                    tooltip=__("msg.move_select_color_down"))

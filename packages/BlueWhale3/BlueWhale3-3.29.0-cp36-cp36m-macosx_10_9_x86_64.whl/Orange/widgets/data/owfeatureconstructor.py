...
def __(key):
    return i18n.t("widget.data.data.owfeatureconstructor." + key)
...
class FeatureEditor(QFrame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.funcs_model[:] = chain([__("gbox.select_function")], sorted_funcs)
    ...
    def setEditorData(self, data, domain):
        ...
        self.attrs_model[:] = [__("gbox.select_feature")]
...
class ContinuousFeatureEditor(FeatureEditor):
    ExpressionTooltip = __("tooltip.num_expression")
...
class DiscreteFeatureEditor(FeatureEditor):
    ExpressionTooltip = \
        __("tooltip.result_explicitly")

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        tooltip = \
            __("tooltip.result_value")
        self.valuesedit = QLineEdit(placeholderText=__("placeholder.index"), toolTip=tooltip)
        self.valuesedit.textChanged.connect(self._invalidate)

        layout = self.layout()
        label = QLabel(self.tr(__("label_values")))
...
class StringFeatureEditor(FeatureEditor):
    ExpressionTooltip = __("tooltip.string_expression")
...
class OWFeatureConstructor(OWWidget):
    ...
    class Error(OWWidget.Error):
        more_values_needed = Msg(__("msg_categorical"))
        invalid_expressions = Msg(__("msg_invalid_express"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __("box_var_define"))
        ...
        self.addbutton = QPushButton(
            __("gbox.new"), toolTip=__("tooltip.create_new_var"),
            minimumWidth=120,
        ...
        gui.button(self.buttonsArea, self, __("btn_send"), callback=self.apply, default=True)
    ...
    def send_report(self):
        ...
        for feature in self.featuremodel:
            if isinstance(feature, DiscreteDescriptor):
                items[feature.name] = __("report.categorical_value").format(
                    feature.expression, feature.values,
                    __("report.order") * feature.ordered)
            elif isinstance(feature, ContinuousDescriptor):
                items[feature.name] = __("report.numeric").format(feature.expression)
            elif isinstance(feature, DateTimeDescriptor):
                items[feature.name] = __("report.time").format(feature.expression)
            else:
                items[feature.name] = __("report.text").format(feature.expression)
        self.report_items(
            report.plural(__("report.construct_feature"), len(items)), items)

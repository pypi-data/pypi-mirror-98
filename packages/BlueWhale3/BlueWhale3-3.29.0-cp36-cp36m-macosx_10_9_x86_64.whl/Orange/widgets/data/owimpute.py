...
def __(key):
    return i18n.t("widget.data.data.owimpute." + key)
...
class OWImpute(OWWidget):
    ...
    class Error(OWWidget.Error):
        imputation_failed = Msg(__("msg_imputation_failed"))
        model_based_imputer_sparse = Msg(__("msg_model_not_sparse_data"))

    class Warning(OWWidget.Warning):
        cant_handle_var = Msg(__("msg_default_method_not_handle"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __("box_default_method"))
        ...
        button = QRadioButton(__("btn_default_method_index"))
        ...
        hlayout.addWidget(QLabel(__("btn_time")))
        ...
        box = gui.hBox(self.controlArea, self.tr(__("box_individual_attr")),
                       flat=False)
        ...
        self.reset_button = QPushButton(
            __("btn_restore_all"), enabled=False, default=False,
    ...
    def send_report(self):
        ...
        if specific:
            self.report_items((
                (__("report.default_method"), default.name),
                (__("report.specific_imputers"), ", ".join(specific))
            ))
        else:
            self.report_items(((i18n.t("common.software.method"), default.name),))

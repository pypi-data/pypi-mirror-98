...
def __(key):
    return i18n.t("widget.data.data.owoutliers." + key)
...
def run(data: Table, learner: Learner, state: TaskState) -> Results:
    ...
    callback(0, __("status_initialize"))
...
class SVMEditor(ParametersEditor):
    ...
    def __init__(self, parent):
        ...
        tooltip = __("tooltip_train_err")
        gui.widgetLabel(self.param_box, __("label.nu"), tooltip=tooltip)
        ...
        gui.doubleSpin(self.param_box, self, "gamma",
                       label=__("label.kernel_coefficient"), step=1e-2, minv=0.01,
...
class CovarianceEditor(ParametersEditor):
    ...
    def __init__(self, parent):
        ...
        gui.widgetLabel(self.param_box, __("label.contamination"))
        ...
        gui.checkBox(ebox, self, "empirical_covariance",
                     __("checkbox.support_fraction"), callback=self.parameter_changed)
...
class LocalOutlierFactorEditor(ParametersEditor):
    ...
    def __init__(self, parent):
        ...
        gui.widgetLabel(self.param_box, __("row_contamination"))
        gui.hSlider(self.param_box, self, "cont", minValue=1,
                    maxValue=50, ticks=5, labelFormat="%d %%",
                    callback=self.parameter_changed)
        gui.spin(self.param_box, self, "n_neighbors", label=__("row_neighbor"),
                 minv=1, maxv=100000, callback=self.parameter_changed)
        gui.comboBox(self.param_box, self, "metric_index", label=__("row_metric"),
                     orientation=Qt.Horizontal,
                     items=[i18n.t("common.algorithm.euclidean"), i18n.t("common.algorithm.manhattan"), i18n.t("common.algorithm.cosine"),
                            i18n.t("common.algorithm.jaccard"), i18n.t("common.algorithm.hamming"), i18n.t("common.algorithm.minkowski")],
                     callback=self.parameter_changed)
...
class IsolationForestEditor(ParametersEditor):
    ...
    def __init__(self, parent):
        ...
        gui.checkBox(self.param_box, self, "replicable",
                     i18n.t("common.general.replicable_train"), callback=self.parameter_changed)
...
class OWOutliers(OWWidget, ConcurrentWidgetMixin):
    ...
    class Outputs:
        inliers = Output("Inliers", Table, label=i18n.t("widget.data.data.common.inliers"))
        outliers = Output("Outliers", Table, label=i18n.t("common.general.outliers"))
    ...
    class Warning(OWWidget.Warning):
        disabled_cov = Msg(__("msg_covariance_estimation_many_feature"))

    class Error(OWWidget.Error):
        singular_cov = Msg(__("msg_singular_covariance_matrix"))
        memory_error = Msg(i18n.t("common.software.not_enough_memory"))
    ...
    def init_gui(self):
        box = gui.vBox(self.controlArea, i18n.t("common.software.method"))
    ...
    def _init_editors(self):
        ...
        box = gui.vBox(self.controlArea, i18n.t("common.general.parameter"))
    ...
    def send_report(self):
        if self.n_outliers is None or self.n_inliers is None:
            return
        self.report_items(__("report.data"),
                          ((__("report.input_instance"), len(self.data)),
                           (__("report.inliers"), self.n_inliers),
                           (i18n.t("common.general.outliers"), self.n_outliers)))

        params = self.current_editor.get_parameters()
        if self.outlier_method == self.OneClassSVM:
            self.report_items(
                __("report.detection"),
                ((__("report.detection_method"),
                  __("report.svm_rbf")),
                 (__("report.regularization"), params["nu"]),
                 (__("report.kernel_coefficient"), params["gamma"])))
        elif self.outlier_method == self.Covariance:
            self.report_items(
                __("report.detection"),
                ((__("report.detection_method"), __("report.covariance_estimator")),
                 (__("report.contamination"), params["contamination"]),
                 (__("report.support_fraction"), params["support_fraction"])))
        elif self.outlier_method == self.LOF:
            self.report_items(
                __("report.detection"),
                ((__("report.detection_method"), __("report.local_outlier_factor")),
                 (__("report.contamination"), params["contamination"]),
                 (__("report.number_of_neighbor"), params["n_neighbors"]),
                 (i18n.t("common.general.metric"), params["metric"])))
        elif self.outlier_method == self.IsolationForest:
            self.report_items(
                __("report.detection"),
                ((__("report.detection_method"), __("Isolation Forest1")),
                 (__("report.contamination"), params["contamination"])))
        else:
            raise NotImplementedError

...
def __(key):
    return i18n.t("widget.data.data.owrank." + key)
...
# Default scores.
CLS_SCORES = [
    ScoreMeta(__("checkbox.info_gain"), __("label.info_gain"),
              score.InfoGain, ProblemType.CLASSIFICATION, False),
    ScoreMeta(__("checkbox.info_gain_ratio"), __("label.gain_ratio"),
              score.GainRatio, ProblemType.CLASSIFICATION, True),
    ScoreMeta(__("checkbox.gini_decrease"), __("label.gini"),
              score.Gini, ProblemType.CLASSIFICATION, True),
    ScoreMeta(i18n.t("common.general.ANOVA"), i18n.t("common.general.ANOVA"),
              score.ANOVA, ProblemType.CLASSIFICATION, False),
    ScoreMeta(__("checkbox.x_square"), __("label.square"),
              score.Chi2, ProblemType.CLASSIFICATION, False),
    ScoreMeta(i18n.t("common.algorithm.reliefF"), i18n.t("common.algorithm.reliefF"),
              score.ReliefF, ProblemType.CLASSIFICATION, False),
    ScoreMeta(__("checkbox.fcbf"), __("label.fcbf"),
...
class OWRank(OWWidget, ConcurrentWidgetMixin):
    ...
    class Inputs:
        ...
        scorer = Input("Scorer", score.Scorer, multiple=True, label=i18n.t("widget.data.data.common.scorer"))

    class Outputs:
        reduced_data = Output("Reduced Data", Table, default=True, label=i18n.t("widget.data.data.common.reduced_data"))
        scores = Output("Scores", Table, label=i18n.t("widget.data.data.common.scores"))
        features = Output("Features", AttributeList, dynamic=False, label=i18n.t("common.general.features"))
    ...
    class Information(OWWidget.Information):
        no_target_var = Msg(__("msg.data_no_target_var"))
        missings_imputed = Msg(__('msg.miss_data_imputed'))

    class Error(OWWidget.Error):
        invalid_type = Msg(__("msg.cannot_handle_var"))
        inadequate_learner = Msg(__("msg.scorer_inadequate"))
        no_attributes = Msg(__("msg.data_no_attr"))

    class Warning(OWWidget.Warning):
        renamed_variables = Msg(__("msg.renamed_variables"))
    ...
    def __init__(self):
        ...
        for scoring_methods in (CLS_SCORES,
                                REG_SCORES,
                                []):
            box = gui.vBox(None, __("box_scoring_method") if scoring_methods else None)
        ...
        selMethBox = gui.vBox(self.buttonsArea, __("box_select_attribute"))
        ...
        b1 = button(self.tr(i18n.t('common.software.none')), OWRank.SelectNone)
    ...
    def send_report(self):
        if not self.data:
            return
        self.report_domain(i18n.t("common.software.input"), self.data.domain)
        self.report_table(__("report.ranks"), self.ranksView, num_format="{:.3f}")
        if self.out_domain_desc is not None:
            self.report_items(i18n.t("common.software.output"), self.out_domain_desc)

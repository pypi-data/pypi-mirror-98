...
def __(key):
    return i18n.t("widget.data.data.owpivot." + key)
...
class AggregationFunctionsEnum(Enum):
    (Count, Count_defined, Sum, Mean, Min, Max, Mode, Median, Var, Majority) = range(10)
...
class Pivot:
    ...
    def __get_pivot_tab_domain(self, val_var, X, X_h, X_v, X_t, agg_funs):
        ...
        if create_time_var:
            ...
            for x in (X_v, X_t):
                attrs.append([DiscreteVariable(__("label.total"), map_values(0, x))])
        row_var_h = DiscreteVariable(self._row_var.name, values=["Total"])
        aggr_attr = DiscreteVariable(__('label.aggregate'), [__('agg.'+str(f).lower().replace(" ", "_")) for f in agg_funs])
...
class OWPivot(OWWidget):
    ...
    class Outputs:
        pivot_table = Output("Pivot Table", Table, default=True, label=i18n.t("widget.data.data.common.pivot_table"))
        filtered_data = Output("Filtered Data", Table, label=i18n.t("widget.data.data.common.filtered_data"))
        grouped_data = Output("Grouped Data", Table, label=i18n.t("widget.data.data.common.grouped_data"))

    class Warning(OWWidget.Warning):
        # TODO - inconsistent for different variable types
        no_col_feature = Msg(__("msg_column_feature_should_selected"))
        cannot_aggregate = Msg(__("msg_aggregation_cannot_perform"))
        renamed_vars = Msg(__("msg_var_avoid_duplicate"))
        too_many_values = Msg(__("msg_too_many_values"))
    ...
    def _add_control_area_controls(self):
        gui.comboBox(gui.vBox(self.controlArea, box=i18n.t("common.general.row")),
                     self, "row_feature",
                     contentsLength=14,
                     searchable=True,
        ...
        gui.comboBox(gui.vBox(self.controlArea, box=i18n.t("common.general.column")),
                     self, "col_feature",
                     contentsLength=14,
                     searchable=True,
                     model=DomainModel(placeholder=__("placeholder_same_as_row"),
                                       valid_types=DiscreteVariable),
                     callback=self.__feature_changed,
                     orientation=Qt.Horizontal)
        gui.comboBox(gui.vBox(self.controlArea, box=__("box.value")),
                     self, "val_feature",
                     contentsLength=14,
                     searchable=True,
    ...
    def __add_aggregation_controls(self):
        ...
        for agg in self.AGGREGATIONS:
            if agg is None:
                ...
                row = 0
                continue
            check_box = QCheckBox(__('agg.'+str(agg).lower().replace(" ", "_")), inbox)
    ...
    def commit(self):
        ...
        if self.skipped_aggs:
            self.Warning.cannot_aggregate(", ".join([__('agg.'+f.lower().replace(" ", "_"))
                                               for f in self.skipped_aggs.split(', ')]))
        self._update_graph()
    ...
    def send_report(self):
        self.report_items((
            (__("report.row_feature"), self.row_feature),
            (__("report.column_feature"), self.col_feature),
            (__("report.value_feature"), self.val_feature)))
        if self.data and self.val_feature is not None:
            self.report_table("", self.table_view)
        if not self.data:
            self.report_items(((__("report.group_by"), self.row_feature),))
            self.report_table(self.table_view)

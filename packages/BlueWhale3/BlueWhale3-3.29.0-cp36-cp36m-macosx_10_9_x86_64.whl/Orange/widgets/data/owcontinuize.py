...
def __(key):
    return i18n.t("widget.data.data.owcontinuize." + key)
...
class OWContinuize(widget.OWWidget):
    ...
    multinomial_treats = (
        (__("option.first_value"), Continuize.FirstAsBase),
        (__("option.most_frequent"), Continuize.FrequentAsBase),
        (__("option.value_attr"), Continuize.Indicators),
        (__("option.ignore_multinomial"), Continuize.RemoveMultinomial),
        (__("option.remove_categorical"), Continuize.Remove),
        (__("option.ordinal"), Continuize.AsOrdinal),
        (__("option.divide_val"), Continuize.AsNormalizedOrdinal))

    continuous_treats = (
        (__("option.as_they"), True),
        (__("option.standardize"), False),
        (__("option.center"), False),
        (__("option.scale"), True),
        (__("option.minus"), False),
        (__("option.plus"), False)
    )

    class_treats = (
        (__("option.as_it"), Continuize.Leave),
        (__("option.ordinal"), Continuize.AsOrdinal),
        (__("option.divide_val"), Continuize.AsNormalizedOrdinal),
        (__("option.per_val"), Continuize.Indicators),
    )
    ...
    def __init__(self):
        ...
        box = gui.radioButtonsInBox(
            None, self, "multinomial_treatment", box=__("box.category"),
        ...
        box = gui.radioButtonsInBox(
            None, self, "continuous_treatment", box=__("box.numeric"),
        ...
        box = gui.radioButtonsInBox(
            None, self, "class_treatment", box=__("box.categorical"),
    ...
    def send_report(self):
        self.report_items(
            i18n.t("common.software.setting"),
            [(__("report.categorical_feature"),
              self.multinomial_treats[self.multinomial_treatment][0]),
             (__("report.numeric_feature"),
              self.continuous_treats[self.continuous_treatment][0]),
             (__("report.class"), self.class_treats[self.class_treatment][0])])

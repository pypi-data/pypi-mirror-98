...
def __(key):
    return i18n.t("widget.data.data.owneighbors." + key)
...
class OWNeighbors(OWWidget):
    ...
    class Inputs:
        ...
        reference = Input("Reference", Table, label=i18n.t("common.software.reference"))

    class Outputs:
        data = Output("Neighbors", Table, label=i18n.t("common.general.neighbor"))

    class Info(OWWidget.Warning):
        removed_references = \
            Msg(__("msg_removed_references"))

    class Warning(OWWidget.Warning):
        all_data_as_reference = \
            Msg(__("msg_instance_as_reference"))

    class Error(OWWidget.Error):
        diff_domains = Msg(__("msg_data_reference_different"))
    ...
    def __init__(self):
        ...
        gui.comboBox(
            ...
            label=__("row_distance"), items=[d[0] for d in METRICS],
            callback=self.recompute)
        gui.spin(
            box, self, "n_neighbors", label=__("row_num_neighbor"),
    ...
    def _set_input_summary(self):
        ...
        if self.data or self.reference:
            ...
            details = format_multiple_summaries([
                (__("detail.data"), self.data),
                (i18n.t("common.software.reference"), self.reference)
            ])

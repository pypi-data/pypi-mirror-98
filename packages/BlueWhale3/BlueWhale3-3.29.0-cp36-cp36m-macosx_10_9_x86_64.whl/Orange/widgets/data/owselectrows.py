...
def __(key):
    return i18n.t("widget.data.data.owselectrows." + key)
...
class OWSelectRows(widget.OWWidget):
    ...
    class Outputs:
        matching_data = Output("Matching Data", Table, default=True, label=i18n.t("widget.data.data.common.match_data"))
        unmatched_data = Output("Unmatched Data", Table, label=i18n.t("widget.data.data.common.unmatched_data"))
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table, label=i18n.t("widget.data.data.common.data"))
    ...
    Operators = {
        ContinuousVariable: [
            (FilterContinuous.Equal, __("filter.equals")),
            (FilterContinuous.NotEqual, __("filter.not")),
            (FilterContinuous.Less, __("filter.below")),
            (FilterContinuous.LessEqual, __("filter.at_most")),
            (FilterContinuous.Greater, __("filter.greater_than")),
            (FilterContinuous.GreaterEqual, __("filter.at_least")),
            (FilterContinuous.Between, __("filter.between")),
            (FilterContinuous.Outside, __("filter.outside")),
            (FilterContinuous.IsDefined, __("filter.defined")),
        ],
        DiscreteVariable: [
            (FilterDiscreteType.Equal, __("filter.is")),
            (FilterDiscreteType.NotEqual, __("filter.not")),
            (FilterDiscreteType.In, __("filter.one_of")),
            (FilterDiscreteType.IsDefined, __("filter.defined"))
        ],
        StringVariable: [
            (FilterString.Equal, __("filter.equals")),
            (FilterString.NotEqual, __("filter.not")),
            (FilterString.Less, __("filter.before")),
            (FilterString.LessEqual, __("filter.equal_before")),
            (FilterString.Greater, __("filter.after")),
            (FilterString.GreaterEqual, __("filter.equal_after")),
            (FilterString.Between, __("filter.between")),
            (FilterString.Outside, __("filter.outside")),
            (FilterString.Contains, __("filter.contains")),
            (FilterString.StartsWith, __("filter.begins_with")),
            (FilterString.EndsWith, __("filter.ends_with")),
            (FilterString.IsDefined, __("filter.defined")),
        ]
    }
    ...
    for _all_name, _all_type, _all_ops in (
            (__("row.all_variables"), 0,
             [(None, __("filter.are_defined"))]),
            (__("row.numeric_variables"), 2,
             [(v, _plural(t)) for v, t in Operators[ContinuousVariable]]),
            (__("row.string_variables"), 3,
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __('box_condition'), stretch=100)
        ...
        self.add_button = gui.button(
            box2, self, __("btn.add_condition"), callback=self.add_row)
        self.add_all_button = gui.button(
            box2, self, __("btn.add_all_var"), callback=self.add_all)
        self.remove_all_button = gui.button(
            box2, self, __("btn.remove_all"), callback=self.remove_all)
        gui.rubber(box2)

        box_setting = gui.vBox(self.buttonsArea)
        self.cb_pa = gui.checkBox(
            box_setting, self, "purge_attributes", __("checkbox_remove_unused_feature"),
            callback=self.conditions_changed)
        self.cb_pc = gui.checkBox(
            box_setting, self, "purge_classes", __("checkbox_remove_unused_class"),
            callback=self.conditions_changed)
    ...
    def add_all(self):
        if self.cond_list.rowCount():
            Mb = QMessageBox
            if Mb.question(
                    self, __("msg.remove_exist_filter"),
                    __("msg.replace_filter"), Mb.Ok | Mb.Cancel) != Mb.Ok:
                return
            self.remove_all()
    ...
    def send_report(self):
        if not self.data:
            self.report_paragraph(i18n.t("common.software.no_data"))
            return
        ...
        items["Condition"] = __("report.and").join(conditions) or __("report.no_condition")
        self.report_items(__("report.data"), items)
        if describe_domain:
            self.report_items(__("report.match_data"), self.match_desc)
            self.report_items(__("report.non-match_data"), self.nonmatch_desc)
        else:
            match_inst = \
            ...
            self.report_items(
                i18n.t("common.software.output"),
                ((__("report.match_data"),
                  __("report.instance").format(match_inst) if match_inst else i18n.t('common.software.none')),
                 (__("report.no_condition"),
                  nonmatch_inst > 0 and __("report.instance").format(nonmatch_inst))))

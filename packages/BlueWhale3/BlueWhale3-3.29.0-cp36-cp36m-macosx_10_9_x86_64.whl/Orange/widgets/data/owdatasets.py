...
def __(key):
    return i18n.t("widget.data.data.owdatasets." + key)
...
class OWDataSets(OWWidget):
    ...
    HEADER_SCHEMA = [
        ['islocal', {'label': ''}],
        ['title', {'label': __('label.title')}],
        ['size', {'label': i18n.t("common.software.size")}],
        ['instances', {'label': __('label.instance')}],
        ['variables', {'label': i18n.t("common.general.variable")}],
        ['target', {'label': __('label.target')}],
        ['tags', {'label': __('label.tag')}]
    ]  # type: List[str, dict]
    ...
    def __init__(self):
        ...
        self.filterLineEdit = QLineEdit(
            textChanged=self.filter, placeholderText=__("placeholder_search")
        )
        ...
        box = gui.widgetBox(self.splitter, __("box_desc"), addToLayout=False)
        ...
        self.setStatusMessage(__("set_status.initialize"))
    ...
    def commit(self):
        ...
        if di is not None:
            ...
            if not di.islocal:
                ...
                self.setStatusMessage(__("set_status.fetch"))
                self.setBlocking(True)

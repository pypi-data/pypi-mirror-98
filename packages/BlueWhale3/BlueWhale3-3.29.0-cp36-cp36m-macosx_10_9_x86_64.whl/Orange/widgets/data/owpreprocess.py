...
def __(key):
    return i18n.t("widget.data.data.owpreprocess." + key)
...
class DiscretizeEditor(BaseEditor):
    ...
    Names = {
        NoDisc: i18n.t('common.software.none'),
        EqualWidth: __("editor.equal_width"),
        EqualFreq: __("editor.equal_freq"),
        Drop: __("editor.drop"),
        EntropyMDL: __("editor.entropy_mdl")
    }
...
class ContinuizeEditor(BaseEditor):
    ...
    Continuizers = OrderedDict([
        (Continuize.FrequentAsBase, __("editor.frequent_as_base")),
        (Continuize.Indicators, __("editor.indicators")),
        (Continuize.RemoveMultinomial, __("editor.remove_multinomial")),
        (Continuize.Remove, __("editor.remove")),
        (Continuize.AsOrdinal, __("editor.as_ordinal")),
        (Continuize.AsNormalizedOrdinal, __("editor.as_normalized_ordinal"))
    ])
...
class RemoveSparseEditor(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.layout().addWidget(QLabel(__("label_remove_feature")))
        options = [__("options.miss_value"),
                   __("options.zeros")]
        ...
        filter_settings = QGroupBox(title=__('box_threshold'), flat=True)
        ...
        btn_perc = QRadioButton(self, text=__('btn.percentage'), checked=not self.useFixedThreshold)
        ...
        btn_fix = QRadioButton(self, text=__('btn.fix'), checked=self.useFixedThreshold)
...
class ImputeEditor(BaseEditor):
    ...
    Names = {
        NoImputation: __("editor.no_imputation"),
        Constant: __("editor.constant"),
        Average: __("editor.average"),
        Model: __("editor.model"),
        Random: __("editor.random"),
        DropRows: __("editor.drop_rows"),
    }
...
class UnivariateFeatureSelect(QWidget):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        box = QGroupBox(title=__("row.score"), flat=True)
        ...
        box = QGroupBox(title=__("row.number_feature"), flat=True)
        self.__group = group = QButtonGroup(self, exclusive=True)
        self.__spins = {}

        form = QFormLayout()
        fixedrb = QRadioButton(__("btn.fix"), checked=True)
        ...
        percrb = QRadioButton(__("btn.proportion"))
...
class FeatureSelectEditor(BaseEditor):
    MEASURES = [
        (__("editor.information_gain"), preprocess.score.InfoGain),
        (__("editor.gain_ratio"), preprocess.score.GainRatio),
        (__("editor.gini_index"), preprocess.score.Gini),
        (i18n.t("common.algorithm.reliefF"), preprocess.score.ReliefF),
        (__("editor.fast_correlation_filter"), preprocess.score.FCBF),
        (i18n.t("common.general.ANOVA"), preprocess.score.ANOVA),
        (__("editor.chi_two"), preprocess.score.Chi2),
        (__("editor.rrelieff"), preprocess.score.RReliefF),
        (__("editor.univariate_linear_regression"), preprocess.score.UnivariateLinearRegression)
    ]
...
class RandomFeatureSelectEditor(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        box = QGroupBox(title=__("row.num_features"), flat=True)
        self.__group = group = QButtonGroup(self, exclusive=True)
        self.__spins = {}

        form = QFormLayout()
        fixedrb = QRadioButton(__("row.fix"), checked=True)
        ...
        percrb = QRadioButton(__("row.percentage"))
...
class Scale(BaseEditor):
    ...
    Names = {
        NormalizeBySD: __("editor.normalize_by_sd"),
        CenterByMean: __("editor.center_by_mean"),
        ScaleBySD: __("editor.scale_by_sd"),
        NormalizeSpan_NonZeroBased: __("editor.normalize_span_non_zero_based"),
        NormalizeBySpan_ZeroBased: __("editor.normalize_by_span_zero_based"),
    }
...
class Randomize(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.__rand_type_cb.addItems([i18n.t("common.general.classes"),
                                      i18n.t("common.general.features"),
                                      i18n.t("common.general.meta_data")])
        ...
        form.addRow(__("row.random"), self.__rand_type_cb)
        form.addRow(__("row.replicable_shuffle"), self.__rand_seed_ch)
...
class PCA(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        form.addRow(__("row.components"), self.cspin)
        self.layout().addLayout(form)
...
class CUR(BaseEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        form.addRow(__("row.rank"), self.rspin)
        form.addRow(__("row.relative_error"), self.espin)
        self.layout().addLayout(form)
...
class OWPreprocess(widget.OWWidget, openclass=True):
    ...
    class Inputs:
        data = Input("Data", Orange.data.Table, label=i18n.t("widget.data.data.common.data"))

    class Outputs:
        preprocessor = Output("Preprocessor", preprocess.preprocess.Preprocess, dynamic=False,
                              label=i18n.t("common.general.preprocessor"))
        preprocessed_data = Output("Preprocessed Data", Orange.data.Table,
                                   label=i18n.t("widget.data.data.common.preprocessor_data"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __("box_preprocessor"))
        ...
        self.overlay.layout().addWidget(
            QLabel(__("label_drag_item_from_left"), wordWrap=True))
    ...
    def send_report(self):
        ...
        if pp:
            self.report_items(i18n.t("common.software.setting"), pp)

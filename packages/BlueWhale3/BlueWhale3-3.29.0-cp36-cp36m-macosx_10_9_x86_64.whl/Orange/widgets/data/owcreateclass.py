...
def __(key):
    return i18n.t("widget.data.data.owcreateclass." + key)
...
class OWCreateClass(widget.OWWidget):
    ...
    class_name = ContextSetting(__("default_class_name"))
    ...
    class Warning(widget.OWWidget.Warning):
        no_nonnumeric_vars = Msg(__("msg.contains_numeric"))

    class Error(widget.OWWidget.Error):
        class_name_duplicated = Msg(__("msg.class_duplicated"))
        class_name_empty = Msg(__("msg.class_nonempty"))
    ...
    def __init__(self):
        ...
        gui.lineEdit(
            ...
            orientation=Qt.Horizontal, box=__("box.class_name"))

        variable_select_box = gui.vBox(self.controlArea, __("box.variable_select_box"))

        combo = gui.comboBox(
            variable_select_box, self, "attribute", label=__("box.column"),
        ...
        rules_box.addWidget(QLabel(__("box.name")), 0, 1)
        rules_box.addWidget(QLabel(__("box.substring")), 0, 2)
        rules_box.addWidget(QLabel(__("box.count")), 0, 3, 1, 2)
        self.update_rules()
        ...
        optionsbox = gui.vBox(self.controlArea, __("box.options"))
        gui.checkBox(
            optionsbox, self, "match_beginning", __("checkbox_match_begin"),
            callback=self.options_changed)
        gui.checkBox(
            optionsbox, self, "case_sensitive", __("checkbox_case_sensitive"),
            callback=self.options_changed)

        gui.rubber(self.controlArea)

        gui.button(self.buttonsArea, self, i18n.t("common.btn.apply"), callback=self.apply)
        ...
        def _set_labels():
            ...
            for (n_matched, n_total), (lab_matched, lab_total), (lab, patt) in \
                ...
                if n_before and (lab or patt):
                    ...
                    if n_matched:
                        tip = __("tip_match_instance").format(n_before, n_total)
                    else:
                        tip = __("tip_cover_instance")
            ...
            for n_matched, (_, patt) in zip(matches, self.line_edits):
                if not patt.text():
                    patt.setPlaceholderText(
                        __("placeholder.remain_instance") if n_matched else __("placeholder.unused"))
    ...
    def send_report(self):
        ...
        def _count_part():
            if not n_matched:
                return __("msg.all_match_instance_cover_above").format(n_total)
            elif n_matched < n_total and patt:
                return __("msg.match_instance_cover_above").format(n_matched, n_total - n_matched)
            else:
                return __("msg.match_instance").format(n_matched)

        if not self.attribute:
            return
        self.report_items(i18n.t("common.software.input"), [(__("report.source_attribute"), self.attribute.name)])
        output = ""
        ...
        if output:
            self.report_items(i18n.t("common.software.output"), [(__("report.class_name"), self.class_name)])

...
def __(key):
    return i18n.t("widget.data.data.oweditdomain." + key)
...
class DictItemsModel(QStandardItemModel):
    ...
    def __init__(self, parent=None, a_dict=None):
        ...
        self._dict = {}
        self.setHorizontalHeaderLabels([i18n.t("common.general.key"), i18n.t("common.general.value")])
...
class VariableEditor(QWidget):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        form.addRow(__("row.name"), self.name_edit)

        self.unlink_var_cb = QCheckBox(
            __("row.unlink_var_cb"), self,
            toolTip=__("row.unlink_var_cb_tip")
        )
        ...
        form.addRow(__("row.label"), vlayout)
...
class GroupItemsDialog(QDialog):
    ...
    """
    DEFAULT_LABEL = __("default_label")
    ...
    def __init__(
        ...
        label2 = QLabel(__("label.occurrence"))
        label3 = QLabel(__("label.occurrence"))
        label4 = QLabel(__("label.frequent_value"))
        ...
        new_name_label = QLabel(__("label.new_value_name"))
        ...
        buttons.button(QDialogButtonBox.Ok).setText(i18n.t("common.btn.ok"))
        buttons.button(QDialogButtonBox.Cancel).setText(i18n.t("common.btn.cancel"))
    ...
    def __counts(self):
        ...
        for index in map(self.index, range(self.rowCount())):
            ...
            try:
                ...
                warnings.warn(__("tooltip.key_value_error").format(key))
...
class DiscreteVariableEditor(VariableEditor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.move_value_up = QAction(
            ...
            toolTip=__("tooltip.select_move_up"),
        ...
        self.move_value_down = QAction(
            ...
            toolTip=__("tooltip.select_move_down"),
            shortcut=QKeySequence(Qt.ControlModifier | Qt.AltModifier |
                                  Qt.Key_BracketRight),
            shortcutContext=Qt.WidgetShortcut,
        )
        ...
        self.add_new_item = QAction(
            ...
            toolTip=__("tooltip.append_item"),
            shortcut=QKeySequence(QKeySequence.New),
            shortcutContext=Qt.WidgetShortcut,
        )
        self.remove_item = QAction(
            __("label.remove_item"), group,
            iconText="\N{MINUS SIGN}",
            objectName="action-remove-item",
            toolTip=__("tooltip.delete_item"),
            shortcut=QKeySequence(QKeySequence.Delete),
            shortcutContext=Qt.WidgetShortcut,
        )
        self.rename_selected_items = QAction(
            __("label.rename_items"), group,
            iconText="=",
            objectName="action-rename-selected-items",
            toolTip=__("tooltip.rename_selected_items"),
        ...
        form.insertRow(2, __("box.value"), vlayout)
    ...
    def _merge_categories(self) -> None:
        ...
        dlg = GroupItemsDialog(
            ...
            windowTitle=__("window_title_import_option"),
...
class ReinterpretVariableEditor(VariableEditor):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        def decorate(editor: VariableEditor) -> VariableEditor:
            ...
            typecb.addItem(variable_icon(Categorical), __("type.categorical"), Categorical)
            typecb.addItem(variable_icon(Real), __("type.numeric"), Real)
            typecb.addItem(variable_icon(String), __("type.text"), String)
            typecb.addItem(variable_icon(Time), __("type.time"), Time)
            typecb.activated[int].connect(self.__reinterpret_activated)
            form.insertRow(1, __("row.type"), typecb)
...
class OWEditDomain(widget.OWWidget):
    ...
    class Outputs:
        data = Output("Data", Orange.data.Table, label=i18n.t("widget.data.data.common.data"))

    class Error(widget.OWWidget.Error):
        duplicate_var_name = widget.Msg(__("msg_name_duplicated"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(main, i18n.t("common.general.variable"))
        ...
        breset_all = gui.button(
            bbox, self, __("btn.reset_all"),
            objectName="button-reset-all",
            toolTip=__("tooltip.reset_state"),
            autoDefault=False,
            callback=self.reset_all
        )
        breset = gui.button(
            bbox, self, __("btn.reset_select"),
            objectName="button-reset",
            toolTip=__("tooltip.rest_state"),
            autoDefault=False,
            callback=self.reset_selected
        )
        bapply = gui.button(
            bbox, self, i18n.t("common.btn.apply"),
            objectName="button-apply",
            toolTip=__("tooltip.apply_data"),
            default=True,
            autoDefault=False,
    ...
    def _restore_transform(self, var):
        ...
        for t in tr_:
            try:
                ...
                warnings.warn(
                    __("fail_restore_transform").format(t, err),

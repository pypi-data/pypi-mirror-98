...
def __(key):
    return i18n.t("widget.data.data.owsql." + key)
...
class OWSql(OWBaseSql):
    ...
    class Outputs:
        data = Output("Data", Table, label=i18n.t("widget.data.data.common.data"), doc=i18n.t("widget.data.data.common.read_attr_valued"))
    ...
    class Information(OWBaseSql.Information):
        data_sampled = Msg(__("msg.data_description"))

    class Warning(OWBaseSql.Warning):
        missing_extension = Msg(__("msg.data_miss_extension"))

    class Error(OWBaseSql.Error):
        no_backends = Msg(__("msg.data_install_backend"))
    ...
    def _add_tables_controls(self):
        vbox = gui.vBox(self.controlArea, __("box_table"))
        box = gui.vBox(vbox)
        ...
        cb = gui.checkBox(mt, self, 'materialize', __('checkbox.materialize_table'))
        cb.setToolTip(__('tooltip_result_save'))
        le = gui.lineEdit(mt, self, 'materialize_table_name')
        le.setToolTip(__('tooltip_result_save'))

        gui.button(self.custom_sql, self, __('btn.execute'), callback=self.open_table)

        box.layout().addWidget(self.custom_sql)

        gui.checkBox(box, self, "guess_values",
                     __("checkbox.discover_categorical_var"),
                     callback=self.open_table)

        self.downloadcb = gui.checkBox(box, self, "download",
                                       __("checkbox.download_local"),
                                       callback=self.open_table)
    ...
    def refresh_tables(self):
        ...
        self.tables.append(__("row.select_table"))
        self.tables.append(__("row.custom_sql"))
    ...
    def select_table(self):
        ...
        if self.tablecombo.itemText(curIdx) != "Custom SQL":
            ...
            self.data_desc_table = None
            self.database_desc["Table"] = i18n.t("common.software.brackets_none")
            self.table = None
    ...
    def get_table(self):
        ...
        if curIdx <= 0:
            if self.database_desc:
                self.database_desc["Table"] = __("desc_none")
            self.data_desc_table = None
            return None
        ...
        if table.approx_len() > LARGE_TABLE and self.guess_values:
            ...
            confirm.setText(__("msg.auto_discover_attribute"))
            confirm.addButton(__("btn.ok"), QMessageBox.YesRole)
            no_button = confirm.addButton(__("btn.cancel"), QMessageBox.NoRole)
            if is_postgres(self.backend):
                sample_button = confirm.addButton(__("btn.sample"),
                                                  QMessageBox.YesRole)
            confirm.exec()
        ...
        if self.download:
            if table.approx_len() > AUTO_DL_LIMIT:
                if is_postgres(self.backend):
                    confirm = QMessageBox(self)
                    confirm.setIcon(QMessageBox.Warning)
                    confirm.setText(__("msg.download_big_data"))
                    ...
                    if table.approx_len() > MAX_DL_LIMIT:
                        QMessageBox.warning(
                            self, __('msg.warn'), __("msg.download_data"))
                        return None
                    else:
                        confirm = QMessageBox.question(
                            self, __('msg.question'), __("msg.download_data_to_memory"),

...
def __(key):
    return i18n.t("widget.visualize.visualize.owruleviewer." + key)
...
class OWRuleViewer(widget.OWWidget):
    ...
    class Inputs:
        ...
        classifier = Input('Classifier', _RuleClassifier, label=i18n.t("common.general.classifier"))
    ...
    def __init__(self):
        ...
        self.model.set_horizontal_header_labels(
            [__("label.condition"), "", __("label.class"), i18n.t("common.general.distribution"),
             __("label.probability"), __("label.quality"), __("label.length")])
        ...
        gui.checkBox(widget=self.buttonsArea, master=self, value="compact_view",
                     label=__("checkbox_compact_view"), callback=self.on_update)
        gui.rubber(self.buttonsArea)

        original_order_button = gui.button(
            self.buttonsArea, self,
            __("btn_original_order"),
            autoDefault=False,
    ...
    def send_report(self):
        if self.classifier is not None:
            self.report_domain(__("report.data_domain"), self.classifier.original_domain)
            self.report_items(__("report.rule_induction_algorithm"), self.classifier.params)
            self.report_table(__("report.induced_rule"), self.view)

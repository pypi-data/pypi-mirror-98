...
def __(key):
    return i18n.t("widget.visualize.visualize.owmosaic." + key)


class MosaicVizRank(VizRankDialog, OWComponent):
    """VizRank dialog for Mosaic"""
    captionTitle = __("captiontitle_mosaic_ranking")
    ...
    def __init__(self, master):
        ...
        self.max_attr_combo = gui.comboBox(
            box, self, "max_attrs",
            label=__("label_limit_attribute"), orientation=Qt.Horizontal,
    ...
    def max_attr_changed(self):
        ...
        if self.max_attrs != self.last_run_max_attr or self.saved_state is None:
            ...
            self.button.setText(i18n.t("common.software.continue"))
    ...
    def check_preconditions(self):
        ...
        self.Information.add_message("no_attributes", __("msg.no_var_to_rank"))
...
class OWMosaicDisplay(OWWidget):
    ...
    class Warning(OWWidget.Warning):
        incompatible_subset = Msg(__("msg.incompatible"))
        no_valid_data = Msg(__("msg.no_valid_data"))
        no_cont_selection_sql = \
            Msg(__("msg.use_sql_error"))
    ...
    def __init__(self):
        ...
        self.vizrank, self.vizrank_button = MosaicVizRank.add_vizrank(
            box, self, __("btn_informative_mosaics"), self.set_attr)

        box2 = gui.vBox(self.controlArea, box=__("box.interior_coloring"))
        self.color_model = DomainModel(
            order=DomainModel.MIXED, valid_types=DomainModel.PRIMITIVE,
            placeholder=__("placeholder_pearson_residuals"))
        ...
        self.bar_button = gui.checkBox(
            box2, self, 'use_boxes', label=__('checkbox_compare_total'),
    ...
    def migrate_context(cls, context, version):
        if version < 2:
            settings.migrate_str_to_variable(context, none_placeholder=i18n.t("common.software.brackets_none"))

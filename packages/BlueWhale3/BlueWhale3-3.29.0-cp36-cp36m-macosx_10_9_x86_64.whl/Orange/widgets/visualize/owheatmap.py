...
def __(key):
    return i18n.t("widget.visualize.visualize.owheatmap." + key)
...
ClusteringModelData = [
    {
        Qt.DisplayRole: i18n.t("common.software.none"),
        Qt.ToolTipRole: __("gbox.no_clustering"),
        ClusteringRole: Clustering.None_,
    }, {
        Qt.DisplayRole: i18n.t("common.general.cluster"),
        Qt.ToolTipRole: __("gbox.hierarchical_clustering"),
        ClusteringRole: Clustering.Clustering,
    }, {
        Qt.DisplayRole: __("gbox.clustering_opt"),
        Qt.ToolTipRole: __("gbox.optimal_leaf_ordering"),
...
ColumnLabelsPosData = [
    ...
    for name, value in [
        (i18n.t("common.software.none"), HeatmapGridWidget.NoPosition),
        (__("gbox.top"), HeatmapGridWidget.PositionTop),
        (__("gbox.bottom"), HeatmapGridWidget.PositionBottom),
        (__("gbox.top_bottom"), (HeatmapGridWidget.PositionTop |
...
class OWHeatMap(widget.OWWidget):
    ...
    class Inputs:
        data = Input('Data', Table, label=i18n.t("widget.visualize.visualize.common.data"))

    class Outputs:
        selected_data = Output('Selected Data', Table, default=True,
                               label=i18n.t("widget.visualize.visualize.common.selected_data"))
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table,
                                label=i18n.t("widget.visualize.visualize.common.data"))
    ...
    class Information(widget.OWWidget.Information):
        sampled = Msg(__("msg.data_sampled"))
        discrete_ignored = Msg(__("msg.ignore_categorical_feature"))
        row_clust = Msg("{}")
        col_clust = Msg("{}")
        sparse_densified = Msg(__("msg.memory_tip"))

    class Error(widget.OWWidget.Error):
        no_continuous = Msg(__("msg.no_numeric_features"))
        not_enough_features = Msg(__("msg.column_clustering_feature"))
        not_enough_instances = Msg(__("msg.clustering_instance"))
        not_enough_instances_k_means = Msg(__("msg.k-means_merging_instance"))
        not_enough_memory = Msg(__("msg.out_of_memory"))

    class Warning(widget.OWWidget.Warning):
        empty_clusters = Msg(__("msg.remove_empty_clusters"))

    UserAdviceMessages = [
        widget.Message(__("msg.user_advice"), "diverging_palette")]
    ...
    def __init__(self):
        ...
        # GUI definition
        colorbox = gui.vBox(self.controlArea, __("box.color"))
        ...
        mergebox = gui.vBox(self.controlArea, __("box.merge"), )
        gui.checkBox(mergebox, self, "merge_kmeans", __("checkbox.merge_k-means"),
                     callback=self.__update_row_clustering)
        ibox = gui.indentedBox(mergebox)
        gui.spin(ibox, self, "merge_kmeans_k", minv=5, maxv=500,
                 label=__("row.clusters"), keyboardTracking=False,
                 callbackOnReturn=True, callback=self.update_merge)

        cluster_box = gui.vBox(self.controlArea, i18n.t("common.general.cluster"))
        # Row clustering
        ...
        form.addRow(__("row.rows"), self.row_cluster_cb)
        form.addRow(__("row.columns"), self.col_cluster_cb)
        cluster_box.layout().addLayout(form)
        box = gui.vBox(self.controlArea, __("box.split_by"))
        form = QFormLayout(
        ...
        self.row_split_model = DomainModel(
            placeholder=i18n.t("common.software.brackets_none"),
        ...
        self.row_split_cb = cb = ComboBoxSearch(
            ...
            toolTip=__("tooltip_split_by_categorical_column")
        )
        ...
        form.addRow(__("row.rows"), self.row_split_cb)
        form.addRow(__("row.columns"), self.col_split_cb)

        box = gui.vBox(self.controlArea, __('box.annotation_legends'))

        gui.checkBox(box, self, 'legend', i18n.t("common.software.show_legend"),
                     callback=self.update_legend)

        gui.checkBox(box, self, 'averages', __('checkbox.stripes_averages'),
                     callback=self.update_averages_stripe)
        gui.separator(box)
        annotbox = QGroupBox(__("box.row_annotations"))
        form = QFormLayout(
            annotbox,
        ...
        self.annotation_model = DomainModel(placeholder=i18n.t("common.software.brackets_none"))
        ...
        self.row_side_color_model = DomainModel(
            order=(DomainModel.CLASSES, DomainModel.Separator,
                   DomainModel.METAS),
            placeholder=i18n.t("common.software.brackets_none"), valid_types=DomainModel.PRIMITIVE,
        ...
        form.addRow(i18n.t("common.software.text"), self.annotation_text_cb)
        form.addRow(__("row.color"), self.row_side_color_cb)
        box.layout().addWidget(annotbox)
        annotbox = QGroupBox(__("box.column_labels_position"))
        form = QFormLayout(
            annotbox,
            formAlignment=Qt.AlignLeft,
            labelAlignment=Qt.AlignLeft,
        ...
        form.addRow(__("row.position"), cb)
        form.addRow(__("row.color"), self.col_side_color_cb)
        box.layout().addWidget(annotbox)

        gui.checkBox(self.controlArea, self, "keep_aspect",
                     __("row.keep_aspect"), box=__("checkbox.resize"),
        ...
        self.__font_inc = QAction(
            __("lnk_menu.increase_font"), self, shortcut=QKeySequence("ctrl+>"))
        self.__font_dec = QAction(
            __("lnk_menu.decrease_font"), self, shortcut=QKeySequence("ctrl+<"))
    ...
    def _on_view_context_menu(self, pos):
        ...
        menu.addSeparator()
        a = QAction(__("lnk_menu.aspect_ratio"), menu, checkable=True)
    ...
    def send_report(self):
        self.report_items((
            (
            __("report.columns"), i18n.t("common.general.cluster") if self.col_clustering else __("report.no_sorting")),
            (__("report.rows"), i18n.t("common.general.cluster") if self.row_clustering else __("report.no_sorting")),
            (__("report.split"), self.split_by_var is not None and self.split_by_var.name),
            (__("report.row_annotation"), self.annotation_var is not None and self.annotation_var.name),
        ))
        self.report_plot()

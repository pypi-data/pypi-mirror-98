...
def __(key):
    return i18n.t("widget.visualize.visualize.owpythagoreanforest." + key)
...
class OWPythagoreanForest(OWWidget):
    ...
    class Inputs:
        random_forest = Input('Random Forest', RandomForestModel, replaces=["Random forest"],
                              label=i18n.t("widget.visualize.visualize.common.random_forest"))

    class Outputs:
        tree = Output('Tree', TreeModel, label=i18n.t("widget.visualize.visualize.common.tree"))
    ...
    SIZE_CALCULATION = [
        ...
        (__('gbox.logarithmic'), lambda x: log(x + 1)),
    ]
    ...
    def __init__(self):
        ...
        # CONTROL AREA
        # Tree info area
        box_info = gui.widgetBox(self.controlArea, __('box.forest'))
        self.ui_info = gui.widgetLabel(box_info)

        # Display controls area
        box_display = gui.widgetBox(self.controlArea, __('box.display'))
        self.ui_depth_slider = gui.hSlider(
            box_display, self, 'depth_limit', label=__('row.depth'), ticks=False,
        )  # type: QSlider
        ...
        self.ui_zoom_slider = gui.hSlider(
            box_display, self, 'zoom', label=__('row.zoom'), ticks=False, minValue=100,
    ...
    def _clear_info_box(self):
        self.ui_info.setText(__('info_input_no_forest'))

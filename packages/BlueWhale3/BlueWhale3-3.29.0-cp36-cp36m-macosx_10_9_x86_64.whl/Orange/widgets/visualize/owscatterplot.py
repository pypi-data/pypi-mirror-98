...
def __(key):
    return i18n.t("widget.visualize.visualize.owscatterplot." + key)


class ScatterPlotVizRank(VizRankDialogAttrPair):
    captionTitle = i18n.t("common.software.score_plot")
    minK = 10
    attr_color = None
...
class OWScatterPlot(OWDataProjectionWidget):
    ...
    class Inputs(OWDataProjectionWidget.Inputs):
        features = Input('Features', AttributeList, label=i18n.t("common.general.features"))
    ...
    class Warning(OWDataProjectionWidget.Warning):
        missing_coords = Msg(__("msg.plot_display_error"))
        no_continuous_vars = Msg(__("msg.no_continuous_variable"))

    class Information(OWDataProjectionWidget.Information):
        sampled_sql = Msg(__("msg.sql_table_showing"))
        missing_coords = Msg(__("msg.point_display_tip"))
    ...
    def _add_controls(self):
        ...
        gui.checkBox(
            ...
            label=__("checkbox.variable_independent"),
            callback=self.graph.update_regression_line,
            tooltip=__("checkbox.variable_independent_tip"),
    ...
    def _add_controls_axis(self):
        ...
        self.attr_box = gui.vBox(self.controlArea, __("box.axes"),
                                 spacing=2 if gui.is_macstyle() else 8)
        dmod = DomainModel
        self.xy_model = DomainModel(dmod.MIXED, valid_types=ContinuousVariable)
        self.cb_attr_x = gui.comboBox(
            self.attr_box, self, "attr_x", label=__("label.axis_x"),
            callback=self.set_attr_from_combo,
            model=self.xy_model, **common_options,
        )
        self.cb_attr_y = gui.comboBox(
            self.attr_box, self, "attr_y", label=__("label.axis_y"),
            callback=self.set_attr_from_combo,
            model=self.xy_model, **common_options,
        )
        vizrank_box = gui.hBox(self.attr_box)
        self.vizrank, self.vizrank_button = ScatterPlotVizRank.add_vizrank(
            vizrank_box, self, __("btn_informative_projection"), self.set_attr)

    def _add_controls_sampling(self):
        self.sampling = gui.auto_commit(
            self.controlArea, self, "auto_sample", "Sample", box=__("box.sample"),
    ...
    def _vizrank_color_change(self):
        ...
        if self.data is None:
            err_msg = __("msg.input_empty")
        elif self.data.is_sparse():
            err_msg = __("msg.data_sparse")
        elif len(self.xy_model) < 3:
            err_msg = __("msg.rank_error")
        elif self.attr_color is None:
            err_msg = __("msg.select_color_variable")
        elif np.isnan(self.data.get_column_view(
                self.attr_color)[0].astype(float)).all():
            err_msg = __("msg.no_value_color_variable")
    ...
    def set_subset_data(self, subset_data):
        ...
        if isinstance(subset_data, SqlTable):
            if subset_data.approx_len() < AUTO_DL_LIMIT:
                ...
                self.warning(__("msg.data_subset_error"))
                subset_data = None

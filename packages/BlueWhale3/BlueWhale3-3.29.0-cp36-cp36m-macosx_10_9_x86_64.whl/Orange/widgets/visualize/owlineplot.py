...
def __(key):
    return i18n.t("widget.visualize.visualize.owlineplot." + key)
...
class OWLinePlot(OWWidget):
    ...
    class Inputs:
        data = Input('Data', Table, default=True, label=i18n.t("widget.visualize.visualize.common.data"))
    ...
    class Error(OWWidget.Error):
        not_enough_attrs = Msg(__("msg.need_continuous_feature"))
        no_valid_data = Msg(__("msg.valid_data_error"))

    class Warning(OWWidget.Warning):
        no_display_option = Msg(__("msg.no_display_option"))

    class Information(OWWidget.Information):
        hidden_instances = Msg(__("msg.instances_value_error"))
        too_many_features = Msg(__("msg.show_first").format(MAX_FEATURES))
    ...
    def _add_controls(self):
        displaybox = gui.widgetBox(self.controlArea, __("box.display"))
        gui.checkBox(displaybox, self, "show_profiles", __("checkbox.lines"),
                     callback=self.__show_profiles_changed,
                     tooltip=__("checkbox.lines_tip"))
        gui.checkBox(displaybox, self, "show_range", __("checkbox.range"),
                     callback=self.__show_range_changed,
                     tooltip=__("checkbox.range_tip"))
        gui.checkBox(displaybox, self, "show_mean", __("checkbox.mean"),
                     callback=self.__show_mean_changed,
                     tooltip=__("checkbox.mean_tip"))
        gui.checkBox(displaybox, self, "show_error", __("checkbox.error_bars"),
                     callback=self.__show_error_changed,
                     tooltip=__("checkbox.error_bars_tip"))

        self.group_vars = DomainModel(
            placeholder=i18n.t("common.software.none"), separators=False, valid_types=DiscreteVariable)
        self.group_view = gui.listView(
            self.controlArea, self, "group_var", box=__("box.group_by"),

...
def __(key):
    return i18n.t("widget.visualize.visualize.owviolinplot." + key)
...
class OWViolinPlot(OWWidget):
    ...
    class Error(OWWidget.Error):
        no_cont_features = Msg(__("msg.no_cont_features"))
    ...
    def _add_controls(self):
        ...
        self._group_var_model = VariableListModel(placeholder=i18n.t("common.software.none"))
        sorted_model = SortProxyModel(sortRole=Qt.UserRole)
        ...
        box = gui.vBox(self.controlArea, __("box.variable"))
        box.layout().addWidget(self._value_var_view)
        gui.checkBox(box, self, "order_by_importance",
                     __("box.order_by_importance"),
                     tooltip=__("box.order_by_importance_tip"),
                     callback=self.apply_value_var_sorting)

        box = gui.vBox(self.controlArea, __("box.subgroups"))
        box.layout().addWidget(self._group_var_view)
        gui.checkBox(box, self, "order_grouping_by_importance",
                     __("box.order_grouping_by_importance"),
                     tooltip=__("box.order_grouping_by_importance_tip"),
                     callback=self.apply_group_var_sorting)

        box = gui.vBox(self.controlArea, __("box.display"),
                       sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Maximum))
        gui.checkBox(box, self, "show_box_plot", __("box.box_plot"),
                     callback=self.__show_box_plot_changed)
        gui.checkBox(box, self, "show_strip_plot", __("box.strip_plot"),
                     callback=self.__show_strip_plot_changed)
        gui.checkBox(box, self, "show_rug_plot", __("box.rug_plot"),
                     callback=self.__show_rug_plot_changed)
        self._order_violins_cb = gui.checkBox(
            box, self, "order_violins", __("box.order_subgroups"),
            callback=self.__order_violins_changed,
        )
        gui.radioButtons(box, self, "orientation_index",
                         [__("label.horizontal"), __("label.vertical")], label=__("label.orientation"),
                         orientation=Qt.Horizontal,
                         callback=self.__orientation_changed)

        box = gui.vBox(self.controlArea, __("box.density_estimation"),
                       sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Maximum))
        gui.comboBox(box, self, "kernel_index", items=[__("row.normal"), __("row.epanechnikov"), __("row.linear")],
                     label=__("box.kernel"), labelWidth=60, orientation=Qt.Horizontal,
                     callback=self.__kernel_changed)
        self._scale_combo = gui.comboBox(
            box, self, "scale_index", items=[__("row.area"), __("row.count"), __("row.width")],
            label=__("box.scale"), labelWidth=60, orientation=Qt.Horizontal,

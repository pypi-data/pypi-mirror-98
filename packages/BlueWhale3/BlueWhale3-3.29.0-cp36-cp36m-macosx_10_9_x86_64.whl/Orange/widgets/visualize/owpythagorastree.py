...
def __(key):
    return i18n.t("widget.visualize.visualize.owpythagorastree." + key)
...
class OWPythagorasTree(OWWidget):
    ...
    class Inputs:
        tree = Input('Tree', TreeModel, label=i18n.t("widget.visualize.visualize.common.tree"))
    ...
    def __init__(self):
        ...
        self.SIZE_CALCULATION = [
            (i18n.t("common.software.normal"), lambda x: x),
            (__('gbox.square_root'), lambda x: sqrt(x)),
            (__('gbox.logarithmic'), lambda x: log(x * self.size_log_scale + 1)),
        ]

        # CONTROL AREA
        # Tree info area
        box_info = gui.widgetBox(self.controlArea, __('box.tree_info'))
        self.infolabel = gui.widgetLabel(box_info)
        self.info.set_output_summary(self.info.NoOutput)

        # Display settings area
        box_display = gui.widgetBox(self.controlArea, __('box.display_setting'))
        self.depth_slider = gui.hSlider(
            box_display, self, 'depth_limit', label=__('label.depth'), ticks=False,
            callback=self.update_depth)
        self.target_class_combo = gui.comboBox(
            box_display, self, 'target_class_index', label=i18n.t("common.general.target_class"),
            orientation=Qt.Horizontal, items=[], contentsLength=8,
            searchable=True,
            callback=self.update_colors)
        self.size_calc_combo = gui.comboBox(
            box_display, self, 'size_calc_idx', label=i18n.t("common.software.size"),
        ...
        self.log_scale_box = gui.hSlider(
            ...
            label=__('label.log_scale_factor'), minValue=1, maxValue=100, ticks=False,
            callback=self.invalidate_tree)

        # Plot properties area
        box_plot = gui.widgetBox(self.controlArea, __('box.plot_property'))
        self.cb_show_tooltips = gui.checkBox(
            box_plot, self, 'tooltips_enabled', label=__('checkbox.enable_tooltip'),
            callback=self.update_tooltip_enabled)
        self.cb_show_legend = gui.checkBox(
            box_plot, self, 'show_legend', label=i18n.t("common.software.show_legend"),
            callback=self.update_show_legend)

        gui.rubber(self.controlArea)

        gui.button(self.buttonsArea, self, label=__("btn_redraw"), callback=self.redraw)
    ...
    def _update_info_box(self):
        self.infolabel.setText(__('info_node').format(self.tree_adapter.num_nodes) + '\n' + __('info_depth').format(
    ...
    def _clear_info_box(self):
        self.infolabel.setText(__('info_input_no_tree'))
    ...
    def _update_target_class_combo(self):
        ...
        if self.data.domain.has_discrete_class:
            label_text = __('label.target_class')
            values = [c.title() for c in self.data.domain.class_vars[0].values]
            values.insert(0, i18n.t("common.software.none"))
        else:
            label_text = __('label.node_color')

...
def __(key):
    return i18n.t("widget.visualize.visualize.owtreeviewer." + key)
...
class OWTreeGraph(OWTreeViewer2D):
    ...
    class Inputs:
        ...
        tree = Input('Tree', TreeModel, replaces=["Classification Tree", "Regression Tree"],
                     label=i18n.t("widget.visualize.visualize.common.tree"))
    ...
    def __init__(self):
        ...
        self.color_label = QLabel(__("label_target_class"))
    ...
    def ctree(self, model=None):
        ...
        if model is None:
            self.infolabel.setText(__('info.no_tree'))
            self.root_node = None
            self.dataset = None
            self.tree_adapter = None
        else:
            ...
            if class_var.is_discrete:
                self.color_label.setText(__("info.target_class"))
                self.color_combo.addItem(i18n.t("common.software.none"))
                self.color_combo.addItems(self.domain.class_vars[0].values)
                self.color_combo.setCurrentIndex(self.target_class_index)
            else:
                self.color_label.setText(__("info.color_by"))
            ...
            self.infolabel.setText(__('info.node_leave').format(
    ...
    def send_report(self):
        if not self.model:
            return
        items = [(__("report.tree_size"), self.infolabel.text()),
                 (__("report.edge_width"),
                  (__("report.fixed"), __("report.relative_to_root"), __("report.relative_to_parent"))[
                      # pylint: disable=invalid-sequence-index
                      self.line_width_method])]
        if self.domain.class_var.is_discrete:
            items.append((i18n.t("common.general.target_class"), self.color_combo.currentText()))
        elif self.regression_colors != self.COL_DEFAULT:
            items.append((__("report.color_by"), self.COL_OPTIONS[self.regression_colors]))

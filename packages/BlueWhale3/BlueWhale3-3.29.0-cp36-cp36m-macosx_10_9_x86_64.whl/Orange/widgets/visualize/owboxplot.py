...
def __(key):
    return i18n.t("widget.visualize.visualize.owboxplot." + key)
...
class OWBoxPlot(widget.OWWidget):
    ...
    class Inputs:
        data = Input('Data', Orange.data.Table, label=i18n.t("widget.visualize.visualize.common.data"))
    ...
    class Warning(widget.OWWidget.Warning):
        no_vars = widget.Msg(__("msg_no_vars"))
    ...
    def __init__(self):
        ...
        sorted_model.sort(0)
        box = gui.vBox(self.controlArea, __("box_variable"))
        ...
        gui.checkBox(
            ...
            __("checkbox.sort_subgroups"),
            tooltip=__("tooltip_sort_subgroups"),
            callback=self.apply_attr_sorting)

        self.group_vars = VariableListModel(placeholder=i18n.t("common.software.brackets_none"))
        sorted_model = SortProxyModel(sortRole=Qt.UserRole)
        sorted_model.setSourceModel(self.group_vars)
        sorted_model.sort(0)

        box = gui.vBox(self.controlArea, __("box_subgroups"))
        ...
        gui.checkBox(
            ...
            __("checkbox.sort_variable"),
            tooltip=__("tooltip_sort_variable_values"),
        ...
        self.display_box = gui.vBox(
            self.controlArea, i18n.t("common.software.display"),
            sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Maximum))

        gui.checkBox(self.display_box, self, "show_annotations", __("checkbox.annotate"),
                     callback=self.update_graph)
        self.compare_rb = gui.radioButtonsInBox(
            self.display_box, self, 'compare',
            btnLabels=[__("btn.no_comparison"), __("btn.compare_medians"), __("btn.compare_means")],
            callback=self.update_graph)

        # The vertical size policy is needed to let only the list views expand
        self.stretching_box = box = gui.vBox(
            self.controlArea, box=__("box_display"),
            sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Fixed))
        self.stretching_box.sizeHint = self.display_box.sizeHint
        gui.checkBox(
            box, self, 'stretched', __("checkbox.stretch_bars"),
            callback=self.update_graph,
            stateWhenDisabled=False)
        gui.checkBox(
            box, self, 'show_labels', __("checkbox.box_labels"),
            callback=self.update_graph)
        self.sort_cb = gui.checkBox(
            box, self, 'sort_freqs', __("checkbox.sort_subgroup_frequencies"),
            callback=self.update_graph,
            stateWhenDisabled=False)

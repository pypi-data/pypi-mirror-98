...
def __(key):
    return i18n.t("widget.model.model.owrules." + key)
...
class CustomRuleClassifier(_RuleClassifier):
    ...
    def __init__(self, domain, rule_list, params):
        ...
        self.rule_ordering = params[__("report.rule_ordering")]
        self.covering_algorithm = params[__("report.cover_algorithm")]
        self.params = params
...
class CustomRuleLearner(_RuleLearner):
    ...
    def __init__(self, preprocessors, base_rules, params):
        ...
        self.rule_ordering = params[__("box.rule_order")]

        # top-level control procedure (covering algorithm)
        self.covering_algorithm = params[__("box.cover_algorithm")]
        if self.covering_algorithm == "exclusive":
            ...
            self.gamma = params[__("report.gamma")]
            self.cover_and_remove = self.weighted_cover_and_remove

        # bottom-level search procedure (search algorithm)
        self.rule_finder.search_algorithm.beam_width = params[__("report.beam_width")]
        ...
        evaluation_measure = params[__("report.evaluation_measure")]
        ...
        min_rule_cov = params[__("report.minimum_rule_coverage")]
        max_rule_length = params[__("report.maximum_rule_length")]
        ...
        # bottom-level search procedure (over-fitting avoidance heuristics)
        default_alpha = params[__("report.default_alpha")]
        parent_alpha = params[__("report.parent_alpha")]
...
class OWRuleLearner(OWBaseLearner):
    ...
    def add_main_layout(self):
        ...
        middle_box = gui.vBox(widget=self.controlArea, box=__("box.rule_search"))

        gui.comboBox(
            widget=middle_box, master=self, value="evaluation_measure",
            label=__("row.evaluation_measure"), orientation=Qt.Horizontal,
            items=(__("gbox_entropy"), __("gbox_laplace_accuracy"), __("gbox_wracc")),
            callback=self.settings_changed, contentsLength=3)

        gui.spin(
            widget=middle_box, master=self, value="beam_width", minv=1,
            maxv=100, step=1, label=__("row.beam_width"), orientation=Qt.Horizontal,
            callback=self.settings_changed, alignment=Qt.AlignRight,
            controlWidth=80)

        # bottom-level search procedure (over-fitting avoidance bias)
        bottom_box = gui.vBox(widget=self.controlArea, box=__("box.rule_filter"))

        gui.spin(
            widget=bottom_box, master=self, value="min_covered_examples", minv=1,
            maxv=10000, step=1, label=__("row.min_rule_coverage"),
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80)

        gui.spin(
            widget=bottom_box, master=self, value="max_rule_length",
            minv=1, maxv=100, step=1, label=__("row.max_rule_length"),
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80)

        gui.doubleSpin(
            widget=bottom_box, master=self, value="default_alpha", minv=0.0,
            maxv=1.0, step=0.01, label=__("checkbox_statistical_significance"),
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80,
        ...
        gui.doubleSpin(
            ...
            maxv=1.0, step=0.01, label=__("checkbox_relative_significance"),
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80,
    ...
    def get_learner_parameters(self):
        return OrderedDict([
            (__("report.rule_ordering"), self.storage_orders[self.rule_ordering]),
            (__("report.cover_algorithm"), self.storage_covers[self.covering_algorithm]),
            (__("report.gamma"), self.gamma),
            (__("report.evaluation_measure"), self.storage_measures[self.evaluation_measure]),
            (__("report.beam_width"), self.beam_width),
            (__("report.minimum_rule_coverage"), self.min_covered_examples),
            (__("report.maximum_rule_length"), self.max_rule_length),
            (__("report.default_alpha"), (1.0 if not self.checked_default_alpha
                               else self.default_alpha)),
            (__("report.parent_alpha"), (1.0 if not self.checked_parent_alpha

...
def __(key):
    return i18n.t("widget.model.model.owgradientboosting." + key)


class LearnerItemModel(QStandardItemModel):
    LEARNERS = [
        (GBLearner, "", ""),
        (XGBLearner, __("row.extreme_gradient") + "(xgboost)", "xgboost"),
        (XGBRFLearner, __("row.extreme_gradient") + "(xgboost)",
         "xgboost"),
        (CatGBLearner,  __("row.gradient_boosting") + "(catboost)", "catboost"),
    ]
    ...
    def _add_data(self):
        for cls, opt_name, lib in self.LEARNERS:
            ...
            if not imported:
                item.setToolTip(__("row.tip").format(lib))
            self.appendRow(item)
...
class BaseEditor(QWidget, gui.OWComponent):
    ...
    def _add_main_layout(self):
        ...
        self.basic_box = gui.vBox(self._layout, __("box.basic_property"))
        gui.spin(
            self.basic_box, self, "n_estimators", 1, 10000,
            label=__("label.n_estimators"), **common_args
        )
        gui.doubleSpin(
            self.basic_box, self, "learning_rate", 0, 1, 0.001,
            label=__("label.learning_rate"), **common_args
        )
        gui.checkBox(
            self.basic_box, self, "random_state", label=__("label.random_state"),
            callback=self.settings_changed, attribute=Qt.WA_LayoutUsesWidgetRect
        )

        self.growth_box = gui.vBox(self._layout, __("box.growth_control"))
        gui.spin(
            self.growth_box, self, "max_depth", 1, 50,
            label=__("label.max_depth"), **common_args
        )

        self.sub_box = gui.vBox(self._layout, __("box.subsampling"))
...
class RegEditor(BaseEditor):
    ...
    def _add_main_layout(self):
        ...
        gui.widgetLabel(box, __("label.regularization"))
        gui.hSlider(
    ...
    def _set_lambda_label(self):
        self.lambda_label.setText(__("text.lambda").format(self.lambda_))
...
class GBLearnerEditor(BaseEditor):
    ...
    def _add_main_layout(self):
        ...
        gui.doubleSpin(
            ...
            label=__("label.subsample"),
            callback=self.settings_changed
        )
        # Growth control
        gui.spin(
            self.growth_box, self, "min_samples_split", 2, 1000,
            controlWidth=80, label=__("label.min_samples_split"),
...
class CatGBLearnerEditor(RegEditor):
    ...
    def _add_main_layout(self):
        ...
        gui.doubleSpin(
            ...
            controlWidth=80, alignment=Qt.AlignRight,
            label=__("label.colsample_bytree"),
            callback=self.settings_changed
        )
...
class XGBBaseEditor(RegEditor):
    ...
    def _add_main_layout(self):
        ...
        gui.doubleSpin(
            self.sub_box, self, "subsample", 0.05, 1, 0.05,
            label=__("label.subsample"), **common_args
        )
        gui.doubleSpin(
            self.sub_box, self, "colsample_bytree", 0.05, 1, 0.05,
            label=__("label.colsample_bytree"), **common_args
        )
        gui.doubleSpin(
            self.sub_box, self, "colsample_bylevel", 0.05, 1, 0.05,
            label=__("label.colsample_bylevel"), **common_args
        )
        gui.doubleSpin(
            self.sub_box, self, "colsample_bynode", 0.05, 1, 0.05,
            label=__("label.colsample_bynode"), **common_args
        )
...
class OWGradientBoosting(OWBaseLearner):
    ...
    def add_main_layout(self):
        ...
        box = gui.vBox(self.controlArea, __("box.method"))
        gui.comboBox(

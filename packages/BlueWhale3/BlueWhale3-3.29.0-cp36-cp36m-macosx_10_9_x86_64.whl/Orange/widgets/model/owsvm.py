...
def __(key):
    return i18n.t("widget.model.model.owsvm." + key)
...
class OWSVM(OWBaseLearner):
    ...
    class Outputs(OWBaseLearner.Outputs):
        support_vectors = Output("Support Vectors", Table, explicit=True,
                                 ...
                                 label=i18n.t("widget.model.model.common.support_vector"))

    class Warning(OWBaseLearner.Warning):
        sparse_data = Msg(__("msg_input_data_sparse_to_scale"))
    ...
    def _add_type_box(self):
        ...
        form.addWidget(QLabel(__("row.cost")), 0, 1, Qt.AlignRight)
        form.addWidget(self.c_spin, 0, 2)
        form.addWidget(QLabel(
            __("row.regression_loss_epsilon")), 1, 1, Qt.AlignRight)
        form.addWidget(self.epsilon_spin, 1, 2)

        self.nu_radio = gui.appendRadioButton(box, __("btn.v_svm"), addToLayout=False)
        ...
        form.addWidget(QLabel(__("row.regression_cost")), 2, 1, Qt.AlignRight)
        form.addWidget(self.nu_C_spin, 2, 2)
        form.addWidget(QLabel(__("row.complexity_bound")), 3, 1, Qt.AlignRight)
    ...
    def _add_kernel_box(self):
        ...
        box = gui.hBox(self.controlArea, __("box_kernel"))

        self.kernel_box = buttonbox = gui.radioButtonsInBox(
            box, self, "kernel_type", btnLabels={i18n.t("common.general.linear"), __("btn.polynomial"), __("btn.rbf"),
                                                 __("btn.sigmoid")},
        ...
        gui.label(parambox, self, __("row.kernel"))
        ...
        gamma.setSpecialValueText(__("row.auto"))
    ...
    def get_learner_parameters(self):
        items = OrderedDict()
        if self.svm_type == self.SVM:
            items[__("report.svm_type")] = "SVM, C={}, ε={}".format(self.C, self.epsilon)
        else:
            items[__("report.svm_type")] = "ν-SVM, ν={}, C={}".format(self.nu, self.nu_C)
        self._report_kernel_parameters(items)
        items[__("report.numerical_tolerance")] = "{:.6}".format(self.tol)
        items[__("report.iteration_limt")] = self.max_iter if self.limit_iter else __("report.unlimited")
        return items

    def _report_kernel_parameters(self, items):
        gamma = self.gamma or self._default_gamma
        if self.kernel_type == 0:
            items[__("report.kernel")] = i18n.t("common.general.linear")
        elif self.kernel_type == 1:
            items[__("report.kernel")] = __("report.polynomial").format(
                g=gamma, c=self.coef0, d=self.degree)
        elif self.kernel_type == 2:
            items[__("report.kernel")] = __("report.rbf").format(gamma)
        else:
            items[__("report.kernel")] = __("report.sigmoid").format(

...
def __(key):
    return i18n.t("widget.model.model.owlogisticregression." + key)
...
class OWLogisticRegression(OWBaseLearner):
    ...
    penalty_types = (__("gbox_lasso_l1"), __("gbox_ridge_l2"))
    penalty_types_short = ["l1", "l2"]

    class Warning(OWBaseLearner.Warning):
        class_weights_used = Msg(__("msg_class_weights_used"))

    def add_main_layout(self):
        # this is part of init, pylint: disable=attribute-defined-outside-init
        box = gui.widgetBox(self.controlArea, box=True)
        self.penalty_combo = gui.comboBox(
            box, self, "penalty_type", label=__("row_regularization_type"),
            items=self.penalty_types, orientation=Qt.Horizontal,
            callback=self.settings_changed)
        gui.widgetLabel(box, __("row_strength"))
        box2 = gui.hBox(gui.indentedBox(box))
        gui.widgetLabel(box2, __("label.weak")).setStyleSheet("margin-top:6px")
        self.c_slider = gui.hSlider(
            ...
            createLabel=False)
        gui.widgetLabel(box2, __("label.strong")).setStyleSheet("margin-top:6px")
        box2 = gui.hBox(box)
        ...
        self.weights = gui.checkBox(
            box, self,
            "class_weight", label=__("label.class_weight"),
            callback=self.settings_changed,
            tooltip=__("label.class_weight_tip")
        )
    ...
    def get_learner_parameters(self):
        return ((i18n.t("common.algorithm.regularization"), "{}, C={}, class weights={}".format(

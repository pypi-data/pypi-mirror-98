...
def __(key):
    return i18n.t("widget.model.model.owneuralnetwork." + key)
...
class OWNNLearner(OWBaseLearner):
    ...
    def add_main_layout(self):
        ...
        form.addRow(
            __("row.hidden_neuron"),
            gui.lineEdit(
                None, self, "hidden_layers_input",
                orientation=Qt.Horizontal, callback=self.settings_changed,
                tooltip=__("tooltip_define_neuron"),
                placeholderText=__("placeholder.eg")))
        form.addRow(
            __("row.activation"),
            gui.comboBox(
        ...
        form.addRow(
            gui.checkBox(
                None, self, "replicable", label=i18n.t('common.general.replicable_train'),
    ...
    def set_alpha(self):
        ...
        self.reg_label.setText(__("row.regularization").format(self.strength_C))
    ...
    def setup_layout(self):
        ...
        b = gui.button(self.apply_button, self, i18n.t("common.btn.cancel"),
    ...
    def get_learner_parameters(self):
        return ((__("report.hidden_layers"), ', '.join(map(str, self.get_hidden_layers()))),
                (__("report.activation"), self.act_lbl[self.activation_index]),
                (__("report.solver"), self.solv_lbl[self.solver_index]),
                (__("report.alpha"), self.alpha),
                (__("report.max_iterations"), self.max_iterations),
                (i18n.t('common.general.replicable_train'), self.replicable))

...
def __(key):
    return i18n.t("widget.evaluate.evaluate.owtestandscore." + key)
...
class State(enum.Enum):
    ...
    Waiting = __("status.wait")
    #: Executing/running the evaluations
    Running = i18n.t("common.software.run")
    #: Finished running evaluations (success or error)
    Done = __("status.done")
    #: Evaluation cancelled
    Cancelled = __("status.cancel")
...
class OWTestAndScore(OWWidget):
    ...
    class Inputs:
        train_data = Input("Data", Table, default=True, label=i18n.t("widget.evaluate.evaluate.common.data"))
        test_data = Input("Test Data", Table, label=i18n.t("widget.evaluate.evaluate.common.test_data"))
        learner = Input("Learner", Learner, multiple=True, label=i18n.t("common.general.learner"))
        preprocessor = Input("Preprocessor", Preprocess, label=i18n.t("common.general.preprocessor"))

    class Outputs:
        predictions = Output("Predictions", Table, label=i18n.t("widget.evaluate.evaluate.common.prediction"))
        evaluations_results = Output("Evaluation Results", Results, label=i18n.t("widget.evaluate.evaluate.common.evaluation_result"))

    settings_version = 3
    buttons_area_orientation = None
    UserAdviceMessages = [
        widget.Message(__("tooltip_click_table"), __("tooltip_click_header"))]
    ...
    TARGET_AVERAGE = __("placeholder_average_class")
    class_selection = settings.ContextSetting(TARGET_AVERAGE)

    class Error(OWWidget.Error):
        test_data_empty = Msg(__("msg.test_data_empty"))
        class_required_test = Msg(__("msg.class_required_test"))
        too_many_folds = Msg(__("msg.too_many_folds"))
        class_inconsistent = Msg(__("msg.class_inconsistent"))
        memory_error = Msg(__("msg.memory_error"))
        test_data_incompatible = Msg(__("msg.test_data_incompatible"))
        train_data_error = Msg("{}")

    class Warning(OWWidget.Warning):
        missing_data = \
            Msg(__("msg.remove_unknown_target_value"))
        test_data_missing = Msg(__("msg.miss_test_data_input"))
        scores_not_computed = Msg(__("msg.some_score_not_computed"))
        test_data_unused = Msg(__("msg.use_test_data"))
        cant_stratify = Msg(__("msg.cant_stratify"))

    class Information(OWWidget.Information):
        data_sampled = Msg(__("msg.train_data_sampled"))
        test_data_sampled = Msg(__("msg.test_data_sampled"))
        test_data_transformed = Msg(__("msg.test_match_train_data"))
        cant_stratify_numeric = Msg(__("msg.cant_stratify_numeric"))
    ...
    def __init__(self):
        ...
        sbox = gui.vBox(self.controlArea, __("box.sampling"))
        rbox = gui.radioButtons(
            sbox, self, "resampling", callback=self._param_changed)

        gui.appendRadioButton(rbox, __("btn.cross_validation"))
        ibox = gui.indentedBox(rbox)
        gui.comboBox(
            ibox, self, "n_folds", label=__("row_number_of_fold"),
            items=[str(x) for x in self.NFolds],
            orientation=Qt.Horizontal, callback=self.kfold_changed)
        gui.checkBox(
            ibox, self, "cv_stratified", __("checkbox_stratified"),
            callback=self.kfold_changed)
        gui.appendRadioButton(rbox, __("btn.feature_cross_validation"))
        ibox = gui.indentedBox(rbox)
        ...
        gui.appendRadioButton(rbox, __("btn.random_sampling"))
        ibox = gui.indentedBox(rbox)
        gui.comboBox(
            ibox, self, "n_repeats", label=__("row_repeat_train"),
            items=[str(x) for x in self.NRepeats], orientation=Qt.Horizontal,
            callback=self.shuffle_split_changed
        )
        gui.comboBox(
            ibox, self, "sample_size", label=__("row_train_set_size"),
            items=["{} %".format(x) for x in self.SampleSizes],
            orientation=Qt.Horizontal, callback=self.shuffle_split_changed
        )
        gui.checkBox(
            ibox, self, "shuffle_stratified", __("checkbox_stratified"),
            callback=self.shuffle_split_changed)

        gui.appendRadioButton(rbox, __("btn.leave_one_out"))

        gui.appendRadioButton(rbox, __("btn.train_data_test"))
        gui.appendRadioButton(rbox, __("btn.test_data_test"))

        self.cbox = gui.vBox(self.controlArea, i18n.t("common.general.target_Class"))
        ...
        self.modcompbox = box = gui.vBox(self.controlArea, __("box.model_comparison"))
        gui.comboBox(
        ...
        gui.checkBox(hbox, self, "use_rope",
                     __("checkbox_negligible_difference"),
        ...
        box = gui.vBox(self.mainArea, __("box.evaluation_result"))
        box.layout().addWidget(self.score_table.view)

        self.compbox = box = gui.vBox(self.mainArea, box=__("box.model_comparison"))
        ...
        box.layout().addWidget(QLabel(
            __("tooltip_table_show_probability"), wordWrap=True))
    ...
    def _update_compbox_title(self):
        ...
        if criterion < len(self.scorers):
            ...
            self.compbox.setTitle(__("box.model_comparison_name").format(scorer.name))
        else:
            self.compbox.setTitle(__("box.model_comparison"))
        ...
        for key, slot in self.learners.items():
            ...
            if isinstance(results, Try.Fail):
                ...
                if isinstance(results.exception, DomainTransformationError) \
                    ...
                    errors.append(__("msg.failed_with")

...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owhierarchicalclustering." + key)
...
class OWHierarchicalClustering(widget.OWWidget):
    ...
    class Inputs:
        distances = Input('Distances', Orange.misc.DistMatrix, label=i18n.t("common.general.distance"))

    class Outputs:
        selected_data = Output('Selected Data', Orange.data.Table, default=True,
                               label=i18n.t("widget.unsupervised.unsupervised.common.select_data"))
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Orange.data.Table, label=i18n.t("widget.visualize.visualize.common.data"))
    ...
    basic_annotations = [i18n.t("common.software.none"), i18n.t("common.software.enumeration")]

    class Error(widget.OWWidget.Error):
        not_finite_distances = Msg(__("msg_distance_infinite"))
    ...
    def __init__(self):
        ...
        gui.comboBox(
            self.controlArea, self, "linkage",
            items=[__("gbox.single"), __("gbox.average"), __("gbox.weighted"), __("gbox.complete"), __("gbox.ward")],
            box=__("box.linkage"),
        ...
        box = gui.widgetBox(self.controlArea, __("box.annotation"))
        ...
        box = gui.radioButtons(
            self.controlArea, self, "pruning", box=__("box.pruning"),
            callback=self._invalidate_pruning)
        grid = QGridLayout()
        box.layout().addLayout(grid)
        grid.addWidget(
            gui.appendRadioButton(box, i18n.t("common.software.none"), addToLayout=False),
            0, 0
        )
        ...
        grid.addWidget(
            gui.appendRadioButton(
                self.selection_box, __("row.manual"), addToLayout=False),
            0, 0
        )
        grid.addWidget(
            gui.appendRadioButton(
                self.selection_box, __("row.height_ratio"), addToLayout=False),
            1, 0
        )
        ...
        grid.addWidget(
            gui.appendRadioButton(
                self.selection_box, __("row.top_N"), addToLayout=False),
            2, 0
        )
        ...
        self.zoom_slider = gui.hSlider(
            self.buttonsArea, self, "zoom_factor", box=__("box.zoom"),
    ...
    def _set_items(self, items, axis=1):
        ...
        if isinstance(items, Orange.data.Table) and axis:
            ...
            if items.domain.class_vars:
                ...
                self.annotation = i18n.t("common.software.enumeration")
    ...
    def _update_labels(self):
        ...
        if self.root and self._displayed_root:
            ...
            if self.annotation == i18n.t("common.software.none"):
                labels = []
            elif self.annotation == i18n.t("common.software.enumeration"):

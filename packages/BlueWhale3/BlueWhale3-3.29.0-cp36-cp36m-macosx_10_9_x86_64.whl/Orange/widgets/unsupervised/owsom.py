...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owsom." + key)
...
class OWSOM(OWWidget):
    ...
    class Outputs:
        selected_data = Output('Selected Data', Table, default=True, label=i18n.t("widget.unsupervised.unsupervised.common.select_data"))
    ...
    class Warning(OWWidget.Warning):
        ignoring_disc_variables = Msg(__("msg.ignore_discrete_variables"))
        missing_colors = \
            Msg(__("msg.undefined_value"))
        missing_values = \
            Msg(__("msg.not_show"))
        single_attribute = Msg(__("msg.contains_numeric_column"))

    class Error(OWWidget.Error):
        no_numeric_variables = Msg(__("msg.no_numeric_columns"))
        no_defined_rows = Msg(__("msg.rows_contain_undefined_value"))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, box=__("box_som"))
        shape = gui.comboBox(
            box, self, "", items=(__("gbox.hexagonal_grid"), __("gbox.square_grid")))
        shape.setCurrentIndex(1 - self.hexagonal)

        box2 = gui.indentedBox(box, 10)
        auto_dim = gui.checkBox(
            box2, self, "auto_dimension", __("gbox.set_dimensions"),
        ...
        initialization = gui.comboBox(
            ...
            items=(__("gbox.initialize_PCA"), __("gbox.random_initialization"),
                   __("gbox.replicable_random")))

        start = gui.button(
            box, self, __("btn_restart"), callback=self.restart_som_pressed,
            sizePolicy=(QSizePolicy.MinimumExpanding, QSizePolicy.Fixed))

        self.opt_controls = self.OptControls(
            shape, auto_dim, spin_x, spin_y, initialization, start)

        box = gui.vBox(self.controlArea, __("box_color"))
        gui.comboBox(
            box, self, "attr_color", searchable=True,
            callback=self.on_attr_color_change,
            model=DomainModel(placeholder=__("gbox.same_color"),
                              valid_types=DomainModel.PRIMITIVE))
        gui.checkBox(
            box, self, "pie_charts", label=__("checkbox_pie_charts"),
            callback=self.on_pie_chart_change)
        gui.checkBox(
            box, self, "size_by_instances", label=__("checkbox_size_instances"),
    ...
    def enable_controls(self, enable):
        ...
        c.start.setText(__("btn_start") if enable else __("btn_stop"))
    ...
    def _draw_same_color(self, sizes):
        ...
        for y in range(self.size_y):
            for x in range(self.size_x - self.hexagonal * (y % 2)):
                ...
                ellipse.setToolTip(__("tooltip_instances").format(n))
    ...
    def send_report(self):
        ...
        if self.attr_color:
            self.report_caption(
                __("Self-organizing map colored by '{}'").format(self.attr_color.name))

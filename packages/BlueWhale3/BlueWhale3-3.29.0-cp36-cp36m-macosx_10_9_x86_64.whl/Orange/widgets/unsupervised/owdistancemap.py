...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owdistancemap." + key)
...
class OWDistanceMap(widget.OWWidget):
    ...
    class Outputs:
        ...
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Orange.data.Table, label=i18n.t("widget.data.data.common.data"))
        features = Output('Features', widget.AttributeList, dynamic=False, label=i18n.t("common.general.features"))
    ...
    def __init__(self):
        ...
        self.sorting_cb = gui.comboBox(
            self.controlArea, self, "sorting", box=__("box.element_sort"),
            items=[i18n.t("common.software.none"), i18n.t("common.general.cluster"), __("gbox.order_leave")],
            callback=self._invalidate_ordering)

        box = gui.vBox(self.controlArea, __("box.colors"))
        ...
        self.annot_combo = gui.comboBox(
            self.controlArea, self, "annotation_idx", box=__("box.annotation"),
        ...
        self.annot_combo.model()[:] = [i18n.t("common.software.none"), i18n.t("common.software.enumeration")]
    ...
    def set_distances(self, matrix):
        ...
        if matrix is not None:
            N, _ = matrix.shape
            if N < 2:
                self.error(__("err_empty_matrix"))
                matrix = None
    ...
    def set_items(self, items, axis=1):
        ...
        if items is None:
            model[:] = [i18n.t("common.software.none"), i18n.t("common.software.enumeration")]
        elif not axis:
            model[:] = [i18n.t("common.software.none"), i18n.t("common.software.enumeration"), __("gbox.attribute_name")]
        elif isinstance(items, Orange.data.Table):
            annot_vars = list(filter_visible(items.domain.variables)) + list(items.domain.metas)
            model[:] = [i18n.t("common.software.none"), i18n.t("common.software.enumeration")] + annot_vars
            self.annotation_idx = 0
            self.openContext(items.domain)
        elif isinstance(items, list) and \
                all(isinstance(item, Orange.data.Variable) for item in items):
            model[:] = [i18n.t("common.software.none"), i18n.t("common.software.enumeration"), __("gbox.name")]
        else:
            model[:] = [i18n.t("common.software.none"), i18n.t("common.software.enumeration")]
    ...
    def _update_labels(self, ):
        ...
        elif self.annot_combo.model()[self.annotation_idx] == __("gbox.attribute_name"):
    ...
    def send_report(self):
        ...
        if self.annotation_idx <= 1:
            annot = annot.lower()
        self.report_items((
            (__("gbox.sorting"), self.sorting_cb.currentText().lower()),
            (__("box.annotation"), annot)
        ))
        if self.matrix is not None:
            self.report_plot()

...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owtsne." + key)
...
class TSNERunner:
    ...
    def compute_pca(task, state, **_):
        ...
        state.set_status(__("state.computing_PCA"))
    ...
    def compute_initialization(task, state, **_):
        ...
        state.set_status(__("state.preparing_initialization"))
    ...
    def compute_affinities(task, state, **_):
        # Compute affinities
        state.set_status(__("state.find_nearest_neighbors"))
    ...
    def compute_tsne(task, state, progress_callback=None):
        ...
        state.set_status(__("state.running_optimization"))
...
class OWtSNE(OWDataProjectionWidget, ConcurrentWidgetMixin):
    ...
    class Information(OWDataProjectionWidget.Information):
        modified = Msg(__("msg.modify_settings"))

    class Error(OWDataProjectionWidget.Error):
        not_enough_rows = Msg(__("msg.input_data_rows"))
        not_enough_cols = Msg(__("msg.input_data_attributes"))
        constant_data = Msg(__("msg.input_data_constant"))
        no_valid_data = Msg(__("msg.no_projection_error"))
    ...
    def _add_controls_start_box(self):
        ...
        form.addRow(__("row.perplexity"), self.perplexity_spin)
        form.addRow(gui.checkBox(
            box, self, "multiscale", label=__("row.global_structure"),
        ...
        form.addRow(__("row.exaggeration"), sbe)
        ...
        form.addRow(__("row.components"), sbp)

        self.normalize_cbx = gui.checkBox(
            box, self, "normalize", __("checkbox_normalize_data"),
        ...
        self.run_button = gui.button(box, self, __("btn.start"), callback=self._toggle_run)

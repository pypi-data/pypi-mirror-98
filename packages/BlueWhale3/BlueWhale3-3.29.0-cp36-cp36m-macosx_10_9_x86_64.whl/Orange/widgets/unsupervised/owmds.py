...
def __(key):
    return i18n.t("widget.unsupervised.unsupervised.owmds." + key)
...
def run_mds(matrix: DistMatrix, max_iter: int, step_size: int, init_type: int,
    ...
    iterations_done = 0
    init = embedding
    state.set_status(__("set_status_running"))
...
class OWMDS(OWDataProjectionWidget, ConcurrentWidgetMixin):
    ...
    class Inputs(OWDataProjectionWidget.Inputs):
        distances = Input('Distances', DistMatrix, label=i18n.t("common.general.distance"))
    ...
    RefreshRate = [
        (__("gbox.iteration"), 1),
        (__("gbox.five_steps"), 5),
        (__("gbox.ten_steps"), 10),
        (__("gbox.twenty-five_steps"), 25),
        (__("gbox.fifty_steps"), 50),
        (i18n.t("common.software.none"), -1)
    ]
    ...
    class Error(OWDataProjectionWidget.Error):
        not_enough_rows = Msg(__("msg.input_data_line"))
        matrix_too_small = Msg(__("msg.input_matrix_format"))
        no_attributes = Msg(__("msg.no_attributes"))
        mismatching_dimensions = \
            Msg(__("msg.dimensions_error"))
        out_of_memory = Msg(__("msg.insufficient_memory"))
        optimization_error = Msg(__("msg.optimization_error"))
    ...
    def _add_controls(self):
        ...
        self.gui.add_control(
            self._effects_box, gui.hSlider, __("row_show_pairs"),
    ...
    def _add_controls_optimization(self):
        ...
        gui.button(hbox, self, i18n.t("common.algorithm.PCA"), callback=self.do_PCA, autoDefault=False)
        gui.button(hbox, self, i18n.t("common.general.randomize"), callback=self.do_random, autoDefault=False)
        gui.button(hbox, self, __("btn.jitter"), callback=self.do_jitter, autoDefault=False)
        gui.comboBox(box, self, "refresh_rate", label=__("row_refresh"),
                     orientation=Qt.Horizontal,
                     items=[t for t, _ in OWMDS.RefreshRate],
                     callback=self.__refresh_rate_combo_changed)
        self.run_button = gui.button(box, self, __("btn.start"), self._toggle_run)
    ...
    def _toggle_run(self):
        if self.task is not None:
            self.cancel()
            self.run_button.setText(i18n.t("common.software.resume"))
            self.commit()
        else:
            self._run()
    ...
    def _run(self):
        if self.effective_matrix is None \
            ...
            return
        self.graph.pause_drawing_pairs()
        self.run_button.setText(__("btn.stop"))
    ...
    def on_done(self, result: Result):
        ...
        self.run_button.setText(__("btn.start"))
        self.commit()

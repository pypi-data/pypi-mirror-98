...
from mldock.platform_helpers.aws.sagemaker import environment as sagemaker_environment
...
def run_training():
    ...
    environment = sagemaker_environment.Environment()

    PARSER= argparse.ArgumentParser()
    ## User set params which link to Sagemaker params
    PARSER.add_argument('--training', type=str,
                        default=environment.channel_input_dirs["training"],
                        help='Expects a s3 path to directory containing a sparse .npz file containing item user matrix')
    ...
    if ARGS.run_eval:
        ...
        logger.info("Your data is here = {}".format(ARGS.training))

...
# from multiprocessing import Process, Manager
# from algoRsiBb import AlgoRsiBb as Algo
# from openpyxl import load_workbook
# import copy
# 
...
def main():
    ...
    while i < int(sheet.max_row):
        ...
        # exit()
        algo = Algo(sheet[i][0].value, datasource, sheet[i][1].value, tickers, sheet[i][3].value, sheet[i][4].value, sheet[i][5].value, sheet[i][6].value, sheet[i][7].value, sheet[i][8].value  )
        
        # check if the algo has prev day values in the spread sheet
        try:
            if sheet[i][9].value != None:
                algo.wallet = sheet[i][9].value
            else:
                raise Exception("None wallet found in csv")
            # algo.positions[sheet[i][2].value] = sheet[i][8].value
    ...
    while 0 < len(thread_list):
        ...
        for thread in thread_list:
            if not thread.is_alive():
                ...
                sheet[i][9].value = ret[0]
                pos = ret[1]
                total_shares = 0
                for shares in list(pos.values()):
                    total_shares += i
                sheet[i][10].value = total_shares
                share_val = 0
                for p in pos:
                    share_val += pos[p] * algo_finished._get_last_trade(p)
                sheet[i][11].value = share_val
                sheet[i][12].value = share_val + algo_finished.wallet            

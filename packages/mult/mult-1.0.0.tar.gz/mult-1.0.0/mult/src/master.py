...
def default_popen_call_func(slave_filename, job_id):
...
# ジョブの投入
def throw_job(job, data_dir, slave_filename, popen_call_func):
	# ジョブ識別記号
	job_id = job["job_id"]
	# 入力オブジェクト
	...
	# popenをたたく処理
	popen_call_func(slave_filename, job_id)
...
def call(
	...
	progress = False,	# 進捗表示
	popen_call_func = None	# popenをたたく際のカスタム処理 (デフォルトのNone指定で単に「python」コマンドを使う; 引数はslave_filenameおよびjob_idx)
):
	# デフォルトのpopenの場合
	...
	throw_job_partial = lambda job: throw_job(job, data_dir, slave_filename, popen_call_func)	# ジョブの投入
	# jobを列挙
	...
	while True:
		# 進捗表示
		if progress is True: show_progress(job_pool)	# 進捗表示関数
		# 完了判定
		done_n = job_pool.count(status = "done")	# 指定ステータスのジョブの数をカウントする

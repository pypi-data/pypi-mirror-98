...
class Distribution(ABC):
    ...
    def load(json, name):
        ...
        if isinstance(json, list) and isinstance(json[0], str):
            if json[0] == 'n':
                return NormalDistribution(json[1], json[2])
            else:
...
class NormalDistribution(Distribution):
    ...
    def __init__(self, mean_polynomial_coefficient: list, std_polynomial_coefficient: list):
    ...
    def sample(self, x: float = 1):
        return np.random.normal(self.mean.calculate(x), self.std.calculate(x))

...
from utils_minan.TimeCalculator import time_calculator
try:
    import torch as th
    has_torch = True
except ImportError:
...
class ELSPEnv(gym.Env):
    ...
    def __init__(self, env_path, cfg_path, demand_scale=1):
        ...
        self.action_space = gym.spaces.Discrete(len(self.scheduling_unit.scheduling.scheduling_valid_prod))
...
if __name__ == "__main__":
    ...
    env = ELSPEnv(env_path="../elsp_env_00001", cfg_path="..", demand_scale=1)
    ...
    for i in range(1):
        ...
        action = 1
        while True:
            obs, reward, done, inf = env.step(1 if False else action % 4)
            # print(action % 4, obs)
            # 36.04320240231678

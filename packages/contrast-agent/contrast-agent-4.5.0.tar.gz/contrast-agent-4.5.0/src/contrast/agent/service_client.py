...
class _ServiceState(Singleton):
    ...
    def _send_messages(self, messages):
        ...
        if self.resend_startup:
            logger.debug("Resending startup messages")
            messages = self.build_initial_messages() + messages
    ...
    def _build_server_start_message(self, settings):
    ...
    def _build_startup_message(self, settings):
    ...
    def build_initial_messages(self):
        ...
        return [
            self._build_server_start_message(settings),
            self._build_startup_message(settings),
        ]
...
def send_messages_get_responses(messages):
    ...
    return _ServiceState().send_messages_retry(messages, raise_exception=False)
...
def send_startup_messages():
    ...
    responses = _ServiceState().initialize_and_send_messages()
    if responses:

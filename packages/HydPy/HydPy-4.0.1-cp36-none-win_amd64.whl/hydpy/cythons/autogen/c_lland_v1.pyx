...
cdef class InputSequences:
    ...
    cdef public bint _glob_inputflag
    cdef double *_glob_inputpointer
    ...
    cpdef inline void load_data(self, int idx)  nogil:
        ...
        if self._glob_inputflag:
            self.glob = self._glob_inputpointer[0]
        elif self._glob_diskflag:
    ...
    cpdef inline set_pointerinput(self, str name, pointerutils.PDouble value):
        ...
        if name == "glob":
            self._glob_inputpointer = value.p_value
@cython.final
...
cdef class Model:
    ...
    cpdef inline void calc_et0_v1(self)  nogil:
        cdef int k
        for k in range(self.parameters.control.nhru):
            self.sequences.fluxes.et0[k] = self.parameters.control.ke[k] * (                (8.64 * self.sequences.inputs.glob + 93.0 * self.parameters.control.kf[k])                * (self.sequences.fluxes.tkor[k] + 22.0)                / (                    165.0                    * (self.sequences.fluxes.tkor[k] + 123.0)                    * (1.0 + 0.00019 * min(self.parameters.control.hnn[k], 600.0))                )            )

...
class Plotter:
    ...
    def __init__(self, path, file_suffix: str = "_results_fit.csv",
                 data_results_dict: dict = None, model_name: str = None):
        """
...
class MonteCarloPlotter(Plotter):
    ...
    Used by cvnn.data_analysis.MonteCarloAnalyzer.

    As cvnn.data_analysis.Plotter, this class is used to plot evolution graphs with respect to each epoch like loss
        or accuracy but uses the average and/or median result of a monte carlo simulation.
    It also enables the plot of the same figures but with confidence intervals.
    """

    def __init__(self, path):
        """
        :param path: Path where with all the csv files that ends with `_statistical_result.csv`.
            These csv are generated by cvnn.montecarlo.MonteCarlo.
        """
    ...
    def plot_line_confidence_interval(self, key='accuracy', showfig=False, savefig=True, library='matplotlib',
        ...
        :param full_border:
        :param x_axis: What to use for the x_axis, by default, epoch (Not recommended to change)
        :param extension: file extensions (default svg) to be used when saving the file (ignored if library is plotly).
        """
        if library == 'plotly':

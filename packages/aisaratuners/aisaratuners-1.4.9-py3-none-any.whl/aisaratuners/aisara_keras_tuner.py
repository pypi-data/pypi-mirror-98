...
class HpOptimization:
    ...
    def plot_opti_results(self):
        ...
        if self.opti_object_main == 'max':
            min_max_opti_para_main = [para_plot_opti.iloc[self.num_trials * (i):self.num_trials * (i + 1), -4].max() for i in range(int(to_fig_opti_results['Round'][-1].split('_')[-1]))]
            min_max_opti_para_ploting = [max(min_max_opti_para_main[:i + 1]) for i in range(len(min_max_opti_para_main))]
        else:
            min_max_opti_para_main = [para_plot_opti.iloc[self.num_trials * (i):self.num_trials * (i + 1), -4].min() for i in range(int(to_fig_opti_results['Round'][-1].split('_')[-1]))]

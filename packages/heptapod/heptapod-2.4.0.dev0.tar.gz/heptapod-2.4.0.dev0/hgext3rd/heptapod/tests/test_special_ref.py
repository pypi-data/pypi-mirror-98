...
def test_write_delete_gitlab_special_ref(tmpdir):
    ...
    assert read_gitlab_typed_refs(repo, 'special') == {ref_name: base.hex()}
    ...
    assert read_gitlab_typed_refs(repo, 'special') == {ref_name: ctx1.hex()}
    assert special_refs(repo)[ref_name] == ctx1.hex()

    # deletion cases
    assert delete_gitlab_special_ref(repo, ref_name) is True
    assert read_gitlab_typed_refs(repo, 'special') == {}  # without cache

...
class RedisClusterTrackerStore(TrackerStore):
    ...
    def save(self, tracker: DialogueStateTracker, timeout=None, stream_only=False):
        ...
        while current_retries < self.max_retries:
            try:
                self.red_cluster.set(tracker.sender_id, serialised_tracker, ex=timeout)
                ...
                logger.warning(f'[REDIS_CLUSTER_ERROR] - Failed to execute Redis commands, retry in {sleep_time}s. Error: {exp}')
...
class FailSafeTrackerStore(TrackerStore):
    ...
    def is_healthy(self):
        return self._tracker_store.is_healthy()
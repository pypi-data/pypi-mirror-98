...
class GraphicsObject:
    ...
    def _reconfig(self, option, setting):
        ...
        if option not in self.config:
            raise GraphicsError(f"\n\nThe config you have specified ({setting} for {option}) is not valid for {self}")
    ...
    def _update_layer(self):
        if self.drawn:
            ...
            layer_index = 0
            for layer in GraphicsObject.object_layers:
                if layer_index > self.layer:
                    for obj in layer:
                        if obj.drawn and obj not in GraphicsObject.redraw_on_frame[layer_index]:
                            GraphicsObject.redraw_on_frame[layer_index].append(obj)
    ...
    def animate_resize(self, x, y, time=1, easing_x=py_ease_linear(), easing_y=None, allow_duplicate=True,
        ...
        if len(self.animation_queues["resizing"]) > 0:
            initial = [
                self.animation_queues["resizing"][-1]["Initial"][0] + self.animation_queues["resizing"][-1]["Change"][
                    0],
                self.animation_queues["resizing"][-1]["Initial"][1] + self.animation_queues["resizing"][-1]["Change"][
                    1]]
    ...
    def animate_resize_factor(self, x, y, time=1, easing_x=py_ease_linear(), easing_y=None, allow_duplicate=True,
        ...
        animation_data = {"Time": time, "Start": start, "Update": start, "Initial": initial,
                          "Change": [(initial[0] * x) - initial[0], (initial[1] * y) - initial[1]], "EasingX": easing_x,
                          "EasingY": easing_y}
        ...
        for obj in GraphicsObject.animating_objects["resizing"]:
            if obj.graphwin == graphwin and obj.drawn:
                if t - obj.animation_queues["resizing"][0]["Start"] >= obj.animation_queues["resizing"][0]["Time"]:
                    obj.resize(
                        obj.animation_queues["resizing"][0]["Initial"][0] +
                        obj.animation_queues["resizing"][0]["Change"][0],
                        obj.animation_queues["resizing"][0]["Initial"][1] +
                        obj.animation_queues["resizing"][0]["Change"][1])
                    ...
                    perX = obj.animation_queues["resizing"][0]["EasingX"](
                        (t - obj.animation_queues["resizing"][0]['Start']) / obj.animation_queues["resizing"][0][
                            'Time'])
    ...
    def on_left_press(graphwin):
        ...
        for obj in GraphicsObject.objects:
            if obj.graphwin == graphwin:
                if (obj.is_draggable[0] or obj.is_draggable[1]) and obj.is_clicked(mouse_pos):

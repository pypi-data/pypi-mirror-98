...
class ColourCMYK(Colour):
    ...
    def __getitem__(self, item):
        return [self.cyan, self.magenta, self.yellow, self.key][item]
...
class ColourHSV(Colour):
    ...
    def __getitem__(self, item):
        return [self.hue, self.saturation, self.value][item]
...
class ColourHSL(Colour):
    ...
    def __getitem__(self, item):
        return [self.hue, self.saturation, self.luminance][item]
    ...
    if isinstance(colour_start, Colour):
        if isinstance(colour_end, Colour):
            if isinstance(divisions, int):
                red_change = (colour_end.red - colour_start.red) / (divisions - 1)
                green_change = (colour_end.green - colour_start.green) / (divisions - 1)
                blue_change = (colour_end.blue - colour_start.blue) / (divisions - 1)

                cur_red = colour_start.red
                cur_green = colour_start.green
                cur_blue = colour_start.blue

                gradient = []
                for i in range(divisions - 1):
                    cur_red = int(colour_start.red + i * red_change)
                    cur_green = int(colour_start.green + i * green_change)
                    cur_blue = int(colour_start.blue + i * blue_change)

                    gradient.append(ColourRGB(max(cur_red, 0), max(cur_green, 0), max(cur_blue, 0)))
...
def color_gradient(color_start=ColorRGB(255, 255, 255), color_end=ColorRGB(0, 0, 0), divisions=10):
    if isinstance(color_start, Color):
        if isinstance(color_end, Color):
            if isinstance(divisions, int):
                red_change = (color_end.red - color_start.red) / (divisions - 1)
                green_change = (color_end.green - color_start.green) / (divisions - 1)
                blue_change = (color_end.blue - color_start.blue) / (divisions - 1)

                cur_red = color_start.red
                cur_green = color_start.green
                cur_blue = color_start.blue

                gradient = []
                for i in range(divisions - 1):
                    cur_red = int(color_start.red + i * red_change)
                    cur_green = int(color_start.green + i * green_change)
                    cur_blue = int(color_start.blue + i * blue_change)

                    gradient.append(ColorRGB(max(cur_red, 0), max(cur_green, 0), max(cur_blue, 0)))
...
def blend_colour_linear(start, end, t):
    if isinstance(start, Colour):
        if isinstance(end, Colour):
            return ColourRGB(round(linear_interpolation(start.red, end.red, t)),
                             round(linear_interpolation(start.green, end.green, t)),
                             round(linear_interpolation(start.blue, end.blue, t)))
        else:
...
def blend_colour_cosine(start, end, t):
    if isinstance(start, Colour):
        if isinstance(end, Colour):
            return ColourRGB(round(cosine_interpolation(start.red, end.red, t)),
                             round(cosine_interpolation(start.green, end.green, t)),
                             round(cosine_interpolation(start.blue, end.blue, t)))
        else:
            raise GraphicsError(f"\n\nGraphicsError: end colour for blending must be a Colour instance, not {end}")
    else:
        raise GraphicsError(f"\n\nGraphicsError: start colour for blending must be a Colour instance, not {start}")
...
def blend_colour_cubic(start, end, t):
    if isinstance(start, Colour):
        if isinstance(end, Colour):
            return ColourRGB(round(cubic_interpolation(start.red, start.red, end.red,
                                                       end.red, t)),
                             round(cubic_interpolation(start.green, start.green, end.green,
                                                       end.green, t)),
                             round(cubic_interpolation(start.blue, start.blue, end.blue,
                                                       end.blue, t)))
        else:
            raise GraphicsError(f"\n\nGraphicsError: end colour for blending must be a Colour instance, not {end}")
    else:
...
def blend_colour_hermite(start, end, t, tension=1, bias=0):
    if isinstance(start, Colour):
        if isinstance(end, Colour):
            return ColourRGB(round(hermite_interpolation(start.red, start.red, end.red,
                                                         end.red, t, tension=tension, bias=bias)),
                             round(hermite_interpolation(start.green, start.green, end.green,
                                                         end.green, t, tension=tension, bias=bias)),
                             round(hermite_interpolation(start.blue, start.blue, end.blue,

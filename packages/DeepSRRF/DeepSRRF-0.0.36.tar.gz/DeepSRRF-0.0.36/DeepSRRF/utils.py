...
def RF_to_Bmode(rf, bmode =False, bmode_range=70, log_coef=10, normalize = True):
    hlb = hilbert(rf)
    env = np.abs(hlb)
    ...
    if not bmode:
        return log_coef * np.log10(env)

    else:
        l = log_coef * np.log10(env)
        l = 255 * (l + bmode_range) / bmode_range
        l[np.isneginf(l)] = 0
        #
...
def save_model(path, model, elapsed_time):
    """
...
def prepare_image_for_model(img_input, channels=1, ordering_style='tf'):
    """
    ...
    if ordering_style == 'th': # if THEANO style image ordering
...
def augment_image(image, options):
    ...
    if 'fliplr' in options: # flip left to rigt

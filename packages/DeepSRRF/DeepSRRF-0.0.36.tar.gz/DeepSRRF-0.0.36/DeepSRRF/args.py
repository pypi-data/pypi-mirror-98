...
for the of DeepSR object (DeepSRRF.py)
...
parser.add_argument("--activation", type=str, choices=["relu", "tanh", "sigmoid", "elu", "selu", "elu", "exponential"], default='relu',
...
parser.add_argument('--bmode', action='store_true', default=False,
                    help='Boolean. Test to be implemented after RF signal have been converted \
                         to bmode image. Otherwise test is implemented \
                         on the RF signal after logarithmic compression.')

parser.add_argument("--bmode_range", type=float, default=70.0,
                    help="Float. Range value for convertion RF to B-mod. It affects the contrast of the image. Default is 70.0 ")
...
parser.add_argument("--earlystoppingpatience", type=int, default=5,
                    help="Integer. The number of training epochs to wait before early stop if the model does not progress \
...
parser.add_argument("--log_coef", type=float, default=10.0,
                    help="Float. Coefficient (multiplication factor) of log10 function for envelope compression. Default is 10.0 ")
...
parser.add_argument("--predict", type=int, default=40,
                    help='Range value for conversion of RF data to bmode image. Default is 40.')
...
parser.add_argument('--rebuild', action='store_true', default=False,
                    help='Boolean. If its True, model is being rebuilt for each test file, \
                         since some models need to have been feeded with images’s width and height \
                          Otherwise, model is built only once at the beginning of Test method. Default is False.')

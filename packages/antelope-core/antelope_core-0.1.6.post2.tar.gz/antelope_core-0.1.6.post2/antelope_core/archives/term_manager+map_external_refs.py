...
class TermManager(object):
    ...
    def _serialize_factors(self, origin, *quantities, values=False, map_external_refs=None):
        """
        ...
        :param quantities:
        :param values:
        :param map_external_refs: enable quantity refs to be renamed
        :return:
        """
        if len(quantities) == 0:
        ...
        for q in qqs:
            ...
            if len(_sq) > 0:
                ref = self._canonical_q_ref(q)
                if ref in map_external_refs:
                    ref = map_external_refs[ref]
                j[ref] = _sq
        return j

    def serialize(self, origin, *quantities, values=False, map_external_refs=None):
        """

        :param origin:
        :param quantities:
        :param values:
        :param map_external_refs: [None] enable renaming of quantity refs - handles step I, renaming characterization map
        :return: 3-tuple:
        ...
        j = {'Characterizations': self._serialize_factors(origin, *quantities, values=values,
                                                          map_external_refs=map_external_refs)}

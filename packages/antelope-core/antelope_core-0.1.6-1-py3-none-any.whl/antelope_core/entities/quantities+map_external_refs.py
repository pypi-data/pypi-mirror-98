...
class LcQuantity(LcEntity):
    ...
    def serialize(self, map_external_refs=None, **kwargs):
        if map_external_refs is None:
            map_external_refs = dict()
        j = super(LcQuantity, self).serialize(**kwargs)
        if j['externalId'] in map_external_refs:
            j['externalId'] = map_external_refs[j['externalId']]
        return j

...
class BasicArchive(EntityStore):
    ...
    def export_quantities(self, filename, *quantities, domesticate=True, values=True, gzip=False,
        ...
        :param map_external_refs: [None] dict to map quantity external_ref in new serialization
        :return:
        """
        j = super(BasicArchive, self).serialize()
        ...
        j['quantities'] = sorted([q.serialize(domesticate=domesticate, map_external_refs=map_external_refs)
                                  for q in qs],
                                 key=lambda x: x['externalId'])

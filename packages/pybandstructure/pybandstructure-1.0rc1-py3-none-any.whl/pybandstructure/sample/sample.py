...
####### Magic methods ##################################################
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, tuple) and len(key) == self._lattice_dimension and all(isinstance(n,(int, np.integer)) for n in key):
            ii = self._integer_coords_to_idx[key] 
            if type(ii)==tuple:
                return ii[0]
            else:
                return ii
...
class Unit_Cell_Sample(Sample):
    ...
    def __getitem__(self, key):
        if isinstance(key, str):
            return Sample.__getitem__(self,self._special_points[key])
        else:
    ...
    def get_coords(self, key):
        if isinstance(key, tuple) and len(key) == self._lattice_dimension and all(isinstance(n,(int, np.integer)) for n in key):
            if key in self._integer_coords_to_idx:
                index = self._integer_coords_to_idx[key][0]
            else:
                fractional_part = self.get_fractional_part(key)
            ...
            key_tuple = self._special_points[key]
            if key_tuple in self._integer_coords_to_idx:
                index = self._integer_coords_to_idx[key_tuple][0]
            else:
                fractional_part = self.get_fractional_part(key_tuple)
                index, operation = self._integer_coords_to_idx[fractional_part]
            return self._coords[index]
        else:
            raise ValueError("Given Key should be a tuple of ints or a single int")

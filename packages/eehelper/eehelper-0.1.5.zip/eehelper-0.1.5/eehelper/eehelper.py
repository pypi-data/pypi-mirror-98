...
class EEHelper(object):
    ...
    def __init__(self,
                 ...
                 scale_factor=1,
                 index_list=None,
                 auto_ls5_correction=False,
                 auto_ls8_correction=False,
        ...
        :param index_list: list of names of indices to add
                 valid names: ['EVI', 'NDVI', 'SAVI', 'NDWI', 'NBR', 'VARI', 'NDSI']
                 default: None (will list all indices)
        :param auto_ls8_correction: If the ls8 images should be corrected
                                    based on Roy et al 2016 (DOI: 10.1016/j.rse.2015.12.024)
        :param auto_ls5_correction: If the ls8 images should be corrected
                                    based on Sulla-Menashe et al 2016 (DOI: 10.1016/j.rse.2016.02.041)
        :param composite_index: Index to base the composite on (default: 'NDVI',
                                                                valid: 'NBR','VARI','EVI','SAVI','NDWI','NDVI', 'NDSI')
        ...
        self.index_list = index_list if index_list is not None \
            else ['EVI', 'NDVI', 'SAVI', 'NDWI', 'NBR', 'VARI', 'NDSI']
        self.composite_index = composite_index
        self.composite_function = composite_function
        self.auto_ls5_correction = auto_ls5_correction
        self.auto_ls8_correction = auto_ls8_correction
    ...
    def add_indices(self,
                    in_image):
        """
        Function to add indices to an image:  NDVI, NDWI, VARI, NBR, SAVI, NDSI
    ...
    def ls_sr_band_correction(self,
        ...
        ee.Algorithms.If(Condition, TrueCase, FalseCase) --> treats 0 as FalseCase, not 0 (+/-) as TrueCase
        ee.String(" ").compareTo(" ") --> returns 0 when true (string match in order), not 0 when not true

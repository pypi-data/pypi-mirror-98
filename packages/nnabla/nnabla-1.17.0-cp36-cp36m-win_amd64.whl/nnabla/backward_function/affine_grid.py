...
class AffineGridBackward(BackwardFunction):

    @property
    def name(self):
        return 'AffineGridBackward'
    ...
    def backward_impl(self, inputs, outputs, prop_down, accum):
        ...
        # Computation
        if prop_down[1]:
            inp = nn.Variable(x0.shape).apply(data=g_dx0)
            out = nn.Variable(dy.shape)
            self.forward_func.forward([inp], [out])
            if accum[1]:
                g_dy += out.data
            else:
                g_dy.copy_from(out.data)

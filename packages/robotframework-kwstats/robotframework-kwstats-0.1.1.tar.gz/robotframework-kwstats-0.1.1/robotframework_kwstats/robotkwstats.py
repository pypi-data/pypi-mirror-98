...
def generate_report(opts):
    ...
    rows_html = ''
    for line in groupedResult.to_csv(header=False, index=True).split('\n'):
        if line:
            rows = line.split(',')
            source = rows[1].split('.')[0]
            kwname = rows[1].split('.')[1]
            ...
            """.format(source=str(source), kwname=str(kwname), count=str(total_count), perc=str(pass_percentage), min=str(time_min),

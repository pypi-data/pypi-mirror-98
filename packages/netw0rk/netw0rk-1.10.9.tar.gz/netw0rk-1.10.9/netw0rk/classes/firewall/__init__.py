...
class FireWall(object):
	...
	def enable(self):
		...
		if "Firewall is active and enabled on system startup" in output:
			return Response.success(f"Successfully enabled the firewall.")
		else:
			return Response.error(f"Failed to enable the firewall.")
	...
	def disable(self):
		...
		if "Firewall stopped and disabled on system startup" in output:
			return Response.success(f"Successfully disabled the firewall.")
		else:
			return Response.error(f"Failed to disable the firewall.")
	...
	def allow(self, port):
		...
		if "Rule updated" in output or "Rules updated" in output or "Skipping adding existing rule" in output:
			return Response.success(f"Successfully allowed port [{port}].")
		else:
			return Response.error(f"Failed to allow port [{port}], output: [{output}].")
	...
	def deny(self, port):
		...
		if "Rule updated" in output or "Rules updated" in output or "Skipping adding existing rule" in output:
			return Response.success(f"Successfully denied port [{port}].")
		else:
			return Response.error(f"Failed to deny port [{port}], output: [{output}].")
	...
	def allow_all(self):
		...
		for port, info in ports.items():
			...
			if response["error"] != None: return response
		return Response.success(f"Successfully allowed all {len(ports)} ports.")
	def deny_all(self):
		response = self.info()
		if response["error"] != None: return response
	...
	def set_default(self, deny=True):
		...
		if "Default incoming policy changed to " in output:
			return Response.success(f"Successfully set the default action to [{option}].")
		else:
			return Response.error(f"Failed set the default action to [{option}].")
	...
	def info(self):
		...
		# success.
		return Response.success("Successfully retrieved the firewall info.", {
			"status":status,
			"ports":ports,
		})
	...
	def __check_os__(self):
		...
		return Response.success(f"Supported operating system: [{OS}].")

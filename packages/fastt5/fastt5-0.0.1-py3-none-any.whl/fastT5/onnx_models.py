...
from .onnx_exporter import generate_onnx_representation, quantize, get_model_paths, saved_models_path

from transformers import T5Config, T5ForConditionalGeneration, AutoTokenizer, AutoModelForSeq2SeqLM, T5Tokenizer
from transformers.generation_utils import GenerationMixin
from transformers.modeling_outputs import BaseModelOutputWithPast, Seq2SeqLMOutput, BaseModelOutput
import torch
import functools
import operator
...
class T5Encoder(torch.nn.Module):
    ...
    def forward(self,
        ...
        encoder_hidden_state = torch.from_numpy(self.encoder.run(None, {"input_ids": input_ids.cpu().numpy(),
...
class T5DecoderInit(torch.nn.Module):
    ...
    def forward(self, input_ids, encoder_attention_mask, encoder_hidden_states):
        ...
        decoder_outputs = self.decoder.run(None, {"input_ids": input_ids.cpu().numpy(),
        ...
        out_past_key_values = tuple(list_pkv[i:i+4]
...
class T5Decoder(torch.nn.Module):
    ...
    def forward(self,  input_ids, attention_mask, encoder_output, past_key_values):
        ...
        past_key_values = {f'input_{i}': pkv.cpu().numpy()
...
class OnnxT5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        if past_key_values is None:
            ...
            logits = init_onnx_outputs[0]
            past_key_values = init_onnx_outputs[1]

        else:

            onnx_outputs = self.decoder(
                decoder_input_ids, attention_mask, encoder_hidden_states, past_key_values)

            logits = onnx_outputs[0]
            past_key_values = onnx_outputs[1]
...
def get_onnx_model(model_name_or_path, quantized=True):
    ...
    if quantized:
        assert (encoder_path.exists() and decoder_path.exists() and init_decoder_path.exists()), \

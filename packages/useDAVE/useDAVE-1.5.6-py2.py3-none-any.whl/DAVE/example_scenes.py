...
def single_dof_hinge_plus_cable(s):
    b = s.new_rigidbody('block', mass = 1, cog = (10,0,0), fixed=(True,True,True,True,False, True))
    p1 = s.new_point('p1', position = (10, 0, 10))
    p2 = s.new_point('p2', parent='block', position = (20, 0, 0))
...
def demo_cable(s):
    ...
    b.set_free()
    p1 = s.new_point('p1', position=(10, 0, 10))
    p2 = s.new_point('p2', parent='block', position=(0, 0, 0))
...
def demo_two_cable(s):
    ...
    b.set_free()
    s.new_point('p1a', position=(1, 0, 10))
    s.new_point('p1b', position=(-1, 0, 10))
    s.new_point('p2a', parent='block', position=(1, 0, 0))
    s.new_point('p2b', parent='block', position=(-1, 0, 0))
...
def demo_snake(s, n=20):
    ...
    s.new_point('Poi', parent='base{}'.format(n), position = (2, 0, 0))
    s.new_point('Poi_1')
    s['Poi_1'].position = (10.0, 0.0, 0.0)
    s.new_cable('Cable', endA='Poi_1', endB='Poi', length=7.000, EA=100000)
...
def all_elements():
    ...
    b.set_free()
    pa = s.new_point('poi on rigidbody', parent ='rigidbody', position = (2, 2, 2))
    pt = s.new_point('poi on test', parent='test', position = (5, 7, -2))
    pb = s.new_point('global poi', position=(0, 0, 1))
    s.new_cable('cable', length=50, EA=1000, endA=pa, endB= pt, sheaves=[pb])
...
def grid(s, n=5):
    ...
    for i in range(n):
        for j in range(n):
            ...
            s.new_point(name ='poi {} {}'.format(i, j), parent = b)
    ...
    for i in range(n-1):
        for j in range(n-1):
            ...
            s.new_cable(name = 'cable {} {} a'.format(i,j), endA= p1, endB= p2, length = 5, EA = 1000)
            s.new_cable(name='cable {} {} b'.format(i, j), endA=p1, endB=p3, length=5, EA=1000)
...
def seal(s):
    ...
    b.trimesh.load_file(s.get_resource_path('17919_Seal_baby_happy_V1.obj'))
...
def discrete_beam(s, EI, L, n, P, M):
    ...
    for i in range(n):
        ...
        s.new_linear_connector_6d(name='ConLeft{}'.format(i),
                                  secondary='Axis{}'.format(i),
                                  main='Beam{}'.format(i),
                                  stiffness=(0.0, 0.0, 0.0,
                                             0.0, K, 0.0))
        # code for ConRight
        s.new_linear_connector_6d(name='ConRight{}'.format(i),
                                  secondary='Beam{}_right'.format(i),
                                  main='Axis{}'.format(i + 1),
                                  stiffness=(0.0, 0.0, 0.0,
                                             0.0, K, 0.0))
...
def discrete_linearbeam(s, EIy, EIz, EA, GIp, L, n):
    ...
    for i in range(n):
        ...
        s.new_linear_beam(name='Beam{}'.format(i),
                          nodeA='Axis{}'.format(i), nodeB='Axis{}'.format(i + 1),
                          L = L/n,
                          EA=EA,
                          EIy = EIy,
                          EIz = EIz,
...
def crane5000(s):
    ...
    s.new_point(name='Crane5000_top',
    ...
    s.new_point(name='Mast5000_susp_wire_connection',
    ...
    s.new_cable(name='Crane_susp_wire',
                endA='Mast5000_susp_wire_connection',
                endB='Crane5000_top',
                length=59.865,
                EA=100000000.0)
...
def turtle(s):
    ...
    s.new_buoyancy(name='buoyancy',
                   parent='Turtle')
    s['buoyancy'].trimesh.load_file(s.get_resource_path('buoyancy turtle.obj'), scale=(1.0, 1.0, 1.0),
...
def cheetah(s):
    ...
    s.new_buoyancy(name='buoyancy',
                   parent='Cheetah')
    s['buoyancy'].trimesh.load_file(s.get_resource_path('buoyancy cheetah.obj'), scale=(1, 1, 1),
                                    rotation=(0.0, 0.0, 0.0), offset=(0.0, 0.0, 0.0))
...
def octopus(s):
    ...
    s.new_buoyancy(name='buoyancy',
                   parent='Octopus')
    s['buoyancy'].trimesh.load_file(s.get_resource_path('buoyancy octopus.obj'), scale=(1, 1, 1),
                                    rotation=(0.0, 0.0, 0.0), offset=(0.0, 0.0, 0.0))
...
def all_node_types(s):
    ...
    s.new_buoyancy(name='Buoyancy mesh',
                   parent='Axis_2')
    s['Buoyancy mesh'].trimesh.load_file(s.get_resource_path('cog.obj'), scale=(1.0, 1.0, 1.0), rotation=(0.0, 0.0, 0.0),
                                         offset=(0.0, 0.0, 3.0))
    # code for Visual
    s.new_visual(name='Visual',
                 parent='Axis_3',

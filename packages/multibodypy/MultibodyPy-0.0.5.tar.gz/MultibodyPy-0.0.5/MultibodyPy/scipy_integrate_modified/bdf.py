...
class BDF(OdeSolver):
    ...
    def __init__(self, fun, t0, y0, t_bound, max_step=np.inf, min_step=1e-10,
                 rtol=1e-3, atol=1e-6, jac=None, jac_sparsity=None, newton_tol=1e-5,
        ...
        self.y = y0
        f = self.fun(self.t, self.y, update=False)
    ...
    def step(self):
        t = self.t
        D = self.D
        change_D(D, self.order, self.fixed_step / self.old_step)
        ...
        if error_norm > 1:
            ...
            self.h_abs_rec = h_abs  # np.max([self.min_step, h_abs * factor])

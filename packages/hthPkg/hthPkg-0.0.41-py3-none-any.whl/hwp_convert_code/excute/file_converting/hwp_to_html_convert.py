        ...
        with closing(Hwp5File(hwp5path)) as hwp5file:
            with open_dest() as dest:
                try:
                    ...
                    log.close()
                    pass
                except hwp5.dataio.ParseError as ParseError:
                    print(ParseError)
                    log = open(log_path + 'log.txt', 'a')
                    log.write('PARSE_ERROR!:' + str(dest_path) + '\n')
                    log.close()
                    pass
                except Exception as e:
                    print(e)
                    log = open(log_path + 'log.txt', 'a')
                    log.write('ERROR!:' + str(dest_path) + '\n')
                    log.write(str(e) + '\n')
                    log.close()
                    pass
...
def run_list(i_process, path):
    ...
    if path == 'test':
        ...
        lock_path = home_path.lock_path

    elif path == 'server':
        server_path = cf.other_server
        input_path = server_path.targetPattern
        print('input_path : ' + input_path)
        output_path = server_path.output_path
        print('output_path : ' + output_path)
        log_path = server_path.log_path
        lock_path = server_path.lock_path
    else:
